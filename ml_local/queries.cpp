		...
		case WM_INITDIALOG:
			{
				...
				// i came, i saw, i ate it good and proper, will! <3
				SetDlgItemTextW(hwndDlg,IDC_STATIC_INFO, // eat this, dro!
								WASABI_API_LNGSTRINGW(IDS_WILLS_UBER_STRING));
				EnableWindow(GetDlgItem(hwndDlg,IDC_HIDE_EXTINFO),g_config->ReadInt("useminiinfo", 1));
				...
				if (lParam == 1)
				{
					...
					else
					{
						...
						PathCombineA(configDir, g_viewsDir, m_query_list[m_edit_item]->metafn);
						conf = new C_Config(configDir);
						CheckDlgButton(hwndDlg,IDC_HIDE_EXTINFO,conf->ReadInt("midivvis", 1)?0:1);
					}
				...
				else
				{
					...
					extern void makemetafn(char *filename, char **out);
				...
				if(m_editting==0 && g_config->ReadInt("newfilter_showhelp",1))
				{
		...
		case WM_USER+10:
			{ // enable filter combo boxes based on radio buttons
				...
				if (WM_INITDIALOG == uMsg)
				{
					...
					POINT pt = {g_config->ReadInt("smart_x", -1), g_config->ReadInt("smart_y", -1)};
		...
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				...
				case IDC_CHECK_SHOWINFO:
					g_config->WriteInt("newfilter_showhelp",0);
					break;
				...
				case IDOK:
				{
					...
					if(IsDlgButtonChecked(hwndDlg,IDC_RADIO_SIMPLEALBUM))
					{
						if(conf)
						{
							conf->WriteInt("adiv2pos",100000);
							conf->WriteInt("albumartviewmode",2);
						}
					...
					if(conf) {
						...
						conf->WriteInt("midivvis",v);
						
						ComboBox combo(hwndDlg,IDC_COMBO_PRESETS);
						int n = combo.GetItemData(combo.GetSelection());
						if(n>0 && n<=NUM_PRESETS) {
							n--;
							conf->WriteInt("mv_sort_by", presets[n].sort_by);
							conf->WriteInt("mv_sort_dir", presets[n].sort_dir);
							int cnt = 0;
					...
					if(m_editting) PostMessage(lMedia.hwndLibraryParent, WM_USER + 30, 0, 0);
				}
				case IDCANCEL:
				{
					RECT smart_rect = {0};
					GetWindowRect(hwndDlg, &smart_rect);
					g_config->WriteInt("smart_x", smart_rect.left);
					g_config->WriteInt("smart_y", smart_rect.top);
		...
		case WM_INITDIALOG:
			{
				...
				if (NULL != controlWindow)
				{
					SetWindowText(controlWindow,
								(char*)WASABI_API_LOADRESFROMFILE("TEXT", MAKEINTRESOURCE(IDR_QUERIES_TEXT), 0));
				}
			}
			return 1;
  ...
  switch (uMsg)
  {
    case WM_INITDIALOG:
      // populate comboboxes
      {
        ...
        for (x = 0; x < sizeof(strs)/sizeof(strs[0]); x ++)
        {
          if (strs[x])
          {
			int a=SendMessageW(hwndCB, CB_ADDSTRING,0,(LPARAM)WASABI_API_LNGSTRINGW(strs[x]));
            SendMessage(hwndCB,CB_SETITEMDATA,(WPARAM)a,(LPARAM)x);
            cnt++;
          }
        }
		int a=SendMessageW(hwndCB, CB_ADDSTRING,0,(LPARAM)WASABI_API_LNGSTRINGW(IDS_CUSTOM));
              ...
              case FIELD_LENGTH:
                {
                  ...
                  if (v < 60)
                    ...
                    wsprintf(buf,"%d:%02d",v/60,v%60);
                  else
                    wsprintf(buf,"%d:%02d:%02d",v/60/60,(v/60)%60,v%60);
    ...
    case WM_USER+40:
      if (wParam && lParam>0)
      {
        ...
        if (x != CB_ERR)
        {
          Id=SendMessage(hwndCB,CB_GETITEMDATA,x,0);
          ...
          if (x != CB_ERR)
          {
            Op=SendMessage(hwndCB2,CB_GETITEMDATA,x,0);
...
#undef GAP
                ...
                if (myfillt)
                {
                  while (myfillt_len--)
                  {
					int a=SendMessageW(hwndCB2,CB_ADDSTRING,0,
									  (LPARAM)WASABI_API_LNGSTRINGW(myfillt->str));
...
static INT_PTR CALLBACK scrollChildProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam) 
{
	...
	case WM_USER+41: // remove all
		{
			...
			for(size_t i=0; i!=w.size(); i++)
				SendMessage(hwndDlg,WM_USER+34,(WPARAM)(HWND)w[i],-1);
		}
		break;
	...
	case WM_USER+32: // add filter. lParam is mode. 1 means add by filter, 2 means add blank
		...
		if ((lParam == 1 && wParam) || (lParam == 2 && !wParam))
		{
			...
			nde_field_t p=0;
			if (lParam == 2 || (NULL != (p=NDE_Table_GetColumnByID(g_table, NDE_Filter_GetID(filter)))))
			{
...
{
	m_edit_item=n;
	HWND hwnd = WASABI_API_CREATEDIALOGPARAMW(IDD_ADD_VIEW_2, lMedia.hwndLibraryParent, addQueryFrameDialogProc2, 1);
	SetActiveWindow(hwnd);
}
...
static INT_PTR CALLBACK ml_newWndProc2(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uMsg)
	{
		case WM_INITMENUPOPUP:
			...
			{
				myMenu = TRUE;
				if (SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, (WPARAM)&s_menu, IPC_LIBRARY_SENDTOMENU)==(LRESULT)-1)
					s_menu.mode=2;
				myMenu = FALSE;
				if(ml_oldWndProc2) SetWindowLongPtr(hwndDlg,GWLP_WNDPROC,(LONG_PTR)ml_oldWndProc2);
				ml_oldWndProc2 = NULL;
...
void view_queryContextMenu(INT_PTR param1, HWND hHost, POINTS pts, int n)
{
	...
	ml_oldWndProc2 = (WNDPROC)SetWindowLongPtr(hHost, GWLP_WNDPROC, (LONG_PTR)ml_newWndProc2);
	...
	int r=Menu_TrackPopup(menu,TPM_RETURNCMD|TPM_RIGHTBUTTON|TPM_LEFTBUTTON,pt.x,pt.y,hHost,NULL);
	if(ml_oldWndProc2) SetWindowLongPtr(hHost,GWLP_WNDPROC,(LONG_PTR)ml_oldWndProc2);
	switch(r) 
	{
		...
		default:
			if (s_menu.mode == 2)
			{
				...
				if (SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, (WPARAM)&s_menu, IPC_LIBRARY_SENDTOMENU) == (LRESULT)-1)
				{
					...
					LRESULT result = SendMessage(lMedia.hwndWinampParent, WM_WA_IPC,(WPARAM)&s_menu,IPC_LIBRARY_SENDTOMENU);
					if (result != 1)
					{
						s_menu.mode=3;
						s_menu.data_type = ML_TYPE_ITEMRECORDLIST;
						...
						SendMessage(lMedia.hwndWinampParent, WM_WA_IPC,(WPARAM)&s_menu,IPC_LIBRARY_SENDTOMENU);
						freeRecordList(&objA);
					}
					freeRecordList(&obj);
	...
	{
		s_menu.mode=4;
		SendMessage(lMedia.hwndWinampParent, WM_WA_IPC,(WPARAM)&s_menu,IPC_LIBRARY_SENDTOMENU); // cleanup
	}
	main_sendto_hmenu=0;
	EatKeyboard();
}
...
void queriesContextMenu(INT_PTR param1, HWND hHost, POINTS pts) {
	...
	int r=Menu_TrackPopup(menu,TPM_RETURNCMD|TPM_RIGHTBUTTON|TPM_LEFTBUTTON|TPM_NONOTIFY,pt.x,pt.y,hHost,NULL);
	switch(r) {
		case ID_QUERYMENU_ADDNEWQUERY:
			addNewQuery(hHost);
		break;
