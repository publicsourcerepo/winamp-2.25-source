...
static int getFileInfo(const char *filename, char *metadata, char *dest, int len)
{
  ...
  int r = SendMessage(lMedia.hwndWinampParent,WM_WA_IPC,(WPARAM)&efis,IPC_GET_EXTENDED_FILE_INFO); //will return 1 if wa2 supports this IPC call
...
LRESULT IPC_GetFileInfo(INT_PTR param)
{
	...
	char filename2[MAX_PATH]; // full lfn path if set
	makeFilename2(ent->filename, filename2, sizeof(filename2));
	...
	char tmp[1024] = {0, };
	if (getFileInfo(fnp, "title", tmp, sizeof(tmp) - 1))
	{
		if (tmp[0] && !ent->title)
		{
		...
		if (!ent->artist)
		{
			getFileInfo(ent->filename, "artist", tmp, sizeof(tmp) - 1);
			if (tmp[0])
			{
		...
		if (!ent->album)
		{
			getFileInfo(ent->filename, "album", tmp, sizeof(tmp) - 1);
			if (tmp[0])
			{
		...
		if (ent->year <= 0)
		{
			getFileInfo(ent->filename, "year", tmp, sizeof(tmp) - 1);
		...
		if (!ent->genre)
		{
			getFileInfo(ent->filename, "genre", tmp, sizeof(tmp) - 1);
			if (tmp[0])
			{
		...
		if (ent->track <= 0)
		{
			getFileInfo(ent->filename, "track", tmp, sizeof(tmp) - 1);
			if (tmp[0])
			{
				ent->track = atoi(tmp);
			}
		...
		if (ent->length <= 0)
		{
			getFileInfo(ent->filename, "length", tmp, sizeof(tmp) - 1);
			if (tmp[0])
			{
...
static int getFileInfoW(const wchar_t *filename, wchar_t *metadata, wchar_t *dest, int len)
{
  ...
  int r = SendMessage(lMedia.hwndWinampParent,WM_WA_IPC,(WPARAM)&efis,IPC_GET_EXTENDED_FILE_INFOW); //will return 1 if wa2 supports this IPC call
...
LRESULT IPC_GetFileInfoW(INT_PTR param)
{
	...
	wchar_t filename2[MAX_PATH]; // full lfn path if set
	makeFilename2W(ent->filename, filename2, sizeof(filename2));
	...
	wchar_t tmp[1024] = {0, };
	if (getFileInfoW(fnp, L"title", tmp, sizeof(tmp) - 1))
	{
		if (tmp[0] && !ent->title)
		{
		...
		if (!ent->artist)
		{
			getFileInfoW(ent->filename, L"artist", tmp, sizeof(tmp) - 1);
			if (tmp[0])
			{
		...
		if (!ent->album)
		{
			getFileInfoW(ent->filename, L"album", tmp, sizeof(tmp) - 1);
			if (tmp[0])
			{
		...
		if (ent->year <= 0)
		{
			getFileInfoW(ent->filename, L"year", tmp, sizeof(tmp) - 1);
		...
		if (!ent->genre)
		{
			getFileInfoW(ent->filename, L"genre", tmp, sizeof(tmp) - 1);
			if (tmp[0])
			{
		...
		if (ent->track <= 0)
		{
			getFileInfoW(ent->filename, L"track", tmp, sizeof(tmp) - 1);
			if (tmp[0])
			{
				ent->track = _wtoi(tmp);
			}
		...
		if (ent->length <= 0)
		{
			getFileInfoW(ent->filename, L"length", tmp, sizeof(tmp) - 1);
			if (tmp[0])
			{
...
INT_PTR HandleIpcMessage(INT_PTR msg, INT_PTR param)
{
	...
	case ML_IPC_DB_ADDORUPDATEITEMW:
		...
		if (param && g_table)
		{
			...
			if (ret == 1)
			{
				...
				if (item->rating >= 0) 
				{
					...
					if (g_config->ReadInt("writeratings", 0))
					{
						...
						if (item->rating > 0)
						{
							...
							buf[0] = 0;
						updateFileInfo(item->filename, L"rating", buf);
						SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, 0, IPC_WRITE_EXTENDED_FILE_INFO);
					}
				}
				...
				if (item->filesize >= 0)
				{
					// changed in 5.7 to use the 'realsize' if it's available, otherwise map to filesize scaled to bytes (was stored as kb previously)
	...
	case ML_IPC_DB_ADDORUPDATEFILEW:
		...
		if (param && g_table)
		{
			...
			int guess  = (fi->gues_mode == -1) ? g_config->ReadInt("guessmode", 0) : fi->gues_mode;
			int meta = (fi->meta_mode == -1) ? g_config->ReadInt("usemetadata", 1) : fi->meta_mode;
	...
	case ML_IPC_SMARTVIEW_INFO:
		if (param)
		{
			...
			queryItem * q = (m_query_list.begin()+v->smartViewNum)->second;
			if (!q) break;
			if (!q->name || !q->query) break;
			v->treeItemId = (m_query_list.begin()+v->smartViewNum)->first;
