...
static void EnableDisableRecentPlayingControls(HWND hwndDlg, int tracking){
	if(tracking == -1) tracking = !!g_config->ReadInt("trackplays", 1);
	EnableWindow(GetDlgItem(hwndDlg, IDC_CHECK7), tracking);
	EnableWindow(GetDlgItem(hwndDlg, IDC_CHECK8), tracking);
	EnableWindow(GetDlgItem(hwndDlg, IDC_EDIT2), tracking && !!g_config->ReadInt("trackplays_wait_secs", 0));
	EnableWindow(GetDlgItem(hwndDlg, IDC_EDIT3), tracking && !!g_config->ReadInt("trackplays_wait_percent", 0));
...
static INT_PTR CALLBACK Prefs1Proc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			CheckDlgButton(hwndDlg, IDC_REMEMBER_SEARCH, !!g_config->ReadInt("remembersearch", 0));
			CheckDlgButton(hwndDlg, IDC_CHECK3, !!g_config->ReadInt("useminiinfo", 1));

			CheckDlgButton(hwndDlg, IDC_CHECK_ATF, !!g_config->ReadInt("newtitle", 1));
			CheckDlgButton(hwndDlg, IDC_CHECK5, !!g_config->ReadInt("audiorefine", 0));
			CheckDlgButton(hwndDlg, IDC_CHECK6, !g_config->ReadInt("dbloadatstart", 1));

			CheckDlgButton(hwndDlg, IDC_ARTIST_AS_ALBUMARTIST, g_config->ReadInt("artist_as_albumartist", 1));	

			SetDlgItemInt(hwndDlg, IDC_EDIT_QUERYDELAY, g_config->ReadInt("querydelay",250), 0);

			int tracking = !!g_config->ReadInt("trackplays", 1);
			CheckDlgButton(hwndDlg, IDC_CHECK2, tracking);
			CheckDlgButton(hwndDlg, IDC_CHECK7, !!g_config->ReadInt("trackplays_wait_secs", 0));
			CheckDlgButton(hwndDlg, IDC_CHECK8, !!g_config->ReadInt("trackplays_wait_percent", 0));
			SetDlgItemInt(hwndDlg, IDC_EDIT2, g_config->ReadInt("trackplays_wait_secs_lim",5), 0);
			SetDlgItemInt(hwndDlg, IDC_EDIT3, g_config->ReadInt("trackplays_wait_percent_lim",50), 0);
		...
		case WM_COMMAND:
			...
			{
				case IDC_CHECK3:
					g_config->WriteInt("useminiinfo", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK3));
					PostMessage(lMedia.hwndLibraryParent, WM_USER + 30, 0, 0);
				break;
				case IDC_REMEMBER_SEARCH:
					g_config->WriteInt("remembersearch", !!IsDlgButtonChecked(hwndDlg, IDC_REMEMBER_SEARCH));
				break;
				case IDC_CHECK_ATF:
					g_config->WriteInt("newtitle", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK_ATF));
				break;
				case IDC_CHECK5:
					g_config->WriteInt("audiorefine", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK5));
					if (lMedia.hwndLibraryParent) PostMessage(lMedia.hwndLibraryParent, WM_USER + 30, 0, 0);
				break;
				case IDC_CHECK6:
					g_config->WriteInt("dbloadatstart", !IsDlgButtonChecked(hwndDlg, IDC_CHECK6));
				break;
				case IDC_CHECK2:
				{
					int tracking = !!IsDlgButtonChecked(hwndDlg, IDC_CHECK2);
					g_config->WriteInt("trackplays", tracking);
					EnableDisableRecentPlayingControls(hwndDlg, tracking);
				}
				break;
				case IDC_CHECK7:
				{
					g_config->WriteInt("trackplays_wait_secs", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK7));
					EnableDisableRecentPlayingControls(hwndDlg, -1);
				}
				break;
				case IDC_CHECK8:
				{
					g_config->WriteInt("trackplays_wait_percent", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK8));
					EnableDisableRecentPlayingControls(hwndDlg, -1);
				}
				break;
				case IDC_BUTTON2:
				...
				case IDC_EDIT_QUERYDELAY:
				{
					if (HIWORD(wParam) == EN_CHANGE)
					{
						...
						if (t)
						{
							...
							g_config->WriteInt("querydelay", v);
							g_querydelay = v;
						}
					}
				}
				...
				case IDC_EDIT2:
					if (HIWORD(wParam) == EN_CHANGE)
					{
						...
						if (t)
						{
							...
							g_config->WriteInt("trackplays_wait_secs_lim", v);
						}
					}
				break;
				case IDC_EDIT3:
					if (HIWORD(wParam) == EN_CHANGE)
					{
						...
						if(t)
						{
							...
							g_config->WriteInt("trackplays_wait_percent_lim", v);
						}
					}
				break;
			}
...
static void makeMetaStr(int guess_mode, int use_metadata, int subdir, wchar_t *buf, int bufLen)
{
	...
	if (guess_mode >= 0) {
		if (guess_mode == 1) StringCchPrintfW(guessstr, 32, L"-%s", WASABI_API_LNGSTRINGW(IDS_SMART_STR));
		else if (guess_mode == 2) StringCchPrintfW(guessstr, 32, L"-%s", WASABI_API_LNGSTRINGW(IDS_GUESS_STR));
		else StringCchPrintfW(guessstr, 32, L"+%s", WASABI_API_LNGSTRINGW(IDS_SMART_STR));
	}
...
static void saveList()
{
	...
	for (x = 0; x < l; x ++)
	{
		...
		if (buf[0])
		{
			...
			{
				gs.Append(L"<");
				if (guess >= 0) gs.Append(guess == 1 ? L"s" : (guess == 2 ? L"g" : L"S"));
				if (meta >= 0) gs.Append(meta ? L"M" : L"m");
				if (!subdir) gs.Append(L"r");
				gs.Append(L">");
			}
			gs.Append(buf);
		}
...
int autoscan_add_directory(const wchar_t *path, int *guess, int *meta, int *recurse, int noaddjustcheck)
{
	...
	AutoWide s1(_s1, codePage);

	int bufLen = 0;
	wchar_t *s = (wchar_t*)malloc((bufLen = (wcslen(s1) + 2))*sizeof(wchar_t));
	if (s)
	{
		wcsncpy(s, s1, bufLen);
		s[wcslen(s)+1] = 0;
		...
		while (*p)
		{
			...
			if (*p == L'<' && wcsstr(p, L">"))
			{
				...
				while (*p != L'>')
				{
					...
					// g=no guessing
					if (*p == L'M') use_meta = 1;
					else if (*p == L'm') use_meta = 0;
					else if (*p == L'S') do_guess = 0;
					else if (*p == L's') do_guess = 1;
					else if (*p == L'g') do_guess = 2;
					else if (*p == L'r') do_recurse = 0;
					p++;
				}
				p++;
			}
			...
			if (!_wcsnicmp(p, path, wcslen(p)) && (path[wcslen(p)] == L'\\' || !path[wcslen(p)]))
			{
				...
				if (meta) *meta = use_meta;
				if (guess) *guess = do_guess;
				if (recurse) *recurse = do_recurse;
	...
	if (!noaddjustcheck)
	{
		WIN32_FIND_DATAW fd;
		int bufLen = 0;
		wchar_t *s = (wchar_t*)malloc((bufLen = (wcslen(path) + 32))*sizeof(wchar_t));
...
static INT_PTR CALLBACK editDirProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			if (meta) CheckDlgButton(hwndDlg, IDC_CHECK1, meta > 0 ? BST_CHECKED : BST_INDETERMINATE);

			if (guess < 0) CheckDlgButton(hwndDlg, IDC_RADIO6, BST_CHECKED);
			else if (!guess) CheckDlgButton(hwndDlg, IDC_RADIO1, BST_CHECKED);
			else if (guess == 2) CheckDlgButton(hwndDlg, IDC_RADIO8, BST_CHECKED);
			else if (guess > 0) CheckDlgButton(hwndDlg, IDC_RADIO2, BST_CHECKED);
		...
		case WM_COMMAND:
			switch (LOWORD(wParam))
			{
				case IDOK:
				{
					...
					if (meta == BST_CHECKED) meta = 1;
					else if (meta == BST_INDETERMINATE) meta = -1;
					else meta = 0;

					int guess = -1;
					if (IsDlgButtonChecked(hwndDlg, IDC_RADIO1)) guess = 0;
					else if (IsDlgButtonChecked(hwndDlg, IDC_RADIO2)) guess = 1;
					else if (IsDlgButtonChecked(hwndDlg, IDC_RADIO8)) guess = 2;
					int subdir = 1;
					if (!IsDlgButtonChecked(hwndDlg, IDC_CHECK2)) subdir = 0;
...
static INT_PTR CALLBACK confMetaProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			if (g_config->ReadInt("usemetadata", 1)) CheckDlgButton(hwndDlg, IDC_CHECK1, BST_CHECKED);
			{
				int gm = g_config->ReadInt("guessmode", 0);
		...
		case WM_COMMAND:
			...
			{
				case IDOK:
					g_config->WriteInt("usemetadata", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK1));
					{
						int a = SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_GETCURSEL, 0, 0);
						g_guessifany = (a == 0);
						g_config->WriteInt("guessifany", g_guessifany);
					}

					if (IsDlgButtonChecked(hwndDlg, IDC_RADIO1)) g_config->WriteInt("guessmode", 0);
					if (IsDlgButtonChecked(hwndDlg, IDC_RADIO2)) g_config->WriteInt("guessmode", 1);
					if (IsDlgButtonChecked(hwndDlg, IDC_RADIO3)) g_config->WriteInt("guessmode", 2);
				case IDCANCEL:
					EndDialog(hwndDlg, 0);
					break;
				case IDC_CHECK1:
...
static INT_PTR CALLBACK Prefs3Proc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			CheckDlgButton(hwndDlg, IDC_CHECK3, g_config->ReadInt("bgrescan_startup", 0) ? BST_CHECKED : 0);
			CheckDlgButton(hwndDlg, IDC_CHECK2, g_config->ReadInt("bgrescan_compact", 1) ? BST_CHECKED : 0);
			CheckDlgButton(hwndDlg, IDC_CHECK5, g_config->ReadInt("autoaddplays", 0) ? BST_CHECKED : 0);
		...
		case WM_COMMAND:
			...
			{
				case IDC_CHECK5:
					g_config->WriteInt("autoaddplays", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK5));
				break;
				case IDC_CHECK2:
					g_config->WriteInt("bgrescan_compact", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK2));
				break;
				case IDC_CHECK3:
					g_config->WriteInt("bgrescan_startup", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK3));
				break;
				case IDC_CHECK4:
					g_config->WriteInt("bgrescan_do", g_bgrescan_do = !!IsDlgButtonChecked(hwndDlg, IDC_CHECK4));
				...
				case IDC_EDIT3:
				{
					if(HIWORD(wParam) == EN_CHANGE && m_set_rescan_int)
					{
						...
						{
							if (x < 1) x = 1;
							g_config->WriteInt("bgrescan_int", g_bgrescan_int = x);
				...
				case IDC_BUTTON1:
				{
					...
					if (browse.Browse(hwndDlg))
					{
						...
						AutoWide scanlist(_s1, codePage);
						n.Set(scanlist);
						if (scanlist[0]) n.Append(L"|");
						n.Append(path);
						g_config->WriteString("scandirlist", 0);
				...
				case IDC_BUTTON4:
				{
					...
					for (x = 0; x < l; x ++)
					{
						...
						{
							m_edit_idx = x;
							if (WASABI_API_DIALOGBOXW(IDD_EDITDIR, hwndDlg, editDirProc)) s = 1;
							else break;
						}
					}
					if (s)
				...
				case IDC_RESCAN:
					...
					else
					{
						...
						openDb();
						if (lMedia.hwndLibraryParent) SendMessage(lMedia.hwndLibraryParent, WM_USER + 575, 0xffff00dd, 0);
						SendMessage(hwndDlg, WM_TIMER, 100, 0);
					}
				break;
				case IDC_CONFMETA:
...
static void _dosetsel(HWND hwndDlg)
{
	...
	if (sel >= 0 && (sel != g_config->ReadInt("lastprefp", 0) || !subWnd))
	{
		...
		if(!SendMessage(lMedia.hwndWinampParent,WM_WA_IPC,IPC_ISWINTHEMEPRESENT,IPC_USE_UXTHEME_FUNC))
		{
			SendMessage(lMedia.hwndWinampParent,WM_WA_IPC,(WPARAM)tabwnd,IPC_USE_UXTHEME_FUNC);
			SendMessage(lMedia.hwndWinampParent,WM_WA_IPC,(WPARAM)subWnd,IPC_USE_UXTHEME_FUNC);
		}
	}
}
