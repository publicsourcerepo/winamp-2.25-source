...
static int makeFilterQuery(GayStringW *query, ViewFilter *f)
{
	...
	if (!(f->list->GetSelected(0) && f->HasTopItem()))
	{
		...
		int needclose = 0;
		int needor = 0;
		selCount = SendMessageW(f->list->getwnd(), LVM_GETSELECTEDCOUNT, 0, 0L);

		if (selCount && ((f->HasTopItem()) ? selCount < (c - 1) : selCount < c))
		{
			for (x = f->HasTopItem()?1:0; x < c; x ++)
			{
	...
	if (valid)
	{
		...
		do
		{
			...
			if (g_config->ReadInt("writeratings", 0))
			{
				...
				updateFileInfo(filename, L"rating", buf);
	...
	for (index = 0; index < sizeof(controls) / sizeof(LCTRL); index++)
	{
		if (controls[index].id >= GROUP_MIN && controls[index].id <= GROUP_MAX) // group id
		{
			...
			case GROUP_FILTER:
				...
				else if (divX > (rc.right - (17*(numFilters-1)+(numFilters==2?10:0))))
				{
					RECT rw;
				...
				if (divX2 < (rc.left + 15*(numFilters-2)))
				{
					divX2 = rc.left;
					adiv2_nodraw = 1;
				}
				else if (divX2 > (rc.right - 18*2))
				{
					RECT rw;
		...
		case IDC_SEARCHCAPTION:
			SETLAYOUTPOS(pl, rg.left+2+6, rg.top+1, (ri.right - ri.left), (rg.bottom - rg.top));
			rg.left += (pl->cx + 4) + 6;
			break;
		case IDC_CLEAR:
		...
		case IDC_LIST1:
			...
			break;
		case IDC_DIV1:
			SETLAYOUTPOS(pl, rg.left, rg.top + 1, ri.right - ri.left, (rg.bottom - rg.top) - 1);
			rg.left += pl->cx;
			break;
		case IDC_LIST2:
			if (numFilters == 3)
			{
				SETLAYOUTPOS(pl, rg.left, rg.top + 1, divX2- rg.left, (rg.bottom - rg.top) - 1);
			}
			else
			{
				SETLAYOUTPOS(pl, rg.left, rg.top + 1, (rg.right - rg.left) - 3, (rg.bottom - rg.top) -1);
			}
			rg.left += pl->cx;
			break;
		case IDC_DIV2:
		...
		case IDC_LIST3:
			...
			break;
		case IDC_HDELIM:
			SETLAYOUTPOS(pl, rg.left, rg.top, rg.right - rg.left, (ri.bottom - ri.top));
			rg.top += pl->cy;
			break;
		case IDD_VIEW_MEDIA:
			pl->flags |= (rg.top < rg.bottom) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
...
static bool UpdateFilterPane(int pane, char* selconfigname)
{
	...
	ignore_selections = 1;
	char *_selstr = g_config->ReadInt("remembersearch", 0) ? g_view_metaconf->ReadString(selconfigname, "") : NULL;
	AutoWide selstr(_selstr, CP_UTF8);
	int a = 0;
	if (selstr && *selstr)
	{
		wchar_t *t = (wchar_t*)malloc((wcslen(selstr) + 2)*sizeof(wchar_t));
		wcscpy(t, selstr);
...
void SetToolbarButtonBitmap(HWND hwndDlg, int buttonrc, int bitmaprc, ARGB32 fc)
{
	...
	HBITMAP hbm1 = (HBITMAP)LoadImage(lMedia.hDllInstance,MAKEINTRESOURCE(bitmaprc),IMAGE_BITMAP,0,0,LR_VGACOLOR);
...
#define DO_HELP()	\
	...
	if (uMsg == WM_INITDIALOG) {	\
		int x; \
		hTooltipWnd = CreateWindow(TOOLTIPS_CLASS,(LPCSTR)NULL,TTS_ALWAYSTIP|TTS_NOPREFIX, \
		...
		for (x = 0; x < sizeof(helpinfo)/sizeof(helpinfo[0]); x ++) { \
			...
			ti.uId=(UINT_PTR)GetDlgItem(hwndDlg,helpinfo[x].id); ti.hwnd=hwndDlg; ti.lpszText=WASABI_API_LNGSTRING(helpinfo[x].id2);	\
...
INT_PTR CALLBACK view_audioDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		div_mediapos = g_view_metaconf->ReadInt("adiv2pos", 50000);

		if (numFilters == 1)
		{
			div_filterpos = g_view_metaconf->ReadInt("adiv1pos", 100000);
			div_filter2pos = -1;
		}
		else if (numFilters == 2)
		{
			div_filterpos = g_view_metaconf->ReadInt("adiv1pos", 50000);
			div_filter2pos = -1;
		}
		else if (numFilters == 3)
		{
			div_filterpos = g_view_metaconf->ReadInt("adiv1pos", 33333);
			div_filter2pos = g_view_metaconf->ReadInt("adiv3pos", 66667);
		...
		m_media_hwnd = WASABI_API_CREATEDIALOGPARAMW(IDD_VIEW_MEDIA, hwndDlg, view_mediaDialogProc, (LPARAM)!g_config->ReadInt("audiorefine", 0));
		...
		if (!g_view_metaconf->ReadInt("av0_hscroll",0)) MLSkinnedScrollWnd_ShowHorzBar(m_list1.getwnd(), FALSE);
		if (!g_view_metaconf->ReadInt("av1_hscroll",0)) MLSkinnedScrollWnd_ShowHorzBar(m_list2.getwnd(), FALSE);
		if (!g_view_metaconf->ReadInt("av2_hscroll",0) && numFilters == 3) MLSkinnedScrollWnd_ShowHorzBar(m_list3.getwnd(), FALSE);

		FLICKERFIX ff;
		ff.mode = FFM_ERASEINPAINT;
		INT ffcl[] = { IDC_CLEAR, IDC_BUTTON_ARTMODE, IDC_BUTTON_VIEWMODE, IDC_BUTTON_COLUMNS};
		...
		SetDlgItemText(m_media_hwnd, IDC_QUICKSEARCH, "");
		SetDlgItemTextW(m_media_hwnd, IDC_SEARCHCAPTION, WASABI_API_LNGSTRINGW(IDS_REFINE));
		SetDlgItemTextW(m_media_hwnd, IDC_CLEAR, WASABI_API_LNGSTRINGW(IDS_CLEAR_REFINE));
		KillTimer(m_media_hwnd, UPDATE_QUERY_TIMER_ID); // keep it from researching :)

		if (g_config->ReadInt("remembersearch", 0))
		{
			char *query = g_view_metaconf->ReadString("lastquery_a_utf8", "");
			SetDlgItemTextW(hwndDlg, IDC_QUICKSEARCH, AutoWide(query, CP_UTF8));
...

		{
			...
			for (int i = 0; i < numFilters; i++)
			{
				...
				wsprintf(buf, "%s_sort_by_%d",filter[i]->GetConfigId(), i);
				int l_sc = g_view_metaconf->ReadInt(buf, 0);
				wsprintf(buf, "%s_sort_dir_%d",filter[i]->GetConfigId(), i);
			...
			case ID_RATE_0:
			{
				...
				if(rateList(rate))
				{
					...
					while((s = filter[pane]->list->GetNextSelected(s)) != -1)
						filter[pane]->MetaUpdate(s,L"rating",buf);
				}
	...
	case WM_NOTIFY:
	{
		...
		else if (l->code == LVN_COLUMNCLICK)
		{
			...
			wsprintf(buf, "%s_sort_by_%d",filter[which]->GetConfigId(), which);
			int l_sc = g_view_metaconf->ReadInt(buf, 0);

			wsprintf(buf2, "%s_sort_dir_%d",filter[which]->GetConfigId(), which);
		...
		else if (l->idFrom == IDC_LIST1 || l->idFrom == IDC_LIST2 || l->idFrom == IDC_LIST3)
		{
			...
			if (l->code == NM_DBLCLK)
			{
				...
				if (!x && pane>0 && filter[pane]->HasTopItem())
					playList((!!g_config->ReadInt("enqueuedef", PLAY)) ^(!!(GetAsyncKeyState(VK_SHIFT)&0x8000)),pane-1);
				else if (x < c)
					playList((!!g_config->ReadInt("enqueuedef", PLAY)) ^(!!(GetAsyncKeyState(VK_SHIFT)&0x8000)),pane);
			}
		...
		if (l->code == NM_RETURN)
		{
			...
			playFiles(((!!g_config->ReadInt("enqueuedef", 0)) ^(!!(GetAsyncKeyState(VK_SHIFT)&0x8000))), 1);
		}
	}
	break;
	case WM_USER+710:
	...
	case WM_COMMAND:
		...
		case IDC_BUTTON_VIEWMODE:
		{
			...
			{
				wchar_t buf[350], filterName[128];
				buf[0] = L'\0';
			...
			int r = Menu_TrackPopup(menu,TPM_RETURNCMD|TPM_RIGHTBUTTON|TPM_LEFTBUTTON|TPM_NONOTIFY,rc.left,rc.bottom, hwndDlg,NULL);
			DestroyMenu(menu);
			if (r==0) break;
			else if (r == 0x4000)
		...
		case IDC_BUTTON_COLUMNS:
		{
			...
			int r = Menu_TrackPopup(menu, TPM_RETURNCMD | TPM_RIGHTBUTTON | TPM_LEFTBUTTON, rc.left,rc.bottom, hwndDlg, NULL);
	...
	case WM_APP + 4:
	{
		if(!lParam)
		{
			...
			for(int i=0; i<numFilters; i++)
			{
				...
				{
					filter[i]->MetaUpdate(s,L"rating",buf);
	...
	case WM_DESTROY:
		...
		if (numFilters >= 2) g_view_metaconf->WriteInt("adiv1pos", div_filterpos);
		if (numFilters >= 3) g_view_metaconf->WriteInt("adiv3pos", div_filter2pos);
		
		g_view_metaconf->WriteInt("adiv2pos", div_mediapos);
		...
		for (int i=0; i<numFilters; i++)
		{
			...
			const char *confname = "artist_sel_utf8";
		...
		numFilters=0;
		{
			...
			if (s)
			{
				if (s->getBits()) free(s->getBits()); delete s;
