...
struct CreateCacheParameters : public nu::PtrDequeNode
{
...
static nu::PtrDeque2<CreateCacheParameters> artQueue;
static Nullsoft::Utility::LockGuard queueGuard(400);
...
typedef nu::Map<int, nde_table_t> ArtCache;
...
void MigrateArtCache()
{
	...
	for (int i = 0; i < sizeof(size)/sizeof(size[0]); i++)
	{
		if (!size[i])
		{
			...
			art_db = NDE_CreateDatabase(lMedia.hDllInstance);
			wchar_t tableName[MAX_PATH], oldTableName[MAX_PATH], indexName[MAX_PATH], oldIndexName[MAX_PATH];
		...
		{
			const wchar_t *inidir = WASABI_API_APP->path_getUserSettingsPath();
			wchar_t temp[MAX_PATH], tableName[MAX_PATH], oldTableName[MAX_PATH], indexName[MAX_PATH], oldIndexName[MAX_PATH];
...
static bool InitArtCache(int size)
{
	if (!art_db)
	{
		...
		NDE_Table_NewColumnW(artHashes, ARTHASH_FILENAME, L"filename", FIELD_FILENAME);
		NDE_Table_NewColumnW(artHashes, ARTHASH_HASH, L"hash", FIELD_INT128);
		NDE_Table_PostColumns(artHashes);
		NDE_Table_AddIndexByIDW(artHashes, ARTHASH_FILENAME, L"filename");
	}

	if (size && !artcache[size])
	{
		const wchar_t *inidir = WASABI_API_APP->path_getUserSettingsPath();
		wchar_t temp[MAX_PATH], tableName[MAX_PATH], indexName[MAX_PATH];
		PathCombineW(temp, inidir, L"Plugins\\ml");
...
void CreateCache(AlbumArtContainer *container, int w, int h)
{
	...
	assert(artThread);
	container->AddRef();
	CreateCacheParameters *parameters = (CreateCacheParameters *)malloc(sizeof(CreateCacheParameters));
	parameters->container=container;
	parameters->w=w;
	parameters->h=h;
	parameters->status = AlbumArtContainer::CACHE_UNKNOWN;
	Nullsoft::Utility::AutoLock lock(queueGuard);
...
static int DeleteDatabaseAPC(HANDLE handle, void *user_data, intptr_t id)
{
	...
	StringCchPrintfA(search_mask, MAX_PATH, "%s\\art_*.*", g_tableDir);

	char fn[MAX_PATH];
	StringCchPrintfA(fn, MAX_PATH, "%s\\art.idx", g_tableDir);
	DeleteFileA(fn);
	StringCchPrintfA(fn, MAX_PATH, "%s\\art.dat", g_tableDir);
	DeleteFileA(fn);	

	WIN32_FIND_DATAA findData;
	HANDLE hFind = FindFirstFileA(search_mask, &findData);
	if (hFind != INVALID_HANDLE_VALUE)
	{
		do
		{
			if (!_strnicmp(findData.cFileName, "art_", 4))
			{
				StringCchPrintfA(fn, MAX_PATH, "%s\\%s", g_tableDir, findData.cFileName);
				DeleteFileA(fn);
			}
		}
