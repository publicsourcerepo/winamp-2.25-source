...
//// SHELL32 - BEGIN  (this is hidden functions in Win9x;

typedef LPVOID (WINAPI *SH32_SHALLOC)(ULONG);  
typedef void (WINAPI *SH32_SHFREE)(LPVOID);
typedef void (WINAPI *SH32_ILFREE)(LPCITEMIDLIST);
typedef LPITEMIDLIST (WINAPI *SH32_ILCLONE)(LPCITEMIDLIST);
typedef BOOL (WINAPI *SH32_ILISEQUAL)(LPCITEMIDLIST, LPCITEMIDLIST);
typedef BOOL (WINAPI *SH32_ILISPARENT)(LPCITEMIDLIST, LPCITEMIDLIST, BOOL);

static SH32_SHALLOC	sh32_SHAlloc;
static SH32_SHFREE sh32_SHFree;
static SH32_ILFREE sh32_ILFree;
static SH32_ILCLONE sh32_ILClone;
static SH32_ILISEQUAL sh32_ILIsEqual;
static SH32_ILISPARENT sh32_ILIsParent;

static HINSTANCE shell32Dll = NULL;

static BOOL LoadShell32(void);
static void UnloadShell32(void);
...
void ScanFolderBrowser::LoadBookmarks(void)
{
	...
	FreeBookmarks();
	count = sizeof(BOOKMARKS)/sizeof(FOLDER);
	if (!count) return;

	buttons = (FBUTTON*)malloc(sizeof(FBUTTON)*count);
	if (!buttons) return;
	buttonsCount = 0;
	...
	{
		pfolder = &BOOKMARKS[i];
		pfb = &buttons[buttonsCount];
		switch(BOOKMARKS[i].pathtype)
		{
			case PATHTYPE_PIDL:  
				pfb->pidl = sh32_ILClone((LPITEMIDLIST)pfolder->path);  // need to copy it
				break;
			case PATHTYPE_CSIDL:  
...
void ScanFolderBrowser::FreeBookmarks(void)
{
	...
	if (buttons)
	{
		...
		while(buttonsCount--)
		{
			...
			if (pbtn->pidl) { sh32_ILFree(pbtn->pidl); pbtn->pidl = NULL; }
			pbtn++;
		}
		free(buttons);
	}
...
void ScanFolderBrowser::OnInitialized(void)
{
	...
	if (ctrlWnd)
	{
		...
		LPTBBUTTON ptbb;
		ptbb = (LPTBBUTTON)malloc(buttonsCount*sizeof(TBBUTTON));
		ZeroMemory(ptbb, sizeof(TBBUTTON)*buttonsCount);
...
void ScanFolderBrowser::OnSelectionChanged(LPCITEMIDLIST pidl)
{
	for(int i =0; i < buttonsCount; i++)
	{
		::SendMessage(GetDlgItem(IDC_TLB_BOOKMARKS), TB_CHECKBUTTON, (WPARAM)TOOLBAR_BTN_STARTID + i, sh32_ILIsEqual(pidl, buttons[i].pidl));
	}
...
{
	wchar_t buffer[2048], titleStr[64];
...
void ScanFolderBrowser::ShiftWindows(int cx)
{
	...
	hwnd = GetWindow(GetHandle(), GW_CHILD);
	while(hwnd)
	{
...
static BOOL LoadShell32(void)
{
	shell32Dll = LoadLibraryW(L"shell32.dll");
	if (!shell32Dll) return FALSE;

	sh32_SHAlloc = (SH32_SHALLOC) GetProcAddress(shell32Dll, (const char *)196); 
	sh32_SHFree	= (SH32_SHFREE) GetProcAddress(shell32Dll, (const char *)195);
	sh32_ILFree = (SH32_ILFREE) GetProcAddress(shell32Dll, (const char *)155);
	sh32_ILClone = (SH32_ILCLONE) GetProcAddress(shell32Dll, (const char *)18);
	sh32_ILIsEqual = (SH32_ILISEQUAL) GetProcAddress(shell32Dll, (const char *)21);
	sh32_ILIsParent = (SH32_ILISPARENT) GetProcAddress(shell32Dll, (const char *)23);

	if (!sh32_SHAlloc || !sh32_SHFree || !sh32_ILFree || !sh32_ILClone || !sh32_ILIsEqual || !sh32_ILIsParent) UnloadShell32();
	return (NULL != shell32Dll);
}
