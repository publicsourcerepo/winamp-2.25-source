...
BOOL IPC_HookExtInfoW(INT_PTR param)
{
	...
	if (!g_config->ReadInt("newtitle", 1))
		return FALSE;
	...
	int which = 0;
	if (!_wcsicmp(p->metadata, L"artist"))	which = 1;
	else if (!_wcsicmp(p->metadata, L"title"))	which = 2;
	else if (!_wcsicmp(p->metadata, L"album"))	which = 3;
	//else if (!_wcsicmp(p->metadata, L"tuid")) which = 4;
	else if (!_wcsicmp(p->metadata, L"track"))	which = 5;
	else if (!_wcsicmp(p->metadata, L"rating")) which = 6;
	else if (!_wcsicmp(p->metadata, L"playcount")) which = 7;
	else if (!_wcsicmp(p->metadata, L"GracenoteFileID")) which=8;
	else if (!_wcsicmp(p->metadata, L"ispodcast")) which=9;
	...
	if (which)
	{
		...
		if (m_db_found == 1)
		{
			...
			{
				wsprintfW(p->ret, L"%d", m_lasttrack);
				return 1;
			}
			break;
			case 6:
			...
			if (m_playcount != -1)
			{
				wsprintfW(p->ret, L"%d", m_playcount);
				return 1;
			}
			break;
			case 8:
			...
			if (m_ispodcast != -1)
			{
				wsprintfW(p->ret, L"%d", m_ispodcast);
				return 1;
			}
			break;
			}
...
DWORD doGuessProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
	...
	if (NULL != p && 
		(!ret || !p->ret[0]) && 
		g_config->ReadInt("guessmode", 0) != 2)
	{
		...
		if (which)
		{
          ...
          WideCharToMultiByteSZ(CP_ACP, 0, m_lastalbum, -1, p->ret, p->retlen, 0, 0);
				...
				{
					wsprintf(p->ret, "%d", m_lasttrack);
					return 1;
				}
				if (which == 6 && m_rating != -1)
			{
					wsprintf(p->ret, "%d", m_rating);
				return 1;
			}
			if (which == 7 && m_playcount != -1)
			{
					wsprintf(p->ret, "%d", m_playcount);
				return 1;
			}
			}
		}
