...
void AlbumFilter::SortResults(C_Config *viewconf, int which, int isfirst) // sorts the results based on the current sort mode
{
	...
	{
		char buf[64];
		wsprintf(buf,"%s_sort_by_%d",GetConfigId(),which);
		m_sort_by=viewconf->ReadInt(buf,0);
		wsprintf(buf,"%s_sort_dir_%d",GetConfigId(),which);
...
static void readConf(C_Config * c, int col, int * width, int * pos, bool * hidden, int defwidth, int defpos, bool defhidden, char * confid) {
	...
	char b[100];
	wsprintf(b,"%s_col_%s",confid,name);
	*width = c->ReadInt(b,defwidth);
	wsprintf(b,"%s_col_%s_pos",confid,name);
	*pos = c->ReadInt(b,defpos);
	wsprintf(b,"%s_col_%s_hidden",confid,name);
	*hidden = !!c->ReadInt(b,defhidden);
}
...
static void saveConf(C_Config * c, int col, int defwidth, int defpos, bool defhidden, bool exists, char * confid) {
	...
	if(exists) {
		wsprintf(b,"%s_col_%s",confid,name);
		c->WriteInt(b,defwidth);
		wsprintf(b,"%s_col_%s_pos",confid,name);
		c->WriteInt(b,defpos);
	}
	wsprintf(b,"%s_col_%s_hidden",confid,name);
	c->WriteInt(b,defhidden);
}
...
void AlbumFilter::AddColumns2() {
	...
	readConf(g_view_metaconf,4,&width,&pos,&hidden,77,4,!g_config->ReadInt("show_albumgain", 0),config);
...
void AlbumFilter::CustomizeColumns(HWND parent, BOOL showmenu) {
	...
	g_config->WriteInt("show_albumgain", v);
}