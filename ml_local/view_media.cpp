...
void playFiles(int enqueue, int all)
{
	...
	if (!enqueue && !all && g_config->ReadInt("viewplaymode", 1))
	{
		int selcnt = 0;
		for (int i = 0;i < l;i++)
		{
	...
	if (cnt)
	{
		if (foo_selected >= 0)
		{
			SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, foo_selected, IPC_SETPLAYLISTPOS);
			SendMessage(lMedia.hwndWinampParent, WM_COMMAND, 40047, 0); // stop button, literally
			SendMessage(lMedia.hwndWinampParent, WM_COMMAND, 40045, 0); // play button, literally
		}
		else if (!enqueue) SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, 0, IPC_STARTPLAY);
	}
}
...
static void RecycleSelectedItems()
{
	...
	fileOp.pFrom = 0;
	fileOp.pTo = 0;
	fileOp.fFlags = SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, 0, IPC_USES_RECYCLEBIN) ? FOF_ALLOWUNDO : 0;
		...
		if (resultlist.GetSelected(i))
		{
			if (NDE_Scanner_LocateNDEFilename(s, MAINTABLE_ID_FILENAME, FIRST_RECORD, itemCache.Items[i].filename))
			{
				...
				if (!hasdel && MessageBoxW(m_hwnd, WASABI_API_LNGSTRINGW(IDS_SURE_YOU_WANT_TO_REMOVE_SELECTED_FROM_LIBRARY),
				                          WASABI_API_LNGSTRINGW_BUF(IDS_CONFIRMATION,conf,32), MB_YESNO) != IDYES)
				{
					NDE_Table_DestroyScanner(g_table, s);
					LeaveCriticalSection(&g_db_cs);
					return ;
...
static void initColumnsHeader(HWND hwndList)
{
	...
	sortby = g_view_metaconf->ReadInt("mv_sort_by", 1);
	...
	for (index = 0; columnOrder[index] != -1; index++)
	{
		...
		lvc.cx = g_view_metaconf->ReadInt(cl->config_name, cl->defWidth);
		...
		if (columnOrder[index] == MEDIAVIEW_COL_CLOUD)
		{
			if (!cloud_hinst || cloud_hinst == (HINSTANCE)1 ||
				!SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, 0, IPC_GET_CLOUD_ACTIVE))
			{
			...
			else
			{
				...
				lvc.cx = 27;
				MLCloudColumn_GetWidth(lMedia.hwndLibraryParent, &lvc.cx);
			}
		}

		SendMessageW(hwndList, LVM_INSERTCOLUMNW, 0xFFFF, (LPARAM)&lvc);
	
		if (sortby == columnOrder[index]) MLSkinnedListView_DisplaySort(hwndList, index, !g_view_metaconf->ReadInt("mv_sort_dir", 0));
	}
	...
	case WM_COMMAND:
		switch(LOWORD(wParam))
		{
			...
			case IDCANCEL:
				...
				{
					if (IsDlgButtonChecked(hwndDlg, IDC_CHECK1)) g_config->WriteInt("noshowadddlg", 1);
					EndDialog(hwndDlg, 0);
				}
				break;
			case ID_ADD_FILES:
			...
			case IDC_BTN_LINK_PROMO:
				if (BN_CLICKED == HIWORD(wParam)) ShellExecuteA(lMedia.hwndWinampParent, "open", "http://www.winamp.com/player/overview", NULL, ".", 0);
				break;
		}
		break;
	case WM_TIMER:
	...
	if (iItem < (UINT)itemCache.Size)
	{
		if (g_table && newRating != itemCache.Items[iItem].rating)
		{
			...
			if (NDE_Scanner_LocateNDEFilename(s, MAINTABLE_ID_FILENAME, FIRST_RECORD, itemCache.Items[iItem].filename))
			{
				...
				if (g_config->ReadInt("writeratings", 0))
				{
					wchar_t buf[64];
					if (newRating > 0)
					{
						...
						buf[0] = 0;
					updateFileInfo(itemCache.Items[iItem].filename, L"rating", buf);
					SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, 0, IPC_WRITE_EXTENDED_FILE_INFO);
				}
			}
			NDE_Table_DestroyScanner(g_table, s);
		...
		if (newRating == itemCache.Items[iItem].rating && hwndList)
		{
			...
			MLRatingColumn_Animate(lMedia.hwndLibraryParent, &ratingColumn);
...
static BOOL Header_OnItemChanging(HWND hwndDlg, NMHEADERW *phdr, LRESULT *pResult, UINT uMsg)
{
	if (phdr->pitem && (HDI_WIDTH & phdr->pitem->mask))
	{
		...
		if (MEDIAVIEW_COL_RATING == columnOrder[phdr->iItem])
		{
			...
			if (MLRatingColumn_GetWidth(lMedia.hwndLibraryParent, &rw)) phdr->pitem->cxy = rw.width;
			if (0 == phdr->iItem)
			{
				RATINGBACKTEXT rbt;
				...
				MLRatingColumn_FillBackString(lMedia.hwndLibraryParent, &rbt);
			}
		...
		else if (MEDIAVIEW_COL_CLOUD == columnOrder[phdr->iItem])
		{
			...
			else
			{
				...
				if (MLCloudColumn_GetWidth(lMedia.hwndLibraryParent, &width))
				{
...
static BOOL Header_OnRightClick(HWND hwndDlg, NMHDR *pnmh, LRESULT *pResult)
{
	...
	POINT p;
	GetCursorPos(&p);
	int r = Menu_TrackPopup(menu, TPM_RETURNCMD | TPM_RIGHTBUTTON | TPM_LEFTBUTTON, p.x, p.y, hwndDlg, NULL);
	switch (r)
	{
...
static BOOL ListView_OnItemChanged(HWND hwndDlg, NMLISTVIEW *pnmv)
{
	...
	else
	{
		if (isMixablePresent)
		{
			SetDlgItemText(hwndDlg, IDC_MIXABLE, "");
...
static BOOL ListView_OnKeyDown(HWND hwndDlg, NMLVKEYDOWN *pnkd)
{
	...
	switch (pnkd->wVKey)
	{
		case '3':
			if (GetAsyncKeyState(VK_MENU)&0x8000)
			{
				...
				PostMessageW(hwndDlg, WM_NEXTDLGCTL, (WPARAM)pnkd->hdr.hwndFrom, (LPARAM)TRUE);
			}
			break;
		case 'A':
			if (GetAsyncKeyState(VK_CONTROL))
			{
				...
				SendMessageW(pnkd->hdr.hwndFrom, LVM_SETITEMSTATE, (WPARAM)-1, (LPARAM)&item);
			}
			break;
		case 'E':
			if (GetAsyncKeyState(VK_CONTROL) && resultlist.GetSelectedCount() > 0)
			{
				editInfo(hwndDlg);
				PostMessageW(hwndDlg, WM_NEXTDLGCTL, (WPARAM)pnkd->hdr.hwndFrom, (LPARAM)TRUE);
			}
...
static void Dialog_OnContextMenu(HWND hwndDlg, HWND hwndFrom, int x, int y)
{
	...
	IPC_LIBRARY_SENDTOMENU = SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, (WPARAM)&"LibrarySendToMenu", IPC_REGISTER_WINAMP_IPCMESSAGE);
	if (IPC_LIBRARY_SENDTOMENU > 65536 && SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, (WPARAM)0, IPC_LIBRARY_SENDTOMENU) == (LRESULT)-1)
	{
		s.mode = 1;
		s.hwnd = hwndDlg;
	...
	if (n != -1)
	{
		...
		// (BigG): Check the ini settings for viewing vs playing and create the menu accordingly
		// Keeping this here in case we want to use the ini setting:
		//StringCchPrintfW(str, 2048, WASABI_API_LNGSTRINGW( (g_viewnotplay != 0) ? IDS_VIEW_ALL_FILES_FROM : IDS_PLAY_ALL_FILES_FROM), album ? album : L"");	
	...
	SendMessage(lMedia.hwndLibraryParent, WM_ML_IPC, (WPARAM)&message_build, ML_IPC_SEND_PLUGIN_MESSAGE);
	...
	SendMessage(lMedia.hwndLibraryParent, WM_ML_IPC, (WPARAM)&message_result, ML_IPC_SEND_PLUGIN_MESSAGE);
	...
	switch (r)
	{
					...
					if (NDE_Scanner_LocateNDEFilename(s, MAINTABLE_ID_FILENAME, FIRST_RECORD, itemCache.Items[x].filename))
					{
						...
						if (g_config->ReadInt("writeratings", 0))
						{
							...
							if (rate > 0)
							{
								...
								buf[0] = 0;
							updateFileInfo(itemCache.Items[x].filename, L"rating", buf);
							SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, 0, IPC_WRITE_EXTENDED_FILE_INFO);
						}
					}
				}
		...
		default:
		{
			...
			{
				s.menu_id = r;
				if (SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, (WPARAM)&s, IPC_LIBRARY_SENDTOMENU) == (LRESULT)-1)
				{
					// build my data.
					s.mode = 3;
					...
					s.data = (void*) & myObj;
					LRESULT result = SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, (WPARAM) & s, IPC_LIBRARY_SENDTOMENU);
					if (result != 1)
					{
						s.mode = 3;
						s.data_type = ML_TYPE_ITEMRECORDLIST;
...
static BOOL ListView_OnFindItem(HWND hwndDlg, NMLVFINDITEMW *pfi, LRESULT *pResult, UINT uMsg)
{
	...
	int by = g_view_metaconf->ReadInt("mv_sort_by", MEDIAVIEW_COL_ARTIST);
...
{
	int l_sc = g_view_metaconf->ReadInt("mv_sort_by", MEDIAVIEW_COL_ARTIST);
	int l_sd = g_view_metaconf->ReadInt("mv_sort_dir", 0);
	...
	g_view_metaconf->WriteInt("mv_sort_by", l_sc);
	g_view_metaconf->WriteInt("mv_sort_dir", l_sd);
	g_view_metaconf->Flush();
...
static BOOL ListView_OnBeginDrag(HWND hwndDlg, NMLISTVIEW *pnmv)
{
	switch (columnOrder[pnmv->iSubItem])
	{
		case MEDIAVIEW_COL_RATING:
			...
			MLRatingColumn_BeginDrag(lMedia.hwndLibraryParent, &ratingColumn);
		break;
	}
...
{
	SendMessage(hwndDlg, WM_COMMAND, ((!!(GetAsyncKeyState(VK_SHIFT)&0x8000)) ^(!!g_config->ReadInt("enqueuedef", 0)))
		...
		case(CDDS_SUBITEM | CDDS_ITEMPREPAINT):
			switch (columnOrder[plvcd->iSubItem])
			{
				case MEDIAVIEW_COL_RATING:
					...
					if (MLRatingColumn_Paint(lMedia.hwndLibraryParent, &ratingColumnPaint))
					{
						*pResult = CDRF_SKIPDEFAULT;
						return TRUE;
					}
				...
				case MEDIAVIEW_COL_CLOUD:
					...
					if (MLCloudColumn_Paint(lMedia.hwndLibraryParent, &cloudColumnPaint))
					{
						*pResult = CDRF_SKIPDEFAULT;
						return TRUE;
					}
	...
	if (pnmitem->iItem != -1)
	{
		switch (columnOrder[pnmitem->iSubItem])
		{
			case MEDIAVIEW_COL_RATING:
				...
				ratingColumn.fStyle = RCS_DEFAULT;
				if (!MLRatingColumn_Click(lMedia.hwndLibraryParent, &ratingColumn)) return FALSE;
			...
			case MEDIAVIEW_COL_CLOUD:
			{
				...
				if (cloud_menu)
				{
					int r = Menu_TrackPopup(cloud_menu, TPM_RETURNCMD | TPM_RIGHTBUTTON | TPM_LEFTBUTTON, itemRect.right, itemRect.top, pnmitem->hdr.hwndFrom, NULL);
...
static BOOL ListView_OnHotTrack(HWND hwndDlg, NMLISTVIEW *pnmlv, LRESULT *pResult)
{
	...
	switch (columnOrder[pnmlv->iSubItem])
	{
		case MEDIAVIEW_COL_RATING:
			...
			ratingColumn.bRedrawNow = TRUE;
			MLRatingColumn_Track(lMedia.hwndLibraryParent, &ratingColumn);
		break;
	}
		...
		switch (l->code)
		{
			...
			case TTN_NEEDTEXTW:
			{
				...
				if (lvh.iItem != -1 && lvh.iSubItem == cloudcol)
				{
					...
					{
						if (!cloud_hinst) cloud_hinst = (HINSTANCE)SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, 0, IPC_GET_CLOUD_HINST);
						if (cloud_hinst && cloud_hinst != (HINSTANCE)1)
						{
...
static INT_PTR Dialog_OnInit(HWND hwndDlg, HWND hwndFocus, LPARAM lParam)
{
	...
	if (hwndList)
	{
		...
		styleEx = LVS_EX_DOUBLEBUFFER | LVS_EX_HEADERDRAGDROP | LVS_EX_ONECLICKACTIVATE; /*LVS_EX_ONECLICKACTIVATE - needed to hottracking work prior WinXp */
		SendMessageW(hwndList, LVM_SETEXTENDEDLISTVIEWSTYLE, styleEx, styleEx);

		hwndHeader = (HWND)SendMessage(hwndList, LVM_GETHEADER, 0, 0L);
		if (hwndHeader) SetWindowLongPtrW(hwndHeader, GWLP_ID, IDC_LIST2HEADER);
		...
		MLSkinWindow(lMedia.hwndLibraryParent, &skin);
	}
	...
	int cloudcol = -1;
	int l = g_view_metaconf->ReadInt("nbcolumns", 0);
	if (l)
	{
		...
		{
			char tmp[128];
			StringCchPrintf(tmp, 128, "column%d", index);
	...
	if (cloudcol == -1 && !g_view_metaconf->ReadInt("cloud", 1))
	{
		g_view_metaconf->WriteInt("cloud", 1);
		for(int i = l; i != 0; i--)
		{
	...
	pszTextA = (g_config->ReadInt("remembersearch", 0)) ? g_view_metaconf->ReadString("lastquery_utf8", "") : "";
	...
	if (g_table && !NDE_Table_GetRecordsCount(g_table) && !g_config->ReadInt("noshowadddlg", 0))
	{
...
//	pszTextA = (char *)SENDMLIPC(lMedia.hwndLibraryParent, ML_IPC_SEND_PLUGIN_MESSAGE, (WPARAM)&p);
//	predixisExist = (pszTextA != NULL && pszTextA[0] != 0);
// predixis out  - end
	if (!predixisExist)
	{
		ShowWindow(GetDlgItem(hwndDlg, IDC_BUTTON_MIX), SW_HIDE);
		ShowWindow(GetDlgItem(hwndDlg, IDC_MIXABLE), SW_HIDE);
	}
...
static BOOL Dialog_OnNotify(HWND hwndDlg, INT idCtrl, NMHDR* pnmh, LRESULT *pResult)
{
	...
	case IDC_LIST2:
		...
		case LVN_KEYDOWN:		return ListView_OnKeyDown(hwndDlg, (NMLVKEYDOWN*)pnmh);
...
static void Dialog_OnMouseMove(HWND hwndDlg, UINT nFlags, POINTS pts)
{
	if (GetCapture() == hwndDlg)
	{
		...
		if (MLRatingColumn_Drag(lMedia.hwndLibraryParent, &m.p))  return;
...
static void Dialog_OnLButtonUp(HWND hwndDlg, UINT nFlags, POINTS pts)
{
	if (GetCapture() == hwndDlg)
	{
		...
		if (MLRatingColumn_EndDrag(lMedia.hwndLibraryParent, &ratingColumn))
		{
			SetRating(ratingColumn.iItem, ratingColumn.value, ratingColumn.hwndList);
			return;
		}
...
static void Dialog_OnCommand(HWND hwndDlg, UINT idCtrl, INT nCode, HWND hwndCtrl)
{
	...
	case IDC_BUTTON_ENQUEUE:
	{
		...
		playFiles(idCtrl == IDC_BUTTON_ENQUEUE, i == l);
	}
	break;
	case IDC_BUTTON_CREATEPLAYLIST:
#if 0
	...
	{		
		...
		if (AGAVE_API_PLAYLISTMANAGER->Save(fn, &playlist) == PLAYLISTMANAGER_SUCCESS)
		{
			...
			SendMessage(lMedia.hwndLibraryParent,WM_ML_IPC,(WPARAM)&p,ML_IPC_PLAYLIST_ADD);
			DeleteFileW(fn);
		}
	}
#endif
	...
	case IDC_BUTTON_MIX:
	{
		...
		SendMessage(lMedia.hwndLibraryParent, WM_ML_IPC, (WPARAM)&p, ML_IPC_SEND_PLUGIN_MESSAGE);
	...
	case 6600:
		...
		if (m_last_selitem >= 0 && m_last_selitem < itemCache.Size)
		{
			if (predixisExist)
			{
				...
				else SetDlgItemText(hwndDlg, IDC_MIXABLE, "");
			}
...
static void Dialog_OnDestroy(HWND hwndDlg)
{
	...
	if (hwndList && IsWindow(hwndList))
	{
		for (i = 0; columnOrder[i] != -1; i++)
		{
			...
			g_view_metaconf->WriteInt(cl->config_name, SendMessageW(hwndList, LVM_GETCOLUMNWIDTH, i, 0L));
		}
	}

	//Save the column order
	for (i = 0; columnOrder[i] != -1; i++);
	g_view_metaconf->WriteInt("nbcolumns", i);
	for (j = 0; j < i; j++)
	{
		char tmp[128];
		StringCchPrintf(tmp, 128, "column%d", j);
...
static void Dialog_OnSyncHeaderOrder(HWND hwndDlg, HWND hwndHeader)
{
	...
	LVCOLUMNW column;
	wchar_t buffer[128];
	INT count, order, index, *pOrder, sort;
	signed char tempOrder[MAX_COLUMN_ORDER];
	...
	sort = MLSkinnedListView_GetSort(hwndList);
	count = (INT)SendMessageW(hwndHeader, HDM_GETITEMCOUNT, 0, 0L);
	if (count > 0)
	{
		index = count + 1;
		pOrder = (INT*)malloc(sizeof(INT)*count);
			...
			for (order = 0; order < count; order++)
			{
				...
				if (tempOrder[pOrder[order]] == MEDIAVIEW_COL_CLOUD)
				{
					...
					else
					{
						...
						column.cx = 27;
						MLCloudColumn_GetWidth(lMedia.hwndLibraryParent, &column.cx);
					}
				}
...
INT_PTR CALLBACK view_mediaDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_APP + 3:      // send by bgthread
		...
		else if (wParam == 0x69)
		{
			...
			ms = (UINT)(querytime.QuadPart * 1000 / freq.QuadPart);
	...
	case WM_USER + 32:
	{
		for (int i = 0; edit_columnOrder[i] != -1; i++)
		{
			...
			if (column_id == IDS_CLOUD || column_id == IDS_CLOUD_HIDDEN)
			{
				column_id = ((!cloud_hinst || cloud_hinst == (HINSTANCE)1 ||
							 !SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, 0, IPC_GET_CLOUD_ACTIVE)) ? IDS_CLOUD_HIDDEN : IDS_CLOUD);
			}
		...
		for (int i = 0; i < sizeof(columnList) / sizeof(headerColumn); i++)
		{
			...
			if (edit_columnOrder[j] == -1)
			{
				...
				SendMessage(m_availlistbox_hwnd, LB_SETITEMDATA, r, i);
			}
		}
	}
	break;
			...
			//remove column
		{
			for (int i = 0;i < SendMessage(m_curlistbox_hwnd, LB_GETCOUNT, 0, 0);i++)
			{
				if (SendMessage(m_curlistbox_hwnd, LB_GETSEL, i, 0))
				{
					...
					SendMessage(m_availlistbox_hwnd, LB_SETITEMDATA, r, c);
				}
			}
...
bool isMixable(itemRecordW &song)
{
	...
	char *text = (char *)SendMessage(lMedia.hwndLibraryParent, WM_ML_IPC, (WPARAM) & p, ML_IPC_SEND_PLUGIN_MESSAGE);
