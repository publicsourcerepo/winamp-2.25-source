/*
** Copyright (C) 2003-2007 Nullsoft, Inc.
**
...
void allocRecordList(itemRecordList *obj, int newsize, int granularity)
{
	...
	obj->Items=(itemRecord*)_aligned_realloc(obj->Items,sizeof(itemRecord)*obj->Alloc, 16);
...
void compactRecordList(itemRecordListW *obj)
{
	...
	{
		obj->Alloc = obj->Size;
		obj->Items = (itemRecordW *)_aligned_realloc(obj->Items, sizeof(itemRecordW) * obj->Alloc, 16);
	}
}
...
void convertRecord(itemRecord *output, const itemRecordW *input)
{
	...
	// copy across extended values so legacy support is more complete
...
void convertRecord(itemRecordW *output, const itemRecord *input)
{
	...
	COPY_EXTENDED_INT(bpm);
	COPY_EXTENDED_INT(bitrate);
	// TODO: copy input's extended fields or just force 'realsize' across
	// copy across extended values so legacy support is more complete
}
#undef COPY_EXTENDED_STR
#undef COPY_EXTENDED_INT
