...
int Init()
{
	...
	waMenuID = (WORD)SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, 0, IPC_REGISTER_LOWORD_COMMAND);
	WASABI_API_SVC = (api_service *)SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, 0, IPC_GET_API_SERVICE);
	if (!WASABI_API_SVC || WASABI_API_SVC == (api_service *)1) return ML_INIT_FAILURE;

	Tataki::Init(WASABI_API_SVC);

	WASABI_API_SVC->service_register(&mldbApiFactory);
	...
	WASABI_API_START_LANG(lMedia.hDllInstance,MlLocalLangGUID);
	...
	ML_IPC_MENUFUCKER_BUILD = (int)SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, (WPARAM)&"menufucker_build", IPC_REGISTER_WINAMP_IPCMESSAGE);
	ML_IPC_MENUFUCKER_RESULT = (int)SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, (WPARAM)&"menufucker_result", IPC_REGISTER_WINAMP_IPCMESSAGE);
	IPC_CLOUD_ENABLED = SendMessage(lMedia.hwndWinampParent, WM_WA_IPC, (WPARAM)&"WinampCloudEnabled", IPC_REGISTER_WINAMP_IPCMESSAGE);
...
void Quit()
{
	...
	WASABI_API_SVC->service_deregister(&mldbApiFactory);
	quit();
...
INT_PTR MessageProc(int message_type, INT_PTR param1, INT_PTR param2, INT_PTR param3)
{
	...
	case ML_MSG_NAVIGATION_CONTEXTMENU:
	{
		...
		HNAVITEM myItem = MLNavCtrl_FindItemById(lMedia.hwndLibraryParent, m_query_tree);
		if (hItem == myItem)
		{
	...
	case ML_MSG_TREE_ONKEYDOWN:
	{
		...
		{
			if (!ctrl)
				addNewQuery(lMedia.hwndLibraryParent);
			else
				if (!g_bgscan_scanning) SendMessage(lMedia.hwndLibraryParent, WM_USER + 575, 0xffff00dd, 0);

			PostMessageW(lMedia.hwndLibraryParent, WM_NEXTDLGCTL, (WPARAM)param3, (LPARAM)TRUE);
			return 1;
		}
		else if (m_query_list[param1])
		{
			...
			{
				queryDeleteItem(lMedia.hwndLibraryParent, param1);
			}
			else
				break;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
winampMediaLibraryPlugin lMedia = { MLHDR_VER, "nullsoft(ml_local.dll)", Init, Quit, MessageProc, 0, 0, 0, };
