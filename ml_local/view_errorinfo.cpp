...
static void LayoutWindows(HWND hwnd, BOOL fRedraw)
{
	...
	SetWindowPos(temp, NULL, 20, 20, rc.right - rc.left - 40, rc.bottom - rc.top - 45,
	...
	switch(uMsg)
	{
		case WM_INITDIALOG:
			SetWindowText(GetDlgItem(hwndDlg, IDC_DB_ERROR),
						  (char*)WASABI_API_LOADRESFROMFILE("TEXT", MAKEINTRESOURCE((nde_error ? IDR_NDE_ERROR : IDR_DB_ERROR)), 0));
