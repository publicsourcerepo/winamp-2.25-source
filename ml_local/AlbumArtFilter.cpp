...
AlbumArtFilter::AlbumArtFilter(HWND hwndDlg, int dlgitem, C_Config *c) : hwndDlg(hwndDlg), dlgitem(dlgitem),
		...
		hbmpNames(NULL), ratingHotItem(-1), bgBrush(NULL)
{
	mode = c->ReadInt("albumartviewmode",1);
	icons_only = c->ReadInt("albumarticonmode",0);
...
static COLORREF GetWAColor(INT index)
{
	...
	if (!wad_getColor) *(void **)&wad_getColor=(void*)SendMessage(lMedia.hwndLibraryParent,WM_ML_IPC,1,ML_IPC_SKIN_WADLG_GETFUNC);
...
ARGB32 * loadRrc(int id, char * sec, int *w, int *h, bool data=false)
{
	...
	HGLOBAL resourceHandle = WASABI_API_LOADRESFROMFILE(sec, MAKEINTRESOURCE(id), &size);
	if(resourceHandle)
	{
...
void AlbumArtFilter::drawArt(AlbumArtContainer *art, DCCanvas *pCanvas, RECT *prcDst, int itemid, int imageIndex)
{
	...
	if (res != AlbumArtContainer::DRAW_SUCCESS)
	{
		...
		if (!noart || noart->isInvalid())
		{
			...
			{
				int w, h;
				w = prcDst->right - prcDst->left;
				h = prcDst->bottom - prcDst->top;
				DrawRect(pCanvas->getHDC(), prcDst->left, prcDst->top, w, h);
				wchar_t str1[32], str2[32];
...
BOOL AlbumArtFilter::DrawItemIcon(NMLVCUSTOMDRAW *plvcd, DCCanvas *pCanvas, UINT itemState, RECT *prcClip, BOOL bWndActive)
{
	...
	SetRect(&rcText, plvcd->nmcd.rc.left, plvcd->nmcd.rc.bottom - (textHeight + 2), plvcd->nmcd.rc.right, plvcd->nmcd.rc.bottom - (icons_only ? (textHeight + 6) : 0));

	// background
	SetRect(&re, plvcd->nmcd.rc.left, plvcd->nmcd.rc.top, plvcd->nmcd.rc.right, (icons_only ? plvcd->nmcd.rc.bottom - (textHeight + 6) : rcText.top));
	...
	if (IntersectRect(&ri, &rcText, prcClip))
	{
		...
		if (p && *p)
		{
			SetRect(&ri, rcText.left + of, rcText.top + 1, rcText.right - of, rcText.bottom -1);
			DrawTextW(hdc, p, -1, &ri, DT_CENTER | DT_BOTTOM | DT_SINGLELINE | DT_WORD_ELLIPSIS | DT_NOPREFIX);
		}
	}
...
BOOL AlbumArtFilter::PrepareDetails(HDC hdc)
{
	...
	for (size_t i=0; i < showncolumns.size(); i++)
	{
		...
		int of = getStrExtent(hdcTmp, showncolumns[i]->name);
...
BOOL AlbumArtFilter::DrawItemDetail(NMLVCUSTOMDRAW *plvcd, DCCanvas *pCanvas, UINT itemState, RECT *prcClip, BOOL bWndActive, HDC hdcNames, INT namesWidth)
{
	...
	if (g_config->ReadInt("alternate_items", config_use_alternate_colors) && imageIndex==0 && plvcd->nmcd.dwItemSpec%2)
		imageIndex=3;
	...
	if (ri.left < ri.right)
	{
		for (size_t i=0; i < showncolumns.size() && ri.top < limCY; i++, ri.top += textHeight)
		{
			...
			p = CopyText2(plvcd->nmcd.dwItemSpec,i,buf,100);
			...
			if ((INT)i == ratingrow)  // this is the ratings column, so draw graphical stars
			{
				...
				MLRating_Draw(lMedia.hwndLibraryParent,&p);
			}
			else {
				//ri.left = re.left;
...
BOOL AlbumArtFilter::CalcuateItemHeight(void)
{
	...
	if (hdc)
	{
		hFont = (HFONT)SendMessageW(hwndList, WM_GETFONT, 0, 0L);
	...
	if (isDetailsMode(mode))
	{
		...
		RECT r;
		MLRating_CalcRect(lMedia.hwndLibraryParent, NULL, 5, &r);
		r.bottom -= r.top;
		...
		newHeight = max(h, textHeight * ((INT)showncolumns.size() - bypassColumnCount)) + 12;
	...
	else
	{
		...
		SendMessageW(hwndList, LVM_SETICONSPACING, 0, MAKELPARAM(w + HORZ_SPACING, h + (icons_only ? 0 : textHeight + 6) + VERT_SPACING));
...
INT_PTR AlbumArtFilter::DialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam)
{
	switch (uMsg)
	{
		...
		case WM_DISPLAYCHANGE:
		{
			...
			if(WM_DISPLAYCHANGE == uMsg) PostMessageW(GetDlgItem(hwndDlg,dlgitem),uMsg,wParam,lParam);
			else ShowWindow(GetDlgItem(hwndDlg,dlgitem), SW_SHOWNORMAL);
		}
			break;
		...
		case WM_USER+602:
			...
			if (lParam == 1)
			{
				...
				for (int x = 0; x < c; x ++)
				{
					if(list->GetSelected(x) && albumList.Items[x+1].art)
					{
						...
						d.showCancelAll = 1;
						int r = (int)SendMessage(lMedia.hwndWinampParent,WM_WA_IPC,(LPARAM)&d,IPC_FETCH_ALBUMART);
			...
			else if (lParam == 3)
			{
				...
				if (sel != LB_ERR)
				{
					...
					SendMessage(lMedia.hwndWinampParent,WM_WA_IPC,(LPARAM)url,IPC_OPEN_URL);
				}
			}
			break;
		...
		case WM_USER+700: // hit-info
			{
				...
				if(mode && info->hwnd == hwndRealList && info->item >= 0 && info->item < albumList.Size)
				{
					int w,h;
					getImgSize(mode,w,h);
					RATINGHITTESTPARAMS ratingHitTest = {sizeof(ratingHitTest),{info->x,info->y},{16+w+namesWidth,3+textHeight*ratingrow,0,0},5,RDS_NORMAL,NULL,-1,-1};
					ratingHitTest.rc.bottom = ratingHitTest.rc.top + textHeight;
					ratingHitTest.rc.right = ratingHitTest.rc.left + 200;
					
					LONG hitVal = MLRating_HitTest(lMedia.hwndLibraryParent,&ratingHitTest);
...
static int getWidth(int def, int col)
{
	...
	wsprintf(b,"av_art_col_%s",name);
	return g_view_metaconf->ReadInt(b,def);
}
...
void AlbumArtFilter::Fill(itemRecordW *items, int numitems, int *killswitch, int numitems2)
{
	...
	wchar_t albbuf[100],albbuf2[100];
	...
	wchar_t buf[64], sStr[16], langBuf[64];;
	if (isbl)
	{
...
{
	int mid = (r >> 16), newMode, w1, w2, h1, h2;
	...
	c->WriteInt("albumartviewmode",mode);
	c->WriteInt("albumarticonmode",icons_only);
	SaveColumnWidths();
...
{
	if(_wcsicmp(metaItem,L"rating")==0)
