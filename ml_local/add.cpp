...
{
	filename2[0]=0;
	GetLongPathName(filename, filename2, filename2_len);
	if (!strcmp(filename,filename2)) filename2[0]=0;
}
...
{
	// Do we need to make a copy of the filename pointer here?
	// ToDo: (BigG) need to move these to the correct function(s) for removal
	//WASABI_API_SYSCB->syscb_issueCallback(api_mldb::SYSCALLBACK, api_mldb::MLDB_CLEARED, 0, 0);
	...
	int type=0, the_length_sec=0;
	wchar_t m_artist[1024] = {0}, tmp[1024] = {0};

	if(getFileInfoW(filename, L"type",tmp,sizeof(tmp)/sizeof(wchar_t)))
	{
		if(tmp[0]) { type=_wtoi(tmp); db_setFieldInt(s,MAINTABLE_ID_TYPE,type); hasttype++; }
	}

	if (getFileInfoW(filename, L"length",tmp,sizeof(tmp)/sizeof(wchar_t)))
	{
		if(tmp[0]) { db_setFieldInt(s,MAINTABLE_ID_LENGTH,the_length_sec=(_wtoi(tmp)/1000)); hastlength++; }
	}

	if (getFileInfoW(filename, L"bitrate",tmp,sizeof(tmp)/sizeof(wchar_t)))
	{
		if(tmp[0]) { db_setFieldInt(s,MAINTABLE_ID_BITRATE,_wtoi(tmp)); hasbitrate++; }
	}

	if(use_metadata && getFileInfoW(filename,L"title",tmp,sizeof(tmp)/sizeof(wchar_t))) 
	{
		if(tmp[0]) { db_setFieldStringW(s,MAINTABLE_ID_TITLE,tmp); hasttitle++; }
		getFileInfoW(filename,L"artist",tmp,sizeof(tmp)/sizeof(wchar_t));
		if(tmp[0]) { StringCchCopyW(m_artist, 1024, tmp); db_setFieldStringW(s,MAINTABLE_ID_ARTIST,tmp); hastartist++; }
		getFileInfoW(filename,L"album",tmp,sizeof(tmp)/sizeof(wchar_t));
		if(tmp[0]) { db_setFieldStringW(s,MAINTABLE_ID_ALBUM,tmp); hastalbum++; }

		GetOptionalField(s, filename, L"comment", MAINTABLE_ID_COMMENT);

		getFileInfoW(filename,L"year",tmp,sizeof(tmp)/sizeof(wchar_t));
		...
		GetOptionalField(s, filename, L"genre", MAINTABLE_ID_GENRE);

		getFileInfoW(filename,L"track",tmp,sizeof(tmp)/sizeof(wchar_t));
		if(tmp[0])
		{
			int track, tracks;
		...
		getFileInfoW(filename,L"disc",tmp,sizeof(tmp)/sizeof(wchar_t));
		if(tmp[0])
		{
			int disc, discs;
		...
		getFileInfoW(filename,L"albumartist",tmp,sizeof(tmp)/sizeof(wchar_t));
		if(tmp[0]) { db_setFieldStringW(s,MAINTABLE_ID_ALBUMARTIST,tmp); hasalbumartist++; }
		GetOptionalField(s, filename, L"publisher", MAINTABLE_ID_PUBLISHER);
		GetOptionalField(s, filename, L"composer", MAINTABLE_ID_COMPOSER);
		GetOptionalField(s, filename, L"replaygain_album_gain", MAINTABLE_ID_ALBUMGAIN);
		GetOptionalField(s, filename, L"replaygain_track_gain", MAINTABLE_ID_TRACKGAIN);
		GetOptionalFieldInt(s, filename, L"bpm", MAINTABLE_ID_BPM);
		GetOptionalField(s, filename, L"GracenoteFileID", MAINTABLE_ID_GRACENOTEFILEID);
		GetOptionalField(s, filename, L"GracenoteExtData", MAINTABLE_ID_GRACENOTEEXTDATA);
		GetOptionalFieldInt(s, filename, L"lossless", MAINTABLE_ID_LOSSLESS);
		GetOptionalField(s, filename, L"category", MAINTABLE_ID_CATEGORY);
		GetOptionalField(s, filename, L"codec", MAINTABLE_ID_CODEC);
		GetOptionalField(s, filename, L"director", MAINTABLE_ID_DIRECTOR);
		GetOptionalField(s, filename, L"producer", MAINTABLE_ID_PRODUCER);
		GetOptionalFieldInt(s, filename, L"width", MAINTABLE_ID_WIDTH);
		GetOptionalFieldInt(s, filename, L"height", MAINTABLE_ID_HEIGHT);
		if (g_config->ReadInt("writeratings", 0))
			GetOptionalFieldInt(s, filename, L"rating", MAINTABLE_ID_RATING);
		else
			GetNonBlankFieldInt(s, filename, L"rating", MAINTABLE_ID_RATING);
		GetOptionalField(s, filename, L"mime", MAINTABLE_ID_MIMETYPE);
	}
	int guessmode=guess_mode;
	if (guessmode != 2 && 
        (
          (!hasttitle) + (!hastartist) + (!hastalbum) + (!hasttrack) >= 
          (g_guessifany ? 1 : 4)
        )
     )
	{
	...
	{
		wchar_t *p = PathFindFileNameW(filename);
		wchar_t *dup=_wcsdup(p);
	...
	if (!hasalbumartist)
	{
		if (hastartist && g_config->ReadInt("artist_as_albumartist", 1))
