...
{
	if (!scan_extlist) 
		scan_extlist=(wchar_t*)SendMessage(lMedia.hwndWinampParent,WM_WA_IPC,0,IPC_GET_EXTLISTW);
...
static void CountFolder(const wchar_t *folder, int recurse, HANDLE cancelswitch, volatile int *found)
{
	...
	{
		if (g_bgrescan_status_hwnd)
		{
			wchar_t status[150+MAX_PATH];
...
struct ScanFolderParams
{
	...
	{
		path = _wcsdup(_path);
		guess = _guess >= 0 ? _guess : g_config->ReadInt("guessmode",0);;
		meta = _meta >= 0 ? _meta : g_config->ReadInt("usemetadata",1);
		recurse = _recurse;
	}
	~ScanFolderParams()
	{
...
static VOID CALLBACK ScanFoldersAPC(ULONG_PTR param)
{
	...
	for (size_t i=0;i!=params->count;i++)
	{
		...
		int guess = params->guess[i] >= 0 ? params->guess[i] : g_config->ReadInt("guessmode",0);;
		int meta = params->meta[i] >= 0 ? params->meta[i] : g_config->ReadInt("usemetadata",1);
...
void Scan_ScanFolders(HWND parent, size_t count, wchar_t **paths, int *guess, int *meta, int *recurse)
{
	...
	if (g_table && ScanCreateThread())
	{
		...
		WASABI_API_DIALOGBOXPARAMW(IDD_ADDSTUFF,parent,ScanFileUI, (LPARAM)params);
...
void Scan_ScanFolder(HWND parent, const wchar_t *path, int guess, int meta, int recurse)
{
	...
	if (ScanCreateThread())
	{
		wchar_t **paths = (wchar_t **)malloc(sizeof(wchar_t*));
		int *guesses = (int *)malloc(sizeof(int));
		int *metas = (int *)malloc(sizeof(int));
		int *recs = (int *)malloc(sizeof(int));
		*guesses = guess;
		*metas = meta;
		*recs = recurse;
...
static VOID CALLBACK BackgroundScanAPC(ULONG_PTR param)
{
	...
	if (s)
	{
		...
		while (*p && !ScanCancelled(events, 2))
		{
			while (*p == L'|') p++;

			int use_metadata=g_config->ReadInt("usemetadata",1);
			int guess_mode=g_config->ReadInt("guessmode",0);
			int recurse=1;
			if (*p == L'<' && wcsstr(p,L">"))
			{
				p++;
	...
	if (!ScanCancelled(events, 2) && g_config->ReadInt("bgrescan_compact",1))
	{
		EnterCriticalSection(&g_db_cs);
...
void Scan_RemoveFiles(HWND parent) 
{
	...
	if (g_table && ScanCreateThread())
	{
		...
		WASABI_API_DIALOGBOXPARAMW(IDD_ADDSTUFF,parent, RemoveFilesUI, (LPARAM)params);
		PostMessage(lMedia.hwndWinampParent,WM_WA_IPC,NDE_Table_GetRecordsCount(g_table),IPC_STATS_LIBRARY_ITEMCNT);
	}
}