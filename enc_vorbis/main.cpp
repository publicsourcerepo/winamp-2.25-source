...
void readconfig(char *configfile, configtype *cfg)
{
	...
	if (configfile) GetPrivateProfileStruct("audio_ogg","conf",cfg,sizeof(configtype),configfile);
		cfg->cfg_mode=0; // VBR, fuckers.
}

void writeconfig(char *configfile, configtype *cfg)
{
	if (configfile) WritePrivateProfileStruct("audio_ogg","conf",cfg,sizeof(configtype),configfile);
}
...
extern "C"
{
	unsigned int __declspec(dllexport) GetAudioTypes3(int idx, char *desc)
	{
		if (idx==0)
		{
			...
			StringCchPrintf(desc, 1024, "%s %s (aoTuV b6.03)", WASABI_API_LNGSTRING(IDS_ENC_VORBIS_DESC), ENC_VERSION);
	...
	BOOL CALLBACK DlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam) 
	{
		if (uMsg == WM_USER+667)
		{
			int p=SendDlgItemMessage(hwndDlg,IDC_VBRQUALITY,TBM_GETPOS,0,0)-10;
			char tmp[512] = {0};
			...
			StringCchPrintf(tmp, 512, WASABI_API_LNGSTRING(IDS_QUALITY_FACTOR_F_KBPS), dispVal, br);

			SetDlgItemText(hwndDlg,IDC_VBRVAL,tmp);
		...
		else if (uMsg == WM_COMMAND)
		{
			...
			{
				SendMessage(winampwnd, WM_WA_IPC, (WPARAM)"http://www.geocities.jp/aoyoume/aotuv/", IPC_OPEN_URL);
			}
		}
