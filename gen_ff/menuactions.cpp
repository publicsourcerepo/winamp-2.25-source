...
static LRESULT sendMlIpc(int msg, WPARAM param) {
	...
	if(!mlwnd) {
		int IPC_GETMLWINDOW = (INT)SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&"LibraryGetWnd", IPC_REGISTER_WINAMP_IPCMESSAGE);
		if(IPC_GETMLWINDOW > 65536) mlwnd = (HWND)SendMessage(plugin.hwndParent,WM_WA_IPC,0,IPC_GETMLWINDOW);
	}
	if(!mlwnd) return 0;
	...
	case _ACTION_CONTROLMENU:
		{
			...
			TrackPopupMenu(ctrlmenu, TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON, p1, p2, 0, wa2.getMainWindow(), NULL);
			break;
		}
...
#define ID_RATING0                      40401
			...
			LRESULT IPC_LIBRARY_SENDTOMENU = SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&"LibrarySendToMenu", IPC_REGISTER_WINAMP_IPCMESSAGE);
			HMENU menu = 0;
			memset(&mainSendTo, 0, sizeof(mainSendTo));
			if (IPC_LIBRARY_SENDTOMENU > 65536 && SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)0, IPC_LIBRARY_SENDTOMENU) == 0xffffffff)
			{
				char stStr[32];
				MENUITEMINFO mii = {sizeof(mii), MIIM_SUBMENU | MIIM_TYPE, MFT_STRING, };
				...
				InsertMenuItem(songinfomenu, 3, TRUE, &mii);
			...
			pluginMessage message_build = {SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&"menufucker_build", IPC_REGISTER_WINAMP_IPCMESSAGE),(intptr_t)&mf,0};
			sendMlIpc(ML_IPC_SEND_PLUGIN_MESSAGE,(WPARAM)&message_build);
			
			int ret = TrackPopupMenu(songinfomenu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON|TPM_RETURNCMD, p1, p2, 0, wa2.getMainWindow(), NULL);
			
			pluginMessage message_result = {SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&"menufucker_result", IPC_REGISTER_WINAMP_IPCMESSAGE),(intptr_t)&mf,ret,0};
			...
			if (menu)
			{
				if (mainSendTo.mode == 2)
				{
					...
					if (SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&mainSendTo, IPC_LIBRARY_SENDTOMENU) == 0xffffffff)
					{
						wchar_t buf[1041];
						wchar_t *end=buf;
						size_t endSize;
						StringCchCopyExW(buf, 1040, wa2.getFileW(wa2.getCurPlaylistEntry()), &end, &endSize, 0);
						end[1]=0; // double null terminate (since we passed 1040 for a 1041 size buffer, this is always safe)

						mainSendTo.mode = 3;
						mainSendTo.data = buf;
						mainSendTo.data_type = ML_TYPE_FILENAMESW;
						SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&mainSendTo, IPC_LIBRARY_SENDTOMENU);
					}
				}
				// remove sendto
			...
			{
				mainSendTo.mode = 4;
				SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&mainSendTo, IPC_LIBRARY_SENDTOMENU); // cleanup
				memset(&mainSendTo, 0, sizeof(mainSendTo));
			}
			InsertMenuW(songinfomenu, 3, MF_BYPOSITION | MF_STRING, WINAMP_TOGGLE_AUTOSCROLL, olditemstr);
			if (ret) SendMessage(wa2.getMainWindow(), WM_COMMAND, ret, 0); // TODO?
	...
	case ACTION_SENDTO:
		{
			...
			int ret = TrackPopupMenu(menu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON|TPM_RETURNCMD, p1, p2, 0, wa2.getMainWindow(), NULL);
			if (menu)
			{
				if (mainSendTo.mode == 2)
				{
