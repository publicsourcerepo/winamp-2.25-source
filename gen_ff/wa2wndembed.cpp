...
{
	SendMessage(wa2.getWnd(IPC_GETWND_VIDEO), WM_TIMER, 12345, 0);
}
...
ifc_window *Wa2WndEmbed::createWindowByGuid(GUID g, ifc_window *parent)
{
		           ...
		           !memcmp(embedWndGuids.Data4,g.Data4,8))*/
	{
		...
		if (0 == (WS_BORDER & GetWindowLongPtr(ws->me, GWL_STYLE)))
		{
			RECT r = {11, 20, 8, 14};
			host->oswndhost_setRegionOffsets(&r);
		}
		...
		if (rw == w)
		{
			...
			if (!f.mustLeave())
			{
				...
				// in any case, we need to hide the window here, at least temporarily in the case of skin unloading
				{
					if (ee->whichwnd > 1024)
					{
						...
						hContent = (NULL != hHost) ? (HWND)SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)hHost, IPC_FF_GETCONTENTWND) : NULL;
						
						if (NULL != hContent)
						{
							ws = (embedWindowState *)GetWindowLongPtr(hContent, GWLP_USERDATA);
						}
						else
						{
						...
						{	
							...
							if (!x->isTransfering() && wa2.isValidEmbedWndState(ws))
							{
								...
								{
									SendMessage(ws->me, WM_USER + 101, 0, 0);
								}
			...
			if (ee->whichwnd > 1024 && ws)
			{
				if (forcedoffwnds.haveItem(ws->me))
				{
					...
					SetWindowPos(ws->me, NULL, r.left + 20000, r.top + 20000, 0, 0, SWP_NOSIZE | SWP_NOREDRAW | SWP_NOSENDCHANGING | SWP_NOZORDER);
...
int Wa2WndEmbed::embedRememberProc(embedWindowState *p, embedEnumStruct *parms)
{
	...
	if(going_freeform && !m_loading_at_startup)
	{
		...
		{
			WritePrivateProfileString("gen_ff", "classicmlwidth", StringPrintf("%d", ws->position.right - ws->position.left), INI_FILE);
			WritePrivateProfileString("gen_ff", "classicmlheight", StringPrintf("%d", ws->position.bottom - ws->position.top), INI_FILE);
		}
	}
	return 0;
}
...
void Wa2WndEmbed::rememberVisibleWindows()
{
	...
	for (int i = 0;i < sizeof(embedTable) / sizeof(embedTable[0]);i++)
	{
		...
		if (going_freeform)
		{
			if (embedTable[i] == IPC_GETWND_PE)
			{
				...
				if (!m_loading_at_startup)
				{
					WritePrivateProfileString("gen_ff", "classicplwidth", StringPrintf("%d", pewidth), INI_FILE);
					WritePrivateProfileString("gen_ff", "classicplheight", StringPrintf("%d", peheight), INI_FILE);
				}
				int classicpews = wa2.isWindowShade(IPC_GETWND_PE);
				if (!m_loading_at_startup || GetPrivateProfileInt("gen_ff", "classicplws", -1, INI_FILE) == -1)
					WritePrivateProfileString("gen_ff", "classicplws", classicpews ? "1" : "0", INI_FILE);
				if (classicpews)
					SendMessage(wa2.getMainWindow(), WM_COMMAND, WINAMP_OPTIONS_WINDOWSHADE_PL, 0);
				GetWindowRect(w, &ws->position);
			}
		}
		wa2wndstatus.addItem(ws);
	...
	SendMessage(wa2.getMainWindow(), WM_WA_IPC, (WPARAM)&cs, IPC_EMBED_ENUM);
}
	...
	for (int i = 0;i < n;i++)
	{
		...
		if (going_fixedform && !m_loading_at_startup)
		{
			...
			if(GetWindow(ws->wnd, GW_CHILD) == mlwnd)
			{
				...
				int mlwidth = GetPrivateProfileInt("gen_ff", "classicmlwidth", -1, INI_FILE);
				int mlheight = GetPrivateProfileInt("gen_ff", "classicmlheight", -1, INI_FILE);
