...
BOOL CALLBACK ffPrefsProc5(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
	case WM_INITDIALOG:
		...
		if (m_are_we_loaded)
		{
			if (WASABI_API_SKIN->group_exists(L"skin.about.group"))
			{
				...
				{
					SetDlgItemText(hwndDlg, IDC_STATIC_EMPTY, WASABI_API_LNGSTRING(IDS_ERROR_WHILE_LOADING_SKIN_WINDOW));
				}
			}
			else
			{
		...
		{
			SetDlgItemText(hwndDlg, IDC_STATIC_EMPTY, WASABI_API_LNGSTRING(IDS_NO_SKIN_LOADED));
		}
		return 1;
	case WM_DESTROY:
		destroyskinabout();
