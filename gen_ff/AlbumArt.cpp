...
void AlbumArt::ArtLoaded(int _w, int _h, ARGB32 *_bits)
{
	...
	if (!noMakiCallback)
	{
		// TODO when this has _bits=0 then it seems to trigger a crash
		AlbumArt::script_vcpu_onAlbumArtLoaded(
			SCRIPT_CALL, this->getScriptObject(),
			MAKE_SCRIPT_BOOLEAN((int)bits));
	}
}
...
int AlbumArt::corecb_onUrlChange(const wchar_t *filename)
{
	...
	if (forceRefresh || (!noAutoRefresh && isVisible()))
	{
		...
		if(WCSICMP(src_file, L"")) filename = src_file;
...
SkinBitmap *AlbumArt::getBitmap()
{
	...
	if (bits)
	{
		artBitmap = new HQSkinBitmap(bits, w, h);
		return artBitmap;
	}
...
{
	if (xuihandle != _xuihandle) return ALBUMART_PARENT::setXuiParam(_xuihandle, attrid, name, strval);
	switch (attrid)
	{
	...
	case ALBUMART_VALIGN:
		if (!WCSICMP(strval, L"top")) valign = -1;
		else if (!WCSICMP(strval, L"bottom")) valign = 1;
		else valign = 0;
		deferedInvalidate();
		break;
	case ALBUMART_ALIGN:
		if (!WCSICMP(strval, L"left")) align = -1;
		else if (!WCSICMP(strval, L"right")) align = 1;
		else align = 0;
		deferedInvalidate();
		break;
	case ALBUMART_STRETCHED:
		if (!WCSICMP(strval, L"0") || !WCSICMP(strval, L"")) stretched = false;
		else stretched = true;
		deferedInvalidate();
		break;
	case ALBUMART_NOREFRESH:
		if (!WCSICMP(strval, L"0") || !WCSICMP(strval, L"")) noAutoRefresh = false;
		else noAutoRefresh = true;
		break;
	default:
		return 0;
	}
