...
void fm_itemClicked(HWND wnd, int pos)
{
	...
	for (int i = 0;i < n;i++)
	{
		if (mappingList.GetSelected(i))
		{
			...
			int s = WTOI(scale);
			SendMessage(GetDlgItem(wnd, IDC_SLIDER_SCALE), TBM_SETPOS, 1, s);
...
void fm_newMapping(HWND wnd)
{
	...
	int v = (int)SendMessage(GetDlgItem(wnd, IDC_SLIDER_SCALE), TBM_GETPOS, 0, 0);
...
void fillFontList2(HWND list, int reset = 1)
{
	...
	foreach(fontlist)
	int idx = ComboBox_AddStringW(list, fontlist.getfor()->face.v());
	...
	case WM_INITDIALOG:
		{
			...
			fillFontList2(GetDlgItem(hwndDlg, IDC_COMBO_FONTS), 1);
			fillSkinFontList(GetDlgItem(hwndDlg, IDC_COMBO_SKINFONTS));
			fm_rescanList(lv);
			...
			SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALE), TBM_SETRANGEMAX, 0, 200);
			SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALE), TBM_SETRANGEMIN, 0, 25);
			SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALE), TBM_SETPOS, 1, 100);
			return 1;
		}
	case WM_COMMAND:
	...
	case WM_HSCROLL:
		{
			...
			{
				int t = (int)SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALE), TBM_GETPOS, 0, 0);
...
void updateBFReplacement(HWND hwndDlg)
{
	...
	EnableWindow(GetDlgItem(hwndDlg, IDC_COMBO_TTFOVERRIDE), replace);
...
BOOL CALLBACK ffPrefsProc2(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
	case WM_INITDIALOG:
		{
			...
			fillFontList(GetDlgItem(hwndDlg, IDC_COMBO_TTFOVERRIDE));
			fillFontList2(GetDlgItem(hwndDlg, IDC_COMBO_DEFAULTFONT), 1);
			int pos = -1;
			...
			ComboBox_SetCurSel(GetDlgItem(hwndDlg, IDC_COMBO_TTFOVERRIDE), pos);
			...
			ComboBox_SetCurSel(GetDlgItem(hwndDlg, IDC_COMBO_DEFAULTFONT), pos);

			SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALETTFOVERRIDE), TBM_SETRANGEMAX, 0, 200);
			SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALETTFOVERRIDE), TBM_SETRANGEMIN, 0, 25);
			SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALETTFOVERRIDE), TBM_SETPOS, 1, cfg_options_ttfoverridescale.getValueAsInt());
			SetDlgItemText(hwndDlg, IDC_STATIC_TTFSCALE, StringPrintf("%d%%", cfg_options_ttfoverridescale.getValueAsInt()));
			SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALEDEFAULTFONT), TBM_SETRANGEMAX, 0, 200);
			SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALEDEFAULTFONT), TBM_SETRANGEMIN, 0, 25);
			SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALEDEFAULTFONT), TBM_SETPOS, 1, cfg_options_defaultfontscale.getValueAsInt());
			SetDlgItemText(hwndDlg, IDC_STATIC_SCALEDEFAULTFONT, StringPrintf("%d%%", cfg_options_defaultfontscale.getValueAsInt()));
			...
			else
			{
				...
				ComboBox_AddString(charmaps, "Unicode");
				ComboBox_AddString(charmaps, "Apple Roman");
				ComboBox_AddString(charmaps, "Adobe Latin-1");
				ComboBox_AddString(charmaps, "Adobe Standard");
				ComboBox_AddString(charmaps, "Adobe Custom");
				ComboBox_AddString(charmaps, "Adobe Expert");
				ComboBox_AddString(charmaps, "SJIS");
				ComboBox_AddString(charmaps, "Big5");
				ComboBox_AddString(charmaps, "Wansung");
				ComboBox_AddString(charmaps, "Johab");
	...
	case WM_HSCROLL:
		{
			if ((HWND) lParam == GetDlgItem(hwndDlg, IDC_SLIDER_SCALETTFOVERRIDE))
			{
				...
				SetDlgItemText(hwndDlg, IDC_STATIC_TTFSCALE, StringPrintf("%d%%", t));
			...
			{
				int t = (int)SendMessage((HWND)lParam, TBM_GETPOS, 0, 0);
				SetDlgItemText(hwndDlg, IDC_STATIC_SCALEDEFAULTFONT, StringPrintf("%d%%", t));
	...
	case WM_TIMER:
		{
			if (wParam == 100)
			{
				...
				int t = (int)SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALETTFOVERRIDE), TBM_GETPOS, 0, 0);
				cfg_options_ttfoverridescale.setValueAsInt(t);
			}
			else if (wParam == 101)
			{
				KillTimer(hwndDlg, 101);
				int t = (int)SendMessage(GetDlgItem(hwndDlg, IDC_SLIDER_SCALEDEFAULTFONT), TBM_GETPOS, 0, 0);
