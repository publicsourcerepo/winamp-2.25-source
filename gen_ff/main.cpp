...
extern int lumidiff(int a, int b);
...
static void doplaylistcolors()
{
	...
	if (s.bm || s.elems) SendMessage(wa2.getMainWindow(), WM_WA_IPC, (WPARAM)&s, IPC_SETPLEDITCOLORS);
}
...
static ifc_window *createEmb(embedWindowState *ws, bool startHidden)
{
	...
	{		
		...
		SetWindowPos(ws->me, NULL, r.left - 20000, r.top - 20000, 0, 0, SWP_NOSIZE | SWP_NOREDRAW | SWP_NOSENDCHANGING | SWP_NOZORDER | SWP_NOCOPYBITS);
...
void onSkinSwitch()
{
	...
	buf[0] = 0;
	wchar_t *p = (wchar_t *)SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)buf, IPC_GETSKINW);
	{
		...
		if (!_waccess(t.getValue(), 0))
		{
			...
			else
			{
				...
				//SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&cs, IPC_EMBED_ENUM);
	...
	if (subWnd != NULL && IsWindow(subWnd)) SendMessage(subWnd, WM_INITDIALOG, 0, 0);
	...
	if (IsWindow(tabwnd))
	{
		...
		{
			if (TabCtrl_GetItemCount(tabwnd) == 4)
			{
				TCITEM item;
				item.mask=TCIF_TEXT;
				item.pszText=WASABI_API_LNGSTRING(IDS_CURRENT_SKIN);
...
{
	SendMessage(p->me, WM_DISPLAYCHANGE, 0, 0);
	return 0;
}
...
int embedCreateProc(embedWindowState *ws, embedEnumStruct *parms)
{
	...
	if (!rw && ws->me && IsWindowVisible(ws->me))
	{
...
static LRESULT WINAPI wa_newWndProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (uMsg == WM_COMMAND && LOWORD(wParam) == WINAMP_REFRESHSKIN)
	{
		...
		if (prefs)
		{
			...
			HWND hOwner = (HWND)(LONG_PTR)GetWindowLongPtr(prefs, GWLP_HWNDPARENT);
			if (NULL != hOwner && hOwner != hwndDlg)
				SetWindowLongPtr(prefs, GWLP_HWNDPARENT, (LONG_PTR)hwndDlg);
		}
		m_in_skinrefresh++;
		...
		{
			SendMessage(wa2.getMainWindow(), WM_COMMAND, WINAMP_OPTIONS_PREFS, 0);
...
#define UPDATE_DISPLAY_TIMER			38
	...
	if (m_are_we_loaded)
	{
		...
		else if (uMsg == WM_INITMENUPOPUP)
		{
			...
			{
				int IPC_LIBRARY_SENDTOMENU = SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&"LibrarySendToMenu", IPC_REGISTER_WINAMP_IPCMESSAGE);
				if (IPC_LIBRARY_SENDTOMENU > 65536 && SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&mainSendTo, IPC_LIBRARY_SENDTOMENU) == 0xffffffff)
					mainSendTo.mode = 2;
			}
		...
		else if (uMsg == WM_WA_IPC)
		{
			if (lParam == DEFERREDCALLBACKMSG && DEFERREDCALLBACKMSG > 65536)
			{
				...
				else if (wParam == 2)
				{
					...
					embedEnumStruct cs = { embedCreateProc, 0 };
					SendMessage(hwndDlg, WM_WA_IPC, (WPARAM)&cs, IPC_EMBED_ENUM);
					...
					if (make_sure_library_is_here_at_startup)
						SendMessage(wa2.getMainWindow(), WM_COMMAND, ID_FILE_SHOWLIBRARY, 0);
			...
			else if (lParam == UPDATEDIALOGBOXPARENTMSG && UPDATEDIALOGBOXPARENTMSG > 65536)
			{
				...
				case IPC_CB_ONSHOWWND:
					...
					case IPC_CB_WND_VIDEO:
						...
						break;
					default:
						DebugString("embedWnd : Got IPC_ONSHOW\n");
						if (IsWindow((HWND)wParam))
						{
							HWND hTarget = (HWND)wParam;
							embedWindowState *ws = (embedWindowState*)GetWindowLongPtr(hTarget, GWLP_USERDATA);
							if (ws && ws->me == hTarget)
							{
								GUID thisguid = EmbedWndGuid(ws).getGuid();
				...
				case IPC_CB_ONHIDEWND:
					...
					default:
						DebugString("embedWnd : Got IPC_ONHIDE\n");
						if (IsWindow((HWND)wParam))
						{
							HWND h = (HWND)wParam;
							embedWindowState *ws = (embedWindowState *)GetWindowLongPtr(h, GWLP_USERDATA);
				...
				case IPC_GET_GENSKINBITMAP:
					if (m_are_we_loaded)
					{
						if (wParam == 0)
						{
							...
							{
								HBITMAP bm;
								wchar_t bitmapfilename[MAX_PATH];
				...
				if (l != NULL)
				{
					...
					if (c)
					{
						...
						if (l)
						{
							...
							return SendMessage(l->gethWnd(), WM_SYSCOMMAND, wParam, lParam);
						}
					}
				}
				break;
...
void initFFApi()
{
	...
	{
		m_loaded_at_all = 1;
		api_service *svc = (api_service *)SendMessage(wa2.getMainWindow(), WM_WA_IPC, 0, IPC_GET_API_SERVICE);
		...
		ServiceBuild(AGAVE_API_DOWNLOADMANAGER, DownloadManagerGUID);
...
void shutdownFFApi()
{
	if (m_loaded_at_all)
	{
		...
		ServiceRelease(AGAVE_API_DOWNLOADMANAGER, DownloadManagerGUID);
...
void init_inst()
{
	...
	StdWnd::setLayeredWnd(wa2.getMainWindow(), 1);
	StdWnd::setLayeredAlpha(wa2.getMainWindow(), 0);
	...
	if (wasminimized) ShowWindow(wa2.getMainWindow(), SW_RESTORE);
	...
	{
		if (!m_loading_at_startup) WritePrivateProfileString("gen_ff", "classicpe", "1", INI_FILE);
	}
	else
		if (!m_loading_at_startup) WritePrivateProfileString("gen_ff", "classicpe", "0", INI_FILE);
	...
	if (wa2.isWindowVisible(IPC_GETWND_EQ))
	{
		...
		gothrueqmsg = 0;
		if (!m_loading_at_startup) WritePrivateProfileString("gen_ff", "classiceq", "1", INI_FILE);
	}
	else
		if (!m_loading_at_startup) WritePrivateProfileString("gen_ff", "classiceq", "0", INI_FILE);

	removeEq();

	if (SendMessage(plugin.hwndParent, WM_WA_IPC, 0, IPC_ISMAINWNDVISIBLE))
	{
		SendMessage(plugin.hwndParent, WM_COMMAND, WINAMP_MAIN_WINDOW, 0);
		if (!m_loading_at_startup) WritePrivateProfileString("gen_ff", "classicmw", "1", INI_FILE);
	}
	else
		if (!m_loading_at_startup) WritePrivateProfileString("gen_ff", "classicmw", "0", INI_FILE);
	...
	if (wa2.export_sa_setreq) wa2.export_sa_setreq(1);
	...
	if (wa2.getWnd(IPC_GETWND_VIDEO))
	{
		...
		if (!oldVideoWndProc) oldVideoWndProc = (WNDPROC)SetWindowLongPtrW(oldVideoWnd, GWLP_WNDPROC, (LONG_PTR)newVideoWndProc);
	}

	if (wasminimized) ShowWindow(wa2.getMainWindow(), SW_MINIMIZE);
	if (eggstat) SetTimer(wa2.getMainWindow(), UPDATE_EGG, 25, NULL);
	//ExitProcess(-1);
}
...
void quit_inst()
{
	...
	int classicmw = GetPrivateProfileInt("gen_ff", "classicmw", 1, INI_FILE);
	if (classicmw && !SendMessage(plugin.hwndParent, WM_WA_IPC, 0, IPC_ISMAINWNDVISIBLE))
		SendMessage(plugin.hwndParent, WM_COMMAND, WINAMP_MAIN_WINDOW, 0);

	CheckMenuItem(wa2.getPopupMenu(), WINAMP_MAIN_WINDOW, classicmw ? MF_CHECKED : MF_UNCHECKED);

	if (GetPrivateProfileInt("gen_ff", "classiceq", 1, INI_FILE) && !wa2.isWindowVisible(IPC_GETWND_EQ))
	{
		gothrueqmsg = 1;
		SendMessage(plugin.hwndParent, WM_COMMAND, WINAMP_OPTIONS_EQ, 0);
		gothrueqmsg = 0;
	...
	if (GetPrivateProfileInt("gen_ff", "classicpe", 1, INI_FILE) && !wa2.isWindowVisible(IPC_GETWND_PE))
	{
		SendMessage(plugin.hwndParent, WM_COMMAND, WINAMP_OPTIONS_PLEDIT, 0);
	}

	// restore the classic main window to be solid
	StdWnd::setLayeredWnd(wa2.getMainWindow(), 0);
}
...
int init()
{
	...
	INI_DIR = (char*)SendMessage(plugin.hwndParent, WM_WA_IPC, 0, IPC_GETINIDIRECTORY);
	...
	DEFERREDCALLBACKMSG = SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)"gen_ff_deferred", IPC_REGISTER_WINAMP_IPCMESSAGE);
	UPDATEDIALOGBOXPARENTMSG = SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)"gen_ff_update", IPC_REGISTER_WINAMP_IPCMESSAGE);
	...
	SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&ffPrefsItem, IPC_ADD_PREFS_DLGW);
...
void RestoreClassicWinamp(int was_loaded)
{
	...
	{
		int classicpews = GetPrivateProfileInt("gen_ff", "classicplws", 0, INI_FILE);
		int classicwidth = GetPrivateProfileInt("gen_ff", "classicplwidth", 275, INI_FILE);
		int classicheight = GetPrivateProfileInt("gen_ff", "classicplheight", 145, INI_FILE);
		int classicmw = GetPrivateProfileInt("gen_ff", "classicmw", 1, INI_FILE);
		int classiceq = GetPrivateProfileInt("gen_ff", "classiceq", 1, INI_FILE);
		char buf[64];

		wsprintf(buf, "%d", classicheight);
		if (classicpews)
		{
			WritePrivateProfileString("winamp", "pe_height", "14", INI_FILE);
			WritePrivateProfileString("winamp", "pe_height_ws", buf, INI_FILE);
		}
		else
		{
			WritePrivateProfileString("winamp", "pe_height", buf, INI_FILE);
			WritePrivateProfileString("winamp", "pe_height_ws", "", INI_FILE);
		}
		wsprintf(buf, "%d", classicwidth);
		WritePrivateProfileString("winamp", "pe_width", buf, INI_FILE);

		WritePrivateProfileString("winamp", "eq_open", classiceq ? "1" : "0", INI_FILE);
		WritePrivateProfileString("winamp", "mw_open", classicmw ? "1" : "0", INI_FILE);
	}
}
...
BOOL CALLBACK aboutProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		C_Text t(ver);
		t.setText(StringPrintfW(L". (C) 2003-2013 Nullsoft, Inc. %s", VERSION));
...
void config()
{
	if (m_loaded_at_all)
	{
		...
		if ((!skin.iscaseequal(L"No skin loaded")) || guiTree->getNumObject() > 0)
		{
...
void updateControlMenu(ifc_window *w)
{
	...
	if (u != 50 && u != 75 && u != 100 && u != 150 && u != 200 && u != 250 && u != 300)
	{
...
void appControlMenu(ifc_window *w)
{
	...
	if (w)
	{
		...
		updateAppBarMenu(w);
		HMENU ctrlmenu = GetSubMenu(controlmenu, 0);
		TrackPopupMenu(ctrlmenu, TPM_LEFTALIGN | TPM_TOPALIGN | TPM_LEFTBUTTON, x, y, 0, wa2.getMainWindow(), NULL);
	}
	WASABI_API_WND->appdeactivation_setbypass(0);
}
...
int onCreateExternalWindowGuid(GUID g)
{
	...
	{
		SendMessage(wa2.getMainWindow(), WM_COMMAND, WINAMP_HELP_ABOUT, 0);
		return 1;
	}
	else if (g == lightning_bolt_guid)
	{
		SendMessage(wa2.getMainWindow(), WM_COMMAND, WINAMP_LIGHTNING_CLICK, 0);
		return 1;
	}
	else if (g == colorthemes_guid)
	{
		last_page.setData(L"3");
		SendMessage(plugin.hwndParent, WM_WA_IPC, (WPARAM)&ffPrefsItem, IPC_OPENPREFSTOPAGE);
		return 1;
	}
	else if (g == avs_guid)
	{
...
{
	if (!toggle_from_wa2 && subWndId == 0 && IsWindow(subWnd)) SendMessage(subWnd, WM_INITDIALOG, 0, 0);
}
...
void onCustomAltF4()
{
	SendMessage(wa2.getMainWindow(), WM_CLOSE, 0, 0);
}
...
void onGoFullscreen()
{
	...
	SendMessage(wa2.getMainWindow(), APPBAR_CALLBACK, ABN_FULLSCREENAPP, 1);
...
{
	// hidden windows will not receive APPBAR_CALLBACK, so forward it to winamp's main
	SendMessage(wa2.getMainWindow(), APPBAR_CALLBACK, ABN_FULLSCREENAPP, 0);
...
int processGenericHotkey(const char *hk)
{
	...
	if (VCPU::getComplete())
	{
		DebugString("HOTKEY: %s trapped by script\n", hk);
		return 1;
	}
	return 0;
}
