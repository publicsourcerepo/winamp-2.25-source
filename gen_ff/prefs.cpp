...
void _dosetsel(HWND hwndDlg) 
{
	...
	if (sel >= 0 && (sel != last_page.getValueAsInt() || !subWnd))
	{
		...
		switch (sel) 
		{
			...
			case 1: t=IDD_PREFS_ALPHA;		p=ffPrefsProc4; subWndId = 1;	break;
...
// frame proc
BOOL CALLBACK ffPrefsProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam) {
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			item.pszText=WASABI_API_LNGSTRINGW(IDS_ALPHA_BLENDING);
