...
const wchar_t *Core::getPlaystring()
{
	...
	if (!wcsstr(m_playstring, L"://"))
		m_playstring = StringPrintfW(L"file://%s", m_playstring);
	return m_playstring;
}
...
const wchar_t *Core::core_getDecoderName(const wchar_t *filename)
{
	...
	wchar_t fn[MAX_PATH+3];
	WCSNPRINTF(fn, MAX_PATH, L"hi.%s", filename);
	In_Module *player = (In_Module *)wa2.CanPlay(fn);
	if (player)
	{
		// cope nicely with the 5.7+ input plug-ins with unicode descriptions
		if ((player->version & ~IN_UNICODE) == IN_VER_U)
		{
