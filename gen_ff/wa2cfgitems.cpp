...
void syncOutputCrossfade() {
  ...
  SendMessage(output_ipc, WM_DS_IPC, crossfade_time.getValueAsInt() * 1000, DS_IPC_SET_CROSSFADE_TIME);
  if (using_dsound) SendMessage(output_ipc, WM_DS_IPC, cfg_audiooptions_crossfader.getValueAsInt(), DS_IPC_SET_CROSSFADE);
}
...
void syncGenFFCrossfade() {
  syncing_crossfade = 1;
  if (output_ipc == NULL) return;
  crossfade_time.setValueAsInt(SendMessage(output_ipc, WM_DS_IPC, 0, DS_IPC_GET_CROSSFADE_TIME) / 1000);
  cfg_audiooptions_crossfader.setValueAsInt(SendMessage(output_ipc, WM_DS_IPC, 0, DS_IPC_GET_CROSSFADE));
  syncing_crossfade = 0;
}
...
void setCrossfader(int crossfade) {
  ...
  if (crossfade && (!output_ipc || !using_dsound)) {
	...
	char title[256];
	int r = MessageBox(last_dlg_parent ? last_dlg_parent : wa2.getMainWindow(),
					   WASABI_API_LNGSTRING(IDS_CROSSFADER_ONLY_UNDER_OUT_DS),
					   WASABI_API_LNGSTRING_BUF(IDS_NOT_SUPPORTED,title,256), MB_YESNO);
    if (r == IDYES) {
      SendMessage(wa2.getMainWindow(),WM_WA_IPC,32,IPC_OPENPREFSTOPAGE);
    }
  }
  syncOutputCrossfade();
}
...
void hookOutputIPC() {
  oldOutputIPCProc = (WNDPROC)SetWindowLongPtr(output_ipc, GWLP_WNDPROC, (LONG_PTR)outputIPCProc);
}

// not called in fact, since we never need to let go of it
void unhookOutputIPC() {
  SetWindowLongPtr(output_ipc, GWLP_WNDPROC, (LONG_PTR)oldOutputIPCProc);
}

BOOL CALLBACK findOutputIPCProc(HWND hwnd, LPARAM lParam) {
  char cn[256];
  GetClassName(hwnd, cn, 255); cn[255] = 0;
...
void Crossfader::onOutputChanged() {
  ...
  PathParser pp(wa2.getOutputPlugin());
