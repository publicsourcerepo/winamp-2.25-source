...
void MyPathCompactPath(const wchar_t *in, wchar_t *out, int outlen, HWND hwnd, int len=-1, HDC *pdc=NULL, HFONT *phfOld=NULL)
{
	if (len == -1)
	{
		RECT r={0};
	...
	if (!pdc || !*pdc)
	{
		...
		hfOld = (HFONT)SelectObject(dc,hf);
		if (phfOld) *phfOld = hfOld;
		if (pdc) *pdc = dc;
	}
	else
		dc = *pdc;
...
INT_PTR CALLBACK autotagger_dlgproc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		for (size_t i=0; i < t->list.size(); i++)
		{
			...
			wchar_t path[2048];
			MyPathCompactPath(t->list[i]->filename,path,2048,hlist,500-25,&dc,&hf);
			t->listview.InsertItem((int)i,path,(LPARAM)t->list[i]);
	...
	case WM_NOTIFY:
		...
		case HDN_ITEMCHANGEDW:
		{
			...
			if (nmh->iItem != COL_FILENAME) break;
			Tagger * t = (Tagger *)(LONG_PTR)GetWindowLongPtr(hwndDlg,GWLP_USERDATA);
			HDC dc=NULL;
			HFONT hf=NULL;
			HWND hlist = GetDlgItem(hwndDlg,IDC_LIST);

			int w = ListView_GetColumnWidth(hlist,COL_FILENAME);

			for (size_t i=0; i < t->list.size(); i++)
			{
				wchar_t path[2048]=L"";
				wchar_t path2[2048]=L"";
				MyPathCompactPath(t->list[i]->filename,path,2048,hlist,w-25,&dc,&hf);
				t->listview.GetText((int)i,COL_FILENAME,path2,2048);
				if (wcscmp(path,path2))
					t->listview.SetItemText((int)i,COL_FILENAME,path);
			}
	...
	case WM_USER:
	{
		...
		wchar_t path[2048];
		MyPathCompactPath(ti->filename,path,2048,GetDlgItem(hwndDlg,IDC_FILENAME));
...
#define PUTINFO2(x,y) tag2_5->get_ ## x ##(&bstr); if(bstr && *bstr) SetDlgItemText(hwndDlg,y,bstr); if(bstr) SysFreeString(bstr); bstr=0;
...
#define PUTINFO2(x,y) tag2_5->get_ ## x ## (&bstr); if(bstr && *bstr) SetFileInfo(fn,y,bstr); if(bstr) SysFreeString(bstr); bstr=0;
...
TagItem::TagItem(const char * filename0) : freefn(true)
{
	filename = AutoWideDup(filename0);
	init();
}

TagItem::TagItem(const wchar_t * filename0, bool copy) : freefn(copy)
{
...
void TagItem::SetStatus(CddbMusicIDStatus status, HWND hwndDlg)
{
	...
	if (status < sizeof(s)/sizeof(int)) SetStatus(WASABI_API_LNGSTRINGW_BUF(s[status],buf,100),hwndDlg);
}
...
Tagger::Tagger(nu::PtrList<TagItem> &_list, ICDDBMusicIDManager3 *musicid) :  hwndDlg(0), abort(0), musicid(musicid), icp(0), m_dwCookie(0)
{
	list.own(&_list);
...
TagItem* Tagger::FindTagItem(const wchar_t * filename)
{
	...
	for (size_t i=0; i<list.size(); i++)
		if (!_wcsicmp(list[i]->filename,filename)) return list[i];
	return NULL;
}
...
#undef PUTINFO2
	...
	{
		wchar_t buf[2048]=L"";
#define PUTINFO(y, x) buf[0]=0; GetFileInfo(filename,x,buf,2048); if(buf[0]) infotag->put_ ## y ##(buf);
#define PUTINFO2(y, x) buf[0]=0; GetFileInfo(filename,x,buf,2048); if(buf[0]) tag2_5->put_ ## y ##(buf);
		PUTINFO(LeadArtist, L"artist");
		PUTINFO(Album, L"album");
		PUTINFO(Title, L"title");
		PUTINFO(Genre, L"genre");
