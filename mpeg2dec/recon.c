...
/* private prototypes */
static void form_prediction(MPEGDecoder *decoder, unsigned char *src[], int sfield,  unsigned char *dst[], int dfield,  int lx, int lx2, int w, int h, int x, int y, int dx, int dy,  int average_flag);
...
void form_predictions(MPEGDecoder *decoder, int bx, int by, int macroblock_type,   int motion_type, int PMV[2][2][2], int motion_vertical_field_select[2][2],  int dmvector[2], int stwtype)
{
  int currentfield;
  ...
  if ((macroblock_type & MACROBLOCK_MOTION_FORWARD) 
   || (decoder->picture_coding_type==P_TYPE))
  {
    if (decoder->picture_structure==FRAME_PICTURE)
    {
        ...
        || !(macroblock_type & MACROBLOCK_MOTION_FORWARD))
      {
        ...
        if (stwtop<2)
          form_prediction(decoder, decoder->forward_reference_frame,0,decoder->current_frame,0,
            decoder->Coded_Picture_Width,decoder->Coded_Picture_Width<<1,16,8,bx,by,
            PMV[0][0][0],PMV[0][0][1],stwtop);

        if (stwbot<2)
          form_prediction(decoder, decoder->forward_reference_frame,1,decoder->current_frame,1,
            decoder->Coded_Picture_Width,decoder->Coded_Picture_Width<<1,16,8,bx,by,
      ...
      else if (motion_type==MC_FIELD) /* field-based prediction */
      {
        ...
        if (stwtop<2)
          form_prediction(decoder, decoder->forward_reference_frame,motion_vertical_field_select[0][0],
            decoder->current_frame,0,decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,8,
            bx,by>>1,PMV[0][0][0],PMV[0][0][1]>>1,stwtop);

        /* bottom field prediction */
        if (stwbot<2)
          form_prediction(decoder, decoder->forward_reference_frame,motion_vertical_field_select[1][0],
            decoder->current_frame,1,decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,8,
            bx,by>>1,PMV[1][0][0],PMV[1][0][1]>>1,stwbot);
      }
      else if (motion_type==MC_DMV) /* dual prime prediction */
      {
        /* calculate derived motion vectors */
        Dual_Prime_Arithmetic(decoder, DMV,dmvector,PMV[0][0][0],PMV[0][0][1]>>1);

        if (stwtop<2)
        {
          /* predict top field from top field */
          form_prediction(decoder, decoder->forward_reference_frame,0,decoder->current_frame,0,
            decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,8,bx,by>>1,
            PMV[0][0][0],PMV[0][0][1]>>1,0);

          /* predict and add to top field from bottom field */
          form_prediction(decoder, decoder->forward_reference_frame,1,decoder->current_frame,0,
            decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,8,bx,by>>1,
            DMV[0][0],DMV[0][1],1);
        }
        ...
        if (stwbot<2)
        {
          ...
          form_prediction(decoder, decoder->forward_reference_frame,0,decoder->current_frame,1,
            decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,8,bx,by>>1,
            DMV[1][0],DMV[1][1],1);
        }
      }
    ...
    {
      /* field picture */
      currentfield = (decoder->picture_structure==BOTTOM_FIELD);

      /* determine which frame to use for prediction */
      if ((decoder->picture_coding_type==P_TYPE) && decoder->Second_Field
         && (currentfield!=motion_vertical_field_select[0][0]))
        predframe = decoder->backward_reference_frame; /* same frame */
      else
        predframe = decoder->forward_reference_frame; /* previous frame */
        ...
        || !(macroblock_type & MACROBLOCK_MOTION_FORWARD))
      {
        ...
        if (stwtop<2)
          form_prediction(decoder, predframe,motion_vertical_field_select[0][0],decoder->current_frame,0,
            decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,16,bx,by,
            PMV[0][0][0],PMV[0][0][1],stwtop);
      }
      else if (motion_type==MC_16X8)
      {
        ...
        {
          form_prediction(decoder, predframe,motion_vertical_field_select[0][0],decoder->current_frame,0,
            decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,8,bx,by,
            PMV[0][0][0],PMV[0][0][1],stwtop);
          ...
          form_prediction(decoder, predframe,motion_vertical_field_select[1][0],decoder->current_frame,0,
            decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,8,bx,by+8,
      ...
      else if (motion_type==MC_DMV) /* dual prime prediction */
      {
        ...
        /* calculate derived motion vectors */
        Dual_Prime_Arithmetic(decoder, DMV,dmvector,PMV[0][0][0],PMV[0][0][1]);

        /* predict from field of same parity */
        form_prediction(decoder, decoder->forward_reference_frame,currentfield,decoder->current_frame,0,
          decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,16,bx,by,
          PMV[0][0][0],PMV[0][0][1],0);

        /* predict from field of opposite parity */
        form_prediction(decoder, predframe,!currentfield,decoder->current_frame,0,
          decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,16,bx,by,
          DMV[0][0],DMV[0][1],1);
      }
      else
  ...
  if (macroblock_type & MACROBLOCK_MOTION_BACKWARD)
  {
    if (decoder->picture_structure==FRAME_PICTURE)
    {
      if (motion_type==MC_FRAME)
      {
        ...
        if (stwtop<2)
          form_prediction(decoder, decoder->backward_reference_frame,0,decoder->current_frame,0,
            decoder->Coded_Picture_Width,decoder->Coded_Picture_Width<<1,16,8,bx,by,
            PMV[0][1][0],PMV[0][1][1],stwtop);

        if (stwbot<2)
          form_prediction(decoder, decoder->backward_reference_frame,1,decoder->current_frame,1,
            decoder->Coded_Picture_Width,decoder->Coded_Picture_Width<<1,16,8,bx,by,
      ...
      else /* field-based prediction */
      {
        /* top field prediction */
        if (stwtop<2)
          form_prediction(decoder, decoder->backward_reference_frame,motion_vertical_field_select[0][1],
            decoder->current_frame,0,decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,8,
            bx,by>>1,PMV[0][1][0],PMV[0][1][1]>>1,stwtop);

        /* bottom field prediction */
        if (stwbot<2)
          form_prediction(decoder, decoder->backward_reference_frame,motion_vertical_field_select[1][1],
            decoder->current_frame,1,decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,8,
    ...
    else /* TOP_FIELD or BOTTOM_FIELD */
    {
      ...
      if (motion_type==MC_FIELD)
      {
        /* field-based prediction */
        form_prediction(decoder, decoder->backward_reference_frame,motion_vertical_field_select[0][1],
          decoder->current_frame,0,decoder->Coded_Picture_Width<<1,decoder->Coded_Picture_Width<<1,16,16,
