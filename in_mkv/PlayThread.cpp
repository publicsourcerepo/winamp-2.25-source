...
static ifc_mkvvideodecoder *FindVideoDecoder(const nsmkv::TrackEntry *track_entry)
{
	...
	while (sf = WASABI_API_SVC->service_enumService(WaSvc::MKVDECODER, n++))
	{
...
again:
			...
			int time_diff = timestamp_ms - realTime;
			...
			if (time_diff > 3)
			{
				...
				int ret= WaitForMultipleObjects(2, handles, FALSE, timestamp_ms-realTime);
				if (ret != WAIT_TIMEOUT)
				{
...
int MKVPlayer::OnVideo(const nsmkv::Cluster &cluster, const nsmkv::BlockBinary &binary)
{
	...
	if (nsmkv::Lacing::GetState(binary.flags, (const uint8_t *)binary.data, binary.data_size, &lacing_state))
	{
		...
		while (nsmkv::Lacing::GetFrame(i++, (const uint8_t *)binary.data, binary.data_size, &frame, &frame_len, &lacing_state))
		{
			...
			video_decoder->DecodeBlock(frame, frame_len, cluster.time_code+binary.time_code);
...
void MKVPlayer::OnSegmentInfo(const nsmkv::SegmentInfo &segment_info)
{
	...
	{
		int bitrate = 		(8ULL * content_length / (uint64_t)g_duration);
...
{
	wchar_t audio_info[256];
	wchar_t video_info[256];
	audio_info[0]=0;
	video_info[0]=0;
...
int MKVPlayer::OnFirstCluster(uint64_t position)
{
	if (video_decoder)
	{
		...
		SetThreadPriority(video_thread, AGAVE_API_CONFIG->GetInt(playbackConfigGroupGUID, L"priority", THREAD_PRIORITY_HIGHEST));
	}
	return MKV_CONTINUE;
}
