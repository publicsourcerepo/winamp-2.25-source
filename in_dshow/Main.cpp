...
wchar_t lastfn[MAX_PATH];	// currently playing file (used for getting info on the current file)
...
wchar_t m_lastfn[2048]; // currently playing file (used for getting info on the current file)
...
extern void getInfo(const wchar_t *fn, wchar_t *linetext, int linetextCch, wchar_t *fulltext, int fulltextCch, int *bitrate);
...
RGBQUAD palette[0x100]; // for RGB8
int m_length=-1;
int g_quit;
...
public:
	...
	void sample_cb(LONGLONG starttime, LONGLONG endtime, IMediaSample *pSample)
	{
		...
		if (l)
		{
			...
			double te=(double)(endtime)/10000;
...
public:
	...
	{
		m_nbframes++;
		if (!m_avgfps_start) m_avgfps_start=GetTickCount();

		if (GetTickCount()-m_laststatus>500)
		{
			...
			if (g_num_free_frames)
	{
		...
		else
		{
			...
			while ((GetTickCount()-m_starttime)<(unsigned int)evtime && !g_quit) Sleep(1);
		}
		if (g_quit) return;
		if (video_mediatype==VIDEO_MAKETYPE('Y','V','1','2'))
		{
...
void quit()
{
	delete pTrackSelector;
	pTrackSelector = NULL;
	free(infoFn); infoFn=0;
	delete infoHeader; infoHeader=0;
}
...
int play(const wchar_t *fn)
{
	...
	if (!classReg)
	{
		...
		wc.hIcon = NULL;
		wc.hCursor = NULL;
		wc.lpszClassName = "in_dshowClass";
		if (!RegisterClass(&wc))
			return 1;
		classReg=1;
	}
	m_notif_hwnd=CreateWindow("in_dshowClass","dshow_notif",NULL,0,0,1,1,NULL,NULL,mod.hDllInstance,NULL);
	...
	if (m_is_capture)
	{
		...
		if (1)
		{
			...
			if (hr != NOERROR)
				hr = pCapture->FindInterface(&PIN_CATEGORY_CAPTURE,				&MEDIATYPE_Video, pCapVidSrcFilter,				IID_IAMStreamConfig, (void **)&pSC);
	...
	// check if audio has been negociated
	{
		...
		else
		{
			...
			if (pHeader->wFormatTag == 0x3/*WAVE_FORMAT_IEEE_FLOAT*//*audio_bps==32 || audio_bps==64*/)
			{
				m_float=1;
				m_src_bps=audio_bps;
	...
	getInfo(fn, m_status, 512, NULL,0, &m_bitrate);
...
void getfileinfo(const wchar_t *filename, wchar_t *title, int *length_in_ms)
{
	...
	else // some other file
	{
		...
		if (title) // get non path portion of filename
		{
			const wchar_t *p=filename+wcslen(filename);
...
static const wchar_t *pExtList[]={L"MPG",L"MPEG",L"M2V",L"AVI",L"MOV",L"FLV",L"FLV1",L"OGV",L"OGA",L"OGM",L"RMVB",L"RM",L"VOB",L"AC3",L"MKV",L"MP4",L"M4V",};
static const int pExtDescIdList[] = {0, 0, 1, 2, 3, 4, 4, 5, 6, 7, 8, 8, 9, 10, 11, 12, 13,};
...
extern "C"
{
	...
	_declspec(dllexport) int winampGetExtendedFileInfoW(const wchar_t *fn, const char *data, wchar_t *dest, int destlen)
	{
		if (!fn || !*fn)
		{
			if (!lstrcmpi(data,"type"))
			{
				lstrcpynW(dest,L"1", destlen); //video
				return 1;
			}
		...
		if (!lstrcmpi(data,"type"))
		{
			if (infoHeader)
			{
				if (infoHeader->has_video)
					...
					lstrcpynW(dest,L"0", destlen); // no video
			}
			else // assume video
			{
				lstrcpynW(dest,L"1", destlen); //video
		...
		if (!lstrcmpi(data,"length"))
		{
			int len = GetFileLength(fn);
			if (len == -1000)
				dest[0]=0;
			else
				StringCchPrintfW(dest, destlen, L"%d", len);
			return 1;
		}
		else if (!lstrcmpi(data, "bitrate"))
		{
			int bitrate;
			getInfo(fn, NULL, 0, NULL,0, &bitrate);
			StringCchPrintfW(dest, destlen, L"%d", bitrate);
			return 1;
		}
		else if (!lstrcmpi(data,"title"))
		{
			if (infoHeader && infoHeader->title)
				lstrcpynW(dest,AutoWide(infoHeader->title), destlen);
			else
				dest[0]=0;
			return 1;
		}
		else if (!lstrcmpi(data,"artist"))
		{
			if (infoHeader && infoHeader->artist)
				lstrcpynW(dest,AutoWide(infoHeader->artist), destlen);
			else
				dest[0]=0;
			return 1;
		}
		else if (!lstrcmpi(data,"comment"))
		{
			if (infoHeader && infoHeader->comment)
				lstrcpynW(dest,AutoWide(infoHeader->comment), destlen);
			else
				dest[0]=0;
			return 1;
		}
		else if (!lstrcmpi(data,"genre"))
		{
			if (infoHeader && infoHeader->genre)
				lstrcpynW(dest,AutoWide(infoHeader->genre), destlen);
			else
				dest[0]=0;
			return 1;
		}
		else if (!lstrcmpi(data,"album"))
		{
			if (infoHeader && infoHeader->album)
				lstrcpynW(dest,AutoWide(infoHeader->album), destlen);
			else
				dest[0]=0;
			return 1;
		}
		else if (!lstrcmpi(data,"composer"))
		{
			if (infoHeader && infoHeader->composer)
				lstrcpynW(dest,AutoWide(infoHeader->composer), destlen);
			else
				dest[0]=0;
			return 1;
		}
		else if (!lstrcmpi(data,"publisher"))
		{
			if (infoHeader && infoHeader->publisher)
				lstrcpynW(dest,AutoWide(infoHeader->publisher), destlen);
			else
				dest[0]=0;
			return 1;
		}
