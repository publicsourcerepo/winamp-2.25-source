...
int dsr_play(char *fn)
{
  ...
  if (!s_bAudioOnly)
  {
    ...
    hr = s_pVideoWindow->put_Owner((OAHWND) s_hVideoWnd);
    hr = s_pVideoWindow->put_WindowStyle(WS_CHILD | WS_CLIPSIBLINGS);
    ...
    if(s_buffering)
    {
      BeginEnumFilters(s_pGraphBuilder, pEF, pBF)
      {
        if(CComQIPtr<IAMNetworkStatus, &IID_IAMNetworkStatus> pAMNS = pBF)
        {
          ...
          {
            wsprintf(lastfn_status,"Buffering (%d%%)",BufferingProgress);
