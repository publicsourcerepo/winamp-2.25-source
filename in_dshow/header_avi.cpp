...
HeaderAvi::HeaderAvi(bool bAllowHttpConnection /* = false */)
		: bAllowHttp(bAllowHttpConnection)
{}
			...
			case mmioFOURCC('I', 'N', 'F', 'O'):
				{
					...
					while (len)
					{
						...
						case mmioFOURCC('I', 'C', 'O', 'M'):
							{
								composer = (char *)malloc(infoSize + 1);
								composer[infoSize] = 0;
						...
						case mmioFOURCC('I', 'P', 'U', 'B'):
							{
								publisher = (char *)malloc(infoSize + 1);
						...
						case mmioFOURCC('I', 'C', 'M', 'T'):
							{
								comment = (char *)malloc(infoSize + 1);
								comment[infoSize] = 0;
...
{
	DWORD bytesRead;
	ReadFile(fh, buffer, size*count, &bytesRead, NULL);
	return bytesRead;
}
