...
void getInfo(const wchar_t *fn, wchar_t *linetext, int linetextCch, wchar_t *fulltext, int fulltextCch, int *bitrate)
{
	wchar_t tmp[512] = {0, };
...
INT_PTR CALLBACK infoProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		{
			wchar_t text[512];
			getInfo(m_fn, NULL, 0, text, 512, NULL);
		...
		if (!_wcsicmp(m_fn, m_lastfn) && pGraphBuilder)
		{
			...
			do
			{
				...
				int a = SendDlgItemMessageW(hwndDlg, IDC_FILTERLIST, LB_ADDSTRING, 0, (LPARAM)fi.achName);
	...
	case WM_COMMAND:
		...
		case IDC_FILTERLIST:
			...
			{
				int sel = SendDlgItemMessage(hwndDlg, IDC_FILTERLIST, LB_GETCURSEL, 0, 0);
				if (sel == LB_ERR) break;
