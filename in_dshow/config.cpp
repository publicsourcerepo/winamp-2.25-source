...
char *getfileextensions()
{
	...
	while (*p)
	{
		...
		if (s) *op++=';';
		s=1;
		while (*p && !ISSEP(*p)) *op++=*p++;
	}
	*op++=0;
	lstrcpyn(op,WASABI_API_LNGSTRING(IDS_VIDEO_FILES_OFD),512);
	while (*op) op++;
	p=config_extlist;
	s=0;
	while (*p)
...
void config_read()
{
  lstrcpyn(config_extlist,default_extlist, 129);
  RS(config_extlist);
}
...
INT_PTR CALLBACK configProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  {
  case WM_INITDIALOG:
    SetDlgItemText(hwndDlg,IDC_TYPES,config_extlist);
  ...
  case WM_COMMAND:
    ...
    case IDC_DEFAULTTYPES:
      SetDlgItemText(hwndDlg,IDC_TYPES,default_extlist);
      break;
    case IDOK:
      GetDlgItemText(hwndDlg,IDC_TYPES,config_extlist,128);
      config_write();
    case IDCANCEL:
      EndDialog(hwndDlg,0);
      break;
