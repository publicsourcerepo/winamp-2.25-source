...
C_ItemList * fileListToItemRecords(wchar_t** files,int l, HWND centerWindow) {
	filenameMap ** map = (filenameMap **)malloc(sizeof(void*) * l);
	filenameMap * m = (filenameMap *)malloc(sizeof(filenameMap) * l);
	for(int i=0; i<l; i++) {
		map[i] = &m[i];
		map[i]->fn = files[i];
	}
...
void filenameToItemRecord(wchar_t * file, itemRecordW * ice)
{
	...
	if (!(file_size == INVALID_FILE_SIZE || file_size == 0))
	{
		...
		// and since 5.7+ we can also return this as a true value
