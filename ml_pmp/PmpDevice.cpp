...
{
	LinkedQueue * txQueue = (isCloudDevice ? &cloudTransferQueue : &transferQueue);
	LinkedQueue * finishedTX = (isCloudDevice ? &cloudFinishedTransfers : &finishedTransfers);
	int txProgress = (isCloudDevice ? cloudTransferProgress : currentTransferProgress);
	double num = (100.0*(double)txQueue->GetSize()) - (double)txProgress;
	double total = (double)100*txQueue->GetSize() + 100*finishedTX->GetSize();
