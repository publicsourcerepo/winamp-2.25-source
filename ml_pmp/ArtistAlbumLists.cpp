...
public:
	...
	TracksList(Device * dev,C_Config * config, ArtistAlbumLists * aaList, bool cloud) :
		...
		if (fieldsBits & SUPPORTS_LASTPLAYED)  fields.Add(new ListField(12+cloud, 100, WASABI_API_LNGSTRINGW(IDS_LAST_PLAYED), config));
...
ArtistAlbumLists::ArtistAlbumLists(Device * dev, int playlistId, C_Config * config, wchar_t ** filterNames, int numFilters0, int type, bool async) {
	...
	if (async && config->ReadInt(L"savefilter", 1))
	{
...
{
	wchar_t filterstr[256],filteritems[300];
...
C_ItemList * ArtistAlbumLists::CompilePrimaryList(const C_ItemList * songs) {
	...
	SortSongItem * songList;
	songList = (SortSongItem*)malloc(sizeof(SortSongItem)*songs->GetSize());
	int l=songs->GetSize();
