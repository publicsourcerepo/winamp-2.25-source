...
static HBITMAP getBitmap(pmpart_t art, Device * dev, HWND parent) {
	...
	ARGB32 * bits = (ARGB32 *)malloc(w*h*sizeof(ARGB32));
	if(!bits) return NULL;
...
static void setBitmap(HBITMAP bm, HWND hwndDlg) {
	editinfo_image* image = (editinfo_image*)malloc(sizeof(editinfo_image));
	...
	switch(uMsg) {
		case WM_INITDIALOG:
		{
			...
			for(int i=0;i<l;i++) {
				...
				if(!disable_artist)
				{
					...
					if(!buf[0]);
					else if(!last_artist[0]) lstrcpy(last_artist,buf);
				...
				if(!disable_albumartist)
				{
					...
					if(!buf[0]);
					else if(!last_albumartist[0]) lstrcpy(last_albumartist,buf);
				...
				if(!disable_publisher)
				{
					...
					if(!buf[0]);
					else if(!last_publisher[0]) lstrcpy(last_publisher,buf);
				...
				if(!disable_composer)
				{
					...
					if(!buf[0]);
					else if(!last_composer[0]) lstrcpy(last_composer,buf);
				...
				if(!disable_album)
				{
					...
					if(!buf[0]);
					else if(!last_album[0]) lstrcpy(last_album,buf);
				...
				if(!disable_genre)
				{
					...
					if(!buf[0]);
					else if(!last_genre[0]) lstrcpy(last_genre,buf);
		...
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				...
				case IDC_CHECK_ALBUMART: enableArt(hwndDlg,false,IsDlgButtonChecked(hwndDlg,IDC_CHECK_ALBUMART)); break;
				case IDC_ART_CLEAR:
				{
					editinfo_image* image = (editinfo_image*)GetWindowLongPtr(hwndDlg,GWLP_USERDATA);
					if(image) {
