...
#include "../playlist/api_playlistdirectorycallback.h"
	...
	: activityRunning(FALSE), navigationItemCreated(FALSE), usedSpace(0), totalSpace(0)
{
	...
	isCloudDevice = (!strcmp(connection_type, "cloud"));
	...
	WASABI_API_THREADPOOL->AddHandle(transfer_thread, transferContext.notifier, TransferThreadPoolFunc, &transferContext, 0, api_threadpool::FLAG_LONG_EXECUTION);
	...
	else
	{
		...
		if (isCloudDevice)
		{
			if (!cloudQueueTreeItem)
			{
				...
				nis.hInsertAfter = MLNavCtrl_InsertItem(plugin.hwndLibraryParent, &nis2);
...
int DeviceView::CreatePlaylist(wchar_t * name, bool silent)
{
	...
	if (NULL == name)
	{
		...
		wchar_t buffer[512], buffer2[ARRAYSIZE(buffer)];
		BOOL foundMatch;
...
bool DeviceView::DeletePlaylist(int playlistId, bool deleteFiles, bool verbal)
{
	...
	if (false != verbal)
	{
		wchar_t message[1024], title[1024], playlistName[256], deviceName[256];
	...
	playlistTreeItems.eraseAt(index);
...
HWND DeviceView::CreateView(HWND parent)
{
	...
	if (currentViewedDevice->isCloudDevice && currentViewedDevice->config->ReadInt(L"media_numfilters", 2) == 1)
		return WASABI_API_CREATEDIALOGPARAMW(IDD_VIEW_CLOUD_SIMPLE, parent, pmp_video_dlgproc, 1);
	else
		return WASABI_API_CREATEDIALOGW((isCloudDevice ? IDD_VIEW_CLOUD_ARTISTALBUM : IDD_VIEW_PMP_ARTISTALBUM), parent, pmp_artistalbum_dlgproc);
}
...
{
	LinkedQueue * txQueue = (isCloudDevice ? &cloudTransferQueue : &transferQueue);
	if(txQueue->GetSize() == 0)
	{
		dev->Eject();
	}
	else
...
{
	wchar_t ** filesW = (wchar_t**)malloc(sizeof(wchar_t*)*num);
	for(int i=0; i<num; i++)
	{
			...
			if (AGAVE_API_PLAYLISTS->GetInfo(index, api_playlists_cloud, &cloud, sizeof(cloud)) == API_PLAYLISTS_SUCCESS)
			{
				...
				if (cloud_hinst && cloud_hinst != (HINSTANCE)1)
				{
					...
					if (gp)
					{
						...
						{
							size_t num_names = mlplugin->MessageProc(0x406, index, 0, 0);
						}
					}
				}
...
void DeviceView::OnActivityStarted()
{
	...
	{
		event_handlers[i]->ActivityStarted(this, this);
	}
}
...
void DeviceView::OnActivityChanged()
{
	for (size_t i=0;i<event_handlers.size();i++)
	{
		event_handlers[i]->ActivityChanged(this, this);
	}
}
...
void DeviceView::OnActivityFinished()
{
	for (size_t i=0;i<event_handlers.size();i++)
	{
		event_handlers[i]->ActivityFinished(this, this);
	}
}
...
void DeviceView::UpdateSpaceInfo(BOOL updateUsedSpace, BOOL notifyChanges)
{
		...
		FALSE != notifyChanges)
	{
		...
		{
			if (0 != ((1 << 0) & changes))
				event_handlers[i]->TotalSpaceChanged(this, totalSpace);
			if (0 != ((1 << 1) & changes))
				event_handlers[i]->TotalSpaceChanged(this, usedSpace);
		}
	}
}
...
void DeviceView::OnNameChanged(const wchar_t *new_name)
{
	for (size_t i=0;i<event_handlers.size();i++)
	{
		event_handlers[i]->DisplayNameChanged(this, new_name);
	}
}
...
void DeviceView::Sync(bool silent)
{
	...
	if(syncAllLibrary)
	{
		...
		results = AGAVE_API_MLDB->Query(querystring);
		if(results)
	...
	if (dev->extraActions(DEVICE_SUPPORTS_PODCASTS, 0, 0, 0) == 0)
	{
		...
		if(podcasteps && podcastsnum > 0)
		{
			...
			int n = 0, alloc = 512;
			s->songs = (songMapping*) malloc(sizeof(songMapping) * alloc);
						...
						if(podcast_name[0] && (all || config->ReadInt(buf,0)))   // if we have a podcast and we want to sync it
						{
							...
							if(podcasts)
							{
								...
								for(int j=0; j<podcasts->Size && (podcasteps == -1 || j < podcasteps); j++)
								{
									...
									{
										alloc += 512;
										s->songs = (songMapping*)realloc(s->songs,sizeof(songMapping) * alloc);
									}
...
bool DeviceView::PlayTracks(C_ItemList * tracks, int startPlaybackAt, bool enqueue, bool msgIfImpossible, HWND parent)
{
	...
	if(serverPort>0 && dev->copyToHardDriveSupported())
	{
		...
		for(int i=0; i<tracks->GetSize(); i++)
		{
			...
			// enqueue file
			enqueueFileWithMetaStructW ef= {wideUrl,buf2,dev->getTrackLength(s)/1000};
...
static void getReverseCopyFilenameFormat(wchar_t* filepath, wchar_t* format, int len, BOOL * uppercaseext)
{
	...
	if(((unsigned int)(ULONG_PTR)iniDirectory) < 65536) iniDirectory=0;
	PathCombineW(inifile, WASABI_API_APP->path_getUserSettingsPath(), L"Plugins\\gen_ml.ini");
	wchar_t buf[2048];
	if(cdrip) GetPrivateProfileString(L"gen_ml_config",L"extractpath",m_def_extract_path,buf,2048,inifile);
	...
	if(cdrip) GetPrivateProfileString(L"gen_ml_config",L"extractfmt2",m_def_filename_fmt,buf,2048,inifile);
	else lstrcpyn(buf,global_config->ReadString(L"extractfmt2",m_def_filename_fmt),2048);
	if(l < len) lstrcpyn(format/*+l*/,buf,len - l);
	if(cdrip) *uppercaseext = GetPrivateProfileInt(L"gen_ml_config",L"extractucext",0,inifile);
...
void DeviceView::CopyTracksToHardDrive(songid_t * tracks, int numTracks)
{
	...
	wchar_t filepath[MAX_PATH], format[2048];
	...
	{
		AddTrackToTransferQueue(new ReverseCopyInst(this,filepath,format,tracks[i],addToLibrary,!!uppercaseext));
	}
	txQueue->unlock();
}
...
void DeviceView::CopyPlaylistToLibrary(int plnum)
{
	...
	*/
	...
	txQueue->lock();
	for(int i=0; i<l; i++)
		AddTrackToTransferQueue(new ReversePlaylistCopyInst(this,filepath,format,dev->getPlaylistTrack(plnum,i),filename,name,i==l-1,addToLibrary));
	txQueue->unlock();
}
