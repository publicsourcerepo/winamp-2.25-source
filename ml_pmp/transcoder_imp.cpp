...
static nu::PtrList<TranscoderImp> transcoders;
...
LRESULT CALLBACK TranscodeMsgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) 
{
	...
	{
		for(size_t i=0; i<transcoders.size(); i++)
		{
			TranscoderImp * t = transcoders[i];
...
static void BuildEncodableFormatsList(FormatList &list, HWND winampWindow, Device *device) 
{
	...
	while (i--)
	{
		if (device->extraActions(DEVICE_VETO_ENCODER, list[i]->fourcc, 0, 0) == 1)
		{
			list.eraseindex(i);
		}
	}
...
int TranscoderImp::CanTranscode(wchar_t * file, wchar_t * ext, int length)
{
	...
	if(fmt) {
		if(ext) {
			...
			if(extA[1]) wcscpy(ext,AutoWide(extA));
