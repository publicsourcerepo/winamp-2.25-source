...
class Device {
	...
	public:
		...
		virtual bool hasArt(songid_t songid){return false;} /* DEPRECATED: not called */
...
typedef struct {
	...
	int (*init)(); // called when winamp is loaded, for any one-time init
	void (*quit)(); // called when winamp is unloaded, for any one-time deinit
