...
winampMediaLibraryPlugin plugin = {MLHDR_VER,"nullsoft(ml_pmp.dll)",init,quit,PluginMessageProc};
C_ItemList devices;
...
static int init()
{
	...
	// TODO: fix thread pool signaling so we don't have to do this
	transfer_thread = WASABI_API_THREADPOOL->ReserveThread(api_threadpool::FLAG_LONG_EXECUTION);
	...
	StringCbPrintfW(szDescription, ARRAYSIZE(szDescription),
					WASABI_API_LNGSTRINGW(IDS_NULLSOFT_PMP_SUPPORT), PLUGIN_VERSION);
	plugin.description = (char*)szDescription;

	hMainThread = GetCurrentThread();
	//InitializeCriticalSection(&listTransfersLock);
	global_config = new C_Config(AutoWide((char*)SendMessage(plugin.hwndWinampParent,WM_WA_IPC,0,IPC_GETINIFILE)));

	wchar_t inifile[MAX_PATH];
	const wchar_t *iniDirectory = WASABI_API_APP->path_getUserSettingsPath();
	PathCombineW(inifile, iniDirectory, L"Plugins");
	CreateDirectoryW(inifile, NULL);
	PathAppendW(inifile, L"gen_ml.ini");
	gen_mlconfig = new C_Config(inifile,L"gen_ml_config");
...
static void quit() 
{
	...
	while(i-- > 0) ((DeviceView*)devices.Get(i))->threadKillswitch = 1;
	...
	i = devices.GetSize();
	while(i-- > 0) ((DeviceView*)devices.Get(i))->dev->Close();
	unloadDevPlugins();
...
static void CALLBACK ProgressTimerTickCb(HWND hwnd, UINT uMsg, UINT_PTR eventId, unsigned long elapsed)
{
	...
	for(int i=0; i<devices.GetSize(); i++)
	{
		...
		LinkedQueue * txQueue = (dev->isCloudDevice ? &cloudTransferQueue : &dev->transferQueue);
		LinkedQueue * finishedTX = (dev->isCloudDevice ? &cloudFinishedTransfers : &dev->finishedTransfers);
		int txProgress = (dev->isCloudDevice ? cloudTransferProgress : dev->currentTransferProgress);
		int device_num = (100*txQueue->GetSize()) - txProgress;
		num += device_num;
		int device_total=100*txQueue->GetSize() + 100*finishedTX->GetSize();
		total += device_total;
	...
	{
		if (-1 == percent)
			WASABI_API_LNGSTRINGW_BUF(IDS_PORTABLES,buf,sizeof(buf)/sizeof(*buf));
		else
			...
			case COMMITTIMERID:
				{
					...
					{
						DeviceView * d = (DeviceView *)devices.Get(i);
						LinkedQueue * txQueue = (d->isCloudDevice ? &cloudTransferQueue : &d->transferQueue);
...
void deviceDisconnected(Device * dev)
{
	...
	for(int i = 0; i < devices.GetSize(); i++) 
	{
		...
		else
		{
			...
			if (strnicmp(d->GetName(), "all_sources", 11) && d->isCloudDevice)
			{
				cloud_count += 1;
			}
		}
...
static void URL_GetParameter(const wchar_t *param_start, char *dest, size_t dest_len)
{
	...
	while (*param_start && dest_len > 1 && *param_start != L'&')
	{
		if (*param_start == '+') 
    {
      param_start++;
						...
						case ML_ACTION_RCLICK:
							{
								...
								switch(r)
								{
									...
									case ID_MAINTREEROOT_HELP:
										SENDWAIPC(plugin.hwndWinampParent, IPC_OPEN_URL, L"http://www.winamp.com/help/Main_Page#Portables_Interface");
										break;
								}
							}
							break;
...
extern "C" {
	...
	__declspec( dllexport ) int winampUninstallPlugin(HINSTANCE hDllInst, HWND hwndDlg, int param) {
		...
		wchar_t title[256];
		StringCbPrintfW(title, ARRAYSIZE(title), L"%s %s",WASABI_API_LNGSTRINGW(IDS_NULLSOFT_PMP_SUPPORT),PLUGIN_VERSION);
