...
static void UpdateStatus(HWND hwndDlg, bool full = false) {
	...
	if (full && currentViewedDevice->isCloudDevice)
	{
		int usedPercent = 0;
...
static void LayoutWindows(HWND hwnd, BOOL fRedraw, INT simple);
	...
	switch (uMsg){
		case WM_INITDIALOG:
		{
			...
			*(void **)&wad_DrawChildWindowBorders=(void*)SendMessage(plugin.hwndLibraryParent,WM_ML_IPC,3,ML_IPC_SKIN_WADLG_GETFUNC);
			...
			if (currentViewedDevice->isCloudDevice && currentViewedDevice->config->ReadInt(L"savefilter", 1))
			{
			...
			INT ffcl[] = { IDC_BUTTON_CLEARSEARCH, IDC_BUTTON_CLEARREFINE, IDC_BUTTON_EJECT, IDC_BUTTON_PLAY,
						   IDC_BUTTON_ENQUEUE, IDC_BUTTON_SYNC, IDC_BUTTON_AUTOFILL, IDC_BUTTON_ARTMODE,
						   IDC_BUTTON_VIEWMODE, IDC_BUTTON_COLUMNS,
		...
		case WM_LBUTTONDOWN:
		{
			...
			{
				hwndDiv = GetDlgItem(hwndDlg, id[i]);
				if (!hwndDiv) continue;
		...
		case WM_PAINT:
		{
			...
			if(m_nodrawtopborders==1) size-=4;
		...
		case WM_USER:
		{
			...
			if (aacontents) aacontents->SelectionChanged(wParam==IDC_LIST_ARTIST?0:(wParam==IDC_LIST_ALBUM?1:2),lists);
			noSearchTimer=true;
			...
			if(l->idFrom==IDC_LIST_ARTIST || l->idFrom==IDC_LIST_ALBUM || l->idFrom==IDC_LIST_ALBUM2)
			{
				switch(l->code) {
					...
					case NM_DBLCLK: // play some songs!
					{
						...
						currentViewedDevice->PlayTracks(items,0,(!!(GetAsyncKeyState(VK_SHIFT)&0x8000)),false);
						delete items;
						break;
					}
					case LVN_KEYDOWN:
			...
			switch(LOWORD(wParam)) {
				...
				case IDC_QUICKSEARCH:
					if (HIWORD(wParam) == EN_CHANGE && !noSearchTimer) {
						KillTimer(hwndDlg, 500);
						SetTimer(hwndDlg, 500, (!currentViewedDevice->isCloudDevice ? 250 : 350), NULL);
		...
		case WM_INITDIALOG:
			...
			*(void **)&wad_DrawChildWindowBorders=(void*)SendMessage(plugin.hwndLibraryParent,WM_ML_IPC,3,ML_IPC_SKIN_WADLG_GETFUNC);
...

			{
				FLICKERFIX ff;
				ff.mode = FFM_ERASEINPAINT;
				INT ffcl[] = { IDC_BUTTON_PLAY, IDC_BUTTON_ENQUEUE, IDC_BUTTON_SORT, IDC_BUTTON_EJECT};
				for (int  i = 0; i < sizeof(ffcl) / sizeof(INT); i++)
				{
					ff.hwnd = GetDlgItem(hwndDlg, ffcl[i]);
					SendMessage(plugin.hwndLibraryParent, WM_ML_IPC, (WPARAM)&ff, ML_IPC_FLICKERFIX);
	...
	switch (uMsg) {
		case WM_INITDIALOG:
		{
			...
			*(void **)&wad_DrawChildWindowBorders=(void*)SendMessage(plugin.hwndLibraryParent,WM_ML_IPC,3,ML_IPC_SKIN_WADLG_GETFUNC);
			...
			// depending on how this is called, it will either do a video only view or will be re-purposed as a 'simple' cloud view
			aacontents = new ArtistAlbumLists(currentViewedDevice->dev, currentViewedPlaylist, currentViewedDevice->config,
											  NULL, 0, (lParam ? -1 : (currentViewedDevice->videoView ? 1 : -1)), !!lParam);
			...
			ff.mode = FFM_ERASEINPAINT;
			INT ffcl[] = { IDC_BUTTON_CLEARSEARCH, IDC_BUTTON_CLEARREFINE, IDC_BUTTON_EJECT, IDC_BUTTON_PLAY,
						   IDC_BUTTON_ENQUEUE, IDC_BUTTON_SYNC, IDC_BUTTON_AUTOFILL,
						   IDC_HEADER_DEVICE_ICON, IDC_HEADER_DEVICE_NAME, IDC_HEADER_DEVICE_BAR,
						   IDC_HEADER_DEVICE_SIZE, IDC_HEADER_DEVICE_TRANSFER,
			};
...
static INT_PTR CALLBACK pmp_common_dlgproc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch (uMsg) {
		...
		case WM_DESTROY:
		{
			if (currentViewedDevice && currentViewedDevice->isCloudDevice)
			{
			...
			if(l->idFrom == IDC_LIST_TRACKS)
			{
				switch(l->code) {
					...
					case NM_DBLCLK:
					{
						...
						int start=0;
						int l=tracksList->listview.GetCount();
						bool enqueue = !!(GetAsyncKeyState(VK_SHIFT)&0x8000);
						int i;
						for(i=0; i<l; i++) if(tracksList->listview.GetSelected(i)) { start=i; break;}
						if(items->GetSize() && (enqueue || !gen_mlconfig->ReadInt(playmode,1))) { void* x = items->Get(i); delete items; items = new C_ItemList; items->Add(x); start=0; }
						currentViewedDevice->PlayTracks(items,start,enqueue,false);
						delete items;
					}
					break;
		...
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				...
				case IDC_BUTTON_ENQUEUE:
				{
					...
					currentViewedDevice->PlayTracks(selected,0,LOWORD(wParam)==IDC_BUTTON_ENQUEUE,true);
					delete selected;
				}
				break;
			}
...
static void LayoutWindows(HWND hwnd, BOOL fRedraw, int simple)
{
	static INT controls[] = 
	{
		...
		GROUP_STATUS, IDC_BUTTON_EJECT, IDC_BUTTON_PLAY, IDC_BUTTON_ENQUEUE, IDC_BUTTON_SYNC, IDC_BUTTON_AUTOFILL, IDC_BUTTON_SORT, IDC_STATUS,
	...
	for (index = 0; index < sizeof(controls) / sizeof(INT); index++)
	{
		if (controls[index] >= GROUP_MIN && controls[index] <= GROUP_MAX) // group id
		{
			...
			switch(controls[index])
			{
				case GROUP_HEADER:
					...
					{
						SetRect(&rg, rc.left, rc.top + 2, rc.right + 1, (!header ? rc.top + 18 : rc.top));
						rc.top = rg.bottom + 3;
					}
					else
						skipgroup = 1;
				...
				case GROUP_SEARCH:
					...
					{
						SetRect(&rg, rc.left - 1, rc.top + (!header ? 3 : 0), rc.right - 2, rc.top + (!header ? 19 : 16));
						rc.top = rg.bottom + 3;
					}
					else
						skipgroup = 1;
				...
				case GROUP_STATUS:
					if (g_displaystatus)  
					{
						SetRect(&rg, rc.left, rc.bottom - 14, rc.right, rc.bottom);
						rc.bottom = rg.top - 3;
					}
				...
				case GROUP_REFINE:
					rg.top = divY + divCY;
					refineHidden = (g_displayrefine) ? ((rg.top + 17) >= rc.bottom) : TRUE;
					SetRect(&rg, rc.left, rg.top, rc.right, (refineHidden) ? rg.top : (rg.top + 17));
					break;
				case GROUP_TRACKS:
					if (!simple)
						SetRect(&rg, rc.left, (divY + divCY) + ((!refineHidden) ? 20 : 0), rc.right, rc.bottom - 26);
					else
						SetRect(&rg, rc.left, rc.top, rc.right, rc.bottom - 26);
					break;
				case GROUP_FILTER:
					if (!simple)
					{
						...
						SetRect(&rg, rc.left, rc.top, rc.right, divY);
					}
					else
						skipgroup = 1;
					break;
		...
		{
			case IDC_HEADER_DEVICE_TRANSFER:
				SETLAYOUTPOS(pl, rg.right - 18, rg.top - 1, 16, 16);
				pl->flags |= (!header && (rg.right - rg.left) > (ri.right - ri.left)) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				if (SWP_SHOWWINDOW & pl->flags) rg.right -= (pl->cx + 3+16);
				break;
			case IDC_HEADER_DEVICE_ICON:
				SETLAYOUTPOS(pl, rg.left, rg.top - 1, 16, 16);
				pl->flags |= (!header && (rg.right - rg.left + 16) > (ri.right - ri.left)) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
			...
			case IDC_HEADER_DEVICE_NAME:
			{
				...
				SETLAYOUTPOS(pl, rg.left + 4, rg.top, s.cx + ((s.cx / len)), 16);
				pl->flags |= (!header && (rg.right - rg.left + 12) > (ri.right - ri.left)) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				if (SWP_SHOWWINDOW & pl->flags) rg.left += (pl->cx + 4);
				break;
			}
			case IDC_HEADER_DEVICE_BAR:
				SETLAYOUTPOS(pl, rg.left + 4, rg.top + 2, 150, 11);
				pl->flags |= (!header && (rg.right - rg.left) > (ri.right - ri.left)) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				if (SWP_SHOWWINDOW & pl->flags) rg.left += (pl->cx + 8);
				break;
			case IDC_HEADER_DEVICE_SIZE:
				SETLAYOUTPOS(pl, rg.left + 4, rg.top, rg.right - rg.left, 16);
				pl->flags |= (!header && (ri.right - ri.left) > 60) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				if (SWP_SHOWWINDOW & pl->flags) rg.left += (pl->cx + 4);
				break;
			case IDC_HDELIM2:
				SETLAYOUTPOS(pl, 0, rg.bottom + 1, rg.right + 35, 0);
			...
			case IDC_BUTTON_COLUMNS:
				...
				if (SWP_SHOWWINDOW & pl->flags) rg.left += (pl->cx + 1);
				break;
			case IDC_SEARCH_TEXT:
				pl->flags |= (rg.top < rg.bottom) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				SETLAYOUTPOS(pl, rg.left + 5, rg.top + 1, (ri.right - ri.left), (rg.bottom - rg.top)); 
				if (SWP_SHOWWINDOW & pl->flags) rg.left += (pl->cx + 4)+6;
				break;
			case IDC_REFINE_TEXT:
				...
				break;
			case IDC_BUTTON_CLEARSEARCH:
				pl->flags |= ((((rg.right - rg.left) - (ri.right - ri.left)) > 40) && (rg.top < rg.bottom)) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				SETLAYOUTPOS(pl, rg.right - (ri.right - ri.left) + 4, rg.top, (ri.right - ri.left), (rg.bottom - rg.top)); 
				if (SWP_SHOWWINDOW & pl->flags) rg.right -=  (pl->cx + 4);
				break;
			case IDC_BUTTON_CLEARREFINE:
				pl->flags |= ((((rg.right - rg.left) - (ri.right - ri.left)) > 40) && (rg.top < rg.bottom)) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				SETLAYOUTPOS(pl, rg.right - (ri.right - ri.left) + 2, rg.top, (ri.right - ri.left), (rg.bottom - rg.top)); 
				if (SWP_SHOWWINDOW & pl->flags) rg.right -=  (pl->cx + 4);
				break;
			case IDC_QUICKSEARCH:
				pl->flags |= ((rg.right > rg.left) && (rg.top < rg.bottom)) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				SETLAYOUTPOS(pl, rg.left, rg.top, rg.right - rg.left + 4, (rg.bottom - rg.top) - 1);
				break;
			case IDC_REFINE:
				pl->flags |= ((rg.right > rg.left) && (rg.top < rg.bottom)) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				SETLAYOUTPOS(pl, rg.left, rg.top, rg.right - rg.left + 2, (rg.bottom - rg.top) - 1);
				break;
			case IDC_BUTTON_EJECT:
				if (currentViewedDevice->isCloudDevice)
				{
				...
				SETLAYOUTPOS(pl, rg.right - (ri.right - ri.left) + 2/* - 1*/, rg.bottom - 18, (ri.right - ri.left), 18);
				pl->flags |= ((rg.right - rg.left) > (ri.right - ri.left)) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				if (SWP_SHOWWINDOW & pl->flags) rg.right -= (pl->cx + 3);
				break;
			case IDC_BUTTON_AUTOFILL:
				if (currentViewedDevice->isCloudDevice)
				{
			...
			case IDC_BUTTON_ENQUEUE:
			case IDC_BUTTON_SORT:
				SETLAYOUTPOS(pl, rg.left, rg.bottom - 18, ri.right - ri.left, 18);
				pl->flags |= ((rg.right - rg.left) > (ri.right - ri.left)) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				if (SWP_SHOWWINDOW & pl->flags) rg.left += (pl->cx + 4);
				break;
			case IDC_STATUS:
				SETLAYOUTPOS(pl, rg.left, rg.bottom - 18, rg.right - rg.left, 17);
				if (SWP_SHOWWINDOW & pl->flags) rg.top = (pl->y + pl->cy + 1);
				break;
			case IDC_HDELIM:
				divCY = ri.bottom - ri.top;
				if (divY > (rg.bottom - 70)) { divY = rg.bottom - divCY; m_nodrawtopborders = 2; }
				else if (divY < (rg.top + 36)) { divY = rg.top; m_nodrawtopborders = 1; }
				else  m_nodrawtopborders = 0;
				SETLAYOUTPOS(pl, rg.left, divY, rg.right - rg.left + 2, (ri.bottom - ri.top));
				break;
			case IDC_LIST_TRACKS:
				pl->flags |= (rg.top < rg.bottom) ? SWP_SHOWWINDOW : SWP_HIDEWINDOW;
				SETLAYOUTPOS(pl, rg.left, rg.top + 1, rg.right - rg.left + 1, (rg.bottom - rg.top) + 20);
				break;
			case IDC_LIST_ARTIST:
				SETLAYOUTPOS(pl, rg.left, rg.top + 1, divX, (rg.bottom - rg.top) - 1);
				rg.left += pl->cx;
				break;
			case IDC_VDELIM:
				SETLAYOUTPOS(pl, rg.left, rg.top + 1, (!(rg.bottom - rg.top) ? 0 : (ri.right - ri.left)), (rg.bottom - rg.top));
				rg.left += pl->cx;
				break;
			case IDC_LIST_ALBUM:
				if(numFilters == 3)
				{
					BOOL hide = ((divX2 - divX - 1) < 15);
					pl->flags |= hide ? SWP_HIDEWINDOW : SWP_SHOWWINDOW;
					SETLAYOUTPOS(pl, rg.left, rg.top + 1, (hide ? 0 : divX2 - divX - 1), (rg.bottom - rg.top) - 1);
				}
				else { SETLAYOUTPOS(pl, rg.left, rg.top + 1, (rg.right - rg.left) + 1, (rg.bottom - rg.top) - 1); }
				rg.left += pl->cx;
				break;
			case IDC_VDELIM2:
				SETLAYOUTPOS(pl, rg.left, rg.top + 1, (numFilters == 2 ? 0 : ri.right - ri.left), (rg.bottom - rg.top));
				rg.left += pl->cx;
				break;
			case IDC_LIST_ALBUM2:
				SETLAYOUTPOS(pl, rg.left, rg.top + 1, (rg.right - rg.left) + 1, (rg.bottom - rg.top) - 2);
				break;
		}
...
static void WINAPI OnDividerMoved(HWND hwnd, INT nPos, LPARAM param)
{
	...
	if (hwndParent)
	{
		...
		switch((INT)param)
		{
			case IDC_VDELIM:
				{
					adiv1pos = (nPos * 100000) / (rc.right - rc.left);
		...
		LayoutWindows(hwndParent, TRUE, FALSE);
	}
}
...
static BOOL AttachDivider(HWND hwnd, BOOL fVertical, DIVIDERMOVED callback, LPARAM param)
{
	...
	pd = (DIVIDER*)malloc(sizeof(DIVIDER));
	if (!pd) return FALSE;
