...
BOOL AlbumArtListView::CalcuateItemHeight(void)
{
	...
	if (isDetailsMode(mode))
	{
		...
		newHeight = max(h, textHeight * (INT)contents->GetNumColumns()) + 12;
...
BOOL AlbumArtListView::DialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam) {
	switch(uMsg) {
		...
		case WM_NOTIFY:
			...
			}
			{
				...
				if(l->code == NM_RCLICK && l->hwndFrom == ListView_GetHeader(listview.getwnd())) {
					...
					wchar_t artist[100], album[100], tracks[10], year[10];
		...
		{
			HWND hwndList, hwndRealList;
			RECT ri;
			hwndList = GetDlgItem(hwndDlg, dlgitem);
			if (hwndList)
			{
				hwndRealList = (HWND)SendMessageW(hwndList, WM_EX_GETREALLIST, 0, 0L);
				if (hwndRealList)
				{
					int w, h;
					getImgSize(mode, w, h);	
