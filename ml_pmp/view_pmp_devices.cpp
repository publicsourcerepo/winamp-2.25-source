...
{
	wchar_t caption[256];
	int pcnum=0,num=0,time=0,pc,total=0;
	...
	for (int i=0; i<devices.GetSize(); i++)
	{
		...
		int s = txQueue->GetSize();
		int n = (s * 100) - txProgress;
		total += s * 100;
		total += 100*finishedTX->GetSize();
		pcnum += n;
		num += s;
		int t = (int)(dev->transferRate * (((double)n)/100.0));
		if (time < t) time = t;
	}
	if (total)
	{
		...
		if (l->idFrom==IDC_LIST_TRANSFERS)
			...
			case LVN_KEYDOWN:
			{
				...
				for (int i=0; i<devices.GetSize(); i++)
				{
					...
					LinkedQueue * txQueue = (device->isCloudDevice ? &cloudTransferQueue : &device->transferQueue);
					LinkedQueue * finishedTX = (device->isCloudDevice ? &cloudFinishedTransfers : &device->finishedTransfers);
					if (!AddSelectedItems(&items, &listTransfers->listview, txQueue, row, dev))
						return 0;
					if (!AddSelectedItems(&items, &listTransfers->listview, finishedTX, row, dev))
						return 0;
				}
				bool foundDev=false;
