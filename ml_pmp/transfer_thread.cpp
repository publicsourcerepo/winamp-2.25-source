...
void ReversePlaylistCopyInst::PostCopyAction()
{
	...
	if(f) {
		fputws(L"#EXTINF:",f);
		wchar_t buf[100]; wsprintf(buf,L"%d",dev->dev->getTrackLength(songid)/1000);
		fputws(buf,f);
		fputws(L",",f);
		wchar_t title[2048];
...
void TransferContext::DoOneTransfer(HANDLE handle)
{
	if (TryEnterCriticalSection(&transfer_lock))
	{
		...
		CopyInst * c = (CopyInst *)txQueue->Peek();
		if (c)
		{
			...
			{ // dupe
				...
				{
					WASABI_API_LNGSTRINGW_BUF(IDS_ALREADY_UPLOADED, c->statusCaption, sizeof(c->statusCaption)/sizeof(wchar_t));
				}
				else
				{
					WASABI_API_LNGSTRINGW_BUF(IDS_DUPLICATE, c->statusCaption, sizeof(c->statusCaption)/sizeof(wchar_t));
				}
