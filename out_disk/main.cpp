...
static int _stdcall browzaproc(HWND wnd,UINT msg,LPARAM lp,LPARAM dat)
{
	if (msg==BFFM_INITIALIZED) SendMessage(wnd,BFFM_SETSELECTION,1,dat);
	return 0;
}

char g_tmp[MAX_PATH], g_tmp_sf[MAX_PATH];
...
BOOL CALLBACK DlgProc(HWND wnd,UINT msg,WPARAM wp,LPARAM lp)
{
	...
	case WM_COMMAND:
		...
		else if (wp==IDOK)
		{
			...
			cfg_output_mode=SendDlgItemMessage(wnd,IDC_MODE,CB_GETCURSEL,0,0);
			...
			cfg_format_mode=SendDlgItemMessage(wnd,IDC_FILENAME_FORMAT,CB_GETCURSEL,0,0);
			cfg_format_index=SendDlgItemMessage(wnd,IDC_FILENAME_INDEX,CB_GETCURSEL,0,0);

			EndDialog(wnd,0);
		}
		else if (wp==IDC_SINGLEFILE_FILE_BUTTON)
		{
			char tmp[MAX_PATH], filter[64];
...
{
	int index = SendMessage(mod.hMainWindow,WM_USER,0,IPC_GETLISTPOS);
	...
	{
		char * dir=cfg_output_dir, tmp[MAX_PATH];
		if (!*dir) dir=GetDefaultSaveToFolder(tmp);
		while(*dir) {*(out++)=*(dir++);}
		if (out[-1]!='\\') *(out++)='\\';
	}
	...
	while(*format_ext) *(out++)=*(format_ext++);
	*out=0;
}
...
static void file_err(char* f,char* t)
{
	wchar_t tmp[512+128];
	StringCchPrintfW(tmp, 512+128, L"%hs: \"%hs\"",t,f);
...
int Open(int sr,int nch,int bps,int xx,int xxx)
{
	...
	else
	{
		...
		if (cfg_show_saveas)
		{
			char filter[64], title[128], title2[128];
	...
	if (hOut==INVALID_HANDLE_VALUE)
	{
		...
		if (hOut==INVALID_HANDLE_VALUE)
		{
			file_err(fn,WASABI_API_LNGSTRING(IDS_CANNOT_CREATE_OUTPUT_FILE));
			return -1;
		}
	}
_ap:
	...
	if (append)
	{
		...
		if (hOut==INVALID_HANDLE_VALUE)
		{
			file_err(fn,WASABI_API_LNGSTRING(IDS_CANNOT_OPEN_FILE));
			return -1;
		}
		{
			DWORD br;
			...
			ReadFile(hOut,&wfs,4,&br,0);
			if (wfs<sizeof(ap_wfx)-2 || wfs>sizeof(ap_wfx)) goto ap_f;
			ReadFile(hOut,&ap_wfx,wfs,&br,0);
			...
			DWORD data_size=0;
			ReadFile(hOut,&data_size,4,&br,0);
		...
		goto ap_ok;
ap_f:
		file_err(fn,WASABI_API_LNGSTRING(IDS_NOT_A_PCM_WAV_FILE));
	...
	if (hOut!=INVALID_HANDLE_VALUE)
	{
		if (riff)
		{
			...
			if (data_ofs)
			{
				...
				WriteFile(hOut,&data_size,4,&bw,0);
			}
			if (fact_ofs)
			{
...
void do_cvt(BYTE* data,UINT size)
{
...
#endif
	{
		...
		if (res)
		{
			...
			MMRESULT rs = acmStreamConvert(hStream,&ahd,flags);
