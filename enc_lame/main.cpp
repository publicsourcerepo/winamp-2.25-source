...
void BuildVersionString(char version[128])
{
	BE_VERSION ver;
	beVersion(&ver);

	if (ver.byBetaLevel)
		StringCchPrintf(version, 128, "%u.%ub%u", (unsigned int)ver.byMajorVersion, (unsigned int)ver.byMinorVersion, (unsigned int)ver.byBetaLevel);
	else if (ver.byAlphaLevel)
		StringCchPrintf(version, 128, "%u.%ua%u", (unsigned int)ver.byMajorVersion, (unsigned int)ver.byMinorVersion, (unsigned int)ver.byAlphaLevel);
	else
		StringCchPrintf(version, 128, "%u.%u", (unsigned int)ver.byMajorVersion, (unsigned int)ver.byMinorVersion);
}
...
void readconfig(char *configfile, configtype *cfg)
{
	...
	if (configfile) GetPrivateProfileStruct("audio_mp3l", "conf", cfg, sizeof(configtype), configfile);
}

void writeconfig(char *configfile, configtype *cfg)
{
	if (configfile) WritePrivateProfileStruct("audio_mp3l", "conf", cfg, sizeof(configtype), configfile);
}
...
extern "C"
{
	unsigned int __declspec(dllexport) GetAudioTypes3(int idx, char *desc)
	{
		if ((g_lamedll != NULL) && (beInitStream != NULL) && (beCloseStream != NULL) && (beEncodeChunk != NULL) && (beDeinitStream != NULL) && (beWriteVBRHeader != NULL) && (beVersion != NULL))
			if (idx == 0)
			{
				...
				StringCchPrintf(desc, 1024, WASABI_API_LNGSTRING(IDS_ENC_LAME_DESC), ENC_VERSION);
	...
	BOOL CALLBACK DlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		if (uMsg == WM_USER + 666)
		{
			int vbr_meth = SendDlgItemMessage(hwndDlg, IDC_VBRMETHOD, CB_GETCURSEL, 0, 0);
		...
		if (uMsg == WM_INITDIALOG)
		{
			...
			if (lParam)
			{
				...
				SendDlgItemMessage(hwndDlg, IDC_VBRMETHOD, CB_ADDSTRING, 0, (LPARAM)"CBR");
				SendDlgItemMessage(hwndDlg, IDC_VBRMETHOD, CB_ADDSTRING, 0, (LPARAM)"VBR default");
				SendDlgItemMessage(hwndDlg, IDC_VBRMETHOD, CB_ADDSTRING, 0, (LPARAM)"VBR old");
				SendDlgItemMessage(hwndDlg, IDC_VBRMETHOD, CB_ADDSTRING, 0, (LPARAM)"VBR new");
				SendDlgItemMessage(hwndDlg, IDC_VBRMETHOD, CB_ADDSTRING, 0, (LPARAM)"VBR mtrh");
				SendDlgItemMessage(hwndDlg, IDC_VBRMETHOD, CB_ADDSTRING, 0, (LPARAM)"ABR");
				SendDlgItemMessage(hwndDlg, IDC_VBRMETHOD, CB_SETCURSEL, wc->cfg.vbr_method + 1, 0);
				...
				SendDlgItemMessage(hwndDlg, IDC_STEREOMODE, CB_SETCURSEL, wc->cfg.stereo_mode, 0);

				{
					...
					{
						char buf[64];
						StringCchPrintf(buf, 64, "%d", g_valid_bitrates[i]);
				...
				SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_ADDSTRING, 0, (LPARAM)"--alt-preset standard");
				SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_ADDSTRING, 0, (LPARAM)"--alt-preset fast standard");
				SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_ADDSTRING, 0, (LPARAM)"--alt-preset extreme");
				SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_ADDSTRING, 0, (LPARAM)"--alt-preset fast extreme");
				SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_ADDSTRING, 0, (LPARAM)"--alt-preset insane");
				SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_ADDSTRING, 0, (LPARAM)"--alt-preset abr");
				SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_ADDSTRING, 0, (LPARAM)"--alt-preset cbr");
				SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_ADDSTRING, 0, (LPARAM)"--alt-preset medium");
				SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_ADDSTRING, 0, (LPARAM)"--alt-preset fast medium");
				SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_SETCURSEL, wc->cfg.quality, 0);
		...
		if (uMsg == WM_DESTROY)
		{
			...
			if (wc)
			{
				wc->cfg.bitrate = SendDlgItemMessage(hwndDlg, IDC_BITRATE , CB_GETITEMDATA, SendDlgItemMessage(hwndDlg, IDC_BITRATE , CB_GETCURSEL, 0, 0), 0);
				wc->cfg.vbr_max_bitrate = SendDlgItemMessage(hwndDlg, IDC_MAXBITRATE, CB_GETITEMDATA, SendDlgItemMessage(hwndDlg, IDC_MAXBITRATE, CB_GETCURSEL, 0, 0), 0);
				wc->cfg.abr_bitrate = SendDlgItemMessage(hwndDlg, IDC_AVGBITRATE, CB_GETITEMDATA, SendDlgItemMessage(hwndDlg, IDC_AVGBITRATE, CB_GETCURSEL, 0, 0), 0);
				int vbr_meth = SendDlgItemMessage(hwndDlg, IDC_VBRMETHOD, CB_GETCURSEL, 0, 0);
				if (vbr_meth == CB_ERR) vbr_meth = 0;
				wc->cfg.vbr_method = vbr_meth - 1;
				wc->cfg.stereo_mode = SendDlgItemMessage(hwndDlg, IDC_STEREOMODE, CB_GETCURSEL, 0, 0);
				wc->cfg.quality = SendDlgItemMessage(hwndDlg, IDC_QUALITY, CB_GETCURSEL, 0, 0);
				wc->cfg.vbr = SendDlgItemMessage(hwndDlg, IDC_VBRQ, CB_GETCURSEL, 0, 0);
	...
	int __declspec(dllexport) SetConfigItem(unsigned int outt, char *item, char *data, char *configfile)
	{
		if (outt == mmioFOURCC('M', 'P', '3', 'l'))
		{
			configtype cfg;
			readconfig(configfile, &cfg);
			if (!lstrcmpi(item, "bitrate")) // assume CBR bitrate
			{
	...
	int __declspec(dllexport) GetConfigItem(unsigned int outt, char *item, char *data, int len, char *configfile)
	{
		if (outt == mmioFOURCC('M', 'P', '3', 'l'))
		{
			...
			if (!lstrcmpi(item, "bitrate"))
			{
				...
				StringCchPrintf(data,len,"%d",bitrate);
			}
			return 1;
		}
		return 0;
	...
	void __declspec(dllexport) SetWinampHWND(HWND hwnd)
	{
		...
		{
			char tmp[2048];
			GetModuleFileName(GetMyInstance(), tmp, sizeof(tmp) - 1);

			char *p = tmp + lstrlen(tmp) - 1;
			while (*p && *p != '\\') p--;
			if (*p == '\\') *p = 0;

			char lamedll[2048];
			StringCchPrintf(lamedll, 2048, "%s\\lame_enc.dll", tmp);

			g_lamedll = LoadLibrary(lamedll);
		}

		if (g_lamedll)
		{
			beInitStream	= (BEINITSTREAM) GetProcAddress(g_lamedll, TEXT_BEINITSTREAM);
