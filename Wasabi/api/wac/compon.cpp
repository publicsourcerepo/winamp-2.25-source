...
public:
   ...
   component_slot(Library *_dll, WaComponent *_wac, ComponentAPI *_api, GUID g, const wchar_t *orig_path) 
     : dll(_dll), wac(_wac), componentapi(_api), guid(g), path(orig_path) {
#else
   component_slot(Library *_dll, WaComponent *_wac, GUID g, const wchar_t *orig_path) 
     : dll(_dll), wac(_wac), guid(g), path(orig_path) {
#endif
    int fnlen = wcslen(Std::filename(orig_path));
    path.trunc(-fnlen);
    postonregsvcs = 0;
    postoncreate = 0;
...
void ComponentManager::load(const wchar_t *filename) {
  ...
  const wchar_t *ext = Std::extension(inifile);
	int len = wcslen(ext);
  inifile.trunc(-len);
  inifile.cat(L"ini");
  IniFile ini(inifile);
...
void ComponentManager::loadAll(const wchar_t *path) {
  ...
  while (dir.next()) {
    ...
    const wchar_t *ext = Std::extension(fn);
