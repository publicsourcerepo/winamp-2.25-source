...
{
#ifdef WASABI_API_CONFIG
  Std::setDefaultFont(cfg_options_defaultfont.getValue());
  Std::setDefaultFontScale(cfg_options_defaultfontscale.getValueAsInt());
#endif
}
...
svc_font *Font::requestSkinFont(const wchar_t *id, int *size, int *gotdefault) 
{
  ...
  */
  ...
  // not found ? try to find it in the windows fonts directory
  {
    ...
    Std::getFontPath(WA_MAX_PATH, fp);
    StringW file;
		file.own(fp);
//  FREE(fp); // benski> no need because we now own it
  ...
  if (f) {
    ...
    if (oldsize != -1 && size) {
      *size = oldsize;
      double f = (double)Std::getDefaultFontScale() / 100.0;
      *size = (int)(*size*f);
    }
    // Query Std:: and build the path to the default font file.
    wchar_t *fontPath = WMALLOC(WA_MAX_PATH);
    Std::getFontPath(WA_MAX_PATH, fontPath);
    wchar_t fontFile[WA_MAX_PATH];
    Std::getDefaultFont(WA_MAX_PATH, fontFile);
		StringW defaultFont;
