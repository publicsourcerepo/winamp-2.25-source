...
int VCPU::addScript(void *mem, int memsize, int id) {
  ...
  if (!translateobjects) {
    ...
    for (int z=0;z<nGuids;z++) {
      ...
      if (t == -1) { 
        DebugString("maki class entry %d not found : %s\n", z, zz);
//        __asm int 3; 
      }
      typetable->addItem(t);
    }
...
//CUT!!!! so annoying      Std::messageBox("Error while loading a script, a component is missing", "Oops", 0);
      DebugString("Tried to link DLF %d (class entry %d) but the class isn't here\n", i, pt - CLASS_ID_BASE);
      //return -1;
    }
    ...
    switch (opcode) {
      ...
      case OPCODE_ADD: {
        ...
        if (v2.v.type == SCRIPT_STRING)
          ...
          if (v1.v.data.sdata) 
					{
            wcscpy(s, v1.v.data.sdata);
            wcscat(s, v2.v.data.sdata);
          } else
            wcscpy(s, v2.v.data.sdata);
      ...
      {
#ifdef _WIN32
		  OutputDebugString("Opcode 0 - NOP encountered, please check!\n");
#else
#warning port me
#endif
        break;
