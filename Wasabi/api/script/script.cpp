...
int Script::addScript(const wchar_t *path, const wchar_t *filename, const wchar_t *id) 
{
  ...
  wchar_t olddir[WA_MAX_PATH];
  Std::getCurDir(olddir, WA_MAX_PATH);
...
#else
      Std::messageBox(StringPrintfW(L"Could not load script \'%s%s\'\r\n\r\nPress \'OK\' to continue.", path, filename), L"Skin Warning", 0);
#endif
      if (redisplay) {
#ifdef WANT_WASABI_API_WNDMGR
...
int Script::codeToSeverity(int code, wchar_t *t) 
{
  switch (code) {
    case GURU_INVALIDHEADER:
    case GURU_OLDFORMAT:
      wcscpy(t, L"Ignoring script");
      return 5;
    case GURU_DIVBYZERO:
      wcscpy(t, L"Returning 0");
      return 4;
    case GURU_NEWFAILED:
      wcscpy(t, L"Returning NULL");
      return 4;
    case GURU_NULLCALLED:
      wcscpy(t, L"Ignoring call");
      return 4;
    case GURU_INCOMPATIBLEOBJECT:
      wcscpy(t, L"Assigning NULL");
      return 4;
    default:
      wcscpy(t, L"Internal error");
      return 9;
  }
}
