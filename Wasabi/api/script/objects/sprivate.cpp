...
public:
	...
	void OnFinish (DownloadToken token)
	{
		if (!validSO)
		{
			DeleteFileW(AGAVE_API_DOWNLOADMANAGER->GetLocation(token));
			delete this;
			return;
		}
		...
		if (http)
		{
			...
			if (204 != replyCode)
			{
				...
				downloadDest = AGAVE_API_DOWNLOADMANAGER->GetLocation(token);
				succeeded = FALSE;
...
scriptVar SPrivate::vcpu_updateLinks(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar version, scriptVar bversion)
{
	...
	AGAVE_API_DOWNLOADMANAGER->Download(url, sp->dlcb);
