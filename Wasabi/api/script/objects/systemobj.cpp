...
{
	if (loaded && started_up) vcpu_onScriptUnloading(SCRIPT_CALL, getScriptObject());
...
{
	SCRIPT_FUNCTION_INIT;
	time_t ts = Std::getTimeStamp();
	struct tm *tm_now;
	tm_now = localtime((const time_t *)&ts);
	uint32_t tcnow = Std::getTickCount();
...
scriptVar SystemObject::vcpu_setClipboard(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar text)
{
	...
	OpenClipboard(0);
	EmptyClipboard();
	clipbuffer = GlobalAlloc(GMEM_DDESHARE, sizeof(wchar_t) * (wcslen(source)+1));
...
scriptVar SystemObject::vcpu_getParam(SCRIPT_FUNCTION_PARAMS, ScriptObject *object)
{
	...
	if (o)
		return MAKE_SCRIPT_STRING(o->getParam());
...
scriptVar SystemObject::vcpu_newGroup(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar groupname)
{
	...
	Group *gr = static_cast<Group *>(g->guiobject_getScriptObject()->vcpu_getInterface(groupGuid));
...
scriptVar SystemObject::vcpu_isDesktopAlphaAvailable(SCRIPT_FUNCTION_PARAMS, ScriptObject *object)
{
	...
	return MAKE_SCRIPT_INT(StdWnd::isDesktopAlphaAvailable() && cfg_uioptions_desktopalpha.getValueAsInt());
#else
	return MAKE_SCRIPT_INT(StdWnd::isDesktopAlphaAvailable());
#endif
}
...
{
	SCRIPT_FUNCTION_INIT;
	return MAKE_SCRIPT_INT(StdWnd::isTransparencyAvailable());
}
...
{
	SCRIPT_FUNCTION_INIT;
	SOM::mid(staticStr, GET_SCRIPT_STRING(str), GET_SCRIPT_INT(start), GET_SCRIPT_INT(len));
	return MAKE_SCRIPT_STRING(staticStr);
}
...
{
	SCRIPT_FUNCTION_INIT;
	SOM::mid(staticStr, GET_SCRIPT_STRING(str), 0, GET_SCRIPT_INT(len));
	return MAKE_SCRIPT_STRING(staticStr);
}
...
scriptVar SystemObject::vcpu_strright(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar _str, scriptVar _len)
{
	...
	SOM::mid(staticStr, str, wcslen(str)-len, len);
	return MAKE_SCRIPT_STRING(staticStr);
}
...
{
	SCRIPT_FUNCTION_INIT;
	wcscpy(staticStr, GET_SCRIPT_STRING(_str));
#ifdef _WIN32
...
{
	SCRIPT_FUNCTION_INIT;
	const wchar_t *pt = Std::extension(GET_SCRIPT_STRING(_str));
	if (pt != NULL) wcscpy(staticStr, pt);
	else *staticStr = '\0';
	return MAKE_SCRIPT_STRING(staticStr);
}
...
scriptVar SystemObject::vcpu_getViewportWidthGO(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar obj)
{
	...
	RECT r;
	Std::getViewport(&r, o->guiobject_getRootWnd()->getOsWindowHandle());
	return MAKE_SCRIPT_INT(r.right-r.left);
}
...
scriptVar SystemObject::vcpu_getViewportHeightGO(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar obj)
{
	...
	RECT full;
	Std::getViewport(&full, o->guiobject_getRootWnd()->getOsWindowHandle(), true);
	if (Std::rectEqual(&full, &r))
		r.bottom--;
...
scriptVar SystemObject::vcpu_getMonitorLeftGO(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar obj)
{
	...
	GuiObject *o = static_cast<GuiObject *>(GET_SCRIPT_OBJECT_AS(obj, guiObjectGuid));
	RECT r;
	Std::getViewport(&r, o->guiobject_getRootWnd()->getOsWindowHandle(), 1);
	return MAKE_SCRIPT_INT(r.left);
}
...
scriptVar SystemObject::vcpu_getMonitorHeightGO(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar obj)
{
	...
	GuiObject *o = static_cast<GuiObject *>(GET_SCRIPT_OBJECT_AS(obj, guiObjectGuid));
	RECT r;
	Std::getViewport(&r, o->guiobject_getRootWnd()->getOsWindowHandle(), true);
	return MAKE_SCRIPT_INT(r.bottom-r.top);
}
...
{
	SCRIPT_FUNCTION_INIT;
	return MAKE_SCRIPT_INT(Std::getTickCount());
}
...
scriptVar SystemObject::vcpu_selectFile(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar extlist, scriptVar id, scriptVar prevfn)
{
	...
	if (!ff.isempty())
	{
		wchar_t *p = const_cast<wchar_t *>(Std::filename(ff));
		if (p != NULL) *p = 0;
		sf.setDefaultDir(ff);
	}
...
scriptVar SystemObject::vcpu_selectFolder(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar title, scriptVar info, scriptVar defPath)
{
	...
	bi.ulFlags = BIF_RETURNONLYFSDIRS | BIF_NEWDIALOGSTYLE; // Dunno if NewDialogStyle is available in win2k?
...
scriptVar SystemObject::vcpu_enumEmbedGUID(SCRIPT_FUNCTION_PARAMS, ScriptObject *object, scriptVar _index)
{
	...
	if (index == 0) // make the table
	{
		...
		SendMessage(wa2.getMainWindow(), WM_WA_IPC, (WPARAM)&param, IPC_EMBED_ENUM);
	}
...
wchar_t SystemObject::staticStr[4096];
wchar_t SystemObject::translateStr[4096];
