...
void Skin::renderBaseTexture(ifc_window *base, Skin *s, ifc_canvas *c, const RECT &r, ifc_window *dest, int alpha)
{
	...
	if (!s)
	{
		DebugString("Warning, base texture from main wnd?!\n");
		s = tha;
	}
...
wchar_t *Skin::enumLoadableSkins(int refresh)
{
	...
	while (skins.next())
	{
		...
		wchar_t *ext = const_cast<wchar_t *>(Std::extension(filename));
	...
	case CHKSKIN_ISWA3OLD:
		{
			...
			int ret = MessageBox(GetActiveWindow(), WASABI_API_LNGSTRING(IDS_SKIN_LOAD_FORMAT_OLD),
...
#ifdef WIN32
			WASABI_API_WND->appdeactivation_setbypass(1);
			int ret = MessageBox(GetActiveWindow(), WASABI_API_LNGSTRING(IDS_SKIN_LOAD_FORMAT_TOO_RECENT),
								 WASABI_API_LNGSTRING_BUF(IDS_SKIN_LOAD_WARNING,title,32),
								 MB_ICONWARNING | MB_YESNO);
			WASABI_API_WND->appdeactivation_setbypass(0);
			if (ret == IDNO) return ;
...
#ifdef WIN32
		WASABI_API_WND->appdeactivation_setbypass(1);
		MessageBox(GetActiveWindow(), WASABI_API_LNGSTRING(IDS_SKIN_LOAD_NOT_SUPPORTED),
