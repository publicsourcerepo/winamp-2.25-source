...
{
	xml_elementtag *i = quickxmltaglist.findItem(xmltag);
	if (i) _xmlReaderOnStartElementCallback( i->id, xmltag, params);
	else _xmlReaderOnStartElementCallback( XML_ELEMENTTAG_UNKNOWN, xmltag, params);
}
...
void SkinElementsMgr::onBeforeLoadingScriptElements(const wchar_t *name, int script_id)
{
	...
	wchar_t *ptr = const_cast<wchar_t *>(Std::filename(buf));
	if (ptr != NULL) *ptr = '\0';
	rootpath = buf;
