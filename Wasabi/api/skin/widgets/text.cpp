...
int Text::getPreferences(int what)
{
	...
	if (alt)
	{
		if (item->getDataAsInt(L"No 7-bit TTF AltFonts", 1))
		{
...
int Text::onDeferredCallback(int p1, int p2)
{
#ifdef WASABI_WIDGETS_COMPBUCK
	switch (p1)
	{
...
#define WINAMP_OPTIONS_REMAINING        40038
		if (!fixedTimerStyle) SendMessage(WASABI_API_WND->main_getRootWnd()->gethWnd(), WM_COMMAND, elapsed ? WINAMP_OPTIONS_ELAPSED : WINAMP_OPTIONS_REMAINING, 0);
#endif
...
const wchar_t *Text::getPrintedText()
{
	...
	if (wantTranslation())
	{
		s = PublicVarManager::translate(name, getGuiObject());
		if (s != NULL)
		{
			printedtxt.swap(s);
			delete s;
...
int Text::triggerEvent(int event, int p1, int p2)
{
...
wchar_t Text::s_txt[WA_MAX_PATH];