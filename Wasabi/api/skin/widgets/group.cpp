	...
	case SUGGESTED_W:
		{
			...
			if (w == AUTOWH) w = getWidthBasedOn();
			if (w == AUTOWH || rw == 1) w = getAutoWidth();
			if (w == AUTOWH && getBaseTexture()) w = getBaseTexture()->getWidth();
	...
	case SUGGESTED_H:
		{
			...
			if (h == AUTOWH) h = getHeightBasedOn();
			if (h == AUTOWH || rh == 1) h = getAutoHeight();
			if (h == AUTOWH && getBaseTexture()) h = getBaseTexture()->getHeight();
	...
	case MAXIMUM_H:
		{
			...
			if (h != AUTOWH) return MIN(h, groupmaxheight);
			return groupmaxheight;
		}
	case MAXIMUM_W:
		{
			int w = GROUP_PARENT::getPreferences(what);
			if (w != AUTOWH) return MIN(w, groupmaxwidth);
			return groupmaxwidth;
		}
	case MINIMUM_H:
		{
			int h = GROUP_PARENT::getPreferences(what);
			if (h != AUTOWH) return MAX(h, groupminheight);
			return groupminheight;
		}
	case MINIMUM_W:
		{
			int w = GROUP_PARENT::getPreferences(what);
			if (w != AUTOWH) return MAX(w, groupminwidth);
...
{
	if (!StdWnd::isDesktopAlphaAvailable()) return 0;
...
{
	if (!StdWnd::isTransparencyAvailable()) return 0;
