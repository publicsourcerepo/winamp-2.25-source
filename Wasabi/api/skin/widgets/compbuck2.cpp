...
void ComponentBucket2::timerCallback(int id) {
  switch (id) {
    case TIMER_SCROLL: {
      ...
      } while (lastticcount < Std::getTickCount() - TIMER_SPEED);
      if (!vertical)
    ...
    case TIMER_SCROLLPAGE: {
      int n = MulDiv(Std::getTickCount()-scrollpage_starttime,256,scrollpage_speed);
...
void ComponentBucket2::prev_page() {
  scrollpage_starttime = Std::getTickCount();
