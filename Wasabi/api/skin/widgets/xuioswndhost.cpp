...
*/
	...
	SetWindowLongPtr(wnd, GWL_STYLE, (style & ~(STYLE_FILTER)) | WS_CHILD);
	style = GetWindowLongPtr(wnd, GWL_EXSTYLE);
	savedExStyle = (style & EXSTYLE_FILTER);
	SetWindowLongPtr(wnd, GWL_EXSTYLE, (style & ~(EXSTYLE_FILTER)) | WS_EX_CONTROLPARENT);
...
void XuiOSWndHost::oswndhost_unhost() {
  ...
  if (IsWindow(wnd)) {
	...
	DWORD style;
	style = GetWindowLongPtr(wnd, GWL_STYLE);
	SetWindowLongPtr(wnd, GWL_STYLE, ((style & ~STYLE_FILTER) | savedStyle));
	style = GetWindowLongPtr(wnd, GWL_EXSTYLE);
	SetWindowLongPtr(wnd, GWL_EXSTYLE, ((style & ~EXSTYLE_FILTER) | savedExStyle));
