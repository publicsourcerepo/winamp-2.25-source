...
int Layout::isOffscreen(ifc_window *w)
{
	...
	Std::getViewport(&r, NULL, &wr, NULL, isontop);
...
void Layout::timerCallback(int id)
{
	if (id == TIMER_TRANSPARENCY_AUTOON)
	{
		...
		int n = Std::getTickCount() - transparency_reenabled_at;
		if (n < 1000) return ;
		else
		{
...
void Layout::saveAllPositions()
{
	for (int i = 0;i < SkinParser::script_getNumContainers();i++)
	{
		...
		for (int j = 0;j < c->getNumLayouts();j++)
		{
...
{
#ifdef WASABI_COMPILE_CONFIG
	if (!getParentContainer() || getParentContainer()->isTranscient()) return ;
...
LRESULT Layout::wndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (forwardMsgWnd != INVALIDOSWINDOWHANDLE)
	{
		...
		{
		case WM_MOUSEWHEEL:
			return (SendMessage(forwardMsgWnd, uMsg, wParam, 0x31337)); // 0x31337 avoid main window callback
			//case WM_DESTROY: //FG> commented out, makes wa quit when layout is destroyed
		case WM_CLOSE:
			return (SendMessage(forwardMsgWnd, uMsg, wParam, lParam));
...
void Layout::getExplorerWindows(OSWINDOWHANDLE *parent, OSWINDOWHANDLE *listview, OSWINDOWHANDLE *webserver)
{
	...
	*parent = FindWindow(NULL, "Program Manager");
	if (!*parent) return ;
...
void Layout::loadSavedState()
{
	...
	*t = 0;
	...
	if (w == 0 || h == 0 || from_scratch)
	{
		...
		RECT sr = {x, y, x + w, y + h};
		if (findrect)
			sr = windowTracker->findOpenRect(Std::makeRect(0, 0, (int)((float)w * getRenderRatio()), (int)((float)h * getRenderRatio())), this);
		sr.right = sr.left + w;
		sr.bottom = sr.top + h;
		BaseWnd::resize(&sr);
	}
	...
	if (getParentContainer() && !getParentContainer()->isTranscient())
	{
		...
		int hidden = WASABI_API_CONFIG->getIntPrivate(tmp, 0);
...
{
	if (want && !StdWnd::isDesktopAlphaAvailable())
	{
...
{
	transparency_reenabled_at = Std::getTickCount();
...
void Layout::onSetVisible(int show)
{
	...
	if (p) p->onChildSetLayoutVisible(this, show);
	disable_auto_alpha = 0;
...
void Layout::center()
{
	...
	getNonClientRect(&r);
	RECT vw;
	Std::getViewport(&vw, NULL, NULL, gethWnd());
