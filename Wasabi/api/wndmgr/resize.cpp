...
{
	screenWidth = Std::getScreenWidth();
	screenHeight = Std::getScreenHeight();

#ifdef WIN32
	WNDCLASS wc;
	if (!GetClassInfo(hInstance, tag, &wc))
	{
	...
	hWnd = CreateWindowEx(0, tag, "", 0, 0, 0, 1, 1, NULL, NULL, hInstance, NULL);
...
{
#ifdef WIN32
	resizeClass *gThis = (resizeClass *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
	if (!gThis)
		return DefWindowProcW(hwnd, uMsg, wParam, lParam);
	else
