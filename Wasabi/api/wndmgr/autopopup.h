...
class AutoPopupEntry {
  ...
  public:
    ...
    AutoPopupEntry(int skinpartid, GUID g, const wchar_t *grpid, const wchar_t *description, const wchar_t *prefered_container=NULL, int required=TRUE) : skinpart(skinpartid), guid(g), groupid(grpid), desc(description), container(prefered_container), container_how(required) { nid = AutoPopup::allocNid(); }
