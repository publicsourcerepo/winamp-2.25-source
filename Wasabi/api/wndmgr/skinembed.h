...
class SkinEmbedEntry {
  public:
  SkinEmbedEntry(api_dependent *d, ifc_window *w, GUID g, const wchar_t *gid, const wchar_t *prefered_container, int container_flag, Container *c, WindowHolder *wh) : dep(d), wnd(w), guid(g), groupid(gid), layout(prefered_container), required(container_flag), container(c), wndholder(wh) { }
