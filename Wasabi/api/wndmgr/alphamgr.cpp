...
void AlphaMgr::addLayout(Layout *l) {
  checkTimer();
  if (layouts.findItem((const wchar_t *)l)) return;
...
int AlphaMgr::getCurve(AlphaMgrEntry *e) {
  ...
  if (e == NULL) {
    n = MulDiv(Std::getTickCount()-getBigEnterLeaveTime(),256,status == STATUS_IN_FADINGON ? fadein_ms : fadeout_ms);
    if (n > 255) n = 255; if (n < 0) n = 0;
  } else {
    if (e->getEnterLeaveTime() == -1) return -1;
    n = MulDiv(Std::getTickCount()-e->getEnterLeaveTime(),256,status == STATUS_IN_FADINGON ? fadein_ms : fadeout_ms);
    if (n > 255) n = 255; if (n < 0) n = 0;
  }
  return n;
}
...
int AlphaMgr::isPointInLayout(Layout *l, int x, int y, api_region **rgn) 
{
  ...
  l->getWindowRect(&r2);
  Std::offsetRect(&r, r2.left, r2.top);
...
int AlphaMgr::isFocusInLayout(Layout *l) {
  ...
  OSWINDOWHANDLE fw = StdWnd::getFocus();
  while (fw) 
  {
    if (fw == l->gethWnd()) return 1;
    fw = StdWnd::getParent(fw);
  }
  return 0;
}
...
int AlphaMgr::isOverExternalWindow() 
{
  ...
  POINT pt;
  pt.x = x; pt.y = y;
  OSWINDOWHANDLE w = StdWnd::getWindowFromPoint(pt);
  if (isOurExternalWindow(w)) return 1;
  pt.x = x-ext; pt.y = y-ext;
  w = StdWnd::getWindowFromPoint(pt);
  if (isOurExternalWindow(w)) return 1;
  pt.x = x; pt.y = y-ext;
  w = StdWnd::getWindowFromPoint(pt);
  if (isOurExternalWindow(w)) return 1;
...
int AlphaMgr::isOurExternalWindow(OSWINDOWHANDLE w)
{
  ...
  while (wnd) 
  {
    if (StdWnd::isPopup(wnd))
    { 
      if (isWasabiWindow(wnd)) 
        return 0; 
      OSWINDOWHANDLE _w = StdWnd::getParent(wnd);
#ifdef _WIN32
...
void AlphaMgr::preHoverCheck(AlphaMgrEntry *e) {
  ...
  if (!getAutoOpacify()) {
    if (!isin && last != 0 && (last > Std::getTickCount() - holdtime_ms)) 
      isin = 1;
  } else {
    if (!isin && lastbig != 0 && (lastbig > Std::getTickCount() - holdtime_ms)) 
      isin = 1;
  }
  if (!isin) {
...
void AlphaMgr::onBigEnterLeave() {
  big_enterleave_time = Std::getTickCount();
}
