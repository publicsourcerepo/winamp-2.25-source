...
#ifdef WIN32
  if (FindAtom(BYPASS_DEACTIVATE_ATOM) != NULL) return 1; // so people don't _need_ an api pointer to bypass us, however, if you can please call api->appdeactivation_setbypass
#else
...
#ifdef WIN32
  if (i) {
    ATOM a = FindAtom(BYPASS_DEACTIVATE_ATOM);
    if (a != NULL) return;
    AddAtom(BYPASS_DEACTIVATE_ATOM);
  } else {
    ATOM a = FindAtom(BYPASS_DEACTIVATE_ATOM);
    if (a != NULL) {
      DeleteAtom(a);
