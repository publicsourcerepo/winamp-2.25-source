...
#warning port me
	...
	for (int i = 0;i < ghosthwnd.getNumItems();i++)
		StdWnd::destroyWnd(ghosthwnd.enumItem(i));
...
int BaseWnd::init(OSMODULEHANDLE moduleHandle, OSWINDOWHANDLE parent, int nochild)
{
	...
	{
		Std::setRect(&r, 0, 0, getPreferences(SUGGESTED_W), getPreferences(SUGGESTED_H));
	}
	...
	hwnd = StdWnd::createWnd(&r, nochild, acceptExternalDrops(), parent, hinstance, static_cast<ifc_window*>(this));
#ifdef __APPLE__
#warning remove me
	StdWnd::showWnd(hwnd);
#endif
...
void BaseWnd::resize(int x, int y, int w, int h, int wantcb)
{
	...
	RECT oldsize, newsize = Std::makeRect(x, y, w, h);
	if (hwnd != NULL)
	...
	if (getOsWindowHandle() != NULL)
	{
		...
		if (MEMCMP(&newsizescaled, &oldsizescaled, sizeof(RECT)))
		{
			...
			//CUT      0);
			StdWnd::setWndPos(getOsWindowHandle(), NULL, x, y, w, h,
...
int BaseWnd::onPostedMove()
{
	...
	if (w2k_alpha && StdWnd::isDesktopAlphaAvailable() && !cloaked)
	{
		RECT r;
		getWindowRect(&r);
	   StdWnd::moveLayeredWnd(hwnd, r.left, r.top);
	}*/
	return FALSE;
}
...
void BaseWnd::move(int x, int y)
{
	...
	StdWnd::setWndPos(getOsWindowHandle(), NULL, x, y, 0, 0,
...
void BaseWnd::validate()
{
	...
	if (hwnd != NULL) StdWnd::validateRect(getOsWindowHandle());
}
...
void BaseWnd::validateRect(RECT *r)
{
	if (hwnd != NULL)
	{
		...
		{
			RECT r2 = *r;
			Std::scaleRect(&r2, getRenderRatio());
...
void BaseWnd::repaint()
{
	...
	if (hwnd != NULL) StdWnd::update(getOsWindowHandle());
}
...
void BaseWnd::getClientRect(RECT *rect)
{
	...
	//ASSERT(hwnd != NULL);
	if (!StdWnd::isValidWnd(hwnd))
	{
		MEMSET(rect, 0, sizeof(RECT));
		return ;
	}
	//CUT  GetClientRect(getOsWindowHandle(), rect);
...
ifc_window *BaseWnd::getDesktopParent()
{
	...
	if (p)
	{
		GetClassNameW(p, cn, 255); cn[255] = 0;
		if (!wcscmp(cn, BASEWNDCLASSNAME))
			return (ifc_window*)GetWindowLongPtr(p, GWLP_USERDATA);
		else if (last != NULL)
			return (ifc_window*)GetWindowLongPtr(last, GWLP_USERDATA);
	}
#else
#warning port me
#endif
	...
	if (!isDestroying()) switch (uMsg)
		...
		case WM_PRINTCLIENT:
			{
				...
				if (lParam & PRF_CHILDREN)
				{
					...
					while (child != NULL) 
					{
						if (GetWindowLongPtr(child, GWL_STYLE) & WS_VISIBLE) 
						{
							RECT child_size;
							...
							if (child_size.right && child_size.bottom)
							{
								...
								SendMessage(child, WM_PRINT, (WPARAM)bitmap.getHDC(),  PRF_CHILDREN | PRF_CLIENT | PRF_NONCLIENT/*| PRF_OWNED*/);
		...
		case WM_LBUTTONDOWN:
		{
			...
			if (isEnabled() && !dragging)
			{
				...
				if (checkDoubleClick(uMsg, xPos, yPos) && wantDoubleClicks()) if (onLeftButtonDblClk(xPos, yPos)) return 0;
				return r;
			}
		}
		break;
...
#define ARRAYSIZE(x) (sizeof(x)/sizeof(*x))
		...
		{
			if (!isVirtual() && StdWnd::isPopup(hwnd))
			{
...
{
	preventcancelcapture = 1;
	if (StdWnd::getCapture() == getOsWindowHandle()) StdWnd::releaseCapture();
	/*  if (oldCapture) {
...
int BaseWnd::virtualOnPaint()
{
	...
	//CUT  if (getRandomRgn) {
	if (StdWnd::haveGetRandomRegion())
	{
		RegionI zap;
		//CUT    getRandomRgn(paintcanvas.getHDC(), zap.getOSHandle(), SYSRGN);
		StdWnd::getRandomRegion(paintcanvas.getHDC(), zap.getOSHandle());
	}
	else
	{
		RECT z;
...
ifc_window *BaseWnd::findRootWndChild(int x, int y, int only_virtuals)
{
	for (int i = getNumRootWndChildren() - 1; i > -1; i--)
	{
		...
		POINT _p = Std::makePoint(_x, _y);
		if (iv /*&& gpa > 0*/ && Std::pointInRect(&r, _p))
		{
			// GROUP
...
{
	Std::scaleRect(r, getRenderRatio());
}
...
//#ifdef COMPILE_WASABI_SKIN // for some reason this isn't being correctly defined
	if (name)
	{
		StdWnd::setWndName(getOsWindowHandle(), name);
	}
	else
		StdWnd::setWndName(getOsWindowHandle(), L"");
...
int BaseWnd::reinit(OSMODULEHANDLE moduleHandle, OSWINDOWHANDLE parent, int nochild)
{
	...
	// destroy old window
	StdWnd::hideWnd(hwnd); //StdWnd::destroyWnd(hwnd);
...
#warning port me - register drag & drop
	...
	if (this_visible) StdWnd::showWnd(hwnd, FALSE);
	if (hadcapture)
	{
		StdWnd::setCapture(hwnd);
	}
...
{
	wnd = _wnd;
	StdWnd::getWindowRect(wnd, &rect);
	StdWnd::screenToClient(wnd, (int *)&(rect.left), (int *)&(rect.top));
	StdWnd::clientToScreen(parentwnd, (int *)&(rect.left), (int *)&(rect.top));
}

void ReparentWndEntry::unparent()
{
	StdWnd::setWndPos(wnd, NULL, rect.left, -30000, 0, 0, TRUE, TRUE, FALSE, FALSE, TRUE);
	StdWnd::setParent(wnd, NULL);
}

void ReparentWndEntry::reparent(OSWINDOWHANDLE newparent)
{
	StdWnd::setParent(wnd, newparent);
	StdWnd::setWndPos(wnd, NULL, rect.left, rect.top, 0, 0, TRUE, TRUE, FALSE, FALSE, TRUE);
}
...
void BaseWnd::redrawHWNDChildren()
{
	// reparent to the new oswindowhandle
	foreach(reparentwnds)
	StdWnd::update(getOsWindowHandle());
	endfor;
}
...
void BaseWnd::maximize(int axis)
{
	...
	Std::getViewport(&dr, NULL, NULL, getOsWindowHandle(), 0);
