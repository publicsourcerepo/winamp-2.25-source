...
int AppBarWnd::registerWinAppBar() 
{
	...
	m_registered = SHAppBarMessage(ABM_NEW, &abd);
	return m_registered;
}
...
int AppBarWnd::appbar_isSideAutoHideSafe(int side) {
	...
	if (cur == NULL || cur == getOsWindowHandle()) {
		RECT primary = {0};
		Std::getViewport(&primary, hwnd, 1);

		// DebugString("primary screen coords = %d,%d -> %d,%d (%dx%d)\n", primary.left, primary.top, primary.right, primary.bottom, primary.right-primary.left, primary.bottom-primary.top);
		int monitor = 0;
		//int g = 0;
		while (1) {
			RECT r;
			int ret = Std::enumViewports(monitor++, &r, 1);
			if (ret == 0) break;

			if (Std::rectEqual(&primary, &r)) continue;

			// DebugString("secondary screen %d = %d,%d -> %d,%d (%dx%d)\n", ++g, r.left, r.top, r.right, r.bottom, r.right-r.left, r.bottom-r.top);
...
int AppBarWnd::appbar_testDock(int x, int y, RECT *dockrect) {
	...
	Std::getViewport(&viewRect, hwnd, 1);
...
void AppBarWnd::setAutoHideTimer(){
	if (!m_autohide_timer_set) {
		SetTimer(getOsWindowHandle(), IDT_AUTOHIDE, 500, NULL);
...
void AppBarWnd::setAutoUnHideTimer(){
	if (!m_autounhide_timer_set) {
		SetTimer(getOsWindowHandle(), IDT_AUTOUNHIDE, 50, NULL);
...
LRESULT AppBarWnd::wndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
	if (m_registered) {
		...
		else if (msg == WM_TIMER) { // // not using multiplexed timer for independent speed
			switch (wparam) {
				case IDT_AUTOHIDE:
					onAutoHideTimer();
				break;
