...
public:
	...
	void Kill()
	{
		InterlockedExchangePointer(&wnd, 0);
		killswitch=1;
...
int QuickPaintContext::QPThreadPoolFunc(HANDLE h, void *user_data, intptr_t t)
{
	...
	else
	{
		...
		QuickPaintWnd *wnd = 0;
		InterlockedExchangePointer(&wnd, context->wnd);
		if (wnd)
		{
			wnd->quickPaint();
...
{
	InterlockedExchangePointer(&paint_canvas, c);
}
...
int QuickPaintWnd::onPaint(Canvas *canvas)
{
	...
	InterlockedExchangePointer(&render_canvas, paint_canvas);
