...
ForeignWnd::ForeignWnd(OSWINDOWHANDLE wndhandle, OSMODULEHANDLE module, int subclass)
{
	...
	if (subclass)
	{
		oldWindowProc = (WNDPROC) GetWindowLongPtr(wndhandle, GWLP_WNDPROC);
		...
		SetWindowLongPtr(thishwnd, GWLP_WNDPROC, (LONG_PTR)foreignWindowProc);
	}
	else
	{
		oldWindowProc = NULL;
...
ForeignWnd::~ForeignWnd()
{
	if (wndprocentry && oldWindowProc)
	{
		...
		delete wndprocentry;
		wndprocentry = NULL;
		SetWindowLongPtr(thishwnd, GWLP_WNDPROC, (LONG_PTR)oldWindowProc);
		oldWindowProc = NULL;
	}
}
