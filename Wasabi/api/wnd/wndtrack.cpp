...
void WindowTracker::invalidateAllWindows()
{
	/*  for (int i=0;i<desktopwnds.getNumItems();i++)
	    if (desktopwnds[i]->isPostOnInit()) {
	      api_window *w = desktopwnds[i];
	      w->invalidate();
	    }*/
...
void WindowTracker::startCooperativeMove(ifc_window *thiswnd)
{
	...
	// FG> we need to prevent windows from activating the shit out of our windows or focus is gonna blow up
...
void WindowTracker::addCooperative(ifc_window *thiswnd)
{
	...
	for (i = 0;i < desktopwnds.getNumItems();i++)
	{
		...
		if (forceall || snap || (touches(r, thisr) && !Std::rectIntersect(&r, &thisr)))
		{
#else
...
if (oldPosition) 
{
  oldPosition->left += z_snapAdjust.left; oldPosition->top += z_snapAdjust.top; }
	...
	if (!disabledock)
	{
		for (i = desktopwnds.getNumItems(); i > -1; i--)
		{
...
#endif
				Std::getViewport(&r, thiswnd->gethWnd());
				wnd = NULL;
			}
			else
			{
			...
			if (((wnd != NULL && (mask & NOINTERSECT) && Std::rectIntersect(z, &r)) || !touches(*z, r)) && !s)
			{
				*z = oz;
				thisdone.x = 0;
				thisdone.y = 0;
	...
	if (coopWnd == thiswnd && oldPosition)
	{
		...
		{
      Std::offsetRect(z, redock.x, redock.y);
			f++;
		}
#ifdef WIN32
		HDWP hd;
