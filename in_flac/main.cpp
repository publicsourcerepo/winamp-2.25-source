...
wchar_t pluginName[256];
char genresFile[MAX_PATH];
void Init()
{
	...
	const char *iniPath = (const char *)SendMessage(plugin.hMainWindow, WM_WA_IPC, 0, IPC_GETINIDIRECTORY);
	PathCombine(genresFile, iniPath, "genres.txt");
	winampINI = (const char *)SendMessage(plugin.hMainWindow, WM_WA_IPC, 0, IPC_GETINIFILE);
	BuildExtensions();
	config_average_bitrate = !!GetPrivateProfileInt("in_flac", "average_bitrate", 1, winampINI);
