...
{
	if (!libflac)
		libflac = LoadLibraryW(L"libflac.dll");

	if (libflac && !versionString)
	{
		versionString = (const char **)GetProcAddress(libflac, "FLAC__VERSION_STRING");
		if (!versionString)
			*versionString = WASABI_API_LNGSTRING_BUF(IDS_LIBFLAC_DLL_MISSING,defaultVersionString,64);
	}
...
void About(HWND hwndParent)
{
	...
	WASABI_API_LNGSTRINGW_BUF(IDS_NULLSOFT_FLAC_DECODER,text,1024);
