/*
** Copyright (C) 2007-2013 Nullsoft, Inc.
**
...
bool FLAC_AlbumArtProvider::IsMine(const wchar_t *filename)
{
	...
	{
		wchar_t exts[128];
		GetPrivateProfileStringW(L"in_flac", L"extensions", DEFAULT_EXTENSIONSW, exts, 128, AutoWide(winampINI));
...
{
	/* TODO: benski> there is a bug where if you call this from Alt+3 (where it's called most of the time)
  it won't really get saved, because clicking "OK" in alt+3 subsequently writes another FLACMetadata object
	with older data.
	Need to somehow signal to the alt+3 dialog to remove the picture, if it's open
	*/
	FLACMetadata metadata;
	...
	if (metadata.Open(filename))
	{
		...
		if (NameToAPICType(type, pictype))
		{
			if (metadata.RemovePicture(pictype))
			{
				if (metadata.Save(filename))
					return ALBUMARTPROVIDER_SUCCESS;
				else
