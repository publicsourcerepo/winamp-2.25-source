...
void draw_reinit_plfont(int update)
{
	EnterCriticalSection(&g_srcdccs);
	{
		...
		font=CreateFontW(-config_pe_fontsize,0,0,0,FW_NORMAL,FALSE,FALSE,FALSE,
			font_charset,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,DRAFT_QUALITY,DEFAULT_PITCH|FF_DONTCARE,font_name);
		...
		osdFontText = CreateFontW(OSD_TEXT_SIZE, 0, 0, 0, FW_SEMIBOLD, FALSE, FALSE, FALSE,
			font_charset/*ANSI_CHARSET*/, OUT_OUTLINE_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, GetFontNameW()/*"Arial"*/);
		...
		/*SelectObject(hdc,shadefont);
		GetTextMetrics(hdc,&tm);
		shadefont_height=tm.tmHeight;*/
	...
	if (update) PostMessage(hMainWindow, WM_WA_IPC, 0, IPC_CB_RESETFONT);
}
