...
void plPF_PTexG(pl_Cam *cam, pl_Face *TriFace) {
  ...
  C1 = C2 = pl_f2i(TriFace->Shades[i0]);
  U1 = U2 = MappingU1;
  V1 = V2 = MappingV1;
  ...
  if (dY) {
    ...
    dC2 = (pl_f2i(TriFace->Shades[i2]) - C1) / dY;
    dU2 = (MappingU3 - U1) / dY;
    dV2 = (MappingV3 - V1) / dY;
  }
  dY = Y1-Y0;
  if (dY) {
    ...
    dC1 = (pl_f2i(TriFace->Shades[i1]) - C1) / dY;
    dU1 = (MappingU2 - U1) / dY;
    dV1 = (MappingV2 - V1) / dY;
    if (dX2 < dX1) {
  ...
  while (Y0--) {
    if (!Y1--) {
      ...
      if (dY) {
        ...
        dC1 = (pl_f2i(TriFace->Shades[i2])-C1)/dY;
        dV1 = (MappingV3 - V1) / dY;
        dU1 = (MappingU3 - U1) / dY;
      }
    }
    ...
    if (Xlen > 0) {
      ...
      do {
        ...
        do {
            pl_uInt16 adv;
            if (CL <= 0) adv=addtable[0];
            else if (CL >= (255<<8)) adv=addtable[255];
            else adv=addtable[CL>>8];
            *gmem++ = remap[adv +
