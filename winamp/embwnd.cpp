...
enum { NO_CAP,TITLE_CAP,TB_CAP,SZ_CAP};
	...
	if (which_cap == TITLE_CAP || (!which_cap && (config_easymove || mouse_y < 14)))
		...
		case 0:
			if (which_cap == TITLE_CAP && mouse_stats & MK_LBUTTON)
			{
				...
				SendMessage(hwnd, WM_USER+0x100, 1, (LPARAM)&pt);
	                            ...
	                            (((state->r.right-state->r.left)-mouse_x + (state->r.bottom - state->r.top)-mouse_y) <= 30)))
	{
		...
		if (which_cap == SZ_CAP)
		{
			...
			if (x != (state->r.right-state->r.left) || y != (state->r.bottom - state->r.top))
			{
				...
				SendMessage(hwnd, WM_USER+0x101, 1, (LPARAM)&pt);
...
static int emb_OnRButtonUp(HWND hwnd, int x, int y, UINT flags)
{
	...
	POINT p;
	GetCursorPos(&p);
	int ret=TrackPopupMenu(main_menu,TPM_LEFTALIGN|TPM_RIGHTBUTTON|TPM_RETURNCMD,p.x,p.y,0,hwnd,NULL);
	if (ret) SendMessage(hMainWindow,WM_COMMAND,ret,0);
	return 1;
}
...
{
	ReleaseCapture();
	embedui_handlemouseevent(hwnd,x,y,-1,flags,(embedWindowState *)GetWindowLongPtr(hwnd,GWLP_USERDATA));
	return 1;
}
...
{
	SetCapture(hwnd);
	embedui_handlemouseevent(hwnd,x,y,1,keyFlags,(embedWindowState *)GetWindowLongPtr(hwnd,GWLP_USERDATA));
	return 1;
}

static int emb_OnMouseMove(HWND hwnd, int x, int y, UINT keyFlags)
{
	embedui_handlemouseevent(hwnd,x,y,0,keyFlags,(embedWindowState *)GetWindowLongPtr(hwnd,GWLP_USERDATA));
	return 1;
}

static BOOL emb_OnNCActivate(HWND hwnd, BOOL fActive, HWND hwndActDeact, BOOL fMinimized)
{
	embedWindowState *state=(embedWindowState *)GetWindowLongPtr(hwnd,GWLP_USERDATA);
	if (fActive==FALSE)
	{
...
static void EmbedWindow_OnShowWindow(HWND hwnd, BOOL fShow, UINT status)
{
	...
	if (NULL != hChild && 0 != toggleResult)
	{
		...
		SendMessage(hChild, WM_NOTIFY, (WPARAM)embedShow.hdr.idFrom, (LPARAM)&embedShow);
	}
}
...
extern "C"
{
	...
	LRESULT CALLBACK emb_WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		switch (uMsg)
		{
		case WM_INITMENUPOPUP: return SendMessage(hMainWindow,uMsg,wParam,lParam); // for popup menus
...
case WM_KEYDOWN: case WM_KEYUP: case WM_SYSKEYDOWN: case WM_SYSKEYUP:
		{
			...
			{
				//     HWND hh=FindWindowEx(hwnd,NULL,NULL,NULL);
				//    if (hh) PostMessage(hh,uMsg,wParam,lParam);
		...
		{
			HWND hh=FindWindowEx(hwnd,NULL,NULL,NULL);
			if (hh) PostMessage(hh,WM_CLOSE,0,0);
		}
		return 0;
		case WM_USER+102:
			{
		...
		{
			HWND hh=FindWindowEx(hwnd,NULL,NULL,NULL);
			if (hh) SendMessage(hh,uMsg,wParam,lParam);
		}
		...
		case WM_WINDOWPOSCHANGING:
		{
			...
			*/
			...
			if (state && state->reparenting)
			{
				if (0 != (WS_CHILD & GetWindowLongPtr(windowPos->hwnd, GWL_STYLE)))
		...
		case WM_WINDOWPOSCHANGED:
		{
			...
			if (state && 0 == (SWP_NOSIZE & ((WINDOWPOS*)lParam)->flags))
			{
				...
				hh = FindWindowEx(hwnd,NULL,NULL,NULL);
				...
				if (hh)
				{
					...
					if (IsWindowVisible(hh)) SendMessage(hh, WM_USER + 0x201, MAKEWPARAM(0,0), (LPARAM)rgnChild);
					SetWindowPos(hh,NULL, 0, 0, cx, cy, 	SWP_NOMOVE | SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOREDRAW | SWP_NOCOPYBITS);
					if (IsWindowVisible(hh)) SendMessage(hh, WM_USER + 0x201, 0, (LPARAM)NULL);
				...
				if (0 == (SWP_NOREDRAW & ((WINDOWPOS*)lParam)->flags))
				{
					...
					if (GetUpdateRect(hwnd, NULL, FALSE)) GetUpdateRgn(hwnd, rgnWnd, FALSE);
					if (rgnChild)
					{
					...
					if (rgnWnd) DeleteObject(rgnWnd);
				}

				if (rgnChild) DeleteObject(rgnChild);
			}
		}
		return 0;
		...
		case WM_CREATE:
		{
			EMBEDWND *pew = (EMBEDWND*)malloc(sizeof(EMBEDWND));
			ZeroMemory(pew, sizeof(EMBEDWND));
		...
		case WM_ACTIVATE:
			if (WA_INACTIVE == LOWORD(wParam))
			{
				...
				if (pew)
				{
					...
					if (!IsChild(hwnd, pew->hLastFocus)) pew->hLastFocus = NULL;
				}
			...
			else
			{
				if (WA_CLICKACTIVE == LOWORD(wParam))
				{
					...
					if (pew)
					{
						...
						if (hTarget && hTarget != hwnd) pew->hLastFocus = hTarget;
					}
				}
		...
		case WM_SETFOCUS:
		{
			...
			if (pew)
			{
				...
				{
					if (IsWindowEnabled(pew->hLastFocus) && IsWindowVisible(pew->hLastFocus) && 
						0 != (WS_TABSTOP & GetWindowLongPtrW(pew->hLastFocus, GWL_STYLE)))
					{
						hTab = pew->hLastFocus;
						break;
					}
			...
			if (!hTab)	hTab = (hChild) ? GetNextDlgTabItem(hwnd, hChild, FALSE) : hwnd;
			if (hTab && hTab != hwnd)
			{
				WCHAR szName[128];
				DWORD lcid = MAKELCID(MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), SORT_DEFAULT);
				if (GetClassNameW(hChild, szName, sizeof(szName)/sizeof(WCHAR)) &&
					CSTR_EQUAL == CompareStringW(lcid, NORM_IGNORECASE, szName, -1, L"#32770", -1))
				{
	...
	void SnapWindowToAllWindows(RECT *outrc, HWND hwndNoSnap)
	{
		...
		while (state)
		{
			if (state->me != hwndNoSnap && IsWindowVisible(state->me)) SnapWindowToWindow(outrc,state->r);
			state=state->link;
		}
		LeaveCriticalSection(&embedcs);
