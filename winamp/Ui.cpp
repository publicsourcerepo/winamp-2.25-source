...
static int do_clutterbar()
{
	...
	if (inreg(11,24,19,31))
	{
		...
		if (mouse_type == -1)
		{
			...
			TrackPopupMenu(hmenu,TPM_LEFTALIGN|TPM_RIGHTBUTTON,p.x,p.y,0,hMainWindow,NULL);
		}
		if (mouse_stats & MK_LBUTTON)
		{
		wchar_t songnameStr[128];
	...
	{
		en=2;
		if (mouse_type == -1) SendMessage(hMainWindow,WM_COMMAND,WINAMP_OPTIONS_AOT,0);
		if (mouse_stats & MK_LBUTTON) 
		{
			wchar_t songnameStr[128];
			if (config_aot)
	...
	{
		en=4;
		if (mouse_type == -1) SendMessage(hMainWindow,WM_COMMAND,WINAMP_OPTIONS_DSIZE,0);
		if (mouse_stats & MK_LBUTTON) 
		{
			wchar_t songnameStr[128];
	...
	if (inreg(11,56,19,62))
	{
		...
		if (mouse_type == -1) 
		{
			...
			ClientToScreen(hMainWindow,&p);
			TrackPopupMenu(g_submenus_vis,TPM_LEFTALIGN|TPM_RIGHTBUTTON,p.x,p.y,0,hMainWindow,NULL);
		}
		if (mouse_stats & MK_LBUTTON)
		{
			wchar_t songnameStr[128];
...
{					 		    
	...
	if (mouse_type == -2)
		return 0;
	if (do_titlebar_clicking || config_easymove || mouse_y < 14) switch (mouse_type)
	{
		case 1:
			if (!do_titlebar_clicking)
			{
				EnterCriticalSection(&embedcs);
				{
					...
					rec *a = (rec *) alloca(cnt * sizeof(rec));
					...
					if (!!config_snap ^ !!(mouse_stats & MK_SHIFT))
					{
						...
						{
							int b;
							if (a[x].open&&a[x].at) for (b = 0; b < cnt; b ++)
							{
...
static int do_titlebuttons() 
{
	...
	if (inreg(254,3,262,12)) // windowshade button
	{
		if (mouse_type == -1)
		{
			SendMessage(hMainWindow,WM_COMMAND,WINAMP_OPTIONS_WINDOWSHADE,0);
...
static int do_shuffle() 
{
	...
	if (inreg(133+7-4+29,75+15,181+7-4-4+29,87+15)) 
	{
		...
		case -1:
			...
			SendMessage(hMainWindow,WM_COMMAND,WINAMP_FILE_SHUFFLE,0);
			break;
		}
		return 1;
	} 
...
static int do_repeat() 
{
	...
	if (inreg(182+7-4-4+29,75+15,182+29+7-4-4+29,87+15)) 
	{
		...
		case -1:
			...
			SendMessage(hMainWindow,WM_COMMAND,WINAMP_FILE_REPEAT,0);
			break;
		}
		return 1;
	} 
...
static int do_icon()
{
	...
	{
		if (mouse_type == -1) 
			SendMessage(hMainWindow,WM_COMMAND,WINAMP_LIGHTNING_CLICK,0);
		return 1;
	}
	return 0;
}
...
static int do_volbar()
{
	...
	if (do_volbar_active) 
	{
		v = mouse_x - do_volbar_clickx - (107);
		//    v = (v * 256) / (195-132-12);
		v = (v*255)/51;
		if (v < 0) v = 0;
		if (v > 255) v = 255;
...
static int do_panbar()
{
	...
	if (do_panbar_active) 
	{
		v = mouse_x - do_panbar_clickx - (177);
		if (v < 0) v = 0;
		if (v > 24) v = 24;
		v-=12;
		// changed in 5.7 to have a lower limit (~16% vs 24%) and for
		...
		in_setpan(config_pan);
		{
			...
			if (v)
			{
				wchar_t lorrStr[32], balanceStr[64];
...
void ui_handlecursor(void)
{
	...
	}, b_windowshade[] = 
	{
		...
		if (0 != (WS_DISABLED & GetWindowLongPtr(hMainWindow, GWL_STYLE)))
