  ...
  switch (uMsg)
  {
    case WM_LBUTTONDOWN:
			if (!listDragging) 
      {
        ...
        {
				  int x=SendMessage(hwndDlg,LB_ITEMFROMPOINT,0,MAKELPARAM(p.x-r.left,p.y-r.top-2));
				  if (!HIWORD(x))
				  {
					  listDragging=1;
					  listSel=x;
...
//	DO_HELP();
		...
		case WM_CTLCOLORLISTBOX:
            ...
            int thisp;
              ...
              if (thisp >= 0 && listSel >= 0 && thisp < len && listSel < len) {
                char fn[MAX_PATH];
                char file1[MAX_PATH],title1[4096];
                FILE *fp,*fpo;
                Bookmark_getfn(fn);
                fp=fopen(fn,"rt");
    ...
    return 0;
		case WM_INITDIALOG:
      {
        ...
        FILE *fp;
        OldBookListProc=(WNDPROC)SetWindowLong(GetDlgItem(hwndDlg,IDC_SELBOX),GWL_WNDPROC,(LONG)BookListProc);
        listDragging=0;
        listSel=-1;
        Bookmark_getfn(fn);
        fp=fopen(fn,"rt");
