...
{
	if (s) SetWindowText(rf_statusWnd,s);
	if (rf_dlgWnd) SendMessage(rf_dlgWnd,WM_USER+1,r,0);
	else rf_rv=r;
	return r;
}

#define rfta(s) return _rftabort(!success,s)
#define sets(s) SetWindowText(rf_statusWnd,s)
...
static DWORD WINAPI rf_ThreadProc(LPVOID p666)
{
	...
	SetThreadName(-1, "HTTP Retrieve File");
	if (hws)
	{
...
_redirect_goto:
	...
	if (t) 
	{
		StringCchCopy(url_buf,1024,t+3);
	}
	else 
	{
		StringCchCopy(url_buf,1024,rf_url);
	}
	...
	if (p>=url_buf) 
	{
	    *p++=0;
		...
		if (lstrlen(url_lp)>0)
		{
			StringCchCopy(authstr,1024,"Authorization: Basic ");
			encodeMimeStr(url_lp,authstr+lstrlen(authstr));
			StringCchCat(authstr,1024,"\r\n");
		}
	}
	...
	{
		StringCchCopy(proxytmp,256,config_proxy);
		proxy=proxytmp;
	...
	if (*proxy) 
	{
		...
		{
			*p++=0;
			StringCchCopy(proxy_lp,1024,"Proxy-Authorization: Basic ");
			encodeMimeStr(proxy,proxy_lp+lstrlen(proxy_lp));
			StringCchCat(proxy_lp,1024,"\r\n");
			proxy=p;
		}
		...
		if (*p)
		{
			*p++=0;
			if (*p) 
			connect_port=(unsigned short)atoi(p);
			else connect_port=80;
		}
	}
	else
	{
		lstrcpyn(connect_host,url_host,sizeof(connect_host)/sizeof(*connect_host));
	...
	sets(getString(IDS_HTTP_SEND_REQUEST,errorStr,1024));
	{
		if (*proxy) 
		{
			StringCchPrintf(buf,4096,"GET http://%s:%d/%s",url_host,url_port,url_url);
		}
		else
		{
			StringCchPrintf(buf,4096,"GET /%s",url_url);
		}
		StringCchPrintf(buf+lstrlen(buf),4096-lstrlen(buf)," HTTP/1.0\r\n"
						"Host: %s:%d\r\n"
						"User-Agent: Winamp/%s\r\n"
						"%s%s"
					    "Accept: */*\r\n\r\n",
						url_host,url_port,
						app_version,proxy_lp,authstr);
	...
	{ // get the standard HTTP 1.0 200 OK
		...
		if (strstr(buf," 302") || strstr(buf,"301")) redirect=1;
		else redirect=0;
		if (!strstr(buf,"OK") && !redirect)
		{
			StringCchCat(buf,1024,getString(IDS_HTTP_CONNECTION_CLOSED,errorStr,1024));
			closesocket(sock);
			WSACleanup();
			FreeLibrary(hws);
			rfta(buf);
	...
	while (1)
	{
		...
		if (!lstrcmpi(buf,"Content-Length") && (*p >= '0' && *p <= '9'))
		{
			content_length=0;
			while (*p >= '0' && *p <= '9') 
			{
		...
		if (!lstrcmpi(buf,"Location") && redirect)
		{
			StringCchCopy(locbuf,1024,p);
			rf_url=locbuf;
			closesocket(sock);
			...
			while (1)
			{
				...
				else // x > 0
				{
					...
					if (lb/16384 != total_bytes/16384)
					{
						...
						if (content_length)
						{
							StringCchPrintf(buf,4096,getString(IDS_HTTP_RETRIEVE_FILE_LENGTH,errorStr,1024),
										 (total_bytes*100)/content_length,
										 total_bytes,content_length,bps/1000,(bps/10)%100
										 );
						}
						else
						{
							StringCchPrintf(buf,4096,getString(IDS_HTTP_RETRIEVE_FILE,errorStr,1024),
										 total_bytes,bps/1000,(bps/10)%100);
						}
						sets(buf);
					}
				}
...
static BOOL CALLBACK rf_DlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam)
{
	...
	switch (uMsg)
	{
		case WM_USER+1:
			...
			{
				g_rv=0;
				PostMessage(hwndDlg,WM_USER,0,0); // make it go quick
			}
			else 
			{
				SetDlgItemText(hwndDlg,IDCANCEL,getString(IDS_HTTP_CLOSE,NULL,0));
				r=5;
		...
		case WM_TIMER:
			if (wParam == 123)
			{
				...
				{
				char s[32];
					StringCchPrintf(s,32,getString(IDS_CLOSE_COUNTDOWN,NULL,0),r--);
					SetDlgItemText(hwndDlg,IDCANCEL,s); 					
				}
			}
			return 0;
		case WM_ERASEBKGND:
		...
		case WM_INITDIALOG:
			{
				...
				if (WADlg_initted())
				{
					SetWindowLong(GetDlgItem(hwndDlg, IDCANCEL), GWL_STYLE, GetWindowLong(GetDlgItem(hwndDlg, IDCANCEL), GWL_STYLE) | BS_OWNERDRAW);
				}
				rf_dlgWnd=hwndDlg;
				rf_statusWnd=GetDlgItem(hwndDlg,IDC_STATUS);
				SetWindowTextW(hwndDlg,rf_dlgtitle);
				if (strstr(rf_url,"client.winamp.com/update"))
					SetDlgItemText(hwndDlg,IDC_URL,getString(IDS_HTTP_WINAMP_UPDATE_SITE,errorStr,1024));
				else 
					SetDlgItemText(hwndDlg,IDC_URL,rf_url);

				SetDlgItemText(hwndDlg,IDC_STATUS,getString(IDS_HTTP_INIT,errorStr,1024));
				rf_abort=0;
...
#endif
							SetDlgItemText(hwndDlg,IDCANCEL,getString(IDS_HTTP_ABORT,errorStr,1024));
					}
					else
					{
						g_rv=1;
...
//			MessageBox(hwnd,getString(IDS_ONLYHTTP,NULL,0),"Winamp",MB_OK|MB_ICONSTOP);
	...
	else EnableWindow(hwnd,1);

	DestroyWindow(dlgWnd);
	if (activated) SetForegroundWindow(hwnd);
	i = g_rv;
	rf_in=0;
	return i;
}
