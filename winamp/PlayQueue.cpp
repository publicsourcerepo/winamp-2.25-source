...
int PlayQueue_OnEOF()
{
	...
	nextItem = SendMessage(hMainWindow, WM_WA_IPC, 0, IPC_GET_NEXT_PLITEM);
		...
		if (PlayList_getlength() && !g_stopaftercur)
		{
			...
			else
			{
				...
				if (config_shuffle && !PlayList_current_hidden())
				{
					...
					if ((q = PlayList_randpos(1) || PlayList_getlength() == 1)  // this isn't really the place for this check
					    && !config_repeat)
					{
						g_fullstop = 2;
						StopPlaying(0);
