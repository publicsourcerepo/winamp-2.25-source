...
{
	if (id == WINAMP_BUTTON1_SHIFT)	SendMessage(hwnd, WM_COMMAND, WINAMP_REW5S, 0);
	...
	{
		int nitem = SendMessage(hMainWindow, WM_WA_IPC, 0, IPC_GET_PREVIOUS_PLITEM);
		if (nitem != -1)
		{
...
int Main_OnButton5(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify)
{
	if (id == WINAMP_BUTTON5_SHIFT)
		SendMessage(hwnd, WM_COMMAND, WINAMP_FFWD5S, 0);
	...
	{
		int nitem = SendMessage(hMainWindow, WM_WA_IPC, 0, IPC_GET_NEXT_PLITEM);
		if (nitem != -1)
		{
			PlayList_setposition(nitem);
			PlayList_getcurrent(FileName, FileTitle, FileTitleNum);
