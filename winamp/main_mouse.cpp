...
int Main_OnRButtonUp(HWND hwnd, int x, int y, UINT flags)
{
	...
	if (x < 266 && x > 105 && y < 35 && y > 24)
	{
		...
		int ret = TrackPopupMenu(hmenu,TPM_LEFTALIGN|TPM_RIGHTBUTTON| TPM_RETURNCMD,p.x,p.y,0,hwnd,NULL);
		...
		if (menu)
		{
			if (mainSendTo.mode == 2)
			{
				...
				if (SendMessage(hMainWindow, WM_WA_IPC, (WPARAM)&mainSendTo, IPC_LIBRARY_SENDTOMENU) == 0xffffffff)
				{
					wchar_t buf[1041];
					wchar_t *end=buf;
					size_t endSize;
					...
					mainSendTo.data = buf;
					mainSendTo.data_type = ML_TYPE_FILENAMESW;
					SendMessage(hMainWindow, WM_WA_IPC, (WPARAM)&mainSendTo, IPC_LIBRARY_SENDTOMENU);
				}
			}
			// remove sendto
		...
		{
			mainSendTo.mode = 4;
			SendMessage(hMainWindow, WM_WA_IPC, (WPARAM)&mainSendTo, IPC_LIBRARY_SENDTOMENU); // cleanup
			memset(&mainSendTo, 0, sizeof(mainSendTo));
		}
...
int Main_OnLButtonDblClk(HWND hwnd, BOOL fDoubleClick, int x, int y, UINT keyFlags)
{
	...
	{
		KillTimer(hwnd,666);
		SendMessage(hwnd,WM_CLOSE,0xDEADBEEF,0xDEADF00D);
	}
	...
	{
		SendMessage(hwnd,WM_COMMAND,WINAMP_EDIT_ID3,0);
	}
	...
	{
		SendMessage(hwnd,WM_COMMAND,WINAMP_OPTIONS_WINDOWSHADE,0);
		UpdateWindow(hwnd);
		Sleep(200);
		while (1)
		{
