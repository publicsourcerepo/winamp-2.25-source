...
#define FILE_BLAH MessageBox(wnd,WASABI_API_LNGSTRING(IDS_UNABLE_TO_LOAD_FILE),0,MB_ICONERROR)
...
static void __setdlgitemint(UINT id,UINT val)
{
	char buf[32];
	_itoa(val,buf,10);
	SetWindowText(__getdlgitem(id),buf);
}
...
static int __getdlgitemint(UINT id)
{
	char buf[32];
	buf[0]=0;
	GetWindowText(__getdlgitem(id),buf,32);
	return atoi(buf);
}
...
static BOOL WINAPI CfgProc(HWND wnd,UINT msg,WPARAM wp,LPARAM lp)
{
	...
	case WM_INITDIALOG:
		...
		SetWindowTextW(wnd,(wchar_t*)mod.description);
		...
		_SetDlgItemText(wnd,_IDC_DLS,cfg_dls_file);
		...
		_SendDlgItemMessage(wnd,_IDC_LOOP_SP,UDM_SETRANGE,0,MAKELONG(999,1));
		{
			...
			for(n=0;n<3;n++)
			{
				int freq = 11025<<n;
				_itoa(freq,tmp,10);
				int idx = SendMessage(w,CB_ADDSTRING,0,(long)tmp);
				if (cfg_freq==freq) SendMessage(w,CB_SETCURSEL,idx,0);
			}
			...
			{
				SendMessage(w,LB_ADDSTRING,0,(long)MIDI_core::FileTypes_GetExtension(n));
				if (cfg_ext_mask & (1<<n)) SendMessage(w,LB_SETSEL,1,n);
			}
			_SetDlgItemText(wnd,_IDC_EXTS_ED,cfg_extra_exts);
			...
			SendMessage(w,CB_ADDSTRING,0,(long)"GM (General MIDI)");
			SendMessage(w,CB_ADDSTRING,0,(long)"GS (Roland)");
			SendMessage(w,CB_ADDSTRING,0,(long)"XG (Yamaha)");
			SendMessage(w,CB_SETCURSEL,cfg_hardware_reset,0);
		}
//		f_num=0;
...
//				if (!edit_sysex_table.is_empty())
			...
			case _IDC_DLS_B:
				{
					char tmp[MAX_PATH];
					_GetDlgItemText(wnd,_IDC_DLS,tmp,MAX_PATH);
					if (DoOpenFile(wnd,tmp,IDS_DLS_FILES,"DLS",0)) _SetDlgItemText(wnd,_IDC_DLS,tmp);
				}
				break;
			}
			break;
