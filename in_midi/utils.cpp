...
static char cb_class[]="CallbackWndClass0";
...
{
	HWND w=CreateWindow((char*)cl,0,0,0,0,0,0,MIDI_callback::GetMainWindow(),0,MIDI_callback::GetInstance(),0);
...
{
	HANDLE f=CreateFile(file,GENERIC_WRITE,0,0,CREATE_ALWAYS,0,0);
...
int sysex_table::file_read(const char* file)
{
	...
	HANDLE f=CreateFile(file,GENERIC_READ,FILE_SHARE_READ,0,OPEN_EXISTING,0,0);
	if (f==INVALID_HANDLE_VALUE) return 0;
...
int sysex_table::print_preview(int idx,char * out) const
{
	...
	wsprintf(out,WASABI_API_LNGSTRING(STRING_MS_FMT),time);
	while(*out) out++;
	int n;
	for(n=0;n<size2;n++)
	{
		wsprintf(out," %02X",data[n]);
		out+=3;
	}
...
void sysex_table::print_edit(int idx,HWND wnd) const
{
	BYTE* data;
	int size,time;
	if (!get_entry(idx,&data,&size,&time)) {SetWindowText(wnd,"");return;}
	if (size<=2) {SetWindowText(wnd,"");return;}
	char *temp = (char*)malloc(3*size);
	char *ptr = temp;
	int n;
	for(n=1;n<size-1;n++)
	{
		wsprintf(ptr,"%02X ",data[n]);
		ptr+=3;
	}
	ptr[-1]=0;
