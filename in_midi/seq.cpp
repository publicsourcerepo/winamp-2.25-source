...
BOOL CALLBACK seq_base::CtrlProc(HWND wnd,UINT msg,WPARAM wp,LPARAM lp)
{
	...
	else s=(seq_base*)GetWindowLong(wnd,DWL_USER);
	if (s)
	{
		s->do_msg(msg,wp,lp);
	}
...
void CreateControl(DWORD ex,HWND hCtrl,const char * cls,const char * name,DWORD style,UINT x,UINT y,UINT dx,UINT dy,HINSTANCE hDll,UINT id)
{
	...
	HWND w=CreateWindowEx(ex,cls,name,WS_CHILD|WS_VISIBLE|style,r.left,r.top,r.right-r.left,r.bottom-r.top,hCtrl,0,hDll,0);
	if (w)
	{
...
void seq_base::do_msg(UINT msg,WPARAM wp,LPARAM lp)
{
	...
	case WM_INITDIALOG:
		{
			HINSTANCE hCCdll=GetModuleHandle("comctl32.dll");
			UINT n;
			HWND w;
			for(n=0;n<16;n++)
			{
				char tmp[16];
				itoa(n,tmp,10);
				CreateControl(0,hCtrl,TRACKBAR_CLASS,0,TBS_VERT | TBS_BOTH | TBS_NOTICKS | WS_TABSTOP,40+n*28,36,18,80,hCCdll,VOL_ID+n);
			...
			SetDlgItemText(hCtrl,IDC_SYSEX1,sysex1);
			SetDlgItemText(hCtrl,IDC_SYSEX2,sysex2);
			for(n=0;n<48;n++)
			{
...
seq_base::~seq_base()
{
	...
	if (hCtrl)
	{
		...
		cfg_ctrl_x=r.left;
		cfg_ctrl_y=r.top;
		GetDlgItemText(hCtrl,IDC_SYSEX1,sysex1,256);
		GetDlgItemText(hCtrl,IDC_SYSEX2,sysex2,256);
		DestroyWindow(hCtrl);
