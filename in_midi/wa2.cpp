...
typedef int (*readerfunc)(HINSTANCE,reader_source**);
static T_RF_create RF_create=0;
T_gzip_writefile gzip_writefile=0;
...
static int reader_process_file(WReader * r,const char * fn,void * &out_data, size_t &out_size)
{
	...
	int size=0;
	char ks=0;
	if (r->Open((char*)fn,&ks)) return 0;
	size = r->GetLength();
	if (size==-1 || size<0x20) return 0;
	data=malloc(size);//scan funcs assume that theres at least 256 data
	if (!data) return 0;
	if (r->Read((char*)data,size,&ks)!=size) {free(data);return 0;}
	if ((*(DWORD*)data==0x04034B50 || *(DWORD*)data==_rv('Rar!') || ((*(DWORD*)data)&0xFFFFFF)==0x088b1f || *(WORD*)data==0xEA60) && RF_create)
	{
		char temp[MAX_PATH];
		get_temp_file(temp);
		HANDLE f=CreateFile(temp,GENERIC_WRITE,0,0,CREATE_ALWAYS,0,0);
...
void InitReadFile()
{
	...
	if (!triedInit)
	{
	const char *pluginDir = (const char *)SendMessage(mod.hMainWindow, WM_WA_IPC, 0, IPC_GETPLUGINDIRECTORY);
	char path[MAX_PATH];
	PathCombine(path, pluginDir, "read_file.dll");

	HINSTANCE hRFdll=LoadLibrary(path);
	if (hRFdll)
	{
		readerfunc r=(readerfunc)GetProcAddress(hRFdll,"readerSource");
		if (!r) {FreeLibrary(hRFdll);hRFdll=0;}
		else
		{
			r(hRFdll,&pRF);
			RF_create=(T_RF_create)GetProcAddress(hRFdll,"RF_create");
			gzip_writefile=(T_gzip_writefile)GetProcAddress(hRFdll,"gzip_writefile");
		}
	}
	triedInit=1;
	}
...
static int Download(char* url,UINT* f_size,BYTE** m_buf)
{
	...
	{
#ifndef WINAMPX
    MessageBox(mod.hMainWindow,WASABI_API_LNGSTRING(STRING_URL_ERROR),ERROR,MB_ICONERROR);
#endif
		return 0;
	}
	else
	{
		...
		f=CreateFile(tmp,GENERIC_READ,0,0,OPEN_EXISTING,0,0);
...
int CMemReader::Open(char* url,char*)
{
	...
	HANDLE f=CreateFile(url,GENERIC_READ,FILE_SHARE_READ|FILE_SHARE_WRITE,0,OPEN_EXISTING,0,0);
	if (f!=INVALID_HANDLE_VALUE)
	{
		sz=GetFileSize(f,0);
		mem=(BYTE*)malloc(sz);
...
class CFileReader : public WReader
{
  public:
	...
	int Open(char *url, char*killswitch) {f=CreateFile(url,GENERIC_READ,FILE_SHARE_READ|FILE_SHARE_WRITE,0,OPEN_EXISTING,0,0);return f==INVALID_HANDLE_VALUE;}
...
static WReader *get_reader(const char* url)
{
	...
	if (strncmp(url,"http://",7)==0 || strncmp(url,"ftp://",6)==0 || strncmp(url,"https://",8)==0) return new CMemReader;
...
void Init()
{
	...
	swprintf(szDescription,WASABI_API_LNGSTRINGW(IDS_NULLSOFT_MIDI_PLAYER),VER);
...
{
#ifndef WINAMPX
  MessageBox(mod.hMainWindow,tx,0,MB_ICONERROR);
#endif
}
