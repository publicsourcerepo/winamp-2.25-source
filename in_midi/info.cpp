...
{
	if (!text[siz - 1]) SetDlgItemText(wnd, id, text);
	else
	{
		char* foo = (char*)alloca(siz + 1);
		memcpy(foo, text, siz);
		foo[siz] = 0;
		SetDlgItemText(wnd, id, foo);
	}
}
...
static BOOL CALLBACK rmiproc(HWND wnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_INITDIALOG:
		SetWindowLong(wnd, DWL_USER, lp);
		{
			...
			{
				SetDlgItemText(wnd, IDC_DISP, mf->title);
			}
			if (mf->rmi_data)
			{
					...
					for (n = 0;n < N_RMI_TAGZ;n++)
					{
						...
						if (GetWindowTextLength(w))
						{
							*(DWORD*)(rmi_info + ptr) = rmi_tagz[n].riff_id;
							...
							GetWindowText(w, foo, rmi_siz - (ptr + 4));
							UINT s = strlen(foo) + 1;
...
{
	if (!strncmp(url, "file://", 7) || !strncmp(url, "partial://", 10)) return 1;
...
char* getfmtstring(MIDI_file* f, char* s)
{
	...
	{
		char foo[32];
		StringCbPrintf(foo, sizeof(foo), WASABI_API_LNGSTRING(STRING_INFO_FORMAT_FMT), f->data[4 + 4 + 1]);
		z = foo;
		while (*z) *(s++) = *(z++);
	}
	if (f->info.e_type)
...
void MIDI_file::GetTitle(char* buf, int maxlen)
{
	...
	lstrcpyn(buf, file_title, maxlen);
}
...
BOOL SaveFile(HWND w, MIDI_file* mf, BOOL info)
{
	...
	BOOL rmi_only = info;
	string tmp;
	if (is_local(mf->path) && strncmp(mf->path, "partial://", 10))
	{
		tmp = mf->path;
...
#undef APPEND
...

	{
		...
		if (do_gzip) rv = gzip_writefile(tmp, buf, buf_size);
		else
		{
			HANDLE f = CreateFile(tmp, GENERIC_WRITE, 0, 0, CREATE_ALWAYS, 0, 0);
		...
		{
			char _m[320];
			StringCbPrintf(_m, sizeof(_m), WASABI_API_LNGSTRING(STRING_WRITE_ERROR_FMT), (const char*)tmp);
			MessageBox(w, _m, ERROR, MB_ICONERROR);
		}
		return rv;
	}
}
...
BOOL WINAPI InfoProc(HWND wnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	case WM_INITDIALOG:
		...
		SetWindowLong(wnd, DWL_USER, lp);
		SetDlgItemText(wnd, IDC_COPYRIGHT, _pr->info.copyright);
		SetDlgItemInt(wnd, IDC_MS, _pr->len, 0);
		SetDlgItemInt(wnd, IDC_TIX, _pr->info.tix, 0);
		{
			char tmp[128];
			getfmtstring(_pr, tmp);
			SetDlgItemText(wnd, IDC_FORMAT, tmp);

			HANDLE f = CreateFile(_pr->path, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, 0, OPEN_EXISTING, 0, 0);
			if (f != INVALID_HANDLE_VALUE)
			{
				StringCbPrintf(tmp, sizeof(tmp), WASABI_API_LNGSTRING(STRING_BYTES_FMT), GetFileSize(f, 0));
				CloseHandle(f);
				SetDlgItemText(wnd, IDC_FSIZE, tmp);
			}
		}
		{
			char tmp[1024];
			if (_pr->info.traxnames)
			{
				...
				{
					StringCbPrintf(tmp, sizeof(tmp), "(%u) %s", n, (const char*)(_pr->info.traxnames[n]));
					SendMessage(lb, LB_ADDSTRING, 0, (LPARAM)tmp);
				}
			}

			StringCbPrintf(tmp, sizeof(tmp), WASABI_API_LNGSTRING(STRING_TRACKS_FMT), _pr->info.ntrax);
			SetDlgItemText(wnd, IDC_NTRAX, tmp);
			if (_pr->title)
			{
				StringCbPrintf(tmp, sizeof(tmp), WASABI_API_LNGSTRING(STRING_MIDI_INFO_FMT1), (const char*)_pr->title, (const char*)_pr->path);
			}
			else
			{
				StringCbPrintf(tmp, sizeof(tmp), WASABI_API_LNGSTRING(STRING_MIDI_INFO_FMT2), (const char*)_pr->path);
			}
			if (_pr->flags&FLAG_INCOMPLETE) StringCbCat(tmp, sizeof(tmp), WASABI_API_LNGSTRING(STRING_INCOMPLETE));
