...
void get_temp_file(char* fn)
{
	...
	if (!tmp_path[0]) GetTempPath(MAX_PATH,tmp_path);
	static DWORD num;
	if (num==0) num=GetTickCount();
	wsprintf(fn,"%sasdf%x.tmp",tmp_path,num++);
}
...
{
	string temp;
	if (!mciGetErrorString(code,string_buffer_a(temp,256),256))
	{
...
BOOL CALLBACK MIDI_core::KarProc(HWND wnd,UINT msg,WPARAM wp,LPARAM lp)
{
	...
	{
	case WM_INITDIALOG:
		SetDlgItemText(wnd,IDC_BLAH,MIDI_core::kmap_data);
	...
	case WM_CLOSE:
		...
		if (!((int)cfg_bugged & BUGGED_BLAH))
		{
			...
			MessageBox(wnd,WASABI_API_LNGSTRING(IDS_TO_ENABLE_LYRICS_DISPLAY),
