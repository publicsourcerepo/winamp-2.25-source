...
#define CePutws(x,h) pISession->CeWriteFile(h,x,wcslen(x)*sizeof(wchar_t),&w,NULL)
#define CePuts(x,h) pISession->CeWriteFile(h,x,strlen(x)*sizeof(char),&w,NULL)
	HANDLE h = pISession->CeCreateFile(pl->fn,GENERIC_WRITE,FILE_SHARE_WRITE,NULL,TRUNCATE_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
	if(h == INVALID_HANDLE_VALUE) return;
	nu::PtrList<Song> *songs = &pl->songs;
	int l=songs->size();
	DWORD w;
	CePuts("<asx version=\"3.0\">\r\n",h);
	for(int j=0; j<l; j++) {
		CePuts("<entry><ref href=\"",h);
		wchar_t safe[fieldlen*2];
...
public:
	...
	void StartTag(const wchar_t *xmlpath, const wchar_t *xmltag, ifc_xmlreaderparams *params) {
		if(!wcscmp(xmlpath,L"ASX\fENTRY\fREF")) {
			...
			int l=mpl->songs.size();
			Song s;
			lstrcpyn(s.fn,path,MAX_PATH);
			nu::PtrList<Song>::iterator p = std::lower_bound(mpl->songs.begin(),mpl->songs.end(),&s,mplSearch2());
			int f = p - mpl->songs.begin();
...
ASDevice::~ASDevice() {
	...
	for(unsigned int k=0; k<devices.size(); k++) {
		if(devices[k] == this) devices.eraseindex(k);
	}
	pIDevice->Release();
	pISession->Release();
...
void ASDevice::Eject() {
	ejectedDevice *e = (ejectedDevice *)malloc(sizeof(ejectedDevice));
...
int ASDevice::transferTrackToDevice(const itemRecordW * track,void * callbackContext,void (*callback)(void * callbackContext, wchar_t * status),songid_t * songid,int * killswitch) {
	wchar_t ext[10]=L"";
	wchar_t file[2048];
	wcscpy(file,track->filename);
	{wchar_t * e = wcsrchr(file,L'.'); if(e) wcscpy(ext,e+1);}
	...
	wchar_t *path = (wchar_t*)malloc((wcslen(musicFolder)+wcslen(track->artist)+wcslen(track->album)+wcslen(track->title)+100)*sizeof(wchar_t));
...
void ASDevice::deleteTrack(songid_t songid) {
	...
	for(unsigned int i=0; i<playlists.size(); i++) {
		unsigned int l = playlists[i]->songs.size();
		for(unsigned int j=0; j<l; j++) {
			if(playlists[i]->songs[j] == song) {
				playlists[i]->songs.eraseindex(j);
				l--;
...
int ASDevice::getPlaylistCount() { return playlists.size(); }
void ASDevice::getPlaylistName(int playlistnumber, wchar_t * buf, int len) { lstrcpyn(buf,playlists[playlistnumber]->name,len); }
int ASDevice::getPlaylistLength(int playlistnumber) { return playlists[playlistnumber]->songs.size(); }
...
void ASDevice::playlistSwapItems(int playlistnumber, int posA, int posB) {
	nu::PtrList<Song> &songs = playlists[playlistnumber]->songs;
	Song * a = songs[posA];
	Song * b = songs[posB];
	songs[posA] = b;
	songs[posB] = a;
...
void ASDevice::removeTrackFromPlaylist(int playlistnumber, int songnum) {
	playlists[playlistnumber]->songs.eraseindex(songnum);
	playlists[playlistnumber]->dirty=true;
}
...
void ASDevice::deletePlaylist(int playlistnumber) {
	...
	playlists.eraseindex(playlistnumber);
}
