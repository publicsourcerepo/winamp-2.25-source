...
nu::PtrList<ASDevice> devices;

nu::PtrList<ejectedDevice> ejected;
...
static void enumDevices() {
	...
	for(unsigned int i=0; i<ejected.size(); i++) if(!ejected[i]->marked) { free(ejected[i]); ejected.eraseindex(i); }
}

static void init2() {
	// set up device connection/disconnection notifications
	pMyRapiSink = new MyRAPISink();
	pIRapiDesktop->Advise(pMyRapiSink,&pMyRapiSink->RAPISinkContext);
