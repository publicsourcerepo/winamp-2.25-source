...
static unsigned short Crc_12Table[] =
{
...
/*********************************************************
    ...
    function:    BuildCrc_12Table
    description: Function for generating the lookup table
                 for a 12 bit crc (polynomial: 0x80f).
    return:     -
...
void BuildCrc_12Table (void) {
  ...
  for (i=0; i < 256; i++) {
    ...
    Crc_12Table[i] = (unsigned short) (crc & 0x0fff);
  }
}
...
/*********************************************************
    ...
    function:    PrintCrc_12Table
    description: print 12 bit crc lookup table
    return:     -

**********************************************************/
void PrintCrc_12Table(void) {
  int i=0, j=0;
  unsigned int crc_tab=0;

  /* print */

  fprintf(stdout,"\n- polynomial 0x%x lookup table -\n",CRCPOLY);
  for( i=0; i<256; i++ ) {
    if( !(i%8) ) fprintf( stdout, "\n" );
    fprintf(stdout, "0x%.2x, ", Crc_12Table[i] );
  }
  fprintf(stdout,"\n\n- end of lookup table ----------\n");
  ...
  for( i=0; i<256; i++ ) {
    crc_tab = (unsigned int)Crc_12Table[i];
    for( j=0; j<256; j++ ) {
      if( crc_tab == (unsigned int)Crc_12Table[j] && i!=j ) {
        fprintf(stderr,"\nError: double entry in lookup table!\n");
      }
    }
  }
}
...
/*********************************************************
    ...
    function:    Crc_12
    description: calculate a 12-bit crc with
                 polynomial 0x80F and init 0x0fff
    return:      crc

**********************************************************/
unsigned short Crc_12(unsigned char *data, int len) {
  int i;
  unsigned short crc = 0x0fff;

  for (i=0; i<len; i++) {
    crc = 0x0fff & (((crc<<8)&0xff00) ^ Crc_12Table[((crc>>4) ^ *data++) & 0xff]);
  }
