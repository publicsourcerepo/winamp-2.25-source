 ...
 * $Id: bitstream.cpp,v 1.1 2009/04/28 20:17:43 audiodsp Exp $
 */
...
unsigned int CBitStream::Get8Bits()
{
  unsigned short tmp,tmp1;
    
  unsigned int nWordNdx   = (m_BitNdx>>3);
  unsigned int nBitsAvail = 8 - (m_BitNdx & 7);
  ...
  if( 8 > nBitsAvail )
  {
    nWordNdx  = (nWordNdx+1) & (m_nBytes-1);
    tmp1      = m_Buf[nWordNdx];
    tmp1    >>= (8-(m_BitNdx&7));
    tmp      |= tmp1;
  }

  m_BitNdx     = (m_BitNdx+8) & (m_nBits-1);
  m_BitCnt    += 8;
  m_ValidBits -= 8;
