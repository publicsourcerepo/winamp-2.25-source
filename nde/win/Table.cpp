...
//---------------------------------------------------------------------------
Table::Table(const wchar_t *TableName, const wchar_t *Idx, BOOL Create, Database *_db, BOOL _Cached)
...
BOOL Table::Open(void)
{
	...
	else
	{
		...
		// Load default index
		IndexField *field;
		field = new IndexField(PRIMARY_INDEX, -1, -1, L"None");
...
ColumnField *Table::NewColumn(unsigned char FieldID, const wchar_t *FieldName, unsigned char FieldType, BOOL indexUnique)
{
	...
	if (f) {
		...
		f->SetFieldName((wchar_t *)FieldName);
		return NULL;
	}
aok:
...
bool Table::Compact_ColumnWalk(Record *record, Field *entry, void *context_in)
{
	...
	ColumnField *ccol = ctable->NewColumn(field->GetFieldId(), field->GetFieldName(), field->GetDataType(), FALSE);
	return true;
}
