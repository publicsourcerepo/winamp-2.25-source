...
void IDScanner::SetGracenoteData(BSTR filename, BSTR tagID, BSTR extData)
{
	...
	{
		hr=playlistMgr->FileSetExtDataSerialized(filename, extData, CDDB_UPDATE_NONE);
		CComBSTR test;
		hr=playlistMgr->FileGetExtDataSerialized(filename, &test, 0); // benski> 24 Jul 2007 - there is a currently a bug that makes this always E_FAIL
		if (test)
			foundExt=true;
	}
	...
	{
		hr=playlistMgr->FileSetTagID(filename, tagID, CDDB_UPDATE_EXTENDED);

		// write back to Media Library database
		CComBSTR extData;
		hr=playlistMgr->FileGetExtDataSerialized(filename, &extData, 0); // benski> 24 Jul 2007 - there is a currently a bug that makes this always E_FAIL
		if (extData)
			AGAVE_API_MLDB->SetField(filename, "GracenoteExtData", extData);
	}
	else
		hr=playlistMgr->FileSetTagID(filename, tagID, CDDB_UPDATE_NONE);
}
	...
	if (SetupPlaylistSDK())
	{
		...
		if (results)
		{
			...
			for (int i=0;i<results->Size;i++)
			{
				...
				hr=playlistMgr->AddEntry(filename);					// Add entry to gracenote DB
...
int IDScanner::MLDBFileAddedOnThread(HANDLE handle, void *user_data, intptr_t id)
{
	...
	hr=playlistMgr->AddEntry(filename);	// Add the file entry to the gracenote DB
	...
	if (hr == S_OK /*&& results->Size == 1*/)
	{
		...
		if (result->album && result->album[0])
			hr=playlistMgr->FileSetFieldVal(filename, gnpl_crit_field_album_name, result->album);

		if (result->artist && result->artist[0])
			hr=playlistMgr->FileSetFieldVal(filename, gnpl_crit_field_track_artist_name, result->artist);

		// Populate title, file size, and length information so that we have more complete data.
		if (result->title && result->title[0])
			hr=playlistMgr->FileSetFieldVal(filename, gnpl_crit_field_track_name, result->title);
		...
		{
			hr=playlistMgr->FileSetFieldVal(filename, gnpl_crit_field_xdev1, L"2"); // move to phase 2, we can skip phase 1 
