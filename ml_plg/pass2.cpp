...
static ICddbFileInfoList *CreateScanList(volatile int *killswitch)
{
	...
	hr = playlistMgr->FindClose(pFindData);
	infoList->AddRef();
	return infoList;
}
...
void IDScanner::Pass2()
{
	if (SetupMusicID())
	{
		...
		{
			long match_code = 666;
			HRESULT hr = musicID->LibraryID(infoList, MUSICID_RETURN_EXACT_ONLY | MUSICID_GET_FP_FROM_APP | MUSICID_GET_TAG_FROM_APP | MUSICID_PREFER_WF_MATCHES);
