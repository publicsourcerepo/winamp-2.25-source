...
bool SetupPlaylistSDK()
{
	if (!playlistMgr)
	{
		...
		if (icp)
		{
			HRESULT hr = icp->Advise(static_cast<IDispatch *>(&events), &m_dwCookie);
			icp->Release();
		}
	}
...
void ShutdownPlaylistSDK()
{
	if (playlistMgr)
	{
		...
		playlistMgr->Release();
//		playlistMgr->Release(); // TODO: benski> i'm not sure why we have an extra reference
...
{
	wchar_t path_to_create[MAX_PATH];
...
void CheckForResetError(HRESULT error)
{
	...
	{
		wchar_t message[256];
		StringCchCopyW(message, 256, WASABI_API_LNGSTRINGW(IDS_ERROR_RESET));
		//MessageBoxW(plugin.hwndWinampParent, message, WASABI_API_LNGSTRINGW(IDS_NULLSOFT_PLAYLIST_GENERATOR), MB_OK| MB_ICONERROR);
		MessageBoxW(0, message, WASABI_API_LNGSTRINGW(IDS_NULLSOFT_PLAYLIST_GENERATOR), MB_OK| MB_ICONERROR);
	}
}
...
void CheckForShutdownError(HRESULT error)
{
	...
	{
		wchar_t message[256];
		StringCchCopyW(message, 256, L"Unexpected Error. Cannot shut down the playlist generator engine.");
		MessageBoxW(plugin.hwndWinampParent, message, WASABI_API_LNGSTRINGW(IDS_NULLSOFT_PLAYLIST_GENERATOR), MB_OK| MB_ICONERROR);
	}
}
...
void playPlaylist(Playlist &pl, bool enqueue=false, int startplaybackat=0/*-1 for don't start playback*/, int useSeed=FALSE)
{
	...
	if(useSeed)
	{
		for (size_t i = 0; i < number_of_seeds; i++)
		{
			...
			enqueueFileWithMetaStructW s={seedFilename,NULL,-1};
  ...
  for(size_t i=0; i<listLength; i++)
	{
		// ToDo: Change this to MAX_PATH
		wchar_t filename[256];
		pl.GetItem(i,filename,256);
		...
		//	continue;
		enqueueFileWithMetaStructW s={filename,NULL,-1};
		SendMessage(plugin.hwndWinampParent, WM_WA_IPC, (WPARAM)&s, IPC_PLAYFILEW);
  }
...
{	
	...
	const int number_of_files = pl->GetNumItems();
