...
void UpdateStats(void)
{
	...
	wchar_t stats[MAX_STATS];
	wchar_t lengthText[MAX_STATS];
	wchar_t sizeText[MAX_STATS];
	int count = currentPlaylist.GetNumItems();
	...
	if (useSeed == TRUE)
	{
		count += seedPlaylist.GetNumItems();
...
INT_PTR CALLBACK GenerateProcedure(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (msg)
	{
		case WM_INITDIALOG:
		{
			...
			POINT pt = {GetPrivateProfileInt(L"ml_plg", L"generate_x", -1, mediaLibrary.GetWinampIniW()),
						GetPrivateProfileInt(L"ml_plg", L"generate_y", -1, mediaLibrary.GetWinampIniW())};
...
int AddResultListItem(Playlist *playlist, int index, int position, bool seed)
{
	...
	wchar_t filename[MAX_INFO];
	wchar_t info[MAX_INFO] = L"";
	wchar_t *seedText = 0;
...
{
	const int MAX_INFO = 256;
	int listLength = -1;
	int seedLength = -1;

	//SendMessage(GetDlgItem(hwndDlgCurrent, IDC_LIST_RESULTS2),LVM_DELETEALLITEMS,0,0);		// Clear the listview
	//LVM_DELETEALLITEMS 

	// Add all of the seed tracks to the listview
	listLength = (playlist) ? playlist->GetNumItems() : 0;
	seedLength = seedPlaylist.GetNumItems();
