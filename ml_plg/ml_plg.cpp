...
wchar_t mlQuery[MAX_ML_QUERY_SIZE];		// Storage for the custom query
...
int Init()
{
	...
	playlistGeneratorFactory.Register(WASABI_API_SVC, &playlistGeneratorAPI);
	...
	mldbWatcher.WatchWith(WASABI_API_SVC);
...
void Quit()
{
	...
	playlistGeneratorFactory.Deregister(WASABI_API_SVC);
...
void NotInMLWarning(const wchar_t *filename)
{
	...
	MessageBoxW(plugin.hwndLibraryParent, message, WASABI_API_LNGSTRINGW(IDS_NULLSOFT_PLAYLIST_GENERATOR), MB_OK| MB_ICONINFORMATION);
}

void MultipleInstancesWarning(void)
{
	wchar_t message[256];
	StringCchCopy(message, 256, WASABI_API_LNGSTRINGW(IDS_THERE_CAN_BE_ONLY_ONE));
	MessageBoxW(plugin.hwndLibraryParent, message, WASABI_API_LNGSTRINGW(IDS_NULLSOFT_PLAYLIST_GENERATOR), MB_OK | MB_ICONINFORMATION);
}
...
int AddSeedTracks(menufucker_t *mf)
{
	...
	while (position >= 0 && position < count)
	{
		...
		item = &mf->extinf.mediaview.items->Items[position];
...
{
	const int count = mf->extinf.mlplaylist.pl->GetNumItems();
	itemRecordW *item = 0;
	...
	while (position >= 0)
	{
		...
		item = AGAVE_API_MLDB->GetFile(filename);
...
INT_PTR MessageProc(int message_type, INT_PTR param1, INT_PTR param2, INT_PTR param3)
{
	...
	if(message_type == ML_IPC_MENUFUCKER_BUILD && pluginEnabled)
	{
		...
		if(mf->type == MENU_MEDIAVIEW)
		{
			...
			StringCchPrintf(str,100,WASABI_API_LNGSTRINGW(IDS_PLAY_TRACKS_SIMILAR_TO),ice->title);
		...
		else if(mf->type == MENU_MLPLAYLIST)
		{
			...
			StringCchPrintf(str,100,WASABI_API_LNGSTRINGW(IDS_PLAY_TRACKS_SIMILAR_TO),title);
			FixStrForMenu(str,100);
			mii.dwTypeData = str;
			mii.cch = (UINT)wcslen(str);
		...
		else if(mf->type == MENU_PLAYLIST)
		{
			...
			InsertMenuItem(mf->menu,40208,FALSE,&mii);
			mii.wID = 0xdeadc0de;
			mii.fType = MFT_SEPARATOR;
			InsertMenuItem(mf->menu,40208,FALSE,&mii);
		}
