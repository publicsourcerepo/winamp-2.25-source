...
genHotkeysAddStructLocalised WADefCommands[] = {
	...
	{IDS_UI__PREFERENCES,{0, HKF_BRING_TO_FRONT, WM_WA_IPC, -1, IPC_OPENPREFSTOPAGE, "ghkdc prefs"}},
	...
	{IDS_RATING__DEC,{0, 0, WM_WA_IPC, -1, IPC_SETRATING, "ghkdc dec rating"}},
...
int AddCommand(genHotkeysAddStruct *ghas)
{
	...
	if (WACommands)
	{
		...
		while (l--)
		{
			if (!lstrcmpi(WACommands[l].id, ghas->id))
			{
	...
	WACommands[idx].id = _strdup(ghas->id);
...
inline char *GetCommandId(unsigned int i)
{
	if (i < WACommandsNum)
...
int GetCommandIdx(char *id)
{
	if (WACommands)
	{
		...
		while (l--)
		{
		if (!lstrcmpi(WACommands[l].id, id))
		{
			return l;
		}
		}
