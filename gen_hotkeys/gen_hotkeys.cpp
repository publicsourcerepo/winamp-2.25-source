...
HOTKEY_DATA g_defhkds[] = {
                              {MAKEWORD(VK_INSERT, HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc play"},
                              {MAKEWORD(VK_HOME, HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc pause"},
                              {MAKEWORD(VK_END, HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc stop"},
                              {MAKEWORD(VK_PRIOR, HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc prev"},
                              {MAKEWORD(VK_NEXT, HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc next"},
                              {MAKEWORD(VK_UP, HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc vup"},
                              {MAKEWORD(VK_DOWN, HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc vdown"},
                              {MAKEWORD(VK_RIGHT, HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc forward"},
                              {MAKEWORD(VK_LEFT, HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc rewind"},
                              {MAKEWORD('J', HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc jump"},
                              {MAKEWORD('L', HOTKEYF_ALT | HOTKEYF_CONTROL | HOTKEYF_EXT), -1, "ghkdc file"},
                              //multimedia keyboard stuff
                              {2226, -1, "ghkdc stop"},
                              {2227, -1, "ghkdc play/pause"},
                              {2225, -1, "ghkdc prev"},
                              {2224, -1, "ghkdc next"},
                          };
...
int pluginInit()
{
	...
	g_iniFile = (char*) SendMessage(psPlugin.hwndParent, WM_WA_IPC, 0, IPC_GETINIFILE);
	if ((unsigned int)(uintptr_t) g_iniFile < 65536) g_iniFile = "winamp.ini";
	...
	appcommand = GetPrivateProfileInt("gen_hotkeys", "appcommand", 0, g_iniFile);
...
void hotkeysInit()
{
	...
	int enabled = GetPrivateProfileInt("gen_hotkeys", "enabled", 0, g_iniFile);
	...
	int l = GetPrivateProfileInt("gen_hotkeys", "nbkeys", -1, g_iniFile);
	int ver = GetPrivateProfileInt("gen_hotkeys", "version", 1, g_iniFile);
	if (l != -1)
	{
		if (ver == 2)
		{
			...
			if (hkds)
			{
				for (i = 0; i < l; i++)
				{
					...
					char action[1024];
					int hotkey;
					StringCchPrintf(tmp, 1024, "action%d", i);
					GetPrivateProfileString("gen_hotkeys", tmp, "", action, 1024, g_iniFile);
					StringCchPrintf(tmp, 1024, "hotkey%d", i);
					hotkey = GetPrivateProfileInt("gen_hotkeys", tmp, 0, g_iniFile);
					...
					if (hkds[i].iCommand >= 0)
						...
						hkds[i].szCommand = _strdup(action);
				}
		...
		else if (ver == 1)
		{
			...
			if (hkds) 
			{
				...
				{
					char tmp[512];
					StringCchPrintf(tmp, 512, "msg%d", i);
					unsigned int msg = GetPrivateProfileInt("gen_hotkeys", tmp, 0, g_iniFile);
					if (msg)
					{
						StringCchPrintf(tmp, 512, "wparam%d", i);
						WPARAM wparam = GetPrivateProfileInt("gen_hotkeys", tmp, 0, g_iniFile);
						StringCchPrintf(tmp, 512, "lparam%d", i);
						LPARAM lparam = GetPrivateProfileInt("gen_hotkeys", tmp, 0, g_iniFile);
						for (int j = 0;WACommands[j].name;j++)
						{
							if (WACommands[j].uMsg == msg && WACommands[j].wParam == wparam && WACommands[j].lParam == lparam)
							{
								StringCchPrintf(tmp, 512, "hotkey%d", i);
								int hotkey = GetPrivateProfileInt("gen_hotkeys", tmp, 0, g_iniFile);
...
void writePrivateProfileInt(char *section, int val)
{
	char s[32];
	StringCchPrintf(s, 32, "%d", val);
	WritePrivateProfileString("gen_hotkeys", section, s, g_iniFile);
}
...
void hotkeysSave(HOTKEY_DATA *hkds, DWORD num)
{
	int enabled = GetPrivateProfileInt("gen_hotkeys", "enabled", 0, g_iniFile);
	appcommand = GetPrivateProfileInt("gen_hotkeys", "appcommand", 0, g_iniFile);
	WritePrivateProfileString("gen_hotkeys", NULL, NULL, g_iniFile);
	writePrivateProfileInt("nbkeys", num);
	writePrivateProfileInt("version", 2);
	writePrivateProfileInt("enabled", enabled);
	writePrivateProfileInt("appcommand", appcommand);
	if (!hkds || !num)
	{
		return ;
	}
	for (size_t i = 0; i < num; i++)
	{
		...
		StringCchPrintf(tmp, 1024, "action%d", i);
		if (hkds[i].iCommand >= 0)
			WritePrivateProfileString("gen_hotkeys", tmp, GetCommandId(hkds[i].iCommand), g_iniFile);
		else if (hkds[i].szCommand)
			WritePrivateProfileString("gen_hotkeys", tmp, hkds[i].szCommand, g_iniFile);
		StringCchPrintf(tmp, 1024, "hotkey%d", i);
				...
				if (cmd > 0)
				{
					for (unsigned int i = 0; i < g_dwHotkeys; i++)
					{
						if (g_hotkeys[i].hkd.iCommand < 0
							...
							&& !lstrcmpi(g_hotkeys[i].hkd.szCommand, GetCommandId(cmd)))
						{
