	...
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			if (hwHKList && hwHK)
			{
				...
				colwidth2 = GetPrivateProfileInt("gen_hotkeys","col2",-1,g_iniFile);
				...
				colwidth1 = GetPrivateProfileInt("gen_hotkeys","col1",-1,g_iniFile);
			...
			{
				bool unicode = 0;
				char *cmdname = GetCommandName(i, &unicode);
				if (*cmdname)
				{
					LRESULT pos;
					if(unicode) pos = SendMessageW(hwCombo, CB_ADDSTRING, 0, (LPARAM) cmdname);
					else pos = SendMessageA(hwCombo, CB_ADDSTRING, 0, (LPARAM) cmdname);
					SendMessage(hwCombo, CB_SETITEMDATA, pos, i);
				}
			}

			// Set the enabled checkbox
			CheckDlgButton(hwndDlg,IDC_ENABLED,GetPrivateProfileInt("gen_hotkeys","enabled",0,g_iniFile));
			CheckDlgButton(hwndDlg,IDC_ENABLED_WM_APPCOMMAND,GetPrivateProfileInt("gen_hotkeys","appcommand",0,g_iniFile));
		...
		{
			int checked = (SendMessage(GetDlgItem(hwndDlg, IDC_ENABLED), BM_GETCHECK, 0, 0) == BST_CHECKED);
			writePrivateProfileInt("enabled", checked);
			writePrivateProfileInt("appcommand",
			...
			writePrivateProfileInt("col1",ListView_GetColumnWidth(hwHKList,0));
			writePrivateProfileInt("col2",ListView_GetColumnWidth(hwHKList,1));
...
void SetListItem(HWND hwHKList, HWND hwHK, HOTKEY_DATA *hk_data, int idx, int failed)
{
	...
	if (idx >= 0 || hk_data_allocated)
	{
		...
		LVITEM lvi = {
						...
						pos,
						0,
						LVIS_SELECTED | INDEXTOSTATEIMAGEMASK(failed ? 1 : 0) | LVIS_FOCUSED,
