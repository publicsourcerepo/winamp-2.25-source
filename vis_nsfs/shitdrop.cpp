...
static void readconfig(struct winampVisModule *this_mod)
{
	...
	g_width=GetPrivateProfileInt(szAppName,"sdwidth",640,ini_file);
	g_height=GetPrivateProfileInt(szAppName,"sdheight",480,ini_file);
	g_divx=GetPrivateProfileInt(szAppName,"sdx",16,ini_file);
	g_divy=GetPrivateProfileInt(szAppName,"sdy",12,ini_file);
	g_fadeout=GetPrivateProfileInt(szAppName,"sdfadeout",2,ini_file);
}
...
static void writeInt(char *name, int value)
{
	...
	WritePrivateProfileString(szAppName,name,buf,ini_file);
}
...
{	
	...
	writeInt("sdwidth",g_width);
	writeInt("sdheight",g_height);
	writeInt("sdx",g_divx);
	writeInt("sdy",g_divy);
	writeInt("sdfadeout",g_fadeout);
	running=0;
}
