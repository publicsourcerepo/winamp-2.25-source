...
/*BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
	...
	__declspec( dllexport ) winampVisHeader* winampVisGetHeader(HWND hwndParent)
	{
		...
		char temp[256];
		wsprintfA(szDescription,"%s "PLUGIN_VERSION,WASABI_API_LNGSTRING_BUF(IDS_NULLSOFT_TINY_FULLSCREEN,temp,256));
...
void initwindow(struct winampVisModule *this_mod, int w, int h)
{
	...
	WNDCLASS wc={0,WndProc,0,0,this_mod->hDllInstance,0,0,(HBRUSH)GetStockObject(BLACK_BRUSH),0,szAppName};
	UnregisterClass(szAppName,this_mod->hDllInstance);
	RegisterClass(&wc);
	hwndParent=this_mod->hwndParent;
	g_hwnd = CreateWindowEx(WS_EX_APPWINDOW|WS_EX_TOPMOST,szAppName,this_mod->description,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	if (message == WM_CREATE){
		...
		HDC hdc = GetDC(hwnd);
		hFont = CreateFont(-MulDiv(12, GetDeviceCaps(hdc, LOGPIXELSY), 72),
						   0, 0, 0, FW_BOLD, 0, 0, 0, 0, 0, 0, 0, 0, "Arial");
		ReleaseDC(hwnd,hdc);
	}
	...
	else if (message == WM_TIMER)
	{
		...
		wchar_t *p = g_title;
		while (*p) p = CharNextW(p); p = CharPrevW(g_title,p);
		while (p >= g_title)
		{
...
static void config_read(struct winampVisModule *this_mod)
{
	...
	if (!GetPrivateProfileStruct(szAppName,"colors",colpoints,sizeof(colpoints),ini_file))
	{
	...
	GetPrivateProfileStruct(szAppName,"config",&configst,sizeof(configst),ini_file);
}

void config_write(struct winampVisModule *this_mod)
{
	init_inifile(this_mod);
	WritePrivateProfileStruct(szAppName,"colors",colpoints,sizeof(colpoints),ini_file);
	WritePrivateProfileStruct(szAppName,"config",&configst,sizeof(configst),ini_file);
}

void do_vprintcol(HWND hwndDlg, unsigned char *t)
{
	CHOOSECOLOR cs={sizeof(cs),hwndDlg,};
	cs.hInstance = 0;
...
static HRESULT WINAPI _cb(LPDDSURFACEDESC lpDDSurfaceDesc,  LPVOID lpContext)
{
	if (lpDDSurfaceDesc->ddpfPixelFormat.dwRGBBitCount==8)
	{
		LRESULT idx;
		char s[32];
		wsprintf(s,"%dx%d",lpDDSurfaceDesc->dwWidth,lpDDSurfaceDesc->dwHeight);
		idx=SendMessage((HWND)lpContext,CB_ADDSTRING,0,(LPARAM)s);
