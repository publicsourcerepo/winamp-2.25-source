...
: url(strdup(root_url))
{
	...
	WASABI_API_DLMGR->DownloadEx(full_url, new SongListDownloader(url, this), api_downloadManager::DOWNLOADEX_CALLBACK);
	}
	else
	{
...
HRESULT WifiDevice::Attach(HWND hostWindow)
{
	if (!connect_active)
	{
		...
		WASABI_API_DLMGR->DownloadEx(full_url, new PairDownloader(this), api_downloadManager::DOWNLOADEX_CALLBACK);
	}
