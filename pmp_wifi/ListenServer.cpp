#include "api.h"
#include "../jnetlib/multicastlisten.h"
...
#include <windows.h>
static Nullsoft::Utility::LockGuard connections_guard;
...
void WAFAListener::SSDPCallback_OnServiceConnected(Wasabi2::nx_uri_t location, Wasabi2::nx_string_t type, Wasabi2::nx_string_t usn)
{
	...
	{
		Nullsoft::Utility::AutoLock auto_lock(connections_guard);
		...
		WASABI_API_DLMGR->DownloadEx(cached_location, downloader, api_downloadManager::DOWNLOADEX_CALLBACK);
	}		
}
...
void OnInfoDownloadDone(InfoDownloader *_info)
{
	...
	else
	{
		if (connection.location)
		{
			...
			WASABI_API_DLMGR->DownloadEx(cached_location, connection.downloader, api_downloadManager::DOWNLOADEX_CALLBACK);		
		}
	}
}
