...
namespace conformance_inverse4x4 {
...
static void print_4x4_results(const h264_int_macroblock_t output, int pos_y, int pos_x)
{
		for (int i=0;i<4;i++)
	{
		for (int j=0;j<4;j++)
		{
			printf("%+08i ", output[i+pos_y][j+pos_x]);
		}
		printf("\r\n");
	}
}

static bool match_4x4_results(const h264_int_macroblock_t a,const h264_int_macroblock_t b,int pos_y, int pos_x)
{
		for (int i=0;i<4;i++)
	{
		for (int j=0;j<4;j++)
...
static bool test_4x4(const h264_int_macroblock_t coefficients)
{
	bool any_failed=false;
		h264_int_macroblock_t output1, output2;
		for (int y=0;y<12;y++)
		for (int x=0;x<12;x++)
		{
			conformance_inverse4x4::inverse4x4(coefficients, output1, y, x);
			conformance_inverse4x4::inverse4x4_sse2_x86(coefficients, output2, y, x);
...
bool test_inverse4x4()
{
		bool any_failed=false;
	h264_int_macroblock_t rand_coefficients;
	...
	for (int i=0;i<100;i++)
	{
		...
		if (test_4x4(rand_coefficients) == false)
		{
			printf(" *** coefficients for failure: ***\r\n");
