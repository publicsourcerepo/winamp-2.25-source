...
//        SendMessage(mod.hMainWindow,WM_USER,timems,106);
  ...
  static void stopPlayback(const char *fn)
  {
    ...
    {
      isplaying=SendMessage(mod.hMainWindow,WM_USER,0,104);
      if (isplaying)
      {
        timems=SendMessage(mod.hMainWindow,WM_USER,0,105);
...
extern "C" 
{
    ...
    if(strcmp(fn,m_set_lfile)) 
    {
      closeset();

      lstrcpyn(m_set_lfile,fn,sizeof(m_set_lfile));

      HANDLE hFile = CreateFile(fn,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ,NULL,OPEN_EXISTING,0,NULL);
    ...
    if (p) while (pos < m_set_lhdr.metadata_len)
    {
      ...
      if (!strncmp(lp,data,strlen(data)) && lp[strlen(data)]=='=') 
      {
        ...
        {
          m_set_lhdr.metadata_len = lp - (char*)m_set_lhdr.metadata;
        }
        else
        {
          memcpy(lp,p,m_set_lhdr.metadata_len - (p-(char*)m_set_lhdr.metadata));
          m_set_lhdr.metadata_len -= p-lp;
        }
        break;
      }
    }
    ...
    if (val && *val)
    {
      ...
      int nmdl=m_set_lhdr.metadata_len + 5 + strlen(data) + strlen(val);
      ...
      m_set_lhdr.metadata_len+=strlen(data);
      ...
      m_set_lhdr.metadata_len+=strlen(val);
    ...
    if (m_set_lfile[0])
    {
      ...
      HANDLE hFile = CreateFile(m_set_lfile,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ,NULL,OPEN_EXISTING,0,NULL);
      if (hFile == INVALID_HANDLE_VALUE)
      {
//        MessageBox(mod.hMainWindow,"Error opening NSV file for update","NSV Tag Error",MB_OK);
      ...
      else
      {
        ...
        char tmpfn[1024+8];
        char tmpfn2[1024+8];
        wsprintf(tmpfn,"%s.new",m_set_lfile);
        wsprintf(tmpfn2,"%s.old",m_set_lfile);

        HANDLE hTempFile=CreateFile(tmpfn,GENERIC_READ|GENERIC_WRITE,0,NULL,CREATE_ALWAYS,0,NULL);
  ...
  //        MessageBox(mod.hMainWindow,"Can't create tempfile - can't update tag!","NSV Tag Editor Error",MB_ICONSTOP|MB_OK);
        ...
        if (!MoveFile(m_set_lfile,tmpfn2))
        {
          DeleteFile(tmpfn);
          restartPlayback();
        ...
        if (!MoveFile(tmpfn,m_set_lfile))
        {
          MoveFile(tmpfn2,m_set_lfile);
          DeleteFile(tmpfn);
          restartPlayback();
...
void closeNsv(HWND hwndDlg)
{
  ...
  SetDlgItemText(hwndDlg,IDC_LENBYTES,"");
  SetDlgItemText(hwndDlg,IDC_LENMS,"");
  SetDlgItemText(hwndDlg,IDC_AVGBITRATE,"");
  CheckDlgButton(hwndDlg,IDC_TOC,0);
  SetDlgItemText(hwndDlg,IDC_TOCSIZE,"");
  SetDlgItemText(hwndDlg,IDC_SETTOCSIZE,"");
  SetDlgItemText(hwndDlg,IDC_FN,"");
  int x;
  int cnt=SendDlgItemMessage(hwndDlg,IDC_METADATA,LB_GETCOUNT,0,0);
  for ( x= 0; x < cnt; x ++)
  {
  ...
  SetDlgItemText(hwndDlg,IDC_TAG_LEN,WASABI_API_LNGSTRING(IDS_TOTAL_TAG_SIZE));
  SetDlgItemText(hwndDlg,IDC_METADATA_LEN,WASABI_API_LNGSTRING(IDS_NO_METADATA));
...
void makeNewHeaderInfo(HWND hwndDlg)
{
  ...
  char buf[128];
  if (g_filehdr.header_size) wsprintf(buf,WASABI_API_LNGSTRING(IDS_TOTAL_TAG_SIZE_X_BYTES),g_filehdr.header_size);
  else WASABI_API_LNGSTRING_BUF(IDS_NO_TAG,buf,128);
  SetDlgItemText(hwndDlg,IDC_TAG_LEN,buf);
}
...
void populateInfo(HWND hwndDlg)
{
  ...
  else SetDlgItemText(hwndDlg,IDC_LENBYTES,"?");

  if (g_filehdr.file_lenms != ~0) 
  {
    char buf[128];
    unsigned int timems=g_filehdr.file_lenms;
    if (timems < 1000) wsprintf(buf,"%ums",timems);
    else if (timems < 1000*60) wsprintf(buf,"%02u.%03us",timems/1000,timems%1000);
    else if (timems < 1000*60*60) wsprintf(buf,"%02u:%02u.%03us",timems/60000,(timems/1000)%60,timems%1000);
    else wsprintf(buf,"%u:%02u:%02u.%03us",timems/3600000,(timems/60000)%60,(timems/1000)%60,timems%1000);

    SetDlgItemText(hwndDlg,IDC_LENMS,buf);
  }
  else SetDlgItemText(hwndDlg,IDC_LENMS,"?");
  CheckDlgButton(hwndDlg,IDC_TOC,g_filehdr.toc_size?BST_CHECKED:BST_UNCHECKED);
  if (g_filehdr.toc_size) 
  {
    char buf[128];
    wsprintf(buf,"%d%s",g_filehdr.toc_size,g_filehdr.toc_ex ? " (TOC 2.0)":"");
    SetDlgItemText(hwndDlg,IDC_TOCSIZE,buf);
    SetDlgItemInt(hwndDlg,IDC_SETTOCSIZE,g_filehdr.toc_size,FALSE);
  }
  else 
  {
    SetDlgItemText(hwndDlg,IDC_TOCSIZE,"");
    SetDlgItemText(hwndDlg,IDC_SETTOCSIZE,"4096");
  }
  ...
  if (g_filehdr.file_lenms != ~0 && g_filehdr.file_lenbytes != ~0)
  {
    ...
    char buf[1024];
    wsprintf(buf,"%u %s",bitrate/1000,WASABI_API_LNGSTRING(IDS_KBPS));
    SetDlgItemText(hwndDlg,IDC_AVGBITRATE,buf);
  }
  else SetDlgItemText(hwndDlg,IDC_AVGBITRATE,"?");
...
void updateMetaData(HWND hwndDlg)
{
  ...
  g_filehdr.metadata=0;
  g_filehdr.metadata_len=0;
  int n=SendDlgItemMessage(hwndDlg,IDC_METADATA,LB_GETCOUNT,0,0);
  int x;
  int total_size=0;
  ...
  for (x = 0; x < n; x ++)
  {
    ...
    if (bigstr) l = strlen((char *)bigstr)+2;
    else l=SendDlgItemMessage(hwndDlg,IDC_METADATA,LB_GETTEXTLEN,x,0);
  ...
  for (x = 0; x < n; x ++)
  {
    ...
    {
      int l=SendDlgItemMessage(hwndDlg,IDC_METADATA,LB_GETTEXTLEN,x,0);
    ...
    if (*p)
    {
      *metaout++=*p++;
      ...
      {
		char title[64];
        MessageBox(hwndDlg,WASABI_API_LNGSTRING(IDS_ERROR_WRITING_STRING_TO_TAG),
  ...
  g_filehdr.metadata_len=metaout - (char*)g_filehdr.metadata;
  *metaout=0;
}
  ...
  if (g_filehdr.metadata)
  {
    ...
    for (;;)
    {
      ...
      *tmp=0;
      ...
      {
        LRESULT a=SendDlgItemMessage(hwndDlg,IDC_METADATA,LB_ADDSTRING,0,(LPARAM)name);
        SendDlgItemMessage(hwndDlg,IDC_METADATA,LB_SETITEMDATA,(WPARAM)a,(LPARAM)0);
        free(name);
      }
      else
      {
        ...
        lstrcpyn(buf+strlen(buf),name,128);
        strcpy(buf+strlen(buf),"...");
        LRESULT a=SendDlgItemMessage(hwndDlg,IDC_METADATA,LB_ADDSTRING,0,(LPARAM)buf);
        SendDlgItemMessage(hwndDlg,IDC_METADATA,LB_SETITEMDATA,(WPARAM)a,(LPARAM)name);
      }
    }
  }
}
...
{
  closeNsv(hwndDlg);
  g_hFile = CreateFile(g_lastfile,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ,NULL,OPEN_EXISTING,0,NULL);
  SetDlgItemText(hwndDlg,IDC_FN,g_lastfile);
  ...
  for (;;)
  {
    ...
    if (!ret)
    {
      ...
      wsprintf(infobuf,WASABI_API_LNGSTRING(IDS_VIDEO_X_AUDIO_X),
  ...
  SetDlgItemText(hwndDlg,IDC_BSINFO,infobuf);
...
INT_PTR CALLBACK EditProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam) 
{
  if (uMsg == WM_INITDIALOG)
  {
    if (g_metaitem_edit>=0)
    {
      ...
      int x=SendMessage(ctl,LB_GETTEXTLEN,g_metaitem_edit,0);
      if (x != LB_ERR)
      {
        char *t=(char*)malloc(x+1);
        if (SendMessage(ctl,LB_GETTEXT,g_metaitem_edit,(LPARAM)t) != LB_ERR)
        {
          SetDlgItemText(hwndDlg,IDC_EDIT1,t);
        }
        free(t);
      }
    }
  ...
  if (uMsg == WM_COMMAND)
  {
    ...
    {
      int x=SendDlgItemMessage(hwndDlg,IDC_EDIT1,WM_GETTEXTLENGTH,0,0);
      char *t=(char*)malloc(x+3);
      GetDlgItemText(hwndDlg,IDC_EDIT1,t,x+2);
      if (!strstr(t,"="))
      {
		char title[32];
        free(t);
        MessageBox(hwndDlg,WASABI_API_LNGSTRING(IDS_METADATA_STRING_MUST_CONTAIN_ONE_EQUAL_SIGN),
...
int doTagWrite(HWND hwndDlg, int writeheader)
{
  ...
  else if (writeheader || g_oldtag_size)
  {
    ...
    char tmpfn[1024+8];
    char tmpfn2[1024+8];
    wsprintf(tmpfn,"%s.new",g_lastfile);
    wsprintf(tmpfn2,"%s.old",g_lastfile);

    HANDLE hTempFile=CreateFile(tmpfn,GENERIC_READ|GENERIC_WRITE,0,NULL,CREATE_ALWAYS,0,NULL);
    if (hTempFile == INVALID_HANDLE_VALUE)
    {
	  char title[64];
      MessageBox(hwndDlg,WASABI_API_LNGSTRING(IDS_CANNOT_CREATE_TEMPFILE_CANNOT_UPDATE_TAG),
				 WASABI_API_LNGSTRING_BUF(IDS_NSV_TAG_EDITOR_ERROR,title,64),
				 MB_ICONSTOP|MB_OK);
      return 1;
    }
    ...
    if (hdrlen)
    {
      ...
      if (!WriteFile(hTempFile,hdr,hdrlen,&dw,NULL) || (int)dw != hdrlen)
      {
        ...
        CloseHandle(hTempFile);
        DeleteFile(tmpfn);
        MessageBox(hwndDlg,WASABI_API_LNGSTRING(IDS_ERROR_COPYING_SOURCE),
				   WASABI_API_LNGSTRING_BUF(IDS_NSV_TAG_EDITOR_ERROR,title,64),
				   MB_ICONSTOP|MB_OK);
        return 1;
      }
    ...
    for (;;)
    {
      ...
      bytes+=dwout;
      wsprintf(buf,WASABI_API_LNGSTRING(IDS_COPYING_X_BYTES),bytes);
      SetDlgItemText(hwndDlg,IDC_TAG_LEN,buf);
    }
    if (GetFileSize(g_hFile,NULL)-g_oldtag_size != GetFileSize(hTempFile,NULL)-hdrlen)
    {
      ...
      CloseHandle(hTempFile);
      DeleteFile(tmpfn);
      MessageBox(hwndDlg,WASABI_API_LNGSTRING(IDS_ERROR_SIZE_MISMATCH),
				 WASABI_API_LNGSTRING_BUF(IDS_NSV_TAG_EDITOR_ERROR,title,64),
				 MB_ICONSTOP|MB_OK);
      return 1;
    }
    ...
    if (!MoveFile(g_lastfile,tmpfn2))
    {
	  char title[64];
      DeleteFile(tmpfn);
      restartPlayback();
      MessageBox(hwndDlg,WASABI_API_LNGSTRING(IDS_ERROR_RENAMING_SOURCE),
				 WASABI_API_LNGSTRING_BUF(IDS_NSV_TAG_EDITOR_ERROR,title,64),
				 MB_ICONSTOP|MB_OK);
      return 1;
    }
    if (!MoveFile(tmpfn,g_lastfile))
    {
	  char title[64];
      MoveFile(tmpfn2,g_lastfile);
      DeleteFile(tmpfn);
      restartPlayback();
      MessageBox(hwndDlg,WASABI_API_LNGSTRING(IDS_ERROR_RENAMING_NEW_FILE),
				 WASABI_API_LNGSTRING_BUF(IDS_NSV_TAG_EDITOR_ERROR,title,64),
				 MB_ICONSTOP|MB_OK);
      return 1;
    }
...
{
	unsigned int bytesbeenthrough=0;
	unsigned int nframes=0;
	nsv_Unpacketer unpack;
	nsv_InBS bs;
	nsv_GrowBuf framePos;
	...
	for (;;)
	{
		...
		else
		{
			...
			lastPos=SetFilePointer(g_hFile,0,NULL,SEEK_CUR) - (bs.avail()+7)/8 - g_oldtag_size;
			bs.compact();
			nframes++;
			char buf[128];
			wsprintf(buf,WASABI_API_LNGSTRING(IDS_READING_X_FRAMES),nframes);
			SetDlgItemText(hwndDlg,IDC_TAG_LEN,buf);
		}
	}
	if (unpack.isValid() && nframes)
	{
		...
		if (IsDlgButtonChecked(hwndDlg,IDC_TOC))
		{
			...
			if (d && t)
			{
				...
				unsigned int tf=framePos.getlen()/8;
	...
	{
		char title[64];
		MessageBox(hwndDlg,WASABI_API_LNGSTRING(IDS_NSV_ANALYSIS_FAILED),
	...
	switch (uMsg)
	{
		...
		case WM_TIMER:
			if (wParam == 1)
			{
				...
				{
					SetDlgItemText(hwndDlg,IDC_INFO,"");
	...
	switch (uMsg)
	{
		...
		case WM_COMMAND:
			switch (LOWORD(wParam))
			{
				...
				case IDC_EDIT:
				{
					DWORD res=SendDlgItemMessage(hwndDlg,IDC_METADATA,LB_GETCURSEL,0,0);
					if (res != LB_ERR)
					{
						int ptr=SendDlgItemMessage(hwndDlg,IDC_METADATA,LB_GETITEMDATA,res,0);
						if (ptr)
						{
							char title[32];
							MessageBox(hwndDlg,WASABI_API_LNGSTRING(IDS_METADATA_ITEM_CONTAINS_LARGE_AMOUNT_DATA),
...
{
	g_lastfile=fn;
	if (strstr(fn,"://")) return WASABI_API_DIALOGBOXW(IDD_DIALOG4,hwnd,StreamProc);
	else return WASABI_API_DIALOGBOXW(IDD_DIALOG3,hwnd,MainProc);
}