...
#define WNDMENU_CAPTION "Winamp in_nsv"
#define MODAL_ABOUT
...
char g_streaminfobuf[512];
int g_streaminfobuf_used;
...
int has_opened_outmod;
int m_srate; // seek needs this
...
{
	lstrcpyn(stream_url, url, sizeof(stream_url));
	...
	if (len && *data)
	{
		...
		while ((ld = strstr(data, "='")))
		{
			...
			if (data)
			{
				data[0] = 0;
				data += 2;
				if (!lstrcmpi(n, "StreamTitle"))
				{
					lstrcpyn(last_title_sent, ld, sizeof(last_title_sent));
					last_title_sent[sizeof(last_title_sent) - 1] = 0;
					PostMessage(mod.hMainWindow, WM_USER, 0, 243);
				}
				else if (!lstrcmpi(n, "StreamUrl"))
				{
					process_url(ld);
				}
			}
...
int isourfile(const char *fn)
{
	...
	return !strncmp( fn, "unsv://", 7 );
}

NSVDecoder *m_decoder;
...
int play(const char *fn)
{
	...
	if (!decoders_initted)
	{
		...
		char buf[MAX_PATH];
		char *p = buf;
		GetModuleFileName(mod.hDllInstance, buf, sizeof(buf));
		while (*p) p++;
		while (p > buf && *p != '\\') p--;
		*p = 0;
		Decoders_Init(buf);
	...
	lstrcpyn(lastfn, fn, sizeof(lastfn));
	...
	SendMessage(mod.hMainWindow, WM_WA_IPC, 0, IPC_FLUSHAUDITS);
...
int getoutputtime()
{
	...
	{
		LeaveCriticalSection(&g_decoder_cs);
		return m_decoder->getpos() + config_vidoffs;
	}
	LeaveCriticalSection(&g_decoder_cs);
	return 0;
}
...
void getfileinfo(const char *filename, char *title, int *length_in_ms)
{
	if (!filename || !*filename)  // currently playing file
	{
		...
		if (title) // get non-path portion.of filename
		{
			...
			if (error_string[0])
			{
				StringCchPrintf(title, GETFILEINFO_TITLE_LENGTH, "[%s] ", error_string);
			}
			if (!error_string[0] && last_title_sent[0])
			{
				...
				lstrcpyn(title + strlen(title), p, 400);
		}
		LeaveCriticalSection(&g_decoder_cs);
	}
...
DWORD WINAPI DecodeThread(LPVOID b)
{
	int last_bpos = -1;
	int firstsynch = 0;
	unsigned int next_status_time = 0;
	...
	while (!killDecodeThread)
	{
		...
		if (r < 0)
		{
			...
			{
				EnterCriticalSection(&g_decoder_cs);
				lstrcpyn(error_string, m_decoder->get_error(), sizeof(error_string));
				LeaveCriticalSection(&g_decoder_cs);
				PostMessage(mod.hMainWindow, WM_USER, 0, 243);
				Sleep(200);
			}
		...
		else if (!r)
		{
			...
			if (bpos > 255)
			{
				...
				if (last_bpos >= 0)
				{
					...
					if (csa && obuf)
					{
						...
						mod.SAAdd(tempdata, ++obuf, (csa == 3) ? 0x80000003 : csa);
					}
				}
			}
			else
			{
				...
				if (bpos != last_bpos)
				{
					last_bpos = bpos;
					EnterCriticalSection(&g_decoder_cs);
					StringCchPrintf(error_string, 128, WASABI_API_LNGSTRING(IDS_BUFFER_X), (bpos*100) / 256);
					LeaveCriticalSection(&g_decoder_cs);
					int csa = mod.SAGetMode();
			...
			if (GetTickCount() > next_status_time || GetTickCount() < next_status_time - 5000)
			{
				char statusbuf[1024];
				EnterCriticalSection(&g_decoder_cs);
				g_streaminfobuf[0] = 0;
				...
				if (g_streaminfobuf_used)
				{
					...
					p = m_decoder->getServerHeader("server");
				...
				{ // codecs
					...
					if (!*p) StringCchCopyExA(p, size, "?, ", &p, &size, 0);
				...
				next_status_time = GetTickCount() + 2500;
			}
		}
		else
		{
...
extern "C"
{
	...
	bool isFileChanged(const wchar_t* file)
	{
		...
		HANDLE hFind;
		hFind = FindFirstFileW(file, &FindFileData);
	...
	__declspec( dllexport ) int winampGetExtendedFileInfoW(const wchar_t *fn, const char *data, wchar_t *dest, int destlen)
	{
		...
		if (wcscmp(fn, lastextfn) || isFileChanged(fn) || HasFileTimeChanged(fn))
		{
			free(hdr.metadata);
			memset(&hdr, 0, sizeof(hdr));
			valid = 0;

			lstrcpynW(lastextfn, fn, sizeof(lastextfn));
				...
				while (!rdr->iseof())
				{
					...
					if (l <= 0)
					{
						...
						if (!l)
						{
							...
							WIN32_FIND_DATAW FindFileData;
							HANDLE hFind;
							hFind = FindFirstFileW(fn, &FindFileData);
							if (hFind == INVALID_HANDLE_VALUE)
							{
		...
		else if (hdr.metadata)
		{
			...
			if (t) lstrcpynW(dest, AutoWide(t), destlen);
		}
