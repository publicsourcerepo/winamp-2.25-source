...
int ReadWPADFile(LPSTR lpWPADLocation, LPSTR lpIPAddress, int *pnPort)
{
	...
	if(hr == S_OK) {
		if(!(ret = pInternetInitializeAutoProxyDll(0, szTempFile, NULL, &HelperFunctions, NULL))) {
...
{
	unsigned int	b;
	char			szOne[MAX_PATH], szTwo[MAX_PATH];
