...
{
	char zz[128];
	wsprintf(zz,WASABI_API_LNGSTRING(IDS_WAVE_U_MS),SendDlgItemMessage(wnd,IDC_PREBUFFER_1,TBM_GETPOS,0,0));
...
{
	char zz[128];
	wsprintf(zz,WASABI_API_LNGSTRING(IDS_WAVE_U_MS),SendDlgItemMessage(wnd,IDC_PREBUFFER_2,TBM_GETPOS,0,0));
...
{
	char zz[128];
	wsprintf(zz,WASABI_API_LNGSTRING(IDS_WAVE_U_MS),get_buffer(wnd));
	...
	case WM_TIMER:
		{
			...
			SetDlgItemText(wnd,IDC_STATE,z ? poo : WASABI_API_LNGSTRING(IDS_NOT_ACTIVE));
...
static int _do_var(unsigned int v,char* n,bool s)
{
	if (s)
	{
		char tmp[2*sizeof(unsigned int) + 1]; // max 32 bit unsigned int == 4 294 967 296 == 10 digits, 11 if signed
		wsprintf(tmp,"%u",v);
		WritePrivateProfileString(dllfile,n,tmp,inifile);
		return v;
	}
	else
	{
		char tmp[64],tmp_s[2*sizeof(unsigned int) + 1];
		wsprintf(tmp_s,"%u",v);
		GetPrivateProfileString(dllfile,n,tmp_s,tmp,64,inifile);
		return atoui(tmp);
	}
...
void do_cfg(bool s)
{
	...
	if (!dllfile)
	{
		GetModuleFileName(mod.hDllInstance,_dllfile,sizeof(_dllfile));
