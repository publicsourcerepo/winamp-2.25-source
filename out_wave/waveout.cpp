...
int WaveOut::open(WaveOutConfig * cfg)
{
	...
	MMRESULT mr=waveOutOpen(&hWo,cfg->dev-1,&wfx,(DWORD_PTR)hEvent,0,CALLBACK_EVENT);
	if (mr)
	{
		if (mr==32)
		{
			...
			mr=waveOutOpen(&hWo,cfg->dev-1,(WAVEFORMATEX*)&wfxe,(DWORD_PTR)hEvent,0,CALLBACK_EVENT);
		}
		if (mr)
		{
			...
			{
				WASABI_API_LNGSTRING_BUF(IDS_UNKNOWN_MMSYSTEM_ERROR,poo,MAXERRORLENGTH);
			}
			char * e2=0, e2Buf[1024];
			switch(mr)
			{
			case 32:
				wsprintf(_fe,WASABI_API_LNGSTRING(IDS_UNSUPPORTED_PCM_FORMAT),fmt_sr,fmt_bps,fmt_nch);
			...
			if (e2)
			{
				wsprintf(_fe,WASABI_API_LNGSTRING(IDS_ERROR_CODE_WINDOWS_ERROR_MESSAGE),e2,mr,e);
			}
			else
			{
				wsprintf(_fe,WASABI_API_LNGSTRING(IDS_ERROR_CODE),e,mr);
			}
...
bool WaveOut::PrintState(char * z)
{
	...
	{
		rv=1;
		wsprintf(z,WASABI_API_LNGSTRING(IDS_DATA_FORMAT),fmt_sr,fmt_bps,fmt_nch);
		while(*z) z++;
		wsprintf(z,WASABI_API_LNGSTRING(IDS_BUFFER_STATUS),buf_size,n_playing);
		while(*z) z++;
		wsprintf(z,WASABI_API_LNGSTRING(IDS_LATENCY),GetLatency());
	//	while(*z) z++;
...
void WaveOutConfig::SetError(const char * x)
{
	error=(char*)LocalAlloc(LMEM_FIXED,lstrlen(x+1));
	lstrcpy(error,x);
}
