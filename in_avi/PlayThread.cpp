...
public:
	...
	typedef ValueMap<uint32_t, uint32_t> Stats;
	Stats audio_types;
	Stats video_fourccs;
};
...
{
	unsigned int bits_per_sample = AGAVE_API_CONFIG->GetUnsigned(playbackConfigGroupGUID, L"bits", 16);
	...
	while (sf = WASABI_API_SVC->service_enumService(WaSvc::AVIDECODER, n++))
	{
...
{
	size_t output_len = sizeof(audio_output);
...
static bool CheckDSHOW()
{
	if (!checked_in_dshow)
	{
		const char *pluginsDir = (const char *)SendMessage(plugin.hMainWindow, WM_WA_IPC, 0, IPC_GETPLUGINDIRECTORY);
		char in_dshow_path[MAX_PATH];
		PathCombineA(in_dshow_path, pluginsDir, "in_dshow.dll");
		in_dshow = LoadLibraryA(in_dshow_path);
...
static void CALLBACK DSHOWAPC(ULONG_PTR param)
{
	...
	if (in_dshow)
	{
		...
		moduleGetter = (MODULEGETTER)GetProcAddress(in_dshow, "winampGetInModule2");
		if (moduleGetter)
...
{
	wchar_t audio_name[128], audio_properties[256];
	wchar_t video_name[128], video_properties[256];
	wchar_t video_info[512];
...
void PlayLoop(nsavi::avi_reader *reader, bool multiple_readers)
{
	...
	if (demuxer.GetRIFFType(&riff_type) == nsavi::READ_OK)
	{
		...
		if ((audio_no_decoder || video_no_decoder) && CheckDSHOW())
		{
			...
			if (mainThread)
			{
				...
				wchar_t *fn = (wchar_t *)malloc(1024*sizeof(wchar_t *));
	...
	{
		int total_bitrate = 8ULL * content_length / (uint64_t)g_duration;
