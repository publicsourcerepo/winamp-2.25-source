...
void LoadRG()
{
	...
	char path[MAX_PATH];
	GetModuleFileName(plugin.hDllInstance, path, MAX_PATH);
	PathRemoveFileSpec(path);
	PathAppend(path, "ReplayGainAnalysis.dll");
	rgLib = LoadLibrary(path);
	if (rgLib)
	{
	...
	case 8:
		{
			...
			for (size_t x = 0; x != numSamples; x ++)
			{
				...
				if (numChannels == 2)
				{
					...
					right[x] = left[x];		
				peak = fastmax(peak, fabs(left[x]));
				peak = fastmax(peak, fabs(right[x]));
