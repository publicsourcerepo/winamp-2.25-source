...
HRESULT OmService::CreateInstance(UINT nId, LPCWSTR pszName, OmService **instance)
{
	...
	*instance = NULL;
	...
	wchar_t url[256] = {0};
	if (nId == SERVICE_SOURCES)
	{
		Config_GetWebURL(url, 256);
		wcsncat(url, L"sources", 256);
	}
	else if (nId == SERVICE_SIGN_IN)
	{
		Config_GetWebURL(url, 256);
		wcsncat(url, L"account", 256);
	}
...
{
	return StringCchCopy(pszBuffer, cchBufferMax, name);
}
...
HRESULT OmService::GetExternal(IDispatch **ppDispatch)
{
	...
	*ppDispatch = NULL;
	...
	{
		*ppDispatch = (IDispatch*)SendMessage(plugin.hwndWinampParent, WM_WA_IPC, (WPARAM)szBuffer, IPC_JSAPI2_GET_DISPATCH_OBJECT);
