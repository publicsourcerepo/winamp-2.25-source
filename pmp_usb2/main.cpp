...
static Vector<HANDLE> loadingThreads;
...
static void blacklistSave() {
	...
	StringCchPrintfA(buf, 64, "%u",blacklist.size());
	WritePrivateProfileStringA("pmp_usb","blacklistnum",buf,winampini);
	for(size_t i=0; i<blacklist.size(); i++) 
	{
		StringCchPrintfA(buf, 64, "blacklist-%u",i);
		WritePrivateProfileStringA("pmp_usb",buf,AutoChar((const wchar_t*)blacklist.at(i), CP_UTF8),winampini);
	}
}
...
void connectDrive(wchar_t drive, bool checkSize=true, bool checkBlacklist=true) 
{
	...
	for (Vector<USBDevice*>::const_iterator e = devices.begin(); e != devices.end(); e++) 
	{
		if ((*e)->drive == drive) return;
	}
	...
	wchar_t cacheFile[FIELD_LENGTH];
	cacheFile[0] = L'\0';
	...
	if (!PathFileExists(cacheFile))
	{
		...
		wchar_t drvname[100]=L"", titleStr[128]=L"";
		DWORD serial=0;
