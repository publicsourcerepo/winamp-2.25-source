...
extern Vector<USBDevice*> devices;
...
{
	return usbPlaylists.at(playlistnumber)->songs.size();
}
...
void USBDevice::setPlaylistName(int playlistnumber, const wchar_t *buf)
{
	...
	else 
	{
		...
		while(*newFilename == L'.') newFilename++;
...
void USBDevice::playlistSwapItems(int playlistnumber, int posA, int posB)
{
	...
	pl->songs.insert(posA,b);
	pl->songs.eraseAt(posA+1);
	pl->songs.insert(posB,a);
	pl->songs.eraseAt(posB+1);
	pl->dirty = true;
}
...
void USBDevice::deletePlaylist(int playlistnumber)
{
	...
	usbPlaylists.eraseAt(playlistnumber);
	delete pl;
}
...
{
	UsbSong *song = (UsbSong *) songid;
	wchar_t playCount[FIELD_LENGTH];
	playCount[0] = L'\0';
...
// called from ml_pmp
bool USBDevice::hasArt(songid_t songid)
{
// TODO: implement
	return false;
}
...
{
	for (Vector<USBPlaylist*>::const_iterator e = usbPlaylists.begin(); e != usbPlaylists.end(); e++) 
	{
...
USBDevice::USBDevice(wchar_t drive, pmpDeviceLoading * load): transcoder(NULL) {
	...
	GetPrivateProfileString(L"pmp_usb",L"supportedFormats",L"mp3;wav;wma",supportedFormats,sizeof(supportedFormats)/sizeof(wchar_t),iniFile);
...
void USBDevice::fillMetaData(UsbSong *t)
{
	if(!t->filled) 
	{
		...
		if (getFileInfoW(t->filename, L"playcount",tmp,sizeof(tmp)/sizeof(wchar_t))) 
		{
			if(tmp[0]) 
			{
...
UsbSong *USBDevice::findSongInMasterPlaylist(const wchar_t *songfn) {
	...
	for (Vector<UsbSong*>::const_iterator e = mpl->songs.begin(); e != mpl->songs.end(); e++) {
