...
{   
    ...
    SBYTE ppsep;            // Pitch-pan Separation
...
{   
    ...
    ITNOTE       last[64];
    int          old_effect;      // if set, use S3M old-effects stuffs
} IT_HANDLE;
...
// =====================================================================================
static BOOL IT_Test(MMSTREAM *mmfile)
// =====================================================================================
{
    UBYTE id[4];
...
// =====================================================================================
static ML_HANDLE *IT_Init(void)
// =====================================================================================
{
    IT_HANDLE *handle;

    _MM_ALLOCATE(handle, IT_HANDLE, NULL);
    return handle;
}


// =====================================================================================
static void IT_Cleanup(IT_HANDLE *handle)
// =====================================================================================
{
    if(!handle) return;
...
// =====================================================================================
static BOOL IT_GetNumChannels(MMSTREAM *mmfile, UBYTE *mask, SBYTE *remap, UWORD patrows)
// =====================================================================================
...
// Returns 0 on error
{
    ...
    do
    {
        ...
        {
            _mmlog("load_it> unexpected end of file (in patterns)");
            return 0;
        }
...
// =====================================================================================
static BOOL IT_ReadPattern(MMSTREAM *mmfile, IT_HANDLE *h, UWORD patrows, int gxx_memory)
// =====================================================================================
{
    int      row=0,flag,ch;
    ITNOTE  *l, dummy;
        ...
        else
        {
            ...
            {   UNITRK_EFFECT  eff;
                ...
                {   if(note==253)
					...
					{
						if (((volpan>=193) && (volpan<=202)))
						{
							utrk_write_note_ex(h->ut, 1);
						}
						utrk_write_note(h->ut, note+1);
					}
                }

				S3MIT_ProcessCmd(h->ut, h->poslookup, cmd, inf, h->old_effect, gxx_memory,1);

                if (ins && ins<100) utrk_write_inst(h->ut, ins);
                ...
                {   eff.param.s  = 0 - (volpan-105)*32;
                    ...
                    {   eff.effect    = UNI_PITCHSLIDE;
                        eff.framedly += 1;
                        utrk_write_local(h->ut, &eff, STMEM_PITCHSLIDE2);
                    } else
                        utrk_memory_local(h->ut, &eff, STMEM_PITCHSLIDE2, 1);
...
/*                    if(eff.param.u)
					...
					if(eff.param.u)
                        ...
                        utrk_memory_local(h->ut, &eff, STMEM_PITCHSLIDE2, 0);
...
// =====================================================================================
static BOOL IT_Load(IT_HANDLE *h, UNIMOD *of, MMSTREAM *mmfile)
// =====================================================================================
{
    uint        t;
    int         u,lp;
    ...
    if(_mm_feof(mmfile))
    {
        _mmlog("load_it> unexpected end of file (in header)");
        return 0;
    }
    ...
    of->modtype[15] = (h->mh.cmwt >> 8) + 0x30;
    of->modtype[17] = ((h->mh.cmwt >> 4) & 0xf) + 0x30;
    of->modtype[18] = ((h->mh.cmwt) & 0xf) + 0x30;
    of->songname    = DupStrTrim(of->allochandle, h->mh.songname, 26);
    of->reppos      = 0;
    ...
    of->flags      |= UF_LINEAR_PITENV|UF_OVR_PERIOD;

    h->old_effect = 0;
    if (h->mh.flags & 8)
        of->flags |= UF_LINEAR;
//CUT	else of->flags|=UF_AMIGA;
    if (h->mh.cwt>=0x106 && h->mh.flags&16)
        h->old_effect |= 1;
    if (h->mh.flags & 32)
        h->old_effect |= 4;
    ...
    if((h->paraptr=(ULONG*)MikMod_malloc(h->allochandle, (h->mh.insnum+h->mh.smpnum+of->numpat)*sizeof(ULONG))) == NULL)
        return 0;
    ...
    if (h->mh.special & 1)
    {
        ...
        of->comment = ReadComment(of->allochandle, h->mh.msglength, 76, mmfile);
    }
    ...
    for(t=0; t<h->mh.smpnum; t++)
    {
        ...
        if (_mm_feof(mmfile))
	    {
            _mmlog("load_it> unexpected end of file (in samples)");
            return 0;
        }
    ...
    if (of->numins)
    {
        ...
        for(t=0; t<h->mh.insnum; t++)
        {
            ...
            if (_mm_feof(mmfile))
		    {
                _mmlog("load_it> unexpected end of file (in instruments)");
                return 0;
            }
            ...
            if (h->mh.cmwt < 0x200)                         // IT 1.xx header
            {
            ...
            {   ih.nna       = _mm_read_UBYTE(mmfile);
                ...
                ih.fadeout   = _mm_read_I_UWORD(mmfile);
                ih.ppsep     = _mm_read_SBYTE(mmfile);
            ...
            if (h->mh.cmwt < 0x200)                         // IT 1.xx volume envelope
            {
            ...
            else                                            // IT 2.xx vol & chanpan & pitch envs
            {
                ih.volflg    = _mm_read_UBYTE(mmfile);
            ...
            else
            {
                ...
                if (ih.ppsep)
                {
                    d->pitpansep    = ih.ppsep * IT_PANMUL;
...
*/
    ...
    {   
        ...
        {   UWORD packlen;
            ...
            if(packlen < 2048)    // check for bad pattern data...
            {
                ...
                if (!IT_GetNumChannels(mmfile, h->mask, h->remap, packlen))
                    return 0;
            }
        }
    }
    ...
    utrk_local_memflag(h->ut, STMEM_VOLSLIDE, UMF_FRAMEDLY);
...

    {
        ...
        {   // E/F/G share memory, so mark the memory as completely volatile.
            ...
            utrk_local_memflag(h->ut, STMEM_PITCHSLIDE, UMF_EFFECT|UMF_FRAMEDLY);   // Std. Effects column.
...
// =====================================================================================
static CHAR *IT_LoadTitle(MMSTREAM *mmfile)
// =====================================================================================
{
   CHAR s[26];
