...
{   UBYTE what[96];         // (byte) Sample number for all notes
    ENVPT volenv[12];       // (byte) Points for volume envelope
    ENVPT panenv[12];       // (byte) Points for panning envelope
...
// =====================================================================================
static BOOL XM_Test(MMSTREAM *mmfile)
// =====================================================================================
{
    UBYTE id[17];
...
// =====================================================================================
static void *XM_Init(void)
// =====================================================================================
{
    return MikMod_calloc(NULL, 1,sizeof(XMHEADER));
}


// =====================================================================================
static void XM_Cleanup(void *handle)
// =====================================================================================
{
...
// =====================================================================================
static void XM_ReadNote(UTRK_WRITER *ut, XMNOTE *ninf, MMSTREAM *mmfile)
// =====================================================================================
{
    uint          cmp;
    uint          hi, lo;
    ...
    else utrk_write_note(ut, ninf->note);
    ...
    {   
        ...
        case 0xf:                       // Speed / BPM
            if (ninf->dat)
            {
                effdat.param.u  = ninf->dat;
                effdat.framedly = UFD_RUNONCE;
                effdat.effect   = ninf->dat >= 0x20 ? UNI_GLOB_TEMPO : UNI_GLOB_SPEED;
        ...
        case 'R'-55:                    // R - multi retrig note
            ...
            {   
                ...
                // speed only
                    ...
                    utrk_memory_local_ex(ut, &effdat, FT2MEM_RETRIG, UMF_LOWORD, 0);
                // slide only
                else if (hi)
                    utrk_memory_local_ex(ut, &effdat, FT2MEM_RETRIG, UMF_HIWORD, 0);
            }
            else utrk_memory_local_ex(ut, &effdat, FT2MEM_RETRIG, 0, 0);
        break;
...
static void SetMemDefaults(UNIMOD *of)
{
    ...
    utrk_memdef_add(of, &eff, FT2MEM_RETRIG);
}


// =====================================================================================
static BOOL XM_Load(XMHEADER *mh, UNIMOD *of, MMSTREAM *mmfile)
// =====================================================================================
{
    INSTRUMENT  *d;
    UNISAMPLE   *q;
    ...
    if((mh->version<0x102)||(mh->version>0x104)) {
		  _mmlogd("load_xm > Failure: Not a module (1)");
		  return 0;
	  } 
    ...
    of->numins    = mh->numins;
    of->flags    |= UF_INST|UF_XMPERIODS|UF_XM_INST;
    ...
    if((wh = (XMWAVHEADER *)MikMod_calloc(NULL, 2048,sizeof(XMWAVHEADER))) == NULL) return 0;
    d = of->instruments;
    s = wh;
    ...
    {   XMINSTHEADER ih;
        ...
        memset(d->samplenumber,255,120*sizeof(d->samplenumber[0]));
        ...
        if(ih.size > 29)
        {
            ...
            if((ih.numsmp > 0) && (ih.numsmp<=XMNOTECNT) )
            {   uint          u,p;
                XMPATCHHEADER pth;
				...
				_mm_fseek(mmfile,headend,SEEK_SET);
                ...
                if (pth.volflg&4 && pth.volbeg!=pth.volend)
                    d->volflg |= EF_LOOP;
                ...
                if (pth.panflg&4 && pth.panbeg!=pth.panend)
                    d->panflg |= EF_LOOP;
                ...
                {   
                    if (pth.panenv[p].val != 32) inuse = 1;
                    d->panenv[p].val = PAN_LEFT + (pth.panenv[p].val<<2);
                ...
                for(u=0; u<ih.numsmp; u++,s++)
                {
                	...
                	{   _mmlogd1("load_xm > Failure: Unexpected end of file loading sample header %d.",u);
                       // return 0;
					//null lenght sample at EOF, that's all. blah.
						//ih.numsmp=u;
						break;
                    }
                }
            ...
            {   uint  u;
                for(u=headend-_mm_ftell(mmfile); u; u--) 
                {
                  if(_mm_feof(mmfile))
                  {
                    _mmlogd1("load_xm > Failure: Unexpected end of file loading sample header %d.",u);
...
// =====================================================================================
static CHAR *XM_LoadTitle(MMSTREAM *mmfile)
// =====================================================================================
{
    CHAR s[20];
