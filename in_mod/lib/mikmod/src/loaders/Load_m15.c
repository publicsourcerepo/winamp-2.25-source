...
{   CHAR       songname[20];         // the songname..
    ...
    SLONG      smpsize;              // size of all samples
} MODULEHEADER;


/*************************************************************************
*************************************************************************/
static const CHAR nulls[3]    = { 0, 0, 0 };

// =====================================================================================
static void *M15_Init(void)
// =====================================================================================
{
...
// =====================================================================================
static void M15_Cleanup(void *handle)
// =====================================================================================
{
...
// =====================================================================================
static BOOL M15_Test(MMSTREAM *mmfile)
// =====================================================================================
{
    int          t;
    MODULEHEADER mh;
...
// =====================================================================================
static BOOL M15_Load(MODULEHEADER *mh, UNIMOD *of, MMSTREAM *mmfile)
// =====================================================================================
{
    uint         t;
    UNISAMPLE   *q;
	...
	{
        _mmlog("load_m15> unexpected end of file (in header)");
        return 0;
    }
    ...
    // heruistic check
    {
        ...
        for (; t<128; t++)
            if (mh->positions[t] > num)
                num = mh->positions[t];

        if (num > of->numpat)                               // patterns in unused part
        {                                                   // (check - were they saved or not)
            ULONG pos = _mm_ftell(mmfile);
            // check, whether samples will fit in the file
            if (pos + num*64*4 + mh->smpsize <= of->filesize)
                of->numpat = num;
        }
    }
    ...
    if((mh->patdat = MikMod_calloc(NULL, of->numpat*64*4, sizeof(MODNOTE))) == NULL) return 0;
    ...
    {   
        ...
        // Flags?  FLAGS?  Flags!!
        q->format = SF_SIGNED;
        if(s->replen>1 && (q->loopstart < q->length))
            q->flags |= SL_LOOP;
    	...
    	{
            _mmlog("load_m15> invalid value in sample header");
            return 0;
        }
...
// =====================================================================================
static CHAR *M15_LoadTitle(MMSTREAM *mmfile)
// =====================================================================================
{
   CHAR s[20];
