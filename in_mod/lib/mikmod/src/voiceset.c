...
// for the mixing mode are deallocated.
{
    ...
    {   if(md->hardvoices <= num)
        {   md->device.SetHardVoices(md, md->hardvoices = 0);
            MikMod_free(md->allochandle, md->vmh);
        } else
        {   md->hardvoices -= num;
    ...
    {   if(md->softvoices <= num)
        {   md->device.SetSoftVoices(md, md->softvoices = 0);
            MikMod_free(md->allochandle, md->vms);
        } else
        {   md->softvoices -= num;
...
// Returns a handle to the new voice set.   returns -1 on error!
{
    ...
    vs = MikMod_calloc(md->allochandle, 1, sizeof(MD_VOICESET));
...
// =====================================================================================
{
    if(vs)
    {
        ...
        {   MD_VOICESET **cruise = (vs->flags & MDVS_STATIC) ? md->vmh : md->vms;
            ...
            MikMod_free(md->allochandle, vs->vdesc);
        }
...
// =====================================================================================
{
    ...
    vstream = (_VS_STREAM *)MikMod_malloc(sizeof(_VS_STREAM));
...
// =====================================================================================
{
    ...
    stream = (_VS_STREAM *)MikMod_malloc(sizeof(_VS_STREAM);
...
{    
    ...
    _mm_boundscheck(vol, 0, VOLUME_FULL);
...
// =====================================================================================
{
	...
	vs->md->device.VoiceSetSurround(&vs->md->device,vs->vdesc[voice].voice,(pan_lr == PAN_SURROUND) ? MD_ENABLE_SURROUND : 0);
...
// (<0 or >length), or handle is illegal, the loop will not be played.
{
    ...
    //
    // make some asserts on sample's parameters
    // these should be checked/corrected by the loader!
    //

    if (flags & SL_LOOP)
    {
        assert(repend <= length);
        assert(reppos < repend);
    }
    if (flags & SL_SUSTAIN_LOOP)
    {
        assert(susend <= length);
        assert(suspos < susend);
    }
