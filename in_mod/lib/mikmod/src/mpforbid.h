...
{
    if(ps->state.pos_played && (unsigned)ps->state.sngpos<ps->mf->numpos) ps->state.pos_played[ps->state.sngpos][(ps->state.patpos/32)] |= (1<<(ps->state.patpos&31));
}
...
// =====================================================================================
{
    ...
    {
        ps->state.sngpos        = 0;
        ps->state.prev_sngpos   = pf->numpos;
        ps->state.patpos        = 0;
        ...
        memcpy(ps->state.panning, pf->panning, sizeof(ps->state.panning));
        memcpy(ps->state.chanvol, pf->chanvol, sizeof(ps->state.chanvol));
    }
