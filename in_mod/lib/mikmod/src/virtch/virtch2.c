...
static ULONG samples2bytes(ULONG samples)
{
	...
	else if (vc_mode & DMODE_24BITS) samples*=3;
...
static ULONG bytes2samples(ULONG bytes)
{
	...
	else if (vc_mode & DMODE_24BITS) bytes/=3;
...
void VC2_WriteSamples(SBYTE *buf, long todo)
{
    ...
    {   if(TICKLEFT==0)
        ...
        {   portion = MIN(left, samplesthatfit);
            ...
            else if (vc_mode & DMODE_24BITS)
				Mix32to24((SBYTE *) buffer, VC2_TICKBUF, portion);
			else			
...

{
    ...
    if(vc_mode & (DMODE_16BITS|DMODE_24BITS)
        memset(buf,0,todo);
    else
