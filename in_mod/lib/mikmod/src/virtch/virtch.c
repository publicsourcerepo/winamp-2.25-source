...
// =====================================================================================
void VC_RegisterMixerHack(VIRTCH *vc, VMIXER *mixer)
// add mixers to override existing mixers
// =====================================================================================
{
	if (vc)
	{
		mixer->next=vc->mixerlist;
		vc->mixerlist=mixer;
	}
}
...
static void Mix32To24(VIRTCH *vc, UBYTE *dste, SLONG *srce, SLONG count)
{
	for (; count; count--)
	{
		SLONG x1 = *srce++ >> (BITSHIFT-8);

#ifdef BOUNDS_CHECKING
		x1 = (x1 > 0x7FFFFF) ? 0x7FFFFF : (x1 < -0x800000) ? -0x800000 : x1;
#endif
...
// =====================================================================================
{
	...
	else if (vc->mode&DMODE_24BITS) samples*=3;
...
// =====================================================================================
{
	...
	else if (vc->mode & DMODE_24BITS) bytes/=3;
	bytes >>= (vc->channels / 2);
	return bytes;
}

#ifdef _DEBUG
static unsigned samples_done;
#endif
...
// =====================================================================================
{
	...
	if (sinf->format & SF_16BITS)
	{
		...
		else if (vnf->flags & SL_LOOP)
		{
			...
			if (vnf->flags & SL_BIDI)
			{
				...
				for (i=0; i<8; i++)
				{
					sinf->data[(vnf->repend*2) + (i*2)] = sinf->data[(vnf->repend*2) - (i*2) - 2];
					sinf->data[(vnf->repend*2) + (i*2) + 1] = sinf->data[(vnf->repend*2) - (i*2) - 1];
				}
			}
			else
	...
	while (todo > 0)
	{
		...
		if (vnf->flags & SL_REVERSE)
		{
			...
			if (vnf->flags & SL_SUSTAIN_LOOP)
			{
				register const int idxspos = vnf->suspos;

				end = idxspos;
				if (*current_hi <= idxspos)
				{
					...
					if (vnf->flags & SL_SUSTAIN_BIDI)
					{
						...
						*current_hi     = idxspos + (idxspos - *current_hi);
						end             = vnf->susend;
					}
					else
					{
						...
						*current_hi = vnf->susend - (idxspos - *current_hi);
					}
				}
			}
			else if (vnf->flags & SL_LOOP)
			{
				register const int idxlpos = vnf->reppos;

				end = idxlpos;
				if (*current_hi <= idxlpos)
				{
					// the sample is looping, and it has
					// reached the loopstart index
					...
					if (vnf->flags & SL_BIDI)
					{
						...
						vnf->flags     &= ~SL_REVERSE;
						vnf->increment  = -vnf->increment;
						*current_hi     = idxlpos + (idxlpos - *current_hi);
						end             = vnf->repend;
					}
					else
						// normal backwards looping, so set the
						...
						*current_hi = vnf->repend - (idxlpos - *current_hi);
				}
			}
			else
			{
		...
		else
		{
			...
			if (vnf->flags & SL_SUSTAIN_LOOP)
			{
				register const int idxsend = vnf->susend;
			...
			{
				register const int idxlend = vnf->repend;
		...
		{
			INT64S _done = (((INT64S)end << FRACBITS) - vnf->current) / vnf->increment + 1;

			// Instead of hoping that this will never happen I make
			// bounds checking here. This might slow down things a
			// bit, but will prevent angry lock-ups with freaky songs,
			// that use wierd frequencies or do other mess with samples.

			if (_done <= 0) break;
			if (_done > (1ul<<30)) _done = (1ul<<30);
			done = _done;
		}
...
#pragma warning(disable:4020)
			...
			{
				if (vnf->panflg & VC_SURROUND)
					sinf->mixer->NoClickSurround(sinf->data,ptr,vnf->current,vnf->increment,done,&vnf->resfilter, &vnf->volinfo);
				else
					sinf->mixer->NoClick(sinf->data,ptr,vnf->current,vnf->increment,done,&vnf->resfilter, &vnf->volinfo);
			}
		...
		else if (vnf->vol.front.left || vnf->vol.front.right)
		{
			...
			{
				if (vnf->panflg & VC_SURROUND)
					sinf->mixer->MixSurround(sinf->data,ptr,vnf->current,vnf->increment,done,&vnf->resfilter, &vnf->volinfo);
				else
					sinf->mixer->Mix(sinf->data,ptr,vnf->current,vnf->increment,done,&vnf->resfilter, &vnf->volinfo);
			}
		}
#pragma warning(default:4020)
	...
	if (vc=md->device.vc)
	{
		...
		while (todo)
		{
			...
			while (left)
			{
				...
				else if (vc->mode & DMODE_24BITS)
					Mix32To24(vc,(SBYTE*)buffer,vc->TICKBUF,count);
				else
...
//  mode how the buffer is filled)
{
	...
	if (md->device.vc)
	{
		if (md->device.vc->mode & (DMODE_16BITS|DMODE_24BITS))
			memset(buf,0,todo);
		else
...
// =====================================================================================
{
	...
	_MM_ALLOCATE(vc, VIRTCH, NULL);

	vc->volume.front.left  =
	  vc->volume.front.right =
	    vc->volume.rear.left   =
	      vc->volume.rear.right  = VOLUME_FULL;

	if ((vc->sample = (VSAMPLE *)MikMod_calloc(vc->allochandle, vc->samplehandles=64, sizeof(VSAMPLE))) == NULL) return 0;

	if (!vc->TICKBUF)
		if ((vc->TICKBUF=(SLONG *)MikMod_malloc(vc->allochandle, (TICKLSIZE+32) * sizeof(SLONG))) == NULL) return 0;
...
// =====================================================================================
{
	...
	if (!voices)
	{
		...
		MikMod_free(vc->allochandle, vc->vinf);
		MikMod_free(vc->allochandle, vc->vold);
		return 0;
	}
...
// =====================================================================================
{
	...
	memset(md->vc->vinf[voice].mixdat,0,32);
}
...
// =====================================================================================
{
	...
	{
		memset(vc->vinf[voice].mixdat,0,32);
...
//TEST    md->vc->vinf[voice].cutoff    = cutoff;
//TEST    md->vc->vinf[voice].resonance = resonance;
...

{
	...
	*flags |= SF_SIGNED;
	...
	{
		SWORD  *samp16;
		size_t sample_malloc_size;

		if (SizeTAdd(length, 12, &sample_malloc_size) != S_OK || SizeTMult(sample_malloc_size, 2, &sample_malloc_size) != S_OK)
		{
			return -1;
		}
		...
		*flags |= SF_16BITS;
		...
		if ((vc->sample[handle].data = MikMod_malloc(vc->allochandle, sample_malloc_size))==NULL) return -1;
	...
	else
	{
		...
		if (SizeTAdd(length, 12, &sample_malloc_size) != S_OK)
		{
			return -1;
		}
...
// =====================================================================================
{
	if (handle < 8) // < vc->samplehandles))
	{
		...
		if (vc)
		{
			...
			MikMod_free(vc->allochandle, vc->sample[handle].data);
...
//   and dynamic samples.
{
	...
	if (md->vc)
	{
		...
		length = sload->length;
		if (sload->outfmt & SF_STEREO) 
		{
			if (SizeTMult(length, 2, &length) != S_OK)
				return -1;
		}
		...
		if (!(vc->mode & DMODE_SAMPLE_8BIT) && (sload->outfmt & SF_16BITS))
		{
			...
			if (SizeTAdd(length, 16, &sample_malloc_size) != S_OK || SizeTMult(sample_malloc_size, 2, &sample_malloc_size) != S_OK)
			{
				return -1;
			}

			SL_Sample8to16(sload);
			ptr=MikMod_malloc(vc->allochandle, sample_malloc_size);

			if (ptr==NULL) return -1;
			vc->sample[handle].data = ptr;
		...
		else
		{
			...
			if (SizeTAdd(length, 16, &sample_malloc_size) != S_OK)
			{
				return -1;
			}
...
// =====================================================================================
{
	...
	vcs  = MikMod_malloc(sizeof(VCSTATE));

	vcs->numchn = total;
	boom = vcs->boom = MikMod_malloc(sizeof(VCBOOM) * total);
...
void VC_FreeState(PMSTATE *pmstate)
{
	...
	MikMod_free(vcs->boom);
	MikMod_free(pmstate->virtch);
}
