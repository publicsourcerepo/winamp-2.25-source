...
#define PF_LOOP             (1ul<<0)
#define PF_TIMESEEK         (1ul<<1)
#define PF_CONT_LOOP        (1ul<<2)

#define PF_XMPERIODS        UF_XMPERIODS
#define PF_LINEAR           UF_LINEAR
#define PF_LINEAR_PITENV    UF_LINEAR_PITENV
#define PF_INST             UF_INST
#define PF_NNA              UF_NNA

#define PF_XM_INST          UF_XM_INST
#define PF_OVR_PERIOD       UF_OVR_PERIOD
#define PF_FX9_BUG          UF_FX9_BUG

#define PF_NO_PANNING       UF_NO_PANNING
#define PF_NO_RESONANCE     UF_NO_RESONANCE

// ------------------------------------
// -/- Effects flags -/-

#define LOOP_PATTERNSCOPE (1ul<<20)     // used on UNI_LOOP global effect!

// ------------------------------------
// -/- Player constants -/-

#define MIN_TEMPO      32
#define MAX_TIME       100l*64l*1000l*60l
...
//TEST    int    cutoff;       // Resonance filter cutoff (harmonic scale)
//TEST    uint   resonance;    // Resonance filter resonance.
} MP_SIZEOF;
...
// =====================================================================================
{
    ...
    UBYTE  retrig;        // retrig counter
    UBYTE  noteslide;     // noteslide counter
    ULONG  speed;         // what finetune to use

//CUT    UBYTE  glissando;     // glissando (0 means off)
    UBYTE  wavecontrol;   //
...
//CUT    ULONG  soffset;       // last used low order of sample-offset (effect 9)
...
// ============================================================================
{
    ...
    int         prev_sngpos;    // position before loop jump
...
// ============================================================================
{
    ...
    const int    *volume_table;  // volume translation table, taken from mf
...
//CUT    BOOL        *useins;         // flagged true when an instrument is used.
//CUT    BOOL        *usesmp;         // flagged true when a sample is used
...
extern MPLAYER *Player_Dup(MPLAYER *ps);
...
extern void     Player_SetPosition(MPLAYER *mp, uint pos, BOOL wipe);
...
extern long Player_GetPosTime(MPLAYER *ps);
