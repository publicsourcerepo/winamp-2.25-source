...
{
//TEST    long     resfactor, cofactor;   // these are pregen values based on the resonance and cutoff.
//TEST    INT64S   speed, pos;            // running-sum type values, used and modded by thr mixers.
    int dummy;

} VC_RESFILTER;

typedef struct VOLCHAN
{
    int vol, inc;
} VOLCHAN;
...
{   
    ...
    void  (__cdecl *NoClick)(void *srce, SLONG *dest, INT64S index, INT64S increment, SLONG todo, VC_RESFILTER *resfilter, VOLINFO *volinfo);
    void  (__cdecl *NoClickSurround)(void *srce, SLONG *dest, INT64S index, INT64S increment, SLONG todo, VC_RESFILTER *resfilter, VOLINFO *volinfo);

    void  (__cdecl *Mix)(void *srce, SLONG *dest, INT64S index, INT64S increment, SLONG todo, VC_RESFILTER *resfilter, VOLINFO *volinfo);
    void  (__cdecl *MixSurround)(void *srce, SLONG *dest, INT64S index, INT64S increment, SLONG todo, VC_RESFILTER *resfilter, VOLINFO *volinfo);
...
*/
	...
	BYTE mixdat[32];//mixer status for cubic stuff
} VINFO;
...
extern void    VC_RegisterMixerHack(VIRTCH *vc, VMIXER *mixer);
...
// cubic interpolation
extern VMIXER M16_MONO_CUBIC,M16_STEREO_CUBIC,M8_MONO_CUBIC,M8_STEREO_CUBIC;

// fir interpolation
extern VMIXER M16_MONO_FIR,M16_STEREO_FIR,M8_MONO_FIR,M8_STEREO_FIR;
