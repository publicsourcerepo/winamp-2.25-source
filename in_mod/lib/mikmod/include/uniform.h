...
// -/- UniMod flags -/-
// 
// should match corresponding PF_* flags!
//

#define UF_XMPERIODS        (1ul<<10)       // XM periods/finetuning
#define UF_LINEAR           (1ul<<11)       // LINEAR periods
#define UF_LINEAR_PITENV    (1ul<<12)       // linear slides on envelopes?
#define UF_INST             (1ul<<13)       // Instruments are used
#define UF_NNA              (1ul<<14)       // New Note Actions used (set numvoices rather than numchn)

#define UF_XM_INST          (1ul<<15)       // XM instruments
#define UF_OVR_PERIOD       (1ul<<16)       // instrument w/o note overwrites period (IT thing)
#define UF_FX9_BUG          (1ul<<17)       // Protracker FX9 bug emulation
// more to come

#define UF_NO_PANNING       (1ul<<26)       // Disable 8xx panning effects
#define UF_NO_RESONANCE     (1ul<<27)       // Disable Zxx effects
...
#define KEY_KICK        0
#define KEY_OFF         (1ul<<0)
#define KEY_FADE        (1ul<<1)
...
#define TFLG_OVERRIDE_EFFECT    16                          // this is used far more often, so has separate flag
#define TFLG_OVERRIDE           32


// UNITRK :  UniMemory Flags
// --------------------------------------
...
#define UMF_LOWORD      2
#define UMF_HIWORD      4
#define UMF_FRAMEDLY    8
#define UMF_OTHERS      (UMF_FRAMEDLY|UMF_LOWORD|UMF_HIWORD)


// UNITRK :  Framedelay bitwise constants
// --------------------------------------

#define UFD_FXXHACK   255
...
{   UNIMEM_NONE = 0,
    ...
    PTMEM_PORTAMENTO,
    PTMEM_OFFSET,
    PTMEM_NOTESLIDE,            // no, PT does not use this, but I'm just to lazy
    PTMEM_LAST
};
...
//TEST    int    cutoff,          // the cutoff frequency (range 0 to 16384)
//TEST           resonance;       // the resonance factor (range -128 to 128)
...
// the blank rows at the end of the track).  This member effectively makes inuse obsolete.
{
    UBYTE    *buffer;          // unitrk buffer allocation!
		int			 writesize;				 // current buffer size (increases as needed).
...
{   
    ...
    SBYTE  pitpansep;        // pitch pan separation (-128 to 128)
...
// =====================================================================================
{
    ...
    ULONG       filesize;       // size of the module file, in bytes
    ...
    const int  *volume_table;   // volume translation table (129 values!)
    ...
    UE_EFFECT  *memdefs;        // default values for slots' data
    uint        numdefs;        // number of used defaults
...
MMEXPORT UBYTE utrk_blanktrack[5];
...
MMEXPORT void    utrk_local_memflag(UTRK_WRITER *ut, int memslot, uint flags);
MMEXPORT void    utrk_memory_reset(UTRK_WRITER *ut);

MMEXPORT void    utrk_memdef_add(UNIMOD *mf, const UNITRK_EFFECT *data, int memslot);
MMEXPORT void    utrk_memdef_clear(UNIMOD *mf);
...
MMEXPORT void    utrk_memory_local_ex(UTRK_WRITER *ut, const UNITRK_EFFECT *data, int memslot, uint flags, int signs);
...
MMEXPORT void utrk_write_note_ex(UTRK_WRITER *ut, unsigned int noteex);
...
__inline uint PeriodToNote(uint period)
{
    extern const UWORD  npertab[84];
    uint note;
    if (!period) return 0;
...
extern BOOL     AllocPositions(UNIMOD *of,uint32_t total);
...
extern CHAR    *DupStrTrim(MM_ALLOC *allochandle, CHAR *s, UWORD len);
extern CHAR    *ReadComment(MM_ALLOC *allochandle, uint length, uint linelen, MMSTREAM *file);
extern CHAR    *ProcessComment(MM_ALLOC *allochandle, uint length, uint linelen, CHAR *string);
...
MMEXPORT MLOADER  load_med;        // Amiga MED modules (by OctaMED)
MMEXPORT MLOADER  load_far;        // Farandole Composer Module
MMEXPORT MLOADER  load_amf;        // Digital Sound and Music Interface module
MMEXPORT MLOADER  load_okt;        // Amiga Oktalyzer
MMEXPORT MLOADER  load_ptm;        // PolyTracker module
