...
static void mmerr(int crap, const CHAR *crud)
{
	...
	{
		if(!lstrcmp(crud,"Corrupt file or unsupported module type."))
		{
...
// =====================================================================================
{
	...
	WASABI_API_SVC = (api_service*)SendMessage(mikmod.hMainWindow, WM_WA_IPC, 0, IPC_GET_API_SERVICE);
...
BOOL GetPlayParams(const char *fileName, BOOL open, PlayParams *params)
{
    ...
    if (open)
    {
	    ...
	    if (!strncmp(fileName, "http://", 7) || !strncmp(fileName, "https://", 8) ||
            !strncmp(fileName, "ftp://", 6))                // FTP is now currently supported, but still...
	    {
