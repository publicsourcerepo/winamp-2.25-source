...
#include "../gen_ml/M3UWriter.h"
#include "../gen_ml/PLSWriter.h"
#include "./settings.h"
#include <shlwapi.h>
#include <windows.h>
#include <strsafe.h>
	...
	case WM_APP + 1:
		{
			...
			TCHAR szDestination[MAX_PATH], szFormat[MAX_PATH];
			int l = m_rip_params->ntracks;
			done = 1;
			...
			for (int i = m_extract_curtrack + 1;i < l;i++)
			{
				if (m_rip_params->tracks[i])
				{
					...
					wchar_t tmp1[32], tmp2[32], tmp3[32], tmp4[32], tmp5[32];
					FormatFileName(szDestination, ARRAYSIZE(szDestination), szFormat,
					                 i +trackOffset,
									 ...
									 NULL,
									 (m_rip_params->disc && *(m_rip_params->disc)) ? (m_rip_params->disc) : L"");
			...
			if (done && m_rip_params)
			{
				...
				if (m_rip_params->ntracks == m_extract_nb_total && done > 0)
				{
						...
						(S_OK == Settings_GetBool(C_EXTRACT, EF_CREATEPLS, &bVal) && bVal))
					{
						...
						wchar_t str[MAX_PATH], fmt[MAX_PATH];
						...
						FormatFileName(str, ARRAYSIZE(str), fmt, 0xdeadbeef,
