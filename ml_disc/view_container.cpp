...
static void CreateChild(HWND hParent, INT typeId, LPARAM param, BOOL fCreateAlways = TRUE)
{
	...
	switch(typeId)
	{
		...
		case IDD_VIEW_CDROM_EX2:
			...
			break;
		case IDD_VIEW_CDROM_BURN:
			hChild = CreateCDBurnWindow(hParent, (CHAR)(0xFF & param));
			break;
...
static void QueryDriveRecordable(HWND hwnd, CHAR cLetter)
{
	...
	else
	{
		...
		if (pui)
		{
			ZeroMemory(pui, sizeof(DM_UNITINFO_PARAM));
...
static void QueryDiscInfo(HWND hwnd, CHAR cLetter)
{
	...
	switch(cMode)
	{
		case DM_MODE_RIPPING: 
			...
			return;
		case DM_MODE_BURNING:
			if (cLetter == cdburn_IsMeBurning()) CreateChild(hwnd, IDD_VIEW_CDROM_BURN, cLetter); 
			else CreateChild(hwnd, IDD_VIEW_INFO, cLetter); 
			return;
	...
	pmci = (DM_MCI_PARAM*)malloc(sizeof(DM_MCI_PARAM));
    if (pmci)
	{
		ZeroMemory(pmci, sizeof(DM_MCI_PARAM));
		...
		pmci->nTracks = 256;
		pmci->pTracks = (DWORD*)malloc(sizeof(DWORD)*pmci->nTracks);
...
static INT_PTR Window_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
	...
	if (WASABI_API_APP)
	{
		...
		WASABI_API_APP->app_addModelessDialog(hwnd);
	}
	return 0;
}
...
static void Window_OnCommand(HWND hdlg, INT eventId, INT ctrlId, HWND hctrl)
{
	...
	{		
		case IDC_BTN_SHOWINFO:
			switch(eventId)
			{
				case BN_CLICKED: ViewContainer_ShowMiniInfo(hdlg, !pc->bInfoVisible); break;
				case BN_EX_GETTEXT:
					if (hctrl) SetWindowTextW(hctrl, WASABI_API_LNGSTRINGW((pc->bInfoVisible) ? IDS_HIDE_INFO : IDS_SHOW_INFO));
					break;
			}
			break;
...
{
	return g_config->ReadInt(TEXT("useminiinfo"), 1);
}
