...
void cdburn_appendFile(char *file, char driveletter);
void cdburn_appendFile(wchar_t *file, char driveletter);
void cdburn_appendItemRecord(itemRecordListW *obj, char driveletter);
HWND cdburn_FindBurningHWND(char cLetter);
CHAR cdburn_IsMeBurning(void); // returns drive letter if burning..
...
int cdrip_upsell(HWND hwndDlg);// 1 if rip should continue
