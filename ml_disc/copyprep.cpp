...
{
	TCHAR szBuffer[MAX_PATH*2], szFormat[MAX_PATH];
...
{
	TCHAR szFormat[256], szBuffer[MAX_PATH*2];
...
{
	HWND hctrl;
	PREPDLG *ppd = (PREPDLG*)malloc(sizeof(PREPDLG));
	if (!ppd) return 0;

	ZeroMemory(ppd, sizeof(PREPDLG));
...
{	
	...
	PREPDLG *ppd = GetPrepDlg(hdlg);
	if (!ppd) return;
	CALCDISKSIZE *pcs = (CALCDISKSIZE*)malloc(sizeof(CALCDISKSIZE));
	if (!pcs) return;
