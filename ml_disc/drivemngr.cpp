...
BOOL DriveManager_Initialize(DMNPROC DMNProc, BOOL bSuspended)
{
	...
	pMngr = (DRIVEMNGR*)malloc(sizeof(DRIVEMNGR));
	if (!pMngr) return FALSE;

	ZeroMemory(pMngr, sizeof(DRIVEMNGR));
	...
	if (pMngr)
	{
		...
		for (int index =0; index < pMngr->nCount; index++) 
		{
			if (pMngr->pDrives[index].hThread)
			{
				...
				result = WaitForSingleObject(pMngr->pDrives[index].hThread, msExitWaitTime);
				if (WAIT_TIMEOUT == result) 
					TerminateThread(pMngr->pDrives[index].hThread, 1);
		...
		if (pMngr->hPollingThread)
		{
			...
			result = WaitForSingleObject(pMngr->hPollingThread, msExitWaitTime);
...
static DWORD GetDeviceNames(DEVICEINFO *pDevInfo, INT count)
{
	...
	{
		bResult = SetupDiEnumDeviceInterfaces(hDevInfo, 0, (LPGUID)&CdRomClassGuid, index, &spiData);
		if (!bResult)
		{
		...
		if(GetVolumeNameForVolumeMountPoint_DL(pspiDetailData->DevicePath, volume, sizeof(volume)/sizeof(wchar_t)))
		{
			for (int i = 0; i < count; i++)
			{
				...
				{
					pDevInfo[i].pszDevName = (LPWSTR)malloc((cchName + 1)*sizeof(wchar_t));
	...
	for (int i = 0; i < count; i++)
	{
		if (!pDevInfo[i].pszDevName)
		{
			...
			pDevInfo[i].pszDevName = (LPWSTR)malloc(sizeof(szDevName) + 2);
...
{
	MSG msg;
	DWORD start, elapsed, status, timeout, result(0);
...
{
	MSG msg;
	DWORD start, elapsed, status, timeout;
	BOOL bComInit;
...
static void CALLBACK PollMediumInfo(ULONG_PTR param)
{
	...
	while(count--)
	{
		hDevice = CreateFileW(pszDevName[count], GENERIC_READ | GENERIC_WRITE, 
								FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_READONLY, NULL);
...
static void CALLBACK APC_CheckDrives(ULONG_PTR param)
{
	...
	pDevInfo = (DEVICEINFO*)malloc(sizeof(DEVICEINFO)*count);
	if (!pDevInfo) return;
	ZeroMemory(pDevInfo, sizeof(DEVICEINFO)*count);
	...
	for (int i = 0; i < count; i++)
	{
		hDevice = CreateFileW(pDevInfo[i].pszDevName, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, 
...
static void CALLBACK APC_GetTitle(ULONG_PTR param)
{
	...
	if (cLetter && pdtp->pszTitle) 
	{
		...
		if (!mciSendCommandW(0, MCI_OPEN, MCI_OPEN_TYPE | MCI_OPEN_SHAREABLE | MCI_OPEN_TYPE_ID | MCI_OPEN_ELEMENT, (DWORD_PTR)&op))
		{
			...
			present = (!mciSendCommandW(devId, MCI_STATUS, MCI_STATUS_ITEM | MCI_WAIT, (DWORD_PTR)&sp)) ? (BOOL)sp.dwReturn : 0;
