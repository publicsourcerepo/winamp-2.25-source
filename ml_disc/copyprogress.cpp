...
{
	HWND hctrl;
	PROGDLG *ppd = (PROGDLG*)malloc(sizeof(PROGDLG));
	if (!ppd) return 0;
	
	ZeroMemory(ppd, sizeof(PROGDLG));
...
static void ShowErrorBox(HWND hdlg)
{
	...
	TCHAR szBuffer[2048], szFormat[256], szUnknown[64];
	LPTSTR pszMessage;
...
static INT_PTR CopyProgress_OnDestiantionNotExist(HWND hdlg, LPCTSTR pszDestination)
{
    ...
    QUESTIONBOX qb;
	TCHAR szFormat[MAX_PATH],  szMessage[MAX_PATH*2];
	...
	ZeroMemory(&qb, sizeof(QUESTIONBOX));
	qb.hParent =hdlg;
...
{	
	...
	{		
		TCHAR szTemp[1024], szKeyword[64];
		SYSTEMTIME st;
...
static INT_PTR CopyProgress_OnFileAlreadyExist(HWND hdlg, FILECONFLICT *pConflict)
{
	...
	if (!ppd || !ppd->pCopyData) return FALSE;
    QUESTIONBOX qb;
	TCHAR szFormat[128], szMessage[MAX_PATH*2], szPath[MAX_PATH], szFileInfo1[128], szFileInfo2[128];
