...
{
	return WASABI_API_CREATEDIALOGPARAMW(IDD_VIEW_CDROM_BURN, hwndParent, DlgProc, (LPARAM)cLetter);
}

void onUpsell(HWND hwndDlg)
{
	ShellExecute(hwndDlg, L"open", L"http://www.winamp.com/buy", NULL, L".", 0);
...
int getRegVer()
{
	int *x = (int*)malloc(32);
	long s[3];
	long ss[2] = {GetTickCount(), (int)(INT_PTR)x + (int)(INT_PTR)s}; // fucko: better challenge, too
	...
	SendMessage(plugin.hwndWinampParent, WM_WA_IPC, (WPARAM)s, IPC_GETREGISTEREDVERSION);
...
static void startBurn(HWND hwndDlg, char driveletter)
{
	...
	FILE *fp;
	char filename[MAX_PATH], tp[MAX_PATH];
...
static HWND m_burning_other_wnd;

BOOL CALLBACK CDBurnProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			if (rv > 0)
			{
				ShowWindow(GetDlgItem(hwndDlg, IDC_PRO1), SW_HIDE);
				ShowWindow(GetDlgItem(hwndDlg, IDC_PRO2), SW_HIDE);
			}

			wchar_t tmp[512], tmp2[16];
			int idx = LETTERTOINDEX(m_cdrom);
			StringCchPrintf(tmp, 512, WASABI_API_LNGSTRINGW(IDS_BURN_X_TRACKX_ON_X),
							itemCache[idx].Size,
							WASABI_API_LNGSTRINGW_BUF(itemCache[idx].Size == 1 ? IDS_TRACK : IDS_TRACKS,tmp2,16),
							m_cdrom);
			SetWindowText(hwndDlg, tmp);

			char buf[64];
			StringCchPrintfA(buf, 64, "cda://%c.cda", m_cdrom);
			char buf2[512] = "";
			getFileInfo(buf, "cdspeeds", buf2, sizeof(buf2));

			m_max_speed = 0;
			int config_speed = g_config->ReadInt(L"cdburnspeed", 0);
			if (rv < 1 && (config_speed > 2 || !config_speed)) config_speed = 2;
			int sel = 0;
			{
				char *p = buf2;
				char *w = strstr(p, ";");
				...
				while (*p)
				{
					...
					if (!checked1x)
					{
						if (speed != 1)
						{
							int a = (int)(INT_PTR)SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0, (LPARAM)L"1x");
							if (config_speed == 1) sel = a;
						}
						checked1x = 1;
					}
					if (!checked2x)
					{
						if (speed > 2)
						{
							int a = (int)(INT_PTR)SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0, (LPARAM)L"2x");
							if (config_speed == 2) sel = a;
							checked2x = 1;
						}
					}
					if (speed == 1) checked1x = 1;
					if (speed == 2) checked2x = 1;
					wchar_t wtmp[32];
					StringCchPrintfW(wtmp, 32, L"%dx", speed);
					int a = (int)(INT_PTR)SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_ADDSTRING, 0, (LPARAM)wtmp);
					if (config_speed == speed || !config_speed) sel = a;
					if (speed > m_max_speed) m_max_speed = speed;
					p = p2 + 1;
				}
			}
			SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_SETCURSEL, sel, 0);

			if (g_config->ReadInt(L"cdburntestmode", 0)) CheckDlgButton(hwndDlg, IDC_CHECK1, BST_CHECKED);
			if (g_config->ReadInt(L"cdburnproof", 1)) CheckDlgButton(hwndDlg, IDC_CHECK2, BST_CHECKED);
			//        if (g_config->ReadInt(L"cdburnaws",1)) CheckDlgButton(hwndDlg,IDC_CHECK3,BST_CHECKED);
		}
		return 0;
		case WM_COMMAND:
			...
			{
				case IDC_CHECK1:
					g_config->WriteInt(L"cdburntestmode", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK1));
					break;
				case IDC_CHECK2:
					g_config->WriteInt(L"cdburnproof", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK2));
					break;
					//case IDC_CHECK3:
					//        g_config->WriteInt(L"cdburnaws",!!IsDlgButtonChecked(hwndDlg,IDC_CHECK3));
					//        break;
				case IDC_COMBO1:
				{
					int x = (int)(INT_PTR)SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_GETCURSEL, 0, 0);
					if (x != CB_ERR)
					{
						wchar_t tmp[64];
						SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_GETLBTEXT, x, (LPARAM)&tmp);
						int speed = _wtoi(tmp);
						if (speed > 2 && getRegVer() < 1)
						{
							wchar_t title[64];
							SendDlgItemMessage(hwndDlg, IDC_COMBO1, CB_SETCURSEL, 0, 0);
							if (MessageBox(hwndDlg, WASABI_API_LNGSTRINGW(IDS_GET_PRO_FOR_BURN_HIGHER_THAN_2X),
										   WASABI_API_LNGSTRINGW_BUF(IDS_WINAMP_PRO_FEATURE,title,64), MB_YESNO) == IDYES)
							{
								onUpsell(hwndDlg);
							}
						}
						g_config->WriteInt(L"cdburnspeed", speed);
					}
				}
				break;
				case IDC_PRO2:
...
static void doBurnDialog(HWND parent)
{
	int idx = LETTERTOINDEX(m_cdrom);
	if (!itemCache[idx].Size)
	{
		wchar_t title[8];
		MessageBox(parent, WASABI_API_LNGSTRINGW(IDS_NEED_TO_ADD_SOME_TRACKS_TO_BURN),
				   WASABI_API_LNGSTRINGW_BUF(IDS_ERROR,title,8), 0);
		return ;
	}
	if (m_availsecs < 0)
	{
		wchar_t tmp[512], title[8];
		StringCchPrintf(tmp, 512, WASABI_API_LNGSTRINGW(IDS_TOTAL_LENGTH_IS_BIGGER_THAN_MEDIA_CAPACITY), -m_availsecs / 60, -m_availsecs % 60);
		MessageBox(parent, tmp, WASABI_API_LNGSTRINGW_BUF(IDS_ERROR,title,8), 0);
		return ;
	}

	if (WASABI_API_DIALOGBOXW(IDD_BURN, parent, CDBurnProc))
	{
		if (Burn_WaitForCDR(parent, m_cdrom)) return ;
		startBurn(m_hwndstatus, m_cdrom);
	}
}
...
static void SetStatus(HWND hwndDlg, CHAR cLetter)
{
	...
	else
	{
		...
		DM_IMAPI_PARAM *pIMAPI = (DM_IMAPI_PARAM*)malloc(sizeof(DM_IMAPI_PARAM));
		if (pIMAPI)
		{
...
const wchar_t *scanstr_back(const wchar_t *str, const wchar_t *toscan, const wchar_t *defval)
{
	const wchar_t *s = str + wcslen(str) - 1;
	if (wcslen(str) < 1) return defval;
	if (wcslen(toscan) < 1) return defval;
	for (;;)
	{
...
{
	const wchar_t *s = scanstr_back(fn, L".\\", fn - 1);
	if (s < fn) return L"";
	if (*s == '\\') return L"";
	return (s + 1);
}
...
void cdburn_addfile(char* file, Vector<GayString*> *files, api_playlistmanager* plManager, api_playlistloadercallback *plCB);
void cdburn_addfile(wchar_t* file, Vector<GayStringW*> *files, api_playlistmanager* plManager, api_playlistloadercallback *plCB);
void cdburn_addfolder(const char* folder, Vector<GayString*> *files, api_playlistmanager* plManager, api_playlistloadercallback *plCB);
void cdburn_addfolder(const wchar_t* folder, Vector<GayStringW*> *files, api_playlistmanager* plManager, api_playlistloadercallback *plCB);

void cdburn_appendFile(char *file, char cLetter)
{
	Vector<GayString*> files;
	api_playlistmanager *plManager;
	waServiceFactory *plmFactory = WASABI_API_SVC->service_getServiceByGuid(api_playlistmanagerGUID);
	plManager = (plmFactory) ? (api_playlistmanager*)plmFactory->getInterface() : NULL;
	...
	cdburn_addfile(file, &files, (api_playlistmanager*)plManager, (api_playlistloadercallback*)&plCB);
...
void cdburn_addfile(wchar_t* file, Vector<GayStringW*> *files, api_playlistmanager* plManager, api_playlistloadercallback *plCB)
{
	if (!_wcsicmp(extension(file), L"lnk"))
	{
...
void OnBurnDlgInit(HWND hwndDlg, LPARAM lParam)
{
	...
	HANDLE hPrev = (HANDLE) SendDlgItemMessage(hwndDlg,IDC_LOGO,STM_SETIMAGE,IMAGE_BITMAP,
					   (LPARAM)LoadImage(plugin.hDllInstance,MAKEINTRESOURCE(IDB_LOGO_SONIC),
										 IMAGE_BITMAP,0,0, 0));
...
void OnBurnNotify(HWND hwndDlg, DWORD notification, DWORD param)
{
	switch (notification)
	{
		...
		case BURN_FINISHED:
		{
			wchar_t buf1[128], closeStr[16];
			GetDlgItemText(hwndDlg, IDC_CANCEL_BURN, buf1, ARRAYSIZE(buf1));
			if (lstrcmp(buf1, WASABI_API_LNGSTRINGW_BUF(IDS_CLOSE,closeStr,16)))
...
static INT_PTR CALLBACK DlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
			...
			switch (LOWORD(wParam))
			{
				...
				case IDC_ADD:
					if (DM_MODE_BURNING != DriveManager_GetDriveMode(m_cdrom))
					{
						RECT r;
						GetWindowRect((HWND)lParam, &r);
						int x = Menu_TrackPopup(GetSubMenu(g_context_menus, 3), TPM_RIGHTBUTTON | TPM_LEFTBUTTON | TPM_BOTTOMALIGN | TPM_LEFTALIGN | TPM_NONOTIFY | TPM_RETURNCMD, r.left, r.top, hwndDlg, NULL);
						switch (x)
						{
							case ID_BURNADDMENU_FILES:
							{
								...
								temp = (wchar_t *)GlobalAlloc(GMEM_FIXED, len);
								temp[0] = 0;
								l.hwndOwner = hwndDlg;
		...
		case WM_CONTEXTMENU:
		{
			...
			int r = Menu_TrackPopup(menu, TPM_RETURNCMD | TPM_RIGHTBUTTON | TPM_LEFTBUTTON | TPM_NONOTIFY, pt.x, pt.y, hwndDlg, NULL);
			switch (r)
			{
		...
		case WM_DROPFILES:
		{
			...
			y = DragQueryFileA(h, 0xffffffff, temp, sizeof(temp));
