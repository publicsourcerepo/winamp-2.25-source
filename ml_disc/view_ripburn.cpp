...
static void UpdateDriveInfo(HWND hwndDlg, CHAR cLetter)
{
	...
	DM_UNITINFO_PARAM *pui;
	DM_UNITINFO2_PARAM *pui2;
	DM_DISCINFOEX_PARAM *pdi;
	DM_DISCINFO2_PARAM *pdi2;
	...
	// request unitinfo
	pui = (DM_UNITINFO_PARAM*)malloc(sizeof(DM_UNITINFO_PARAM));
	if (pui)
	{
		CopyMemory(&pui->header, &header, sizeof(DM_NOTIFY_PARAM));
		pui->header.fFlags = DMF_DESCRIPTION;
		pui->cchDesc = 128;
		pui->pszDesc = (CHAR*)malloc(sizeof(CHAR)*pui->cchDesc);
		DriveManager_GetUnitInfo(pui);
	}

	// request unitinfo2
	pui2 = (DM_UNITINFO2_PARAM*)malloc(sizeof(DM_UNITINFO2_PARAM));
	if (pui2)
	{
		CopyMemory(&pui2->header, &header, sizeof(DM_NOTIFY_PARAM));
		pui2->header.fFlags = DMF_TYPES;
		pui2->nTypes = 32;
		pui2->pdwTypes = (DWORD*)malloc(sizeof(DWORD)*pui2->nTypes);
		DriveManager_GetUnitInfo2(pui2);
	}

	// request discinfoex
	pdi = (DM_DISCINFOEX_PARAM*)malloc(sizeof(DM_DISCINFOEX_PARAM));
	if (pdi)
	{
	...
	// request discinfo2
	pdi2 = (DM_DISCINFO2_PARAM*)malloc(sizeof(DM_DISCINFO2_PARAM));
	if (pdi2)
	{
...
{
	HWND hwndCtrl;
	hwndCtrl = GetDlgItem(hwndDlg, ctrlId);
	...
	if (hwndCtrl)
	{
		...
		else
		{
			...
			bVisible = ((prcParent->right > rc.right) && (prcParent->bottom > rc.bottom)); 
...
static void ripburn_OnDisplayChanges(void)
{
...
static void ripburn_OnDestroy(HWND hwndDlg)
{
	...
	hwndLB = GetDlgItem(hwndDlg, IDC_LIST_DRIVES);
	if (hwndLB)
	{
...
static void Drive_OnAdded(HWND hwndDlg, CHAR cLetter)
{
	...
	if (hwndLB)
	{
		...
		index = (INT)SendMessageW(hwndLB, LB_ADDSTRING, 0, (LPARAM)str);
		if (LB_ERR != index)
		{
			DM_UNITINFO_PARAM *pui;
			INT idxSelection;
			...
			idxSelection = (int)(INT_PTR)SendMessageW(hwndLB, LB_GETCURSEL, 0,0);
...
static void Drive_OnRemoved(HWND hwndDlg, CHAR cLetter)
{
	...
	{
		INT index;
		index = GetListBoxIndex(hwndLB, cLetter);
		...
		case DMOP_UNITINFO:
			...
			{	
				...
				if (hwndLB)
				{
					...
					{
						DWORD data;
						data = MAKELONG(phdr->cLetter, pui->dwType);
			...
			switch (LOWORD(wParam))
			{
				case IDC_LIST_DRIVES:
					...
					else if (HIWORD(wParam) == LBN_DBLCLK)
					{
						...
						DWORD data;
						HNAVITEM hItem;
						index = (int)(INT_PTR)SendMessage((HWND)lParam, LB_GETCURSEL, 0,0);
						data = (LB_ERR != index) ? (DWORD)SendMessage((HWND)lParam ,LB_GETITEMDATA, index, 0) : 0; 
						hItem = (data) ? Plugin_GetNavItemFromLetter((CHAR)(0xFF & data)) : NULL;
