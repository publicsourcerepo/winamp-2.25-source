...
void CopyComment(wchar_t *&dest, wchar_t *comments)
{
	if (comments)
	{
		...
		dest = (wchar_t *) malloc(sizeof(wchar_t) * (size + numCarriageReturns + 1));
...
static void extractFiles(HWND hwndDlg, CHAR cLetter, int all)
{
	...
	if (cdrip_upsell(hwndDlg)) return ;
	...
	if (ntracks > 0 && ntracks < 256)
	{
		...
		for (x = 0; x < ntracks; x ++)
		{
			...
			if (!lstrcmpiA(buf2, "audio"))
			{
				...
				char titlebuf[400];
				titlebuf[0] = 0;
				mediaLibrary.GetFileInfo(fname, titlebuf, 400, &len);
				pRec = &obj->Items[obj->Size];
...
static void GetGracenoteInfo(HWND hwndDlg, CHAR cLetter, HANDLE hThread = NULL)
{
	...
	int l, x;
	LVITEMW lvitem;
	wchar_t buf[32], titlebuf[256];

	if (hThread)
	{
		APCPARAM *pParam = (APCPARAM*)malloc(sizeof(APCPARAM));
		if (pParam)
		{
	...
	// first, let's try to get artist/album info
	{
		...
		wchar_t artistbuf[256], albumbuf[256], yearbuf[256], genrebuf[256];
	...
	if (l && hwndList) UpdateWindow(hwndList);

	if (Plugin_IsExtractScheduled(cLetter)) extractFiles(hwndDlg, cLetter, 1);
}
...
static void UpdateCDView(HWND hwndDlg, DM_NOTIFY_PARAM *phdr)
{
	...
	if (DMOP_MCIINFO == phdr->opCode)
	{
		...
		for (int i = 0; i < pmci->nTracks; i++)
		{
			...
			time = (0x7FFFFFFF & pmci->pTracks[i])/1000;
			...
			lvitem.lParam	= param;
			index = (INT)SendMessageW(hwndList, LVM_INSERTITEMW, 0, (LPARAM)&lvitem);
...
static void Window_OnCommand(HWND hwndDlg, INT eventId, INT ctrlId, HWND hwndCtrl)
{
	...
	case IDC_BUTTON_PLAY:
	{
		...
		if (hwndList)
		{
			selcnt = (INT)SendMessageW(hwndList, LVM_GETSELECTEDCOUNT, 0, 0L);
	...
	case IDC_BUTTON_EXTRACT:
	{
		...
		int x = Menu_TrackPopup(GetSubMenu(g_context_menus, 0), TPM_RIGHTBUTTON | TPM_LEFTBUTTON | TPM_BOTTOMALIGN | TPM_LEFTALIGN | TPM_NONOTIFY | TPM_RETURNCMD, r.left, r.top, hwndDlg, NULL);
		switch (x)
		{
...
static void Window_OnLButtonUp(HWND hwndDlg, INT vKey, POINTS pts)
{
	...
	if (m.result > 0)
	{
		...
		LVITEMW lvitem;
		int i, l, p, len;
		itemRecordList myObj = {0, 0, 0};
		char trackname[] = "cda://X,%d.cda";
		char name[32], total[512];
...
static void Window_OnContextMenu(HWND hwndDlg, HWND hwndFrom, int x, int y)
{
	...
	int r = Menu_TrackPopup(GetSubMenu(g_context_menus, 1), TPM_RETURNCMD | TPM_RIGHTBUTTON | TPM_LEFTBUTTON | TPM_NONOTIFY, x, y, hwndDlg, NULL);
	switch (r)
	{
		case ID_PE_ID3:
		{
