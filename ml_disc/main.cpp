...
static Vector<LPARAM, 4> driveList;
...
{
	CHAR cLetter = cdburn_IsMeBurning();
	if (cLetter)
	{
		wchar_t buffer[512];
		StringCchPrintfW(buffer, 512, WASABI_API_LNGSTRINGW(IDS_YOU_ARE_CURRENTLY_BURNING_AUDIO_CD_MUST_CANCEL_TO_CLOSE_WINAMP), cLetter);
...
static void QueryTitle(CHAR cLetter)
{
	...
	pdtp = (DM_TITLE_PARAM*)malloc(sizeof(DM_TITLE_PARAM)*4);
	if (pdtp)
	{
		ZeroMemory(pdtp, sizeof(DM_TITLE_PARAM));
		...
		pdtp->cchTitle = 256;
		pdtp->pszTitle = (wchar_t*)malloc(sizeof(wchar_t)*pdtp->cchTitle);
...
{
	wchar_t szInvariant[32];
	
	DRIVE	*pDrive;
	NAVINSERTSTRUCT nis;
	wchar_t szDriveType[32], szDriveCap[64];

	pDrive = (DRIVE*)malloc(sizeof(DRIVE));
	if (!pDrive) return;

	ZeroMemory(pDrive, sizeof(DRIVE));
...
static void CALLBACK Invoke_OnDriveManagerNotify(WORD wCode, INT_PTR param)
{
	...
	switch(wCode)
	{
		...
		case DMW_MODECHANGED:
			...
			{
				htWA = (WASABI_API_APP) ? WASABI_API_APP->main_getMainThreadHandle() : NULL;
				if (htWA)
				{
...
int Init()
{
	// grab the Wasabi service manager
...
void Quit()
{
	...
	waServiceFactory *sf = WASABI_API_SVC->service_getServiceByGuid(AnonymousStatsGUID);
...
static int Root_OnContextMenu(HNAVITEM hItem, HWND hHost, POINTS pts)
{
	...
	int r = Menu_TrackPopup(hMenu, TPM_RETURNCMD | TPM_RIGHTBUTTON | TPM_LEFTBUTTON | TPM_NONOTIFY, pt.x, pt.y, hHost, NULL);
	switch (r)
	{
		case ID_NAVIGATION_PREFERENCES: Plugin_ShowRippingPreferences(); return 1;
		case ID_NAVIGATION_HELP: SENDWAIPC(plugin.hwndWinampParent, IPC_OPEN_URL, L"http://www.winamp.com/help/Main_Page#Ripping_and_Burning_with_Winamp"); return 1;
		break;
	}
	return 0;
}
...
static int Plugin_OnContextMenu(HNAVITEM hItem, HWND hHost, POINTS pts, CHAR cLetter)
{
	...
	switch (r)
	{
		...
		case ID_CDROMMENU_EXTRACT_HELP:  SENDWAIPC(plugin.hwndWinampParent, IPC_OPEN_URL, L"http://www.winamp.com/help/Main_Page#Ripping_and_Burning_with_Winamp"); return 1;
...
static void Plugin_OnSendToBuild(UINT uType, INT_PTR context)
{
	INT i;
	char root[] = "A:\\";
	wchar_t buffer[128];
		...
		case ML_TYPE_FILENAMESW:
			...
			for (i = 0; i < 26; i++)
			{
				if (0x1 & unitmask)
				{
					...
					if(DRIVE_CDROM == GetDriveTypeA(root) && DM_MODE_READY == DriveManager_GetDriveMode(root[0]))
					{
						DWORD dwType;
						dwType = DriveManager_GetDriveType(root[0]);
						if (((DRIVE_CAP_UNKNOWN & dwType) && Drive_IsRecorder(root[0])) || ((DRIVE_CAP_R | DRIVE_CAP_RW) & dwType))
						{
							StringCchPrintfW(buffer, sizeof(buffer)/sizeof(wchar_t), 
												WASABI_API_LNGSTRINGW(IDS_CD_BURNER_ON_X), root[0]);
							s.desc = buffer;
							s.user32 = (INT_PTR)(((INT_PTR)&plugin) + i);
							SENDMLIPC(plugin.hwndLibraryParent, ML_IPC_ADDTOSENDTOW, (WPARAM)&s);
						}
					}
				}
				unitmask = unitmask >> 1;
...
static int Plugin_OnSendToSelect(int msg, INT_PTR param1, INT_PTR param2, INT_PTR param3)
{
	CHAR cLetter;
	INT_PTR type, data;

	if (msg == ML_MSG_ONSENDTOSELECT)
	{
		type = param3 - (INT_PTR)&plugin;
		cLetter = (char)((type >= 0 && type <= 32) ? ('A' + (INT)type) : 0);
		type = param1;
		data = param2;
	}
	else
	{	
		...
		hItem = MLNavCtrl_FindItemById(plugin.hwndLibraryParent, param1);
		if (hItem) pDrive = Plugin_GetDriveFromNavItem(hItem);
		else pDrive = NULL;

		if (pDrive) cLetter = pDrive->cLetter;
		else cLetter = 0x00;

		type = param2;
		data = param3;

		if (!data && cLetter && Drive_IsRecorder(cLetter))
			return (type == ML_TYPE_ITEMRECORDLISTW || type == ML_TYPE_ITEMRECORDLIST ||
					type == ML_TYPE_PLAYLIST || type == ML_TYPE_PLAYLISTS  
					|| type == ML_TYPE_FILENAMES || type == ML_TYPE_FILENAMESW) 
					? 1 : -1;
	}

	if (!cLetter || !data) return 0;

	switch(type)
	{
		case ML_TYPE_ITEMRECORDLIST:
			if (((itemRecordList*)data)->Size) cdburn_appendItemRecord((itemRecordList*)data, cLetter);
			break;
		case ML_TYPE_ITEMRECORDLISTW:
			if (((itemRecordListW*)data)->Size) cdburn_appendItemRecord((itemRecordListW*)data, cLetter);
			break;
		case ML_TYPE_FILENAMES:
			{
				char *p = (char*)data;
				...
				BurnAddStatus_Create(cnt);

				while (*p)
				{
					extern itemRecordListW itemCache[26];
					cdburn_appendFile(p, cLetter);
					p += strlen(p) + 1;
					BurnAddStatus_Step(&itemCache[cLetter]);
				}
				BurnAddStatus_Done();
			}
			break;
		...
		case ML_TYPE_PLAYLIST:
			BurnAddStatus_Create(((mlPlaylist*)data)->numItems);
			cdburn_appendFile((wchar_t *)((mlPlaylist*)data)->filename, cLetter);
			BurnAddStatus_Done();
			break;
		case ML_TYPE_PLAYLISTS:
			{
				...
				while (*playlists)
				{
					numItems += (*playlists)->numItems;
					playlists++;
				}
				BurnAddStatus_Create(numItems);
				playlists = (mlPlaylist **)data;
				while (*playlists)
				{
					mlPlaylist *playlist = *playlists;
					cdburn_appendFile((wchar_t *)(playlist->filename), cLetter);
					playlists++;
				}
				BurnAddStatus_Done();
			}
...
static INT_PTR pluginMessageProc(int msg, INT_PTR param1, INT_PTR param2, INT_PTR param3)
{
	...
	switch (msg)
	{
		...
		case ML_MSG_ONSENDTOBUILD:		Plugin_OnSendToBuild((UINT)param1, param2); break;
		case ML_MSG_ONSENDTOSELECT:
		case ML_MSG_TREE_ONDROPTARGET:	return (INT_PTR)Plugin_OnSendToSelect(msg, param1, param2, param3);
...
/////////////////////////////////////////////////////////////////////////////////
winampMediaLibraryPlugin plugin = { 	MLHDR_VER, "nullsoft(ml_disc.dll)", Init, Quit, pluginMessageProc, 	0, 0, 0, };
