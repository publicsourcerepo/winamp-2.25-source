...
static void myEnumProc(intptr_t user_data, const char *desc, int fourcc)
{
	...
	if (fourcc == OLD_AAC_CODEC) return ;
	...
	if (m_ccs.format == fourcc) { m_has_seled = 1; SendDlgItemMessage(hwndDlg, IDC_ENCFORMAT, CB_SETCURSEL, (WPARAM)a, 0); }
}
	...
	case WM_INITDIALOG:
		{
			...
			if (RegisteredEncoder(m_ccs.format)	&& getRegVer() < 1)
			{
				Settings_GetDefault(C_EXTRACT, EF_FOURCC, &m_ccs.format);
			}
		...
		case IDC_ENCFORMAT:
			if (HIWORD(wParam) != CBN_SELCHANGE) return 0;
			{
				...
				if (sel != CB_ERR)
				{
					...
					{
						m_ccs.format = last;
						int x, l = (INT)(INT_PTR)SendDlgItemMessage(hwndDlg, IDC_ENCFORMAT, CB_GETCOUNT, 0, 0);
						for (x = 0; x < l; x ++)
							if (SendDlgItemMessage(hwndDlg, IDC_ENCFORMAT, CB_GETITEMDATA, x, 0) == m_ccs.format)
								break;
						if (x != l) SendDlgItemMessage(hwndDlg, IDC_ENCFORMAT, CB_SETCURSEL, x, 0);
						// prompt
						poo = 1;
					}
					...
					wchar_t title[64];
					if (poo && MessageBox(hwndDlg,
										  WASABI_API_LNGSTRINGW(IDS_TO_EXTRACT_TO_MP3_NEED_TO_PURCHASE_WINAMP_PRO),
										  WASABI_API_LNGSTRINGW_BUF(IDS_WINAMP_PRO_FEATURE,title,64),
										  MB_YESNO) == IDYES)
					{
						onUpsell(hwndDlg);
					}
...
BOOL CALLBACK upsellProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
			if (lParam)
			{
				...
				MapWindowPoints((HWND)lParam, HWND_DESKTOP, (POINT*)&rc, 2);
				SetWindowPos(hwndDlg, NULL, rc.left + ( (rc.right - rc.left) - (rw.right - rw.left))/2, 
	...
	case WM_COMMAND:
		...
		return 0;
	case WM_DESTROY:
		g_config->WriteInt(L"hasusrip", !!IsDlgButtonChecked(hwndDlg, IDC_CHECK1));
		return 0;
	}
	return 0;
}
...
int cdrip_upsell(HWND hwndDlg)
{
	INT val, def;
	Settings_GetInt(C_EXTRACT, EF_FOURCC, &val);
	Settings_GetDefault(C_EXTRACT, EF_FOURCC, &def);

	if (val == def && !g_config->ReadInt(L"hasusrip", 0) && getRegVer() < 1)
	{
		return (INT)WASABI_API_DIALOGBOXPARAMW(IDD_UPSELL_RIPPING, hwndDlg, upsellProc, (LPARAM)hwndDlg);
	}
	return 0;
}
