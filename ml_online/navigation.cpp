...
HRESULT Navigation::Initialize()
{
	...
	{
		ifc_omservice *service;
		nu::PtrList<ifc_omservice> serviceList;
...
{
	Vector<UINT> *orderList = (Vector<UINT>*)context;
...
HRESULT Navigation::Order(ifc_omservice **list, size_t listSize)
{
	if (listSize < 2) return S_FALSE;
...
void Navigation::ImageChanged(LPCWSTR pszName, INT index)
{
	...
	if (libraryTID != currentTID)
	{
		if (NULL != OMUTILITY)
		{
			IMAGEAPCPARAM *param = (IMAGEAPCPARAM*)malloc(sizeof(IMAGEAPCPARAM));
			if (NULL != param)
			{
				ZeroMemory(param, sizeof(IMAGEAPCPARAM));
...
HRESULT Navigation::RemoveItem(HNAVITEM hItem)
{
	if (NULL == hItem) return E_INVALIDARG;
	if (NULL == hLibrary) return E_UNEXPECTED;
...
HRESULT Navigation::RemoveAll()
{
	if (NULL == hRoot || NULL == hLibrary) return E_UNEXPECTED;

	Vector<HNAVITEM> itemList;
...
HRESULT Navigation::PostMainThreadCallback(NavigationCallback callback, ULONG_PTR param)
{
	...
	NAVASYNCPARAM *async = (NAVASYNCPARAM*)malloc(sizeof(NAVASYNCPARAM));
...
HRESULT Navigation::PatchIconName(LPWSTR pszIcon, UINT cchMaxIconLen, ifc_omservice *service)
{
	...
	{
		WCHAR szTemp[2048];
		hr = StringCchCopy(szTemp, ARRAYSIZE(szTemp), pszIcon);
		if (SUCCEEDED(hr))
		{
...
HRESULT Navigation::OnContextMenu(HNAVITEM hItem, HWND hHost, POINTS pts)
{
	...
	if (NULL != hMenu)
	{
		INT commandId = Menu_TrackPopup(hMenu, 
					TPM_LEFTALIGN | TPM_TOPALIGN | TPM_NONOTIFY | TPM_RETURNCMD, 
					pt.x, pt.y, hHost, NULL);

		Menu_ReleaseMenu(hMenu, menuKind);

		if (0 != commandId && FALSE != Command_ProcessService(service, commandId, NULL))
			commandId = 0;
	...
	if (SUCCEEDED(hr))
	{
		switch(pnmkd->wVKey)
		{
			case VK_DELETE:
				{
					BOOL fSuccess;
					Command_ProcessService(service, ID_SERVICE_UNSUBSCRIBE, &fSuccess);
				}
				break;
		}
