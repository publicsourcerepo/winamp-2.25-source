...
BOOL Listbox::GetCheckboxMetrics(HDC hdc, BOOL checked, UINT state, SIZE *pSize)
{
	...
	{		
		...
		INT stateId = GetCheckboxThemeState(checked, state);
		hr = UxGetThemePartSize(buttonTheme, hdc, BP_CHECKBOX, 
...
LRESULT Listbox::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uMsg)
	{
		...
		case WM_MOUSEMOVE:		
			...
			return 0;
		case WM_LBUTTONDOWN:		OnMouseEvent(uMsg, (UINT)wParam, MAKEPOINTS(lParam), FALSE, &SetupListboxItem::LButtonDown); return 0;
		case WM_LBUTTONUP:		OnMouseEvent(uMsg, (UINT)wParam, MAKEPOINTS(lParam), TRUE, &SetupListboxItem::LButtonUp); return 0;
		case WM_LBUTTONDBLCLK:	OnMouseEvent(uMsg, (UINT)wParam, MAKEPOINTS(lParam), FALSE, &SetupListboxItem::LButtonDblClk); return 0;
		case WM_RBUTTONDOWN:		OnMouseEvent(uMsg, (UINT)wParam, MAKEPOINTS(lParam), FALSE, &SetupListboxItem::RButtonDown); return 0;
		case WM_RBUTTONUP:		OnMouseEvent(uMsg, (UINT)wParam, MAKEPOINTS(lParam), TRUE, &SetupListboxItem::RButtonUp); return 0;
		case WM_MOUSELEAVE:		OnMouseLeave(); return 0;
		case WM_CAPTURECHANGED:	OnCaptureChanged((HWND)lParam); break;
		case WM_ERASEBKGND:		OnEraseBkGround((HDC)wParam); return TRUE;
