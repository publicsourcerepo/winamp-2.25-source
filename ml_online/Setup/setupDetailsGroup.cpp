...
typedef struct __GROUPDETAILSCREATEPARAM
{
	SetupGroup *group;
	LPCWSTR name;
}GROUPDETAILSCREATEPARAM;

typedef struct __GROUPDETAILS
{
	SetupGroup *group;
	LPWSTR name;
	HFONT fontTitle;
...
static INT_PTR GroupDetails_OnInitDialog(HWND hwnd, HWND hFocus, LPARAM lParam)
{
	...
	GROUPDETAILS *details = (GROUPDETAILS*)malloc(sizeof(GROUPDETAILS));
	if (NULL == details) return FALSE;
	ZeroMemory(details, sizeof(GROUPDETAILS));
