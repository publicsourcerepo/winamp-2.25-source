...
typedef nu::PtrList<ifc_omservice> ServiceList;
typedef Vector<UINT> ServiceIdList;
...
typedef struct __AppendServiceToStringData
{
	LPCWSTR formatFirst;
	LPCWSTR formatNext;
	LPWSTR cursor;
...
static HRESULT SetupPage_GetFeaturedServicesUrl(LPWSTR pszBuffer, UINT cchBufferMax)
{
	...
	hr = StringCchCopyEx(cursor, remaining, FEATURED_SERVICES_URL, &cursor, &remaining, STRSAFE_NULL_ON_FAILURE);
	if (FAILED(hr))
		return hr;
...
SetupPage::SetupPage() 
	: ref(1), hwnd(NULL), name(NULL), title(NULL), groupList(NULL), completeEvent(NULL), 
...
HRESULT SetupPage::InitializeServices()
{
	...
	hr = setupSvc->GetWinampWnd(&hWinamp);
...
HRESULT SetupPage::Execute(HWND hwndText)
{
	...
	if (S_OK != groupList->FindGroupById(ID_FEATUREDGROUP, &group))
	{
		WCHAR szBuffer[4096];
			SetupPage_GetFeaturedServicesUrl(szBuffer, ARRAYSIZE(szBuffer));
	...
	if (S_OK != groupList->FindGroupById(ID_KNOWNGROUP, &group))
	{
		group = SetupGroup::CreateInstance(ID_KNOWNGROUP, MAKEINTRESOURCE(IDS_SERVICEGROUP_KNOWN), 
						L"*.ini", &SUID_OmStorageIni, &FUID_SetupKnownGroupFilter,
						SetupGroup::styleSortAlphabetically);
...
static INT_PTR SetupPage_ModalLoop(HWND hwnd, HACCEL  hAccel, HANDLE hCancel)
{
	...
	for (;;)
	{
		status = MsgWaitForMultipleObjectsEx(1, &hCancel, INFINITE, QS_ALLINPUT, MWMO_ALERTABLE | MWMO_INPUTAVAILABLE);
