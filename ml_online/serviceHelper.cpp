...
typedef nu::PtrList<ifc_omstorageasync> AsyncList;

typedef struct __SERVICEHELPER
{
...
HRESULT ServiceHelper_Initialize()
{
	...
	serviceHelper = (SERVICEHELPER*)malloc(sizeof(SERVICEHELPER));
	if (NULL == serviceHelper) return E_OUTOFMEMORY;
	ZeroMemory(serviceHelper, sizeof(SERVICEHELPER));
...
HRESULT ServiceHelper_ResetPermissions(ifc_omservice *service, UINT flags)
{
	...
	{
		WCHAR szText[1024], szFormat[512], szName[128];
...
HRESULT ServiceHelper_ResetSubscription(UINT flags)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			{
				LPWSTR pszUrl;
				if (SUCCEEDED(Plugin_BuildActionUrl(&pszUrl, L"remove", removeList.begin(), removeList.size())))
				{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(webStorage->EndLoad(result, &serviceEnum)))
		{
			...
			if (SUCCEEDED(ServiceHelper_QueryStorage(&localStorage)))
			{
				...
				{
					LPWSTR pszUrl;
					if (SUCCEEDED(Plugin_BuildActionUrl(&pszUrl, L"add", registerList.begin(), registerList.size())))
					{
						ServiceHelper_PostNotificationUrl(pszUrl);
						Plugin_FreeString(pszUrl);
					}
...
static void CALLBACK ServiceHelper_VersionCheckComplete(ifc_omstorageasync *result)
{
	...
	while(index--)
	{
		...
		{
			serviceHelper->versionChecks.eraseindex(index);
			break;
		}
	}
