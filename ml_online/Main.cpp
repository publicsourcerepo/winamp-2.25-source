...
typedef Vector<PLUGINTIMERREC, 4> PluginTimerList;
...
void LoadCacheItem( wchar_t *path )
{
	...
	if ( size == sizeof(FILECACHETYPE ))
	{
		...
		if ( cachefile.version == FILECACHEVERSION )
		{
			...
			{
				char *url = (char *)malloc((size_t)cachefile.urllen);
				size = 0;
...
void LoadCache()
{
	...
	HANDLE hFind;
	wchar_t searchs[2048];
	wchar_t activefile[2048];
		...
		if ( buffer_it->second->expire_time > now )
		{
			...
			if (hFile != INVALID_HANDLE_VALUE) 
			{
				...
				if ( size == sizeof(FILECACHETYPE) )
				{
					...
					WriteFile(hFile, (char *)charUrl ,(DWORD)cachefile.urllen, (LPDWORD)&size, NULL);
					WriteFile(hFile, buffer_it->second->get() , (DWORD)buffer_it->second->getlen(), (LPDWORD)&size, NULL);
					WriteFile(hFile, blank , 1, (LPDWORD)&size, NULL);
				}
				else
				{
					CloseHandle(hFile);
...
void initConfigCache()
{
	...
	int x = g_config->ReadInt("maxbandwidth",350);
...
{
	api_service *serviceApi = (api_service*)SENDWAIPC(plugin.hwndWinampParent, IPC_GET_API_SERVICE,0);
	if (FAILED(WasabiApi_Initialize(plugin.hDllInstance, serviceApi)))
		return 1;
