...
HWND CreateWebInfoWindow(HWND hwndParent, UINT uMsgQuery, INT x, INT y, INT cx, INT cy, INT ctrlId)
{
	...
	pbv = (BROWSERVIEW*)malloc(sizeof(BROWSERVIEW));
	if (!pbv) return NULL;
	ZeroMemory(pbv, sizeof(BROWSERVIEW));
	pbv->uMsgQuery = uMsgQuery;
...
static void DrawInfoLabel(DRAWITEMSTRUCT *pds)
{
	...
	len = GetWindowTextW(pds->hwndItem, szText, sizeof(szText)/sizeof(wchar_t));
...
static LRESULT WINAPI WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uMsg)
	{
		...
		case WM_ML_IPC:				return Window_OnMediaLibraryIPC(hwnd, (INT)lParam, (INT_PTR)wParam);	
	}
