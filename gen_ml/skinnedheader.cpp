...
{
	INT rule;
	rule = g_config->ReadInt("column_resize_mode", 0);
	switch(rule)
	{
...
SkinnedHeader::SkinnedHeader(void) : SkinnedWnd(FALSE), hcurNormal(NULL)
{
...
UINT SkinnedHeader::SizeRuleProportional(HDITEMW *phdi, INT index, UINT uMsg)
{
	...
	for(++o; o < hdrSize.count && 0 != delta; o++)
	{
		...
		r =  delta / (hdrSize.count - o) + ((delta % (hdrSize.count - o)) ? ((delta >0) ? 1 : -1) : 0);
		if (0 == r)
			r = delta;
