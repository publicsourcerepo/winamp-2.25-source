...
HMLIMGFLTRMNGR MLImageFilterI_CreateManager(INT cInitial, INT cGrow)
{
	...
	pMngr = (IMAGEFILTERMGNR*)malloc(sizeof(IMAGEFILTERMGNR));
	if (!pMngr) return NULL;
	
	ZeroMemory(pMngr, sizeof(IMAGEFILTERMGNR));
	pMngr->allocstep = (cGrow < 1) ? 1 : cGrow;
	
	if (cInitial > 0)
	{
		if (cInitial > 40) cInitial = 40;
		pMngr->filters = (IMAGEFILTER*)malloc(sizeof(IMAGEFILTER)*cInitial);
		if (!pMngr->filters)
		{
			free(pMngr);
			return NULL;
