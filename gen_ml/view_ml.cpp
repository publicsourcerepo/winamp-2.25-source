...
#define MEDIALIBRARY_HELP_URL		L"http://www.winamp.com/help/Main_Page#The_Media_Library"
...
static void LayoutWindows(HWND hwnd, INT divX, BOOL fRedraw)
{
	...
	static INT controls[] = {IDC_BTN_LIB, IDC_NAVIGATION, IDC_VDELIM, IDC_CURRENTVIEW};
	RECT rc, ri, roTree;
	...
	if (rc.bottom == rc.top || rc.left == rc.right) return;
	if (rc.bottom > 2) rc.bottom -= 2;

	rgn = CreateRectRgn(0,0,0,0);
	
	divCX = 0;

	if (divX > rc.right - 41) divX = rc.right - 9;
	if (divX < 32) divX = 0;
    else if (divX > (rc.right-rc.left)) divX = (rc.right-rc.left);
	...
	for (int i = 0; i < sizeof(controls)/sizeof(controls[0]); i++)
	{
		...
		{
			case IDC_BTN_LIB:
				btnY = max(rc.top, rc.bottom - (ri.bottom - ri.top));
				SETLAYOUTPOS(pl, rc.left, btnY, rc.left + divX, ri.bottom - ri.top);
				break;
			case IDC_NAVIGATION:
				GetClientRect(pl->hwnd, &roTree);
				NavCtrlI_MapPointsTo(hNavigation, hwnd, (LPPOINT)&roTree, 2);
				SETLAYOUTPOS(pl, rc.left, rc.top, max(0, divX - 1), max(0,(btnY - ri.top) - 3));
                ldiv_paintleftborder = (pl->cx > 0);
				break;
			case IDC_VDELIM:
				divCX = ri.right - ri.left;
				SETLAYOUTPOS(pl, divX, rc.top, divCX, max(0,rc.bottom - rc.top));
				break;
			case IDC_CURRENTVIEW: // current view;
				SETLAYOUTPOS(pl, divX + divCX, rc.top, max(0, rc.right - pl->x), max(0,rc.bottom - rc.top));
				if (!SendMessage(pl->hwnd, WM_USER + 0x200, 0, 0)) pl->flags &= ~(SWP_NOREDRAW);
				
				break;
		}

		if (pl->x == ri.left && pl->y == ri.top) pl->flags |= SWP_NOMOVE;
		if (pl->cx == (ri.right - ri.left) && pl->cy == (ri.bottom - ri.top)) pl->flags |= SWP_NOSIZE;
		if ((SWP_NOMOVE | SWP_NOSIZE) != ((SWP_NOMOVE | SWP_NOSIZE) & pl->flags)) pl++;
	...
	if (pl != layout)
	{
		...
		for(pc = layout; pc < pl && (!useDefer || hdwp); pc++)
		{
			...
			if (useDefer) hdwp = DeferWindowPos(hdwp, pc->hwnd, NULL, pc->x, pc->y, pc->cx, pc->cy, pc->flags);
			else SetWindowPos(pc->hwnd, NULL, pc->x, pc->y, pc->cx, pc->cy, pc->flags);
		}
		...
		for(pc = layout; pc < pl; pc++)
		{
			...
			else if (IDC_CURRENTVIEW == pc->id && pc->rgn)  SendMessage(pc->hwnd, WM_USER + 0x201, 0, 0L);
		...
		if (fRedraw) 
		{
			...
			for(pc = layout; pc < pl; pc++)
			{
				...
				{
					if (pc->rgn) OffsetRgn(pc->rgn, pc->x, pc->y);
					else SetRectRgn(rgnTmp, pc->x, pc->y, pc->x + pc->cx, pc->y + pc->cy);
		...
		else if (g_rgnUpdate) 
		{
			...
			for(pc = layout; pc < pl; pc++)
			{
				...
				{	
					if (pc->rgn) OffsetRgn(pc->rgn, pc->x, pc->y);
					else SetRectRgn(rgn, pc->x, pc->y, pc->x + pc->cx, pc->y + pc->cy);
					CombineRgn(g_rgnUpdate, g_rgnUpdate, (pc->rgn) ? pc->rgn : rgn, RGN_OR);
				}
			}
		}

		for(pc = layout; pc < pl; pc++) if (pc->rgn) DeleteObject(pc->rgn);
	}
...
static void CreateCurrentView(HWND hwndDlg)
{
	...
	if (!m_curview_hwnd) m_curview_hwnd = WASABI_API_CREATEDIALOGPARAMW(resourceId, hwndDlg, proc, 0);
...
static INT CALLBACK OnNavItem_CustomDraw(HNAVCTRL hMngr, HNAVITEM hItem, NAVITEMDRAW_I *pnicd, LPARAM lParam)
{
	...
	if (NICDRF_NOTMINE == result) result = NICDRF_DODEFAULT_I;
	return result;
}
...
{
	if (IsChild(h, g_hwnd)) return h; // if the library window is a child of this window, don't touch it
	if (IsChild(h, g_PEWindow)) return g_PEWindow; // if the playlist window is a child of this window, treat it as the playlist window
...
static void Navigation_DropHelper(HWND hwndDlg, HWND hwndNav, HNAVITEM hItemHit, UINT hitFlags, POINT pt)
{
	if (hItemHit && ((NAVHT_ONITEMINDENT_I | NAVHT_ONITEMRIGHT_I | NAVHT_ONITEM_I | NAVHT_ONITEMBUTTON_I) & hitFlags))
	{
		...
		{
			INT sbCommand;
			if (rc.top <= pt.y && pt.y < (rc.top + 24)) sbCommand = 1;
			else if ((rc.bottom - 24) < pt.y && pt.y <= rc.bottom) sbCommand = 2;
			else sbCommand = 0;
			...
			if(sbCommand)
			{
				...
				{
					if (SetTimer(hwndDlg, TIMER_NAVAUTOSCROLL_ID, TIMER_NAVAUTOSCROLL_DELAY, NULL))  m_nav_autoscroll = TRUE;
				}
			}
		}
		...
		if((NAVHT_ONITEMBUTTON_I & hitFlags) && m_nav_autoexpand_item != hItemHit && !NavItemI_IsExpanded(hItemHit))
		{
			...
			if (SetTimer(hwndDlg, TIMER_NAVAUTOEXPAND_ID, TIMER_NAVAUTOEXPAND_DELAY, NULL))  m_nav_autoexpand_item = hItemHit;
		}
	}
}
...
int handleDragDropMove(HWND hwndDlg, int type, POINT p, int do_cursors)
{
	...
	{		
		...
		else if (h == NavCtrlI_GetHWND(hNavigation))
		{
			...
			if (hItemHit && hItemHit != hItemSel && (!m_query_moving_item || NavItemI_GetParent(m_query_moving_item) != hItemHit)) 
			{
				...
				if (valid) g_treedrag_lastSel = hItemHit;
			}
		}
		else if (NULL != m_curview_hwnd && 
				 NULL != h && 
				 (h == m_curview_hwnd || IsChild(m_curview_hwnd, h)))
		{
			...
			while (1)
			{
				...
				if (dis.result || h == m_curview_hwnd) break;
	...
	if (g_treedrag_lastSel != hItemPrevHilited)
	{
		...
		if (hItemPrevHilited) NavItemI_SetState(hItemPrevHilited, 0, NIS_DROPHILITED_I);
		if (g_treedrag_lastSel) NavItemI_SetState(g_treedrag_lastSel, NIS_DROPHILITED_I, NIS_DROPHILITED_I);
		NavCtrlI_Update(hNavigation);
		ImageList_DragShowNolock(TRUE);
	}

	if (do_cursors) SetCursor((valid) ? hDragNDropCursor : LoadCursor(NULL, IDC_NO));
	return valid;
}
...
static BOOL CALLBACK RatingTweak_OnApplyChanges(UINT fStyle, BOOL bClosing)
{
	...
	g_config->WriteInt("rating_style", ratingGlobalStyle);
	
	MLRatingColumnI_Update();
	if (m_curview_hwnd)	PostMessage(m_curview_hwnd, WM_DISPLAYCHANGE, 0, MAKELPARAM(0, 0));
	if (bClosing) hwndTweak = NULL;
	return TRUE;
}
...
static void OnTimer_NavAutoScroll(HWND hwndDlg)
{
	...
	{
		if (rc.top <= pt.y && pt.y < (rc.top + 24)) command = 1;
		else if ((rc.bottom - 24) < pt.y && pt.y <= rc.bottom) command = 2;
	}
	...
	if (command)
    {
		...
		if ((1 == command && si.nMin == si.nPos) || (2 == command && si.nMax < (INT)(si.nPos + si.nPage))) command = 0;
		else
		{
			ImageList_DragShowNolock(FALSE);
	...
	if (!command) m_nav_autoscroll = FALSE;
}
...
static void OnTimer_NavAutoExpand(HWND hwndDlg)
{
	...
	if (!hwndNav || !IsWindowVisible(hwndNav)) return;
...
static void OnMouseMove(HWND hwndDlg, POINT pt, UINT flags)
{
	...
	if (hWndHit == NavCtrlI_GetHWND(hNavigation))
	{
		...
		if ((NAVHT_ONITEMINDENT_I | NAVHT_ONITEMRIGHT_I | NAVHT_ONITEM_I | NAVHT_ONITEMBUTTON_I) & hitFlags)
		{
			...
			if (tempItem == m_query_moving_item) hItem = m_query_moving_item;
...
static void OnLButtonUp(HWND hwndDlg, POINT pt, UINT flags)
{
		...
		if (m_query_moving_dragplace)
		{
			...
			else
			{
				...
				{	
					...
					if (!whereinsert && !fTest) 	whereinsert = NavItemI_GetParent(m_query_moving_lastdest);
				}
				if (whereinsert && m_query_moving_item != whereinsert) plugin_SendMessage(ML_MSG_TREE_ONDROP, NavItemI_GetId(m_query_moving_item), (INT_PTR)&pt,  NavItemI_GetId(whereinsert));
			}
		}
		else plugin_SendMessage(ML_MSG_TREE_ONDROP, NavItemI_GetId(m_query_moving_item), (INT_PTR)&pt, 0);
	}
	m_query_moving = 0;
	...
	if (m_query_moving_dragplace) CancelNavigationMove();
	ReleaseCapture();
...
{	
	...
	{		
		BOOL fStored = (0 != g_config->ReadInt("visible", 1));
		if (fShow != fStored)
		{
	...
	{
		if (NULL != g_config)
			g_config->WriteInt("visible", (FALSE != fShow));
...
{
	if (NULL != helpInfo && 
		HELPINFO_WINDOW == helpInfo->iContextType)
	{
		...
		if (navigationWindow == helpInfo->hItemHandle)
		{
			...
			if (NULL != selectedItem &&
				0 != plugin_SendMessage(ML_MSG_NAVIGATION_HELP, 
										(INT_PTR)selectedItem, (INT_PTR)navigationWindow, 
										MAKELONG(helpInfo->MousePos.x, helpInfo->MousePos.y)))
			{
				return TRUE;
			}
		}
...
INT_PTR CALLBACK dialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		...
		case WM_LBUTTONUP:
			{
				...
				if (WM_MOUSEMOVE == uMsg) OnMouseMove(hwndDlg, pt, (UINT)wParam);
				else if (WM_LBUTTONUP == uMsg) OnLButtonUp(hwndDlg, pt, (UINT)wParam);
			}
			return TRUE;
		...
		case WM_ML_IPC:				return OnMediaLibraryIPC(hwndDlg, wParam, lParam);
		...
		case WM_USER + 31:
			...
			if(!lParam) PostMessage(hwndDlg, WM_USER + 31, wParam, 1);
			else
			{
...
{
	if (main_sendtomenu || !main_sendto_hmenu || menu != main_sendto_hmenu) return ;
...
void OnDisplayChange(HWND hwndDlg, UINT imgDepth, UINT hRes, UINT vRes)
{
	...
	ratingGlobalStyle = g_config->ReadInt("rating_style", RATING_DEFAULT_STYLE);
	MLRatingColumnI_Update();
	...
	RedrawWindow(hwndDlg, NULL, NULL, RDW_INVALIDATE | RDW_ERASE | RDW_FRAME | RDW_ALLCHILDREN);
}
...
int OnInitDialog(HWND hwndDlg)
{
	...
	if (S_OK == MlDwm_LoadLibrary())
	{
		...
		hr = MlDwm_SetWindowAttribute(GetParent(hwndDlg), DWMWA_NCRENDERING_POLICY, &ncrp, sizeof(ncrp));
		hr = MlDwm_SetWindowAttribute(GetParent(hwndDlg), DWMWA_ALLOW_NCPAINT, &allow, sizeof(allow));
	}

	MLSkinWindow2(hwndDlg, hwndDlg, SKINNEDWND_TYPE_DIALOG, SWS_USESKINCOLORS | SWS_USESKINCURSORS);
	hctrl = GetDlgItem(hwndDlg, IDC_BTN_LIB);
	MLSkinWindow2(hwndDlg, hctrl, SKINNEDWND_TYPE_BUTTON, SWS_USESKINCOLORS | SWS_USESKINCURSORS);
	...
	if (hNavigation) SetWindowPos(GetDlgItem(hwndDlg, IDC_BTN_LIB), NavCtrlI_GetHWND(hNavigation), 0,0,0,0, SWP_NOACTIVATE | SWP_NOSIZE | SWP_NOMOVE);

	divider_pos = g_config->ReadInt("ldivpos", 135);
	...
	HACCEL hAccel;
	hAccel = WASABI_API_LOADACCELERATORSW(IDR_ACCELERATOR_GLOBAL);
	if (hAccel) WASABI_API_APP->app_addAccelerators(hwndDlg, &hAccel, 1, TRANSLATE_MODE_GLOBAL); 
	hAccel = WASABI_API_LOADACCELERATORSW(IDR_ACCELERATOR_MAIN);
	if (hAccel) WASABI_API_APP->app_addAccelerators(hwndDlg, &hAccel, 1, TRANSLATE_MODE_CHILD); 
...
static void OnDestroy(HWND hwndDlg)
{
	...
	{
		wchar_t name[1024];
		if (NavItemI_GetFullName(hItem, name, 1024)) g_config->WriteString("last_view", AutoChar(name, CP_UTF8));
	}
	g_config->WriteInt("ldivpos", divider_pos);
	...
	if (firstShow && fShow)
	{
		...
		if (lastTitleA && *lastTitleA)
		{
			...
			if (MultiByteToWideCharSZ(CP_UTF8, 0, lastTitleA, -1, textW, 2048)) 
				hDefItem = NavCtrlI_FindItemByFullName(hNavigation, NICF_LOCALE_INVARIANT(is_xp_plus), NICF_INVARIANT_I | NICF_DISPLAY_I | NICF_IGNORECASE_I, textW, -1, TRUE);
		}

		if (!hDefItem) hDefItem = NavCtrlI_GetRoot(hNavigation);
