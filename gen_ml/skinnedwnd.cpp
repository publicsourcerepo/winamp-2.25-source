...
SkinnedWnd::SkinnedWnd(BOOL bIsDialog) 
	: hwnd(NULL), fnWndProc(NULL), style(SWS_NORMAL), redrawLock(0),
	  wnddata(SKINNEDWND_TYPE_WINDOW | ((bIsDialog) ? SWS_DIALOG : 0))
{
	minSize.cx = 0;
	minSize.cy = 0;
	maxSize.cx = 0;
...
BOOL SkinnedWnd::Attach(HWND hwndToSkin)
{
	...
	if (S_OK == MlDwm_LoadLibrary())
	{
		...
		hr = MlDwm_SetWindowAttribute(hwnd, DWMWA_NCRENDERING_POLICY, &ncrp, sizeof(ncrp));
		hr = MlDwm_SetWindowAttribute(hwnd, DWMWA_ALLOW_NCPAINT, &allow, sizeof(allow));
	}
...
{
	SkinnedWnd *pWnd;
	pWnd = (SkinnedWnd*)GetPropW(hwnd, WNDDATAPROPW);
	if (!pWnd)  return (IsWindowUnicode(hwnd)) ? DefWindowProcW(hwnd, uMsg, wParam, lParam) : DefWindowProcA(hwnd, uMsg, wParam, lParam);
