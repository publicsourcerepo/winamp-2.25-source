...
extern "C"
{
	...
	winampGeneralPurposePlugin plugin =
	    {
			GPPHDR_VER_U,
	        "nullsoft(gen_ml.dll)", // will be set in  winampGetGeneralPurposePlugin
	        init,
	        config,
	        quit,
	    };
...
void toggleVisible(int closecb)
{
	...
	if (FALSE != fVisible || 1 == closecb)
	{
		if (FALSE == fMinimized && FALSE != fVisible)
		{
			...
			if (hwndFocus == g_ownerwnd || IsChild(g_ownerwnd, hwndFocus))
				SendMessage(plugin.hwndParent, WM_COMMAND, WINAMP_NEXT_WINDOW, 0);
	...
	if (FALSE != fMinimized && 1 != closecb)
	{
		...
		if (NULL != g_config)
			g_config->WriteInt("visible", (FALSE == fVisible));
...
static BOOL Winamp_OnIPC(HWND hwnd, UINT uMsg, INT_PTR param, LRESULT *pResult)
{
	...
	switch(uMsg)
	{
		...
		case IPC_CB_GETTOOLTIPW:
			if (param == 16 && g_config->ReadInt("attachlbolt", 0))
			{
		...
		case IPC_HOOK_TITLESW:
			...
			break;
		case IPC_SETDIALOGBOXPARENT: remote_dialog_parent = (HWND)param; break;
		case IPC_ADD_PREFS_DLGW:
		case IPC_ADD_PREFS_DLG:
			if (param && !((prefsDlgRec*)param)->where)
			{
				prefsDlgRec *p = (prefsDlgRec *)param;
				// now lets check if it is our plugin
				int i = m_plugins.GetSize();
				while (i-- > 0)
				{
					winampMediaLibraryPlugin *mlplugin = (winampMediaLibraryPlugin *)m_plugins.Get(i);
					if (mlplugin->hDllInstance == p->hInst) 
					{
						p->where = (intptr_t)(INT_PTR)&myPrefsItem;
						break;
					}
					// handles localised versions of the ml preference pages (if enabled and loaded successfully)
					// since it has to be that mlplugin->hDllInstance != p->hInst for those cases so do filename
					else
					{
						char filename[MAX_PATH], *f = 0, *ff = 0, lngname[MAX_PATH], *l = 0, *ll = 0;
						unsigned int result;
						result = GetModuleFileName(mlplugin->hDllInstance, filename, ARRAYSIZE(filename));
						if (0 != result && ARRAYSIZE(filename) != result)
						{
							result = GetModuleFileName(p->hInst,lngname, ARRAYSIZE(lngname));
							if (0 != result && ARRAYSIZE(lngname) != result)
							{
								*(f = scanstr_back(filename, ".", filename)) = 0;
								*(l = scanstr_back(lngname, ".", lngname)) = 0;
								if(!lstrcmpi(scanstr_back(filename, "\\/", filename),
											scanstr_back(lngname, "\\/", lngname)))
								{
									p->where = (intptr_t)(INT_PTR)&myPrefsItem;
									break;
								}
...
static LRESULT WINAPI wa_newWndProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch(uMsg)
	{
		...
		case WM_SIZE:
			if (wParam == SIZE_RESTORED)
			{
				if (FALSE != MlWindow_IsMinimizedMode())
				{
					...
					showCommand = (0 != g_config->ReadInt("visible", 1)) ? 
										SW_SHOWNA : SW_HIDE;
		...
		case WM_SYSCOMMAND:
			{
				...
				{
					if (lowP != WINAMP_LIGHTNING_CLICK || g_config->ReadInt("attachlbolt", 0))
					{
						toggleVisible();
						return 0;
					}
		...
		case WM_DWMCOMPOSITIONCHANGED:
			if (g_hwnd && IsWindow(g_hwnd)) PostMessageW(g_hwnd, WM_DWMCOMPOSITIONCHANGED, 0, 0L);
			break;
	}
	...
	if (winampIsUnicode)
		...
		return CallWindowProcA(wa_oldWndProc, hwndDlg, uMsg, wParam, lParam);
}
...
BOOL init2(void)
{
	if (!g_hwnd)
	{
		...
		//xp theme disabling shit
		m_uxdll = LoadLibrary("uxtheme.dll");
		if (m_uxdll)
		{
		...
		myWindowState.r.left = g_config->ReadInt("mw_xpos", 301);
		myWindowState.r.top = g_config->ReadInt("mw_ypos", 29);
		myWindowState.r.right = myWindowState.r.left + g_config->ReadInt("mw_width", 500);
		myWindowState.r.bottom = myWindowState.r.top + g_config->ReadInt("mw_height", 348);
...
int init()
{
	...
	char *dir = (char*)SendMessage(plugin.hwndParent, WM_WA_IPC, 0, IPC_GETINIDIRECTORY);
	if ((size_t) dir < 65536)	return 1;
	PathCombine(g_path, dir, "Plugins");
	CreateDirectory(g_path, NULL);

	dir = (char*)SendMessage(plugin.hwndParent, WM_WA_IPC, 0, IPC_GETPLUGINDIRECTORY);
	if (dir == (char *)1 || dir == 0)
		lstrcpyn(pluginPath, g_path, MAX_PATH);
	else
		lstrcpyn(pluginPath, dir, MAX_PATH);
	...
	char configName[1024 + 32];
	StringCchPrintfA(configName, 1024 + 32, "%s\\gen_ml.ini", g_path);
	g_config = new C_Config(configName);
	config_use_ff_scrollbars = g_config->ReadInt("ffsb", 1);
	config_use_alternate_colors = g_config->ReadInt("alternate_items", 1);
	
	int vis = g_config->ReadInt("visible", 1);
	...
	if (wa_main_menu || wa_windows_menu)
	{
		if (wa_main_menu)
		{
			MENUITEMINFOW i = {sizeof(i), MIIM_ID | MIIM_STATE | MIIM_TYPE, MFT_STRING, vis ? MFS_CHECKED : 0, WA_MENUITEM_ID};
	...
	wa_oldWndProc = (WNDPROC)(LONG_PTR) ( (winampIsUnicode) ? SetWindowLongPtrW(plugin.hwndParent, GWLP_WNDPROC, (LONGX86)(LONG_PTR)wa_newWndProc) :
															SetWindowLongPtrA(plugin.hwndParent, GWLP_WNDPROC, (LONGX86)(LONG_PTR)wa_newWndProc));
	...
	myPrefsItem.where = -1; // to become root based item
	...
	char mldir[MAX_PATH];
	PathCombine(mldir, g_path, "ml");
	CreateDirectory(mldir, NULL);
	PathCombine(mldir, mldir, "views");
	CreateDirectory(mldir, NULL);
...
void quit()
{
	...
	if (g_ownerwnd)
	{
		g_config->WriteInt("mw_xpos", myWindowState.r.left);
		g_config->WriteInt("mw_ypos", myWindowState.r.top);
		g_config->WriteInt("mw_width", myWindowState.r.right - myWindowState.r.left);
		g_config->WriteInt("mw_height", myWindowState.r.bottom - myWindowState.r.top);
