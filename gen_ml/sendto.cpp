...
void SendToMenu::onAddItem(mlAddToSendToStruct *ptr)
{
	...
	InsertMenuItem(_hm,_pos++,TRUE,&mii);
}
	...
	else if (activePlaylist && id == activePlaylist)
	{
		...
		else if (type == ML_TYPE_ITEMRECORDLIST)
		{
			...
			for (int i=0;i < obj->Size;i++)
			{
				...
				TAG_FMT_EXT(wfn, itemrecordTagFunc, fieldTagFuncFree, (void*)&obj->Items[i], title, 400, 1);
		...
		else if (type == ML_TYPE_ITEMRECORDLISTW)
		{
			...
			for (int i=0;i < obj->Size;i++)
			{
				TAG_FMT_EXT(obj->Items[i].filename, itemrecordWTagFunc, fieldTagFuncFree, (void*)&obj->Items[i], title, 400, 1);
				enqueueFileWithMetaStructW enqueueFile;
