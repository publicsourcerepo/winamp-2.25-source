	...
	if (NULL != (hctrl = GetDlgItem(hdlg, IDC_LBL_ADDRESS)) && GetWindowRect(hctrl, &rw))
	{
		...
		{			
			...
			{				
				...
				WCHAR szText[128];
				INT cch;
				cch = (INT)SendMessageW(hctrl, WM_GETTEXT, (WPARAM)(sizeof(szText)/sizeof(szText[0])), (LPARAM)szText);
...
static INT FileView_GetPreferredColumnWidth(HWND hdlg, UINT columnId)
{
	...
	{
		pszText = pfd->pRec[i].Info.cFileName;
...
static BOOL FileView_OnInitDialog(HWND hdlg, HWND hwndFocus, LPARAM lParam)
{
	FILEVIEW *pfv;
	RECT rw;
	pfv = (FILEVIEW*)malloc(sizeof(FILEVIEW));
	if (pfv)
	{
		ZeroMemory(pfv, sizeof(FILEVIEW));
		pfv->style = ((UINT)-1);
		pfv->bAscending = TRUE;
		pfv->sortColumn = -1;
		pfv->yDivider = 240;
...
static BOOL FileView_OnGetStatusText(HWND hdlg, LPWSTR pszText, INT cchTextMax)
{
	...
	WCHAR szTemplate[256], szSize[64];
...
static INT FileView_OnEnqueueSelection(HWND hdlg, UINT uEnqueueFilter, INT *pnFocused)
{
	size_t index;
	FILERECORD *pfr;
	wchar_t szPath[2*MAX_PATH], szAtf[2048];

	enqueueFileWithMetaStructW efs;
	waFormatTitleExtended ft;
