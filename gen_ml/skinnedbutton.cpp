...
#define SPLITAREA_WIDTH	15
#define SPLITAREA_SPACE	2
...
COLORREF SkinnedButton::GetButtonBkColor(BOOL bPressed, BOOL *pbUnified)
{
	...
	return RGB(r/dib.cy, g/dib.cy, b/dib.cy);
...
static void PushButton_CalulateRects(BUTTONPAINTSTRUCT *pbps, RECT *prcText, RECT *prcImage, INT imageWidth, INT imageHeight)
{
	...
	if (0 != (0x0F & pbps->textFormat))
	{
		...
		else if (DT_CENTER & pbps->textFormat) 
		{
			...
			if ((SWBS_DROPDOWNBUTTON | SWBS_SPLITBUTTON) & pbps->skinnedStyle)
			{
	...
	else if (SWBS_DROPDOWNBUTTON & pbps->skinnedStyle) lim -= (SPLITAREA_WIDTH + SPLITAREA_SPACE);
	else lim -= MARGIN_RIGHT;
...
{	
	...
	if ((SWBS_SPLITBUTTON | SWBS_DROPDOWNBUTTON) & pbps->skinnedStyle)
	{
		...
		if (hmlilSplitter)
		{
			...
			left = pbps->rcClient.right - SPLITAREA_WIDTH;
			...
			if (SWBS_SPLITBUTTON  & pbps->skinnedStyle)
			{
				...
				penOld = (HPEN)SelectObject(pbps->hdc, pen);
...
LRESULT SkinnedButton::GetIdealSize(LPCWSTR pszText)
{
	...
	{	HFONT hFont, hfo;
		...
		if (hdc)
		{
			...
			hFont = (HFONT)SendMessageW(hwnd, WM_GETFONT, 0, 0L);
	...
	if ((SWBS_DROPDOWNBUTTON | SWBS_SPLITBUTTON) & style) szButton.cx += (SPLITAREA_WIDTH + SPLITAREA_SPACE);
...
LRESULT SkinnedButton::WindowProc(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (SWS_USESKINCOLORS & style)
	{
		switch(uMsg)
		{
			...
			case WM_ENABLE:
				DisableRedraw();
				r = __super::WindowProc(uMsg, wParam, lParam);
				EnableRedraw(SWR_INVALIDATE | SWR_UPDATE);
				return r;
