...
static INT CompareItemName(LCID Locale, UINT compFlags, LPCWSTR pszString, INT cchLength, HNAVITEM hItem)
{
	...
	if (NICF_INVARIANT_I & compFlags)
	{
		result = (((NAVITM*)hItem)->pszInvariant) ? 
				CompareStringW(NICF_LOCALE_INVARIANT(is_xp_plus), NORM_IGNORECASE, ((NAVITM*)hItem)->pszInvariant, -1, pszString, cchLength) :
				CSTR_LESS_THAN;
...
static BOOL CALLBACK SaveNavigationCB(HWND hwndTree, HTREEITEM hItem, LPVOID user)
{
	...
	{	
  		bCollapsed = (SendMessageW(hwndTree, TVM_GETNEXTITEM, (WPARAM)TVGN_CHILD, (LPARAM)hItem) && !(TVIS_EXPANDED & ((SAVEVIEWDATA*)user)->item.state));
		...
		{
			remain = NavItemI_GetFullName((HNAVITEM)((SAVEVIEWDATA*)user)->item.lParam, ((SAVEVIEWDATA*)user)->buffer, SAVEVIEW_BUFFER_MAX);
			if (remain)
			{
				if ((order != -1 || bCollapsed) && remain < (SAVEVIEW_BUFFER_MAX - 8) )
				{
					if (S_OK == StringCchPrintfW(((SAVEVIEWDATA*)user)->buffer + remain, SAVEVIEW_BUFFER_MAX - remain, L",%d,%d", bCollapsed, order) &&
						S_OK == StringCchPrintfA(((SAVEVIEWDATA*)user)->mlview, ML_VIEW_MAX, MLV_PREFIX_A "%02d", (((SAVEVIEWDATA*)user)->counter_write + 1)))
					{
						((SAVEVIEWDATA*)user)->pConfig->WriteString(((SAVEVIEWDATA*)user)->mlview, AutoChar(((SAVEVIEWDATA*)user)->buffer, CP_UTF8));
...
static NAVITEMSTATEREC *NavCtrlI_FindItemStateRec(HNAVCTRL hNav, HNAVITEM hItem, BOOL fClearOnSuccess)
{
	...
	{
		for (idx = 0; idx < ((NAVMNGR*)hNav)->statesCount; idx++)
		{
...
static LRESULT TreeViewWndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	if (!pTree) return (IsWindowUnicode(hwnd)) ? DefWindowProcW(hwnd, uMsg, wParam, lParam) : DefWindowProcA(hwnd, uMsg, wParam, lParam);
	switch(uMsg)
	{
		case WM_NCDESTROY: // detach
			RemovePropW(hwnd, WNDPROP_SCCTRLW);
...
static BOOL SubclassEditControl(HWND hwndEdit)
{
	...
	SCEDIT *pEdit = (SCEDIT*)malloc(sizeof(SCEDIT));
	if (!pEdit) return FALSE;
	ZeroMemory(pEdit, sizeof(SCEDIT));
...
static BOOL SubclassTreeView(HWND hwndTree)
{
	...
	SCTREE *pTree = (SCTREE*)malloc(sizeof(SCTREE));
	if (!pTree) return FALSE;
	ZeroMemory(pTree, sizeof(SCTREE));
...
static BOOL NavCtrlI_SaveStates(HNAVCTRL hNav)
{
	...
	((NAVMNGR*)hNav)->pConfig->WriteInt(MLV_PREFIX_A "count", save.counter_write);
...
static BOOL NavCtrlI_LoadStates(HNAVCTRL hNav)
{
	...
	count  = ((NAVMNGR*)hNav)->pConfig->ReadInt(MLV_PREFIX_A "count", 0);
	if (count < 1) return TRUE;

	((NAVMNGR*)hNav)->pStates = (NAVITEMSTATEREC*)malloc(sizeof(NAVITEMSTATEREC)*count);
	if (!((NAVMNGR*)hNav)->pStates) return FALSE;

	for (index = 0; index < count; index++)
	{
		StringCchPrintfA(mlview, ML_VIEW_MAX, MLV_PREFIX_A "%02d", index + 1);
		...
		if (data && *data)
		{
			...
			if (len > 4 && len < 4096)
			{
				...
				if (len)
				{
					...
					if (len > 1) 
					{
						((NAVMNGR*)hNav)->pStates[index].pszFullName = (LPWSTR)	malloc(len * sizeof(wchar_t));
...
static BOOL OnTV_SetFocus(HNAVCTRL hNav,  NMHDR *pnmh)
{
	...
	if (0 == (NAVSTYLE_FOCUSED & manager->style))
	{
		...
		selectedItem = NavCtrlI_GetSelection(hNav);
...
HNAVCTRL NavCtrlI_Create(HWND hwndParent)
{
	...
	pMngr = (PNAVMNGR)malloc(sizeof(NAVMNGR));
	if (!pMngr) return NULL;

	ZeroMemory(pMngr, sizeof(NAVMNGR));
	...
	if (!pMngr->hwndHost || !SetPropW(pMngr->hwndHost, NAVMGR_HWNDPROPW, pMngr))  
	{
	...
	// check if we're on xp+ (test loading of uxtheme.dll will suffice as that's xp+ only)
	is_xp_plus = !SendMessage(plugin.hwndParent,WM_WA_IPC,IPC_ISWINTHEMEPRESENT,IPC_USE_UXTHEME_FUNC);
...
BOOL NavCtrlI_UpdateLook(HNAVCTRL hNav)
{
	...
	if (((NAVMNGR*)hNav)->pConfig)
	{
		...
		height = ((NAVMNGR*)hNav)->pConfig->ReadInt("Navigation_ItemHeight", 18);
		...
		((NAVMNGR*)hNav)->style = (((NAVMNGR*)hNav)->style & ~NAVSTYLE_MOUSEDOWNSELECT) | ((0 != ((NAVMNGR*)hNav)->pConfig->ReadInt("Navigation_MouseDownSel", 0)) ? NAVSTYLE_MOUSEDOWNSELECT : 0);
		fFullRowSelect = (((NAVMNGR*)hNav)->pConfig->ReadInt("Navigation_FullRowSel", 1) != 0);
...
HMLIMGLST NavCtrlI_SetImageList(HNAVCTRL hNav, HMLIMGLST hmlil)
{
	...
	if (!((NAVMNGR*)hNav)->pConfig || ((NAVMNGR*)hNav)->pConfig->ReadInt("Navigation_ShowIcons", 1))
	{
...
HNAVITEM NavCtrlI_FindItemByFullName(HNAVCTRL hNav, LCID Locale, UINT compFlags, LPCWSTR pszFullName, INT cchLength, BOOL fAncestorOk)
{
	...
	WCHAR shortname[ITEM_SHORTNAME_MAX];
	LPCWSTR end;
	TVITEMW treeItem;
	HNAVITEM hNavParent;
	...
	{	
		...
		{	
			if (separatorCount)
			{
				...
				{	
					if (CSTR_EQUAL != CompareStringW(NICF_LOCALE_INVARIANT(is_xp_plus), NORM_IGNORECASE, EMPTY_ITEM_NAME, -1, shortname, len))
					{ 
...
HNAVITEM NavCtrlI_InsertItem(HNAVCTRL hNav, HNAVITEM hInsertAfter, HNAVITEM hParent, NAVITEM_I *pnis)
{
	...
	pNavItem = (NAVITM*)malloc(sizeof(NAVITM));
	if (!pNavItem) return NULL;
	ZeroMemory(pNavItem, sizeof(NAVITM));
			...
			if (((NAVMNGR*)hNav)->lockSelected)
			{
				...
				*(HTREEITEM*)&rc = ((NAVMNGR*)hNav)->lockSelected;
				...
				else if (((NAVMNGR*)hNav)->lockSelPos != rc.top)
				{
					WPARAM wCmd;
					INT iHeight;
					iHeight = (INT)SendMessageW(((NAVMNGR*)hNav)->hwndHost, TVM_GETITEMHEIGHT , 0, 0L);

					if (iHeight)
					{
						INT pos, oldPos;
						if (((NAVMNGR*)hNav)->lockSelPos > rc.top) iHeight = 0 - iHeight;
						wCmd = MAKEWPARAM((((NAVMNGR*)hNav)->lockSelPos > rc.top) ? SB_LINEUP : SB_LINEDOWN, 0);
						oldPos = 0xFFFFFF;
