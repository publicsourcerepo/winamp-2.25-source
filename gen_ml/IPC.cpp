...
static INT_PTR IPC_InsertView(UINT msg, INT_PTR param)
{
	...
	switch(msg)
	{
		...
		case ML_IPC_TREEITEM_ADDW:
			if (ML_IPC_TREEITEM_ADD == msg || ML_IPC_TREEITEM_ADDW == msg) nis.id = (INT)((MLTREEITEMW*)param)->id;
			else insertAfterId= (INT)((MLTREEITEMW*)param)->id;
			if (!converted) nis.pszText = ((MLTREEITEMW*)param)->title;
	...
	if (hItem)
	{
		if (ML_IPC_ADDTREEITEM ==msg || ML_IPC_ADDTREEITEM_EX == msg) ((mlAddTreeItemStruct*)param)->this_id = NavItemI_GetId(hItem);
		else ((MLTREEITEMW*)param)->id = NavItemI_GetId(hItem);
	}
...
static INT_PTR IPC_SendToWinamp(INT_PTR param)
{
	...
	{
		int enq = p->enqueue & 1;
		if (!(p->enqueue & 2) && g_config->ReadInt("enqueuedef", 0)) enq ^= 1;
...
static INT_PTR IPC_HandleDrop(INT_PTR param)
{
	if (param)
	{
		...
		{
			int enqueue = (!!g_config->ReadInt("enqueuedef", 0)) ^(!!(GetAsyncKeyState(VK_SHIFT) & 0x8000));
		...
		else if (h == NavCtrlI_GetHWND(hNavigation))
		{
			...
			{
				itemId = NavItemI_GetId(hItemHit);
...
{
	if (!pnavInsert || pnavInsert->item.cbSize != sizeof(NAVITEM)) return NULL;
	...
	switch (msg)
	{
		...
		case ML_IPC_SKIN_WADLG_GETFUNC:
			switch (param)
			{
				...
				case 66:
					if (g_config->ReadInt("plfont_everywhere", 1))
					{
		...
		case ML_IPC_REMOVE_PLUGIN:
			if (param)
			{
				...
				if (msg == ML_IPC_ADD_PLUGIN && p->hDllInstance)
				{
					...
					HMODULE dummyLoad = LoadLibrary(filename);
		...
		case ML_IPC_GET_PARENTAL_RATING: // TODO: store this in api_config
			return g_config->ReadInt("tvrating", 7);
