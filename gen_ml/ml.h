...
typedef struct {
	int version;	// changed 5.7+ - MLHDR_VER will now require a unicode (wchar_t*) description and only work correctly on 5.7+
					...
					//						 so when using MLHDR_VER = 0x16 you will need to cast description to (wchar_t*) to set
	char *description;
...
// return TRUE to block the closing of Winamp (relates to IPC_HOOK_OKTOQUIT)
...
** HMENU media_library_menu = (HMENU)SendMessage(plugin.hwndLibraryParent,WM_ML_IPC,(WPARAM)&mgts,ML_IPC_GETTREE);
...
** HMENU playlists_menu = (HMENU)SendMessage(plugin.hwndLibraryParent,WM_ML_IPC,(WPARAM)&mgts,ML_IPC_GETTREE);
** if(playlists_menu){
**   // show or insert the menu as applicable
** }
*/
...
typedef struct {
	char *desc;			// str (addition 5.7+ - set as L"-" to create a separator and begin with a # to show as grayed)
...
typedef struct {
	wchar_t *desc;		// str (addition 5.7+ - set as L"-" to create a separator and begin with a # to show as grayed)
	intptr_t context;	// context passed by ML_MSG_ONSENDTOBUILD
	intptr_t user32;	// use some unique ptr in memory, you will get it back in ML_MSG_ONSENDTOSELECT...
...
// user32 is unused
// note: the 5.7+ additions for separators and greyed items do not apply to the branch node
...
#define ML_IPC_DB_RUNQUERY_INDEX 0x703 // retrieves item #(int)query [deprecated and doesn't work]
...
#define ML_IPC_FOCUS_TREE 0x757
