...
typedef Vector<PATHID>			PATHLIST;
typedef nu::PtrList<METAINFO>	METARECORS;
...
typedef nu::PtrDeque<DISCOVERJOB> DISCOVERDEQUE;
...
{
public:
	PlMetaLoader(PLAYLISTMETA *plm) { this->plm = plm; plm->nCount = 0; plm->nLength = 0; }
	~PlMetaLoader(){};
...
public:
	void OnFile(const wchar_t *filename, const wchar_t *title, int lengthInMS, ifc_plentryinfo *info) 
	{
		if (plm->nCount < sizeof(plm->szEntries)/sizeof(plm->szEntries[0]))
		{
			...
			if (title && *title) pe->pszTitle = _wcsdup(title);
			else if (filename && *filename) 
			{
				pe->pszTitle = _wcsdup(filename);
			}
			else pe->pszTitle = NULL;
	...
	const wchar_t *GetBasePath() { return L"."; }
...
__inline static int __cdecl FileViewMeta_SearhMetaInfo(const void *elem1, const void *elem2)
{
	...
	if (pKey->pathId != (*((METAINFO**)elem2))->pathId) return (pKey->pathId - (*((METAINFO**)elem2))->pathId);
	return (CompareStringW(CSTR_INVARIANT, NORM_IGNORECASE, 
...
static INT FileViewMeta_GetPathId(LPCWSTR pszPath)
{
	...
	if (metaDb.lastPathIndex < metaDb.pPath.size() && 
		CSTR_EQUAL == CompareStringW(CSTR_INVARIANT, NORM_IGNORECASE, pszPath, -1,
									metaDb.pPath.at(metaDb.lastPathIndex).pszPath, -1)) psr = &metaDb.pPath.at(metaDb.lastPathIndex);
	else psr = (PATHID*)bsearch(pszPath, metaDb.pPath.begin(), metaDb.pPath.size(), sizeof(PATHID), FileViewMeta_SearhPath);
...
static METAINFO *FileViewMeta_GetMeta(LPCWSTR pszPath, LPCWSTR pszFileName, LPCWSTR pszExt, BOOL bCreateIfNotFound)
{
	...
	METAINFO **pr = (METAINFO**)bsearch(&key, metaDb.pRec.begin(), metaDb.pRec.size(), sizeof(METAINFO*), FileViewMeta_SearhMetaInfo);
	if (pr) return *pr;
		
	if (!bCreateIfNotFound) return NULL;
	
	METAINFO *pInfo = (METAINFO*)malloc(sizeof(METAINFO));
	ZeroMemory(pInfo, sizeof(METAINFO));
	pInfo->pathId		= key.pathId;
	pInfo->pszFileName = _wcsdup(key.pszFileName);
	metaDb.pRec.push_back(pInfo);
	qsort(metaDb.pRec.begin(), metaDb.pRec.size(), sizeof(METAINFO*), FileViewMeta_SortMetaInfo);
	return pInfo;
...
{
	FILEMETARECORD *pmr = (FILEMETARECORD*)malloc(sizeof(FILEMETARECORD));
	if (!pmr) return NULL;
	ZeroMemory(pmr, sizeof(FILEMETARECORD));
	switch(uType)
	{
		case FVFT_AUDIO:
...
static BOOL MetaDiscovery_ScheduleJob(HANDLE hWakeEvent, LPCWSTR pszPath, METAINFO *pMeta, UINT fileType, DISCOVERCALLBACK fnCallback, ULONG_PTR param)
{
	...
	DISCOVERJOB *pJob = (DISCOVERJOB*)malloc(sizeof(DISCOVERJOB));
	if (pJob)
	{
		ZeroMemory(pJob, sizeof(DISCOVERJOB));
		pJob->pMeta = pMeta;
