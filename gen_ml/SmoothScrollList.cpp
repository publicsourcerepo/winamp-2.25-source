...
SmoothScrollList_OnCreate(HWND hwnd, CREATESTRUCT *createStruct)
{
	...
	self = (SmoothScrollList *)malloc(sizeof(SmoothScrollList));
	if (NULL == self)
		return -1;

	memset(self, 0, sizeof(SmoothScrollList));
	self->itemHeight = 1;
	self->textHeight = 1;
...
static LRESULT CALLBACK SmoothScrollMsgProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch(uMsg) 
	{
		...
		case WM_NOTIFY:
			{
				...
				{					
					...
					lr = SendMessageW(GetParent(hwnd),uMsg,l->idFrom,lParam);
					return lr;
				}
				else if(l->idFrom == 3) 
				{
		...
		case WM_ML_IPC:			return SmoothScrollList_OnMediaLibraryIPC(hwnd, (INT)lParam, (INT_PTR)wParam);
