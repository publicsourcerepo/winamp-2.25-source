...
HeaderIconList_OnCreate(HWND hwnd, CREATESTRUCT *createStruct)
{
	...
	{
		ListViewData *listView;
		listView = (ListViewData*)malloc(sizeof(ListViewData));
		if (NULL != listView)
		{
...
static LRESULT CALLBACK HeaderIconList(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch(uMsg) 
	{
		...
		case LVM_SETITEMCOUNT:
			{
				...
				if (hwndList) 
				{
					lr = ListView_WindowProc(hwndList, uMsg, wParam, lParam);
		...
		case WM_ML_IPC:		return HeaderIconList_OnMediaLibraryIPC(hwnd, (INT)lParam, (INT_PTR)wParam);
