...
void SkinnedProgressBar::OnPaint()
{
	...
	int radius = (ps.rcPaint.bottom - ps.rcPaint.top - 1) / 2;
	int radius2 = 5;
	...
	if (pos > 0)
	{
		...
		if (val > 0)
		{
			if ((ps.rcPaint.left + val + radius) > ps.rcPaint.right)
			{
				...
				RoundRect(hdc, ps.rcPaint.left - 10, ps.rcPaint.top,
						  val + (radius * 2) - 10, ps.rcPaint.bottom, radius2, radius);
			}
			else
			{
		...
		LineTo(hdc, ps.rcPaint.left, ps.rcPaint.bottom - ((radius)/ 2));
		LineTo(hdc, ps.rcPaint.left, ps.rcPaint.bottom);

		MoveToEx(hdc, ps.rcPaint.left, ps.rcPaint.top, NULL);
		LineTo(hdc, ps.rcPaint.left, ps.rcPaint.top + ((radius)/ 2));

		MoveToEx(hdc, ps.rcPaint.left, ps.rcPaint.top, NULL);
		LineTo(hdc, ps.rcPaint.left + ((radius)/ 2), ps.rcPaint.top);
	}
