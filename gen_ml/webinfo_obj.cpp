...
WebFileInfo::WebFileInfo(HWND hwndParent, IDispatch *pDispWA) : HTMLContainer2(hwndParent), nHomePage(HOMEPAGE_NOTLOADED),
...
HRESULT WebFileInfo::InvokeFileInfo(LPCWSTR pszFileName)
{
	...
	else
	{
		...
		if (S_OK != hr) TRACE_FMT("Error sending webinfo (0x%08X)\n", hr);
