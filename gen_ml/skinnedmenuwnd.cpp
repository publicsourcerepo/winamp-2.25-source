...
static HBITMAP SkinnedMenuWnd_LoadPngResource(INT resourceId, COLORREF rgbBk, COLORREF rgbFg)
{
	...
	hbmp = MLImageLoaderI_LoadDib(&imageSource);
	if (NULL != hbmp)
		MLImageFilterI_Apply(hmlifMngr, &MLIF_FILTER1_UID, hbmp, rgbBk, rgbFg, NULL); 
	return hbmp;
}

SkinnedMenuWnd::SkinnedMenuWnd(UINT menuExStyle, HMLIMGLST hmlil, INT forcedWidth, MENUCUSTOMIZEPROC customProc, ULONG_PTR customParam) : 
	SkinnedWnd(FALSE)
{
...
SkinnedMenuWnd::~SkinnedMenuWnd(void)
{
	...
	if (hMenu)
	{
		...
		{
			MENUITEMINFOW mii;
			SkinnedItemRecord *record;
			mii.cbSize = sizeof(MENUITEMINFOW);
			
			for (INT i = 0; i < skinnedItemCount; i++)
			{
				record = &skinnedItems[i];
...
BOOL SkinnedMenuWnd::AttachMenu(HMENU hMenuToAttach)
{
	...
	if (count > 0)
	{
		skinnedItems = (SkinnedItemRecord*)malloc(sizeof(SkinnedItemRecord) * count);
		if (NULL != skinnedItems)
	...
	ZeroMemory(skinnedItems, sizeof(SkinnedItemRecord) *  skinnedItemCount);
	skinnedItemCursor = 0;
	...
	for (int i = 0; i < count; i++)
	{
		...
		{			
			...
			if (0 == (MFT_OWNERDRAW & mii.fType))
			{
				...
				if (0 == mii.wID || (unsigned int)-1 == mii.wID)
				{
					if (NULL != threadInfo)
					{
...
{		
	...
	newStyle = windowStyle & ~(WS_BORDER | WS_THICKFRAME | WS_DLGFRAME);
	...
	{
		lineWidth -= ((GetSystemMetrics(SM_CXMENUCHECK) - 1) + MENU_BORDER_WIDTH*2);
		if (lineWidth < 0) lineWidth = 0;
	}
	else lineWidth = 0;

	lineHeight = GetLineHeight();
	if (!hmlil || !MLImageListI_GetImageSize(hmlil, &imageWidth, &imageHeight)) { imageWidth = 0; imageHeight = 0; }
	if (hmlilCheck)
	{
		INT imageCX, imageCY;
	...
	if (lineHeight < (imageHeight + 2)) lineHeight = (imageHeight + 2);
...
INT SkinnedMenuWnd::GetLineHeight()
{
	...
	{			
		...
		if (NULL != hf)
		{
			...
			if (GetTextMetricsW(hdc, &tm)) h = tm.tmHeight + 2;
...
INT SkinnedMenuWnd::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS *pncsp)
{
	...
	InflateRect(&pncsp->rgrc[0], -MENU_BORDER_WIDTH, -MENU_BORDER_WIDTH); 
	if (bCalcValidRects)
	{
		InflateRect(&pncsp->rgrc[1], -MENU_BORDER_WIDTH, -MENU_BORDER_WIDTH); 
		InflateRect(&pncsp->rgrc[2], -MENU_BORDER_WIDTH, -MENU_BORDER_WIDTH); 
	}
...
BOOL SkinnedMenuWnd::OnReflectedDrawItem(DRAWITEMSTRUCT *pdis)
{
	...
	if (0 != (SMS_SYSCOLORS & menuExStyle))
	{
		...
		INT backIndex = (0 != (ODS_SELECTED & pdis->itemState)) ?
							COLOR_HIGHLIGHT :
							COLOR_MENU;
	...
	if (0 != (MFT_MENUBARBREAK & mii.fType))
	{
		...
		if (FALSE != GetClientRect(hwnd, &rect))
		{
			...
			rect.right = pdis->rcItem.left - 1;
			rect.left = pdis->rcItem.left - 2;
		...
		case MFT_STRING:
				...
				if (hmlil)
				{
					if (MLImageListI_GetImageSize(hmlil, &imageCX, &imageCY))
					{
						...
						if (-1 != index)
						{
							...
							if (-1 != realIndex)
							{
								ImageList_Draw(himl, realIndex, pdis->hDC, pdis->rcItem.left + 1 + (imageWidth - imageCX)/2, imageTop, ILD_NORMAL);
							}
						}
					}
				}
			...
			if (NULL != mii.hSubMenu && hmlilCheck) 
			{
				...
				{	
					...
					if (-1 != realIndex)
						ImageList_Draw(himl, realIndex, pdis->hDC, pdis->rcItem.right - imageCX - 1, imageTop, ILD_NORMAL);
				}
			}
				...
				FALSE == customProc(MLMENU_ACTION_DRAWTEXT, (HMENU)pdis->hwndItem, pdis->hDC, (LPARAM)pdis, customParam))
			{
				if (mii.cch)
				{
					...
					if (imageWidth && imageHeight)  rt.left += imageWidth + 1;
					rt.right -= imageWidth;

					hFont = GetMenuFont(FALSE);
		...
		case MFT_SEPARATOR:
			{
				...
				ExtTextOutW(pdis->hDC, 0, 0, ETO_OPAQUE, &pdis->rcItem, NULL, 0, NULL);
				MoveToEx(pdis->hDC, pdis->rcItem.left, y, NULL);
		...
		{			
			...
			switch(type)
			{
				case MFT_STRING:
					...
					if(imageHeight > (INT)(pmis->itemHeight + 2)) pmis->itemHeight = (imageHeight + 2);
					if (0 == lineWidth && imageWidth) pmis->itemWidth += imageWidth;
					pmis->itemWidth -= (GetSystemMetrics(SM_CXMENUCHECK) -  imageWidth);
					break;
				case MFT_SEPARATOR:
					pmis->itemHeight = 5;
					break;
		...
		case MN_SIZEWINDOW:
			{
				...
				if (NULL == hMenu) 
				{
					...
					textCX = 0;
					shortcutCX = 0;
					menuToAttach = (HMENU)SendMessageW(hwnd, MN_GETHMENU, 0, 0L);
								...
								0 != (MFS_HILITE & mii.fState))
							{
								...
								if (0 != (WS_VISIBLE & windowStyle))
								{
									...
									else
									{
										...
										iHilite = GetMenuItemCount(hMenu);
