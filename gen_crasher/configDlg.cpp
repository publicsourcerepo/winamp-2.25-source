...
BOOL CALLBACK ConfigDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
		case WM_INITDIALOG:
			...
			BOOL isNT, is51OrLess;
   
			// detect windows version
			wchar_t strBuff[2048], winVer[32], build[32];
			int nWinVer;
			...
			if (hm)
			{
				...
				if(ver.Init())
				{
					...
					is51OrLess = (dwBuf[0] <= 5 && dwBuf[1] <= 1);
				}
				else
				{
