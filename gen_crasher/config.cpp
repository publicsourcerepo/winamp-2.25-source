...
{
	return (fileName) ? WritePrivateProfileStringW(section, name, value, fileName) : FALSE;
}

BOOL ConfigW::Write(const wchar_t *section, const wchar_t *name, double value)
{
	wchar_t tmp[48];
	if (S_OK != StringCchPrintfW(tmp, 48, L"%g", value)) return FALSE;
...
{
	wchar_t tmp[32];
	if (S_OK != StringCchPrintfW(tmp, 32, L"%I64d", value)) return FALSE;
	return Write(section,name, tmp);
}
...
{
	wchar_t tmp[16];
	if (S_OK != StringCchPrintfW(tmp, 16, L"%d", value)) return FALSE;
	return Write(section,name, tmp);
}
...
int ConfigW::ReadInt(const wchar_t *section, const wchar_t *name, int defvalue)
{
	...
	return GetPrivateProfileIntW(section, name, defvalue, fileName);
}
...
long long ConfigW::ReadInt64(const wchar_t *section, const wchar_t *name, long long defvalue)
{
	if (!fileName) return defvalue;
	wchar_t tmp[32];
	if (S_OK != StringCchPrintfW(tmp, 32, L"%I64d", defvalue)) return defvalue;
...
double ConfigW::ReadDouble(const wchar_t *section, const wchar_t *name, double defvalue)
{
	if (!fileName) return defvalue;
	wchar_t tmp[32];
	if (S_OK != StringCchPrintfW(tmp, 32, L"%g", defvalue)) return defvalue;
	const wchar_t *ret = ReadStringW(section, name, tmp);
	return  _wtof(ret);
}
...
const wchar_t* ConfigW::ReadStringW(const wchar_t *section, const wchar_t *name, const wchar_t *defvalue)
{
	...
	buff[0] = 0;
	int len = GetPrivateProfileStringW(section, name, def, buff, BUFF_SIZE, fileName);
...
BOOL ConfigW::SetSection(const wchar_t *section)
{
	...
	if (S_OK != StringCchCopyW(defSection, len, section))
	{
		free(defSection); 
		defSection = NULL;
		return FALSE;
...
BOOL ConfigW::SetIniFile(const wchar_t *file)
{
	...
	if (S_OK != StringCchCopyW(fileName, len, file))
	{
		free(fileName); 
		fileName = NULL;
		return FALSE;
