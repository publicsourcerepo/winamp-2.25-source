...
BOOL CALLBACK CrashDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg) 
	{
		case WM_INITDIALOG: 
			...
			SendDlgItemMessage(hwndDlg,IDC_BMP_LOGO,STM_SETIMAGE,IMAGE_BITMAP,
							   (LPARAM)LoadImage((HINSTANCE)lParam,MAKEINTRESOURCE(IDB_BETA),IMAGE_BITMAP,0,0,LR_SHARED));

			SetDlgItemText(hwndDlg, IDC_LBL_STEP, L"Analyzing settings...");
			settings.ClearTempData();

			wchar_t waPath[2*_MAX_PATH];
		...
		case WM_TIMER:
			if (wParam == 123)
			{
				...
				StringCchPrintf(reporter, 512, L"%s\\plugins\\reporter.exe", waPath);
