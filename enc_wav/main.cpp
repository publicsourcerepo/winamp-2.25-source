...
{
	int l = GetPrivateProfileInt("enc_wav", "fmtsize", 0, INI_FILE);

	EXT_WFX convert_wfx_temp;
	if (GetPrivateProfileStruct("enc_wav", "fmt", &convert_wfx_temp, sizeof(WAVEFORMATEX) + l, INI_FILE))
		memcpy(&config->convert_wfx, &convert_wfx_temp, sizeof(config->convert_wfx));
	else
		config->convert_wfx.wfx = wfx_default; 

	GetPrivateProfileString("enc_wav", "ext", "WAV", config->wav_ext, sizeof(config->wav_ext), INI_FILE);
	config->header = !!GetPrivateProfileInt("enc_wav", "header", 1, INI_FILE);
	config->convert = !!GetPrivateProfileInt("enc_wav", "convert", 0, INI_FILE);
}
...
extern "C"
{
	...
	unsigned int __declspec(dllexport) GetAudioTypes3(int idx, char *desc)
	{
		...
		case 0:
			...
			StringCchPrintf(desc, 1024, WASABI_API_LNGSTRING(IDS_ENC_WAV_DESC), ENC_VERSION);
	...
	int __declspec(dllexport) GetConfigItem(unsigned int outt, char *item, char *data, int len, char *configfile)
	{
		if (outt==mmioFOURCC('A','C','M',' '))
		{
			...
			{
				lstrcpyn(data, config.wav_ext, len);
				return 1;
			}
		}
		return 0;
