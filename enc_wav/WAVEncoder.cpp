...
void WAVEncoder::FinishAudio(const wchar_t *filename)
{
	...
	if (tempfile)
	{
		...
		WriteFile(tempfile, &t, 4, &bw, 0); // size of chunk (4 bytes)
		t = rev32('WAVE');
		WriteFile(tempfile, &t, 4, &bw, 0); // WAVE (4 bytes)
		t = rev32('fmt ');
		WriteFile(tempfile, &t, 4, &bw, 0);// fmt  (4 bytes)
		t = 16;
		WriteFile(tempfile, &t, 4, &bw, 0);// size of chunk (4 bytes)
		WriteFile(tempfile, &inputFormat, 16, &bw, 0); // write WAVEFORMAT out (16 bytes)
		t = rev32('data');
		WriteFile(tempfile, &t, 4, &bw, 0);// data  (4 bytes)
		WriteFile(tempfile, &numBytes, 4, &bw, 0);// size of chunk (4 bytes)
