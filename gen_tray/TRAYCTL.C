...
#define PLUGIN_VERSION	L"2.43"

// Winamp general purpose plug-in mini-SDK
// Copyright (C) 1997, Justin Frankel/Nullsoft
// Modifications and useability enhancements by DrO aka Darren Owen 2006-2010
#include <windows.h>
#include <commctrl.h>
#include <shlwapi.h>
...
void FormCompactText(wchar_t* szTip){
int got = 0;
	...
	if(!update_file){
		...
		got = SendMessage(GetPlaylistWnd(plugin.hwndParent),WM_WA_IPC,IPC_PE_GETINDEXTITLEW,(LPARAM)&file);
	}
	...
	// fall back to the Winamp version just incase
		...
		StringCchPrintf(szTip,64,L"Winamp %c.%s",temp[0],&temp[2]);
	}
}
	...
	for (i = (xporhigher?(NUM_ICONS-1):0); (xporhigher?i>-1:i < NUM_ICONS); (xporhigher?i --:i ++))
	{
		if (config_enabled & (1<<i))
		{
			...
			else
			{
				...
				*icpb = 0;
				...
				if (!Icons[i]){
					...
					GetPrivateProfileString(L"tray icon pack",entry,buf,buf,sizeof(buf),ico_pack);
			...
			NOTIFYICONDATA tnid={sizeof(NOTIFYICONDATA),0};
				...
				tnid.hIcon=Icons[i];
				if(i != 5)
					StringCchPrintf(tnid.szTip,64,L"%s - Winamp", WASABI_API_LNGSTRINGW(tips[i]));
				else
...
BOOL CALLBACK ConfigProc(HWND hwndDlg,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			{
				wchar_t str[512] = {0}, tmp[256], tmp2[256];
			...
			SetWindowText(hwndDlg,szDescription);
			dlg_init = 0;
			...
			if (!custom_enabled)
			{
				if( ico_pack[0])
				{
					wchar_t* icpb = 0, base[MAX_PATH] = {0}, *pack = 0, use[MAX_PATH] = {0};
					StringCchPrintf(use,MAX_PATH,L"%s\\Plugins\\Tray_Control\\",wa_path);
					lstrcpyn(base,ico_pack,sizeof(base));

					if(StrStrI(base,use))
					{
						wchar_t* p = base+lstrlen(use);
						if(p && *p)
						{
							wchar_t rep=0;
							...
							while(icpb && *icpb && *icpb != L'.'){icpb = CharPrev(p,icpb);}
							*icpb = 0;
		...
		case WM_COMMAND:
			...
			{
				SendMessage(plugin.hwndParent,WM_WA_IPC,(WPARAM)"http://www.winamp.com/plugins/language-packs/9",IPC_OPEN_URL);
			}
...
void config_read(void)
{
	...
	if ((unsigned int)ini_file < 65536) ini_file=L"winamp.ini";
	...
	GetPrivateProfileString(PLUGIN_NAME,L"ico_pack",ico_pack,ico_pack,sizeof(ico_pack),ini_file);
	GetPrivateProfileString(PLUGIN_NAME,L"ico_pack_safe",ico_pack_safe,ico_pack_safe,sizeof(ico_pack_safe),ini_file);
}
