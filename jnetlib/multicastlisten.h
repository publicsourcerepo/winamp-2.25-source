...
class JNL_MulticastUDPListen
{
  public:
    JNL_MulticastUDPListen(const char *mcast_ip, unsigned short port, sockaddr *which_interface=0, int family = PF_UNSPEC);
    ~JNL_MulticastUDPListen();

    JNL_UDPConnection *get_connect(int sendbufsize=8192, int recvbufsize=8192);
