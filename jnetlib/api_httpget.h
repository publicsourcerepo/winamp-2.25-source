...
protected:
	...
	~api_httpreceiver() {}
public:
	void open(api_dns *dns=API_DNS_AUTODNS, int recvbufsize=PACKET_SIZE, const char *proxy=NULL);
...
inline void api_httpreceiver::open(api_dns *dns, int recvbufsize, const char *proxy)
{
...
inline const char *api_httpreceiver::getheader(const char *headername)
{
	return _call(API_HTTPRECEIVER_GETHEADER, (const char *)NULL, headername);
}

inline int api_httpreceiver::content_length()
{
	return _call(API_HTTPRECEIVER_GETCONTENTLENGTH, (int)0);
}
