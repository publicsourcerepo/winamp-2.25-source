...
int main(int argc, char **argv)
{
  ...
  case 0:   // client mode
    {
      JNL_AsyncDNS dns;
      JNL_UDPConnection con(&dns); // 0 chooses a random port
  ...
  case 1:   // server (listening) mode
    {
	      JNL_MulticastUDPListen listener("239.255.255.250", 80);
			JNL_UDPConnection *con = listener.get_connect();
      ...
      while(1)
      {
        ...
        while (con->recv_bytes_available()>0)
        {
          ...
          con->get_last_recv_msg_addr(&from, &from_length);
          con->setpeer(from, from_length);
          con->send("blorp",6);
        }
				Sleep(55);
      }
...
int main(int argc, char **argv)
{
  ...
  case 0:   // client mode
    {
      ...
      JNL_UDPConnection con(0, &dns); // 0 chooses a random port
			con.set_ttl(4);
      con.setpeer("239.255.255.250",1900);
      printf("Sending message...\n");
      con.send_string("M-SEARCH * HTTP/1.1\r\n");
			con.send_string("Host: 239.255.255.250:1900\r\n");
			con.send_string("Man: \"ssdp:discover\"\r\n");
			con.send_string("ST: ssdp:all\r\n");
			con.send_string("MX: 3\r\n");
			con.send_string("\r\n");
			DWORD startTime = GetTickCount();
      while ((GetTickCount() - 3000) < startTime)
      {
        con.run();
        if (con.get_state()==JNL_UDPConnection::STATE_ERROR)
        {
  ...
  case 1:   // server (listening) mode
    {
      JNL_MulticastUDPListen listener("239.255.255.250", 1900);
			JNL_UDPConnection *con = listener.get_connect();
			if (!con)
			{
      ...
      while(1)
      {
        ...
        {
          char buf[8192];
          int s=MIN(con->recv_bytes_available(), sizeof(buf)-1);
          con->recv_bytes(buf,s);
					buf[s]=0;
          printf("message received:\n%s\n-----\n", buf);
          // reply on the addr:port from sender
