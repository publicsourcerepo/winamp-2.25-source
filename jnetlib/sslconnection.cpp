...
JNL_SSL_Connection::JNL_SSL_Connection(SSL* pssl, api_dns *dns, size_t sendbufsize, size_t recvbufsize) : JNL_Connection(dns, sendbufsize, recvbufsize),
		forceConnect(false)
{
	m_ssl = pssl;
	m_bsslinit = false;
...
void JNL_SSL_Connection::connect(int s, sockaddr *addr, int length)
{
	/*
...
int JNL_SSL_Connection::socket_send(const char *buf, int len, int options)
{
