		...
		if (_this->m_cache[x].last_used && !_this->m_cache[x].resolved)
		{
			if (!nowinsock) 
			{
				if (_this->m_cache[x].mode==0)
				{
					...
					hints.ai_socktype = _this->m_cache[x].sockettype;

					char portString[32];
					StringCchPrintfA(portString, 32, "%d", (int)_this->m_cache[x].port);

					if (getaddrinfo(_this->m_cache[x].hostname, portString, &hints, &res) == 0)
					{
...
int JNL_AsyncDNS::resolve(char *hostname, short port, addrinfo **addr, int sockettype)
{
...
#define CBCLASS JNL_AsyncDNS
START_DISPATCH;
CB(API_DNS_RESOLVE, resolve);
//CB(API_DNS_REVERSE, reverse);
END_DISPATCH;
#undef CBCLASS
