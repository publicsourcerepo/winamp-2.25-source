...
{
public:
	WS_conInst(JNL_Connection *c, JNL_Listen *l, int which_port);
	~WS_conInst();
...
WS_conInst::WS_conInst(JNL_Connection *c, JNL_Listen *l, int which_port) : m_serv(c), m_listener(l), m_port(which_port), connCb(0)
{
	zlibStream = 0;
	m_pagegen = 0;
	time(&m_connect_time);
...
int WebServerBaseClass::addListenPort(int port, unsigned long which_interface)
{
  ...
  address.sin_addr.s_addr = which_interface?which_interface:INADDR_ANY;
	...
	JNL_Listen *p = new JNL_Listen(port, (sockaddr *)&address, PF_INET);
...
void WebServerBaseClass::attachConnection(JNL_Connection *con, JNL_Listen *listener)
{
	m_connections->Add((void*)new WS_conInst(con, listener, listener->port()));
}
...
void WebServerBaseClass::run(void)
{
	...
	if (m_connections->GetSize() < m_max_con && (nl = m_listeners->GetSize()))
	{
		...
		JNL_Connection *c = l->get_connect(PACKET_SIZE, PACKET_SIZE);
		if (c)
		{
...
void WebServerBaseClass::deflatehelper(WS_conInst *con, char * buf, int l, bool flush) {
	...
	int sent = 0;
	if(flush) l = con->m_serv.bytes_cansend();
...
int WebServerBaseClass::run_connection(WS_conInst *con)
{
	...
	if (s < 3)
	{
		...
		if (compression0)
		{
			...
			char * p = compression;
			while(*p) { if(*p == ' ' || *p == ',') *p=0; p++; }
			for(char * q = compression; q < compression + l; q++) {
				if(!_stricmp(q,"deflate")) { supportsDeflate=true; q+=7; }
				if(!_stricmp(q,"gzip")) { supportsGzip=true; q+=4; }
			}
			free(compression);
			int windowBits = 0;
			if (supportsGzip) {
