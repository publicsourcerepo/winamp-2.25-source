...
#define JNL_CONNECTION_AUTODNS API_DNS_AUTODNS
class JNL_HTTPGet : public api_httpreceiver
{
  public:
		JNL_HTTPGet(api_dns *dns=API_DNS_AUTODNS, int recvbufsize=PACKET_SIZE, const char *proxy=NULL);
    ~JNL_HTTPGet();
  ...
  protected:
    ...
    void do_parse_url(char *url, char **host, unsigned short *port, char **req, char **lp);
