...
static int ParseComments(const void *data, size_t data_len, ParsedComments &parsed)
{
	...
	case 0: // ISO-8859-1
		...
		ParseDescription(parsed.description_data.as8, data_len, parsed.description_cch);
		if (ret != NErr_Success)
			return ret;
	...
	case 1: // UTF-16
		...
		ParseDescription(parsed.description_data.as16, data_len, parsed.description_cch);
		if (ret != NErr_Success)
			return ret;
