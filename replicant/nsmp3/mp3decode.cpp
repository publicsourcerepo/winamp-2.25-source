...
/*
 * $Date: 2011/01/25 18:24:16 $
 * $Id: mp3decode.cpp,v 1.5 2011/01/25 18:24:16 audiodsp Exp $
 */
...
SSC CMp3Decode::Decode(float *pPcm, size_t   cbPcm, size_t  *pcbUsed, unsigned char *ancData, size_t *numAncBytes, int oflOn, unsigned int *startDelay, unsigned int *totalLength)
{
  ...
  //
    nOutBytes = (size_t)m_Bs.GetHdr()->GetSamplesPerFrame() * sizeof(float) * nChannels;
...
SSC CMp3Decode::DecodeNormal(float *pPcm, bool fCrcOk)
{
  //
  ...
  for ( gr=0; gr<(m_Info.IsMpeg1 ? 2:1); gr++ )
    ...
    mp3StereoProcessing(&m_Spectrum[0][0][0], &m_Spectrum[1][0][0],
      ...
      m_Info);
		...
		for (ch=0; ch<nChannels; ch++)
		{
			...
			// antialiasing
			mp3Antialias(&m_Spectrum[ch][0][0], m_Si.ch[ch].gr[gr], m_Info);
    ...
    // polyphase
      pPcm = m_Polyphase.Apply(m_PolySpectrum, pPcm);
    } // for ( gr=0...)
...
SSC CMp3Decode::DecodeOnNoMainData(float *pPcm)
{
	//
...
{
  int nChannels = m_Bs.GetHdr()->GetChannels();
	CPolyphase::Reorder(nChannels, m_PolySpectrum, m_Spectrum);
}
  ...
  if ( numAncBytes != 0 )
    {
      ...
      if ( m_AncOfl.doReadBytes() ) {
        ...
        *numAncBytes = m_Db.GetValidBits() / 8;
        ...
        numAncBits = (size_t)m_Db.GetValidBits();
      }

      for(i=0; i < *numAncBytes; i++) {
        ancData[i] = m_Db.GetBits8(8);
      }

      if ( !m_AncOfl.doReadBytes() && numAncBits % 8) {
        size_t bits = numAncBits - (*numAncBytes-1)*8;
