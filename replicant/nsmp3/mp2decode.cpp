...
CMp2Decode::CMp2Decode(CMpegBitStream &_Bs, DecoderHooks *_hooks) : m_Polyphase(m_Info), m_Bs(_Bs), hooks(_hooks)
{
	// full reset
	Init(true);
...
SSC CMp2Decode::Decode(void *pPcm, size_t cbPcm, size_t *pcbUsed)
{
	...
	//
	nOutBytes = (size_t)m_Bs.GetHdr()->GetSamplesPerFrame() * nChannels * sizeof(float);
...
SSC CMp2Decode::Decode2(void *pPcm)
{
	...
	scfsi=scalefacts;
	i=sblimit2;
	while (i--) if (*bita++) *scfsi++ = (char) m_Bs.GetBits8(2);
	...
	while (i--) if (*bita++)
	{
		...
		{
		case 0: 
			scale[0] = m_Bs.GetBits8(6);
			scale[1] = m_Bs.GetBits8(6);
			scale[2] = m_Bs.GetBits8(6);
			break;
		case 1 : 
			scale[1] = scale[0] = m_Bs.GetBits8(6);
			scale[2] = m_Bs.GetBits8(6);
			break;
		case 2: 
			scale[1] = scale[2] = scale[0] = m_Bs.GetBits8(6);
			break;
		case 3:
			scale[0] = m_Bs.GetBits8(6);
			scale[2] = scale[1] = m_Bs.GetBits8(6);
			break;
		}
		scale+=3;
	}
	...
	for (gr=0;gr<12;gr++) 
	{
			...
			for (channel = 0; channel < nChannels; channel++)
			{
				...
				if (ba) 
				{
					...
					else 
					{
						...
						{
							tab += idx+idx+idx;
							m_PolySpectrum[channel][c_p+0][i] = m_scales[(size_t)tab[0]][m];
							m_PolySpectrum[channel][c_p+1][i] = m_scales[(size_t)tab[1]][m];
							m_PolySpectrum[channel][c_p+2][i] = m_scales[(size_t)tab[2]][m];  
						}
						else
						{
		...
		for (; i < sblimit; i ++)
		{
			...
			if ( (ba=*bita++) )
			{
				...
				else
				{
					...
					{
						tab += idx+idx+idx;
						m_PolySpectrum[0][c_p+0][i] = m_scales[(size_t)tab[0]][ml]; 
						m_PolySpectrum[1][c_p+0][i] = m_scales[(size_t)tab[0]][mr];
						m_PolySpectrum[0][c_p+1][i] = m_scales[(size_t)tab[1]][ml]; 
						m_PolySpectrum[1][c_p+1][i] = m_scales[(size_t)tab[1]][mr];
						m_PolySpectrum[0][c_p+2][i] = m_scales[(size_t)tab[2]][ml]; 
						m_PolySpectrum[1][c_p+2][i] = m_scales[(size_t)tab[2]][mr];
					}
					else 
					{
		...
		if (gr==5 || gr==11)
		{
			if (hooks)
			{
				hooks->layer2_eq(&m_PolySpectrum[0][0][0], nChannels, m_Bs.GetHdr()->GetSampleRate(),18);
			}

			pPcm = m_Polyphase.Apply(m_PolySpectrum, (float *)pPcm, 18);
			c_p=0;
		} 
		else c_p+=3;
	}	
...
SSC CMp2Decode::Decode1(void *pPcm)
{
	...
	if (nChannels==2) 
	{
		...
		sca=scale_index; 
		i=js_top+js_top; 
		while (i--) if ((*al++)) *sca++ = m_Bs.GetBits8(6);
		i=32-js_top;
		while (i--) 
		{
			if ((*al++)) 
	...
	if (hooks)
	{
		hooks->layer2_eq(&m_PolySpectrum[0][0][0], nChannels,m_Bs.GetHdr()->GetSampleRate(),12);
	}
