 ...
 * $Id: mp3read.cpp,v 1.2 2011/01/18 18:22:03 audiodsp Exp $
 */
...
bool mp3SideInfoRead(CBitStream &Bs, MP3SI &Si, const MPEG_INFO &Info, int crc_check)
{
  ...
  if (!crc_check || CrcOk(Bs, Info))
    ...
    if ( Info.IsMpeg1 )
      ...
      if ( Info.stereo == MONO )
        Si.private_bits = Bs.GetBits8(5);
      else
        Si.private_bits = Bs.GetBits8(3);

      for ( ch=0; ch<Info.stereo; ch++ )
        for ( i=0; i<4; i++ )
          Si.ch[ch].scfsi[i] = Bs.Get1Bit();
      }
    else
      {
      // MPEG 2, MPEG 2.5
      Si.main_data_begin = Bs.GetBits8(8);

      if ( Info.stereo == MONO )
        Si.private_bits = Bs.Get1Bit();
      else
        Si.private_bits = Bs.GetBits8(2);
      }
    ...
    for ( gr=0; gr<(Info.IsMpeg1 ? 2:1); gr++ )
      ...
      for ( ch=0; ch<Info.stereo; ch++ )
        ...
        Si.ch[ch].gr[gr].global_gain           = Bs.GetBits8(8);
				if (Info.IsMpeg1)
					Si.ch[ch].gr[gr].scalefac_compress     = Bs.GetBits8(4);
				else
					Si.ch[ch].gr[gr].scalefac_compress     = Bs.GetBits(9);
        Si.ch[ch].gr[gr].window_switching_flag = Bs.Get1Bit();
        ...
        if (Si.ch[ch].gr[gr].window_switching_flag)
          ...
          Si.ch[ch].gr[gr].block_type       = Bs.GetBits8(2);
          Si.ch[ch].gr[gr].mixed_block_flag = Bs.Get1Bit();

          for ( i=0; i<2; i++ )
            Si.ch[ch].gr[gr].table_select[i] = Bs.GetBits8(5);

          // not used, set to zero
          Si.ch[ch].gr[gr].table_select[2] = 0;

          for (i=0; i<3; i++)
            Si.ch[ch].gr[gr].subblock_gain[i] = Bs.GetBits8(3);
          ...
          Si.ch[ch].gr[gr].region0_count = Bs.GetBits8(4);
          Si.ch[ch].gr[gr].region1_count = Bs.GetBits8(3);
          Si.ch[ch].gr[gr].block_type    = 0;
          }

        if ( Info.IsMpeg1 )
          Si.ch[ch].gr[gr].preflag = Bs.Get1Bit();

        Si.ch[ch].gr[gr].scalefac_scale     = Bs.Get1Bit();
        Si.ch[ch].gr[gr].count1table_select = Bs.Get1Bit();
        }
      }
    return true;
    }
    ...
    )
{
  ...
  if ( Info.IsMpeg1 )
    ...
    if ( SiGrCh.block_type == 2 )
      ...
      // scalefactor for last sfb not transmitted
      ScaleFac.l[21]     = 0;
      ScaleFac.l_iip[21] = 0; /* = 1 */

      ScaleFac.l[22]     = 0; /* SCFB 22 */
      ScaleFac.l_iip[22] = 0; /* SCFB 22 , = 1 */
      }
    }
}
...
/***************************************************************************\
 *
 *   functionname: mp3ScaleFactorUpdate
 *   description : update highest MPEG Layer-3 scalefactor;
 *                 when using IS stereo in the last scalefactor band, the
 *                 intensity  information (intensity position
 *                 and illegal is position):
 *                 a) have to be copied from the next to last band if
 *                    the joint stereo boundary scale factor band is below
 *                    the last scale factor band
 *
 *                 b) intensity position has to be set to zero
 *                    illegal intensity position has to be set to one
 *                    if the joint stereo boundary scale factor band equals
 *                    the last scale factor band
 *
 *   returns : <none>
 *
...
{
  static const int last_sfb_long  = 21;
  static const int last_sfb_short = 12;
  ...
  if ( (Info.stereo == STEREO) && (Info.mode == MPG_MD_JOINT_STEREO) ) {
    ...
    if (i_stereo) {
      ...
      if ( SiL.window_switching_flag && (SiL.block_type == 2) ) {
        /* set IS (illegal) position for short and mixed blocks in
           the last scale factor band */
        int pt;
        ...
        if ( SiL.mixed_block_flag ) {
          /* Mixed Blocks only*/
          if ( SiR.zeroSfbStartNdxL == last_sfb_long ) {
            ScaleFac.l[last_sfb_long]     = 0;
            ScaleFac.l_iip[last_sfb_long] = 1;
          }
          else{
            ScaleFac.l[last_sfb_long]     = ScaleFac.l[last_sfb_long - 1];
            ScaleFac.l_iip[last_sfb_long] = ScaleFac.l_iip[last_sfb_long - 1];
          }
        }

        /* common to Short Blocks and Mixed Blocks */
        for ( pt=0; pt<3; pt++ ) {

          if ( SiR.zeroSfbStartNdxS[pt] == last_sfb_short ) {
            ScaleFac.s[pt][last_sfb_short] = 0;
            ScaleFac.s_iip[last_sfb_short] = 1;
          }
          else {
            ScaleFac.s[pt][last_sfb_short]  = ScaleFac.s[pt][last_sfb_short - 1];
            ScaleFac.s_iip[last_sfb_short]  = ScaleFac.s_iip[last_sfb_short - 1];
          }
        }
      }
      else {
        /* set IS (illegal) position for long blocks in
           the last scale factor band */
        if ( SiR.zeroSfbStartNdxL == last_sfb_long ) {
          ScaleFac.l[last_sfb_long]     = 0;
          ScaleFac.l_iip[last_sfb_long] = 1;
