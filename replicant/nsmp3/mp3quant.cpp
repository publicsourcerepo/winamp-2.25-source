 ...
 * $Id: mp3quant.cpp,v 1.5 2011/01/25 18:24:17 audiodsp Exp $
 */
			...
			if (iSample)
			{
				if (iSample > 0)
				{
					if ( iSample < sizeof(powx_43)/sizeof(*powx_43) )
						pFData[indx] = (float) (powx_43[iSample] * fScale);
					else
						pFData[indx] = (float)(pow((double)iSample, 4.0/3.0) * fScale);
				}
				else
				{
					iSample = -iSample;
...
{
	memset(&pFData[SiGrCh.zeroStartNdx], 0, sizeof(pFData[0])*(SBLIMIT*SSLIMIT-SiGrCh.zeroStartNdx));
}
