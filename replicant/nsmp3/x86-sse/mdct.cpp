...
/*
* $Date: 2011/01/20 20:58:10 $
* $Id: mdct.cpp,v 1.4 2011/01/20 20:58:10 audiodsp Exp $
*/

/* ------------------------ includes --------------------------------------*/

#include "nsmp3/mdct.h"
...
/* 2 * cos(PI * (2 * i + 1) / (2 * 18)) */
NALIGN(16) static const float cost36_c1[9] =
{
...
static void overlap_hybrid18(float *prev, float *dest, const float *even, const float *odd, const float *win)
{
	...
	*/
	...
	xmm_prev = _mm_loadu_ps(&prev[9]);
	xmm_odd = _mm_load_ps(&odd[0]);
	xmm_odd = _mm_mul_ps(xmm_odd, _mm_load_ps(&cost36_c1[0]));
	xmm_even = _mm_load_ps(&even[0]);
	xmm_win = _mm_loadu_ps(&win[9]);
	xmm_even_odd = _mm_sub_ps(xmm_even, xmm_odd);
	xmm_interm_res = _mm_mul_ps(xmm_even_odd, xmm_win);
	xmm_interm_res = _mm_add_ps(xmm_prev, xmm_interm_res);
	_mm_storeu_ps(&dest[9], xmm_interm_res);
	...
	*/
	xmm_even_odd = _mm_shuffle_ps(xmm_even_odd, xmm_even_odd, 0x1B); // reverse order
	xmm_win = _mm_loadu_ps(&win[5]);
	xmm_prev = _mm_loadu_ps(&prev[5]);
	xmm_interm_res = _mm_mul_ps(xmm_even_odd, xmm_win);
	xmm_interm_res = _mm_add_ps(xmm_prev, xmm_interm_res);
	_mm_storeu_ps(&dest[5], xmm_interm_res);
	...
	*/
	xmm_even_odd = _mm_add_ps(xmm_even, xmm_odd);
	xmm_win = _mm_loadu_ps(&win[27]);
	xmm_interm_res = _mm_mul_ps(xmm_even_odd, xmm_win);
	_mm_storeu_ps(&prev[9], xmm_interm_res);
	...
	*/
	xmm_even_odd = _mm_shuffle_ps(xmm_even_odd, xmm_even_odd, 0x1B); // reverse order
	xmm_win = _mm_loadu_ps(&win[23]);
	xmm_interm_res = _mm_mul_ps(xmm_even_odd, xmm_win);
	_mm_storeu_ps(&prev[5], xmm_interm_res);
	...
	*/
	xmm_prev = _mm_loadu_ps(&prev[13]);
	xmm_odd = _mm_load_ps(&odd[4]);
	xmm_odd = _mm_mul_ps(xmm_odd, _mm_load_ps(&cost36_c1[4]));
	xmm_even = _mm_load_ps(&even[4]);
	xmm_win = _mm_loadu_ps(&win[13]);
	xmm_even_odd = _mm_sub_ps(xmm_even, xmm_odd);
	xmm_interm_res = _mm_mul_ps(xmm_even_odd, xmm_win);
	xmm_interm_res = _mm_add_ps(xmm_prev, xmm_interm_res);
	_mm_storeu_ps(&dest[13], xmm_interm_res);
	...
	*/
	xmm_even_odd = _mm_shuffle_ps(xmm_even_odd, xmm_even_odd, 0x1B); // reverse order
	xmm_win = _mm_loadu_ps(&win[1]);
	xmm_prev = _mm_loadu_ps(&prev[1]);
	xmm_interm_res = _mm_mul_ps(xmm_even_odd, xmm_win);
	xmm_interm_res = _mm_add_ps(xmm_prev, xmm_interm_res);
	_mm_storeu_ps(&dest[1], xmm_interm_res);
	...
	*/
	xmm_even_odd = _mm_add_ps(xmm_even, xmm_odd);
	xmm_win = _mm_loadu_ps(&win[31]);
	xmm_interm_res = _mm_mul_ps(xmm_even_odd, xmm_win);
	_mm_storeu_ps(&prev[13], xmm_interm_res);
	...
	*/	
	xmm_even_odd = _mm_shuffle_ps(xmm_even_odd, xmm_even_odd, 0x1B); // reverse order
	xmm_win = _mm_loadu_ps(&win[19]);
	xmm_interm_res = _mm_mul_ps(xmm_even_odd, xmm_win);
	_mm_storeu_ps(&prev[1], xmm_interm_res);
	/* do the leftovers */
