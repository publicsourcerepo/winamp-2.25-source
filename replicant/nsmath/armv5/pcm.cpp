	...
	if (IsSigned(source_parameters) && source_parameters->bytes_per_sample == sizeof(int32_t))
	{
		...
		if (IsSigned(destination_parameters) && destination_parameters->bytes_per_sample == sizeof(int16_t))
		{
			...
			if (optional_gain != 0)
			{
				...
				{
				default:
					interleaver->interleave_gain = (InterleaverGain)armv5_vfp_nsmath_pcm_Convert_S32_S16_gain;
			...
			else if (destination_parameters->bits_per_sample < source_parameters->bits_per_sample)
			{
				...
				default:
					...
					{
						interleaver->interleave_shift = (InterleaverShift)armv5_vfp_nsmath_pcm_Convert_S32_S16_shift8;
						return NErr_Success;
					}
					else
					{
						interleaver->interleave_shift = (InterleaverShift)armv5_vfp_nsmath_pcm_Convert_S32_S16_decimate;
						return NErr_Success;
					}
				}
			}
			else
			{
				...
				switch(source_parameters->number_of_channels)
				{		
					interleaver->interleave = (Interleaver)armv5_vfp_nsmath_pcm_Convert_S32_S16;
					return NErr_Success;
				}
			}
		}
		else if (IsSigned(destination_parameters) && destination_parameters->bytes_per_sample == sizeof(int32_t))
		{
			...
			if (optional_gain != 0)
			{
				...
				{
				default:						
					interleaver->interleave_gain = (InterleaverGain)armv5_vfp_nsmath_pcm_Convert_S32_F32;
					interleaver->gain = optional_gain * pow(2.0f, (float)(destination_parameters->bits_per_sample - source_parameters->bits_per_sample));
					return NErr_Success;
				}
			}
		...
		else if (IsSigned(destination_parameters) && destination_parameters->bytes_per_sample == 3)
		{
			...
			if (optional_gain != 0)
			{
				...
				{
				default:
					interleaver->interleave_gain = (InterleaverGain)armv5_vfp_nsmath_pcm_Convert_S32_S24_gain;
					interleaver->gain = optional_gain * pow(2.0f, (float)(destination_parameters->bits_per_sample - source_parameters->bits_per_sample));
					return NErr_Success;
				}
			}
			else if (destination_parameters->bits_per_sample > source_parameters->bits_per_sample)
			{
				...
				interleaver->interleave_shift = (InterleaverShift)armv5_vfp_nsmath_pcm_Convert_S32_S24_pad;				
				return NErr_Success;
			}
			else if (destination_parameters->bits_per_sample < source_parameters->bits_per_sample)
			{
				...
				default:
					interleaver->shift_bits = source_parameters->bits_per_sample - destination_parameters->bits_per_sample;
					interleaver->interleave_shift = (InterleaverShift)armv5_vfp_nsmath_pcm_Convert_S32_S24_decimate;
					return NErr_Success;
				}
			}
			else
			{
				...
				{
				default:						
					interleaver->interleave = (Interleaver)armv5_vfp_nsmath_pcm_Convert_S32_S24;
					return NErr_Success;
				}
			}
		}
	...
	if (source_parameters->format_type == nsaudio::format_type_float)
	{
		if (IsSigned(source_parameters) && source_parameters->bytes_per_sample == sizeof(float))
		{
			if (IsSigned(destination_parameters) && destination_parameters->bytes_per_sample == sizeof(int16_t))
			{
				...
				converter->convert_gain = (ConverterGain)armv5_vfp_nsmath_pcm_Convert_F32_S16;
				if (optional_gain)
			...
			else if (IsSigned(destination_parameters) && destination_parameters->bytes_per_sample == 3)
			{
				...
				converter->convert_gain = (ConverterGain)armv5_vfp_nsmath_pcm_Convert_F32_S24;
				if (optional_gain)
