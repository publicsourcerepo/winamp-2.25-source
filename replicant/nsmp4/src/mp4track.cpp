	...
	bool* pIsSyncSample)
{
	...
	nx_file_t pFile = 0;
	try {
	ns_error_t ret = GetSampleFile(&pFile, sampleId);
	if (ret != NErr_Success)
...
//			PRINT_ERROR(e);
	...
	*pNumBytes = sampleSize;
	...
	uint64_t oldPos = 0;
	if (m_pFile->GetMode() == nx_file_FILE_write_binary) 
		m_pFile->GetPosition(pFile); // only used in mode == 'w'
	try { 
...
ns_error_t MP4Track::GetSampleFile(nx_file_t *sample_file, MP4SampleId sampleId)
{
	...
	if (pUrlAtom->GetFlags() & 1) {
		...
		if (!strncmp(url, "file:", 5)) {
			...
			if (fileName) 
			{
				...
				ns_error_t ret = NXURICreateWithUTF8(&nx_fileName, fileName);
				if (ret == NErr_Success)
				{
					NXFileOpenFile(&pFile, nx_fileName, nx_file_FILE_read_binary);
	...
	if (m_lastSampleFile) {
		NXFileRelease(m_lastSampleFile);
	}
