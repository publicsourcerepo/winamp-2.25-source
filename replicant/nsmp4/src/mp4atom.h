...
public:
	...
	virtual MP4Atom *CreateAtom(MP4Atom* pParentAtom, const char* type);
	const char* m_name;
	bool m_mandatory;
	bool m_onlyOne;
	uint32_t m_count;
...
class MP4AtomInfoSpecific : public MP4AtomInfo
{
public:
	MP4AtomInfoSpecific(const char* name, bool mandatory, bool onlyOne) : MP4AtomInfo(name, mandatory, onlyOne)
	{
	}
	virtual MP4Atom *CreateAtom(MP4Atom* pParentAtom, const char* type)
	{
		return new atom_t(pParentAtom, type);
	}
};
...
class MP4Atom {
public:
	MP4Atom(MP4Atom *parent, const char *type);
	MP4Atom(const char* type = NULL);
	virtual ~MP4Atom();

	static MP4Atom* ReadAtom(MP4File* pFile, MP4Atom* pParentAtom);
	static MP4Atom* CreateAtomFromType(MP4Atom *parent, const char* type);
	static bool IsReasonableType(const char* type);

	MP4Atom *CreateAtom(const char *type);
...
protected:
	...
	void ExpectChildAtom(const char* name, bool mandatory, bool onlyOne = true);
	template <class atom_t>
	void ExpectChildAtom(const char* name, bool mandatory, bool onlyOne = true)
	{
		m_pChildAtomInfos.Add(new MP4AtomInfoSpecific<atom_t>(name, mandatory, onlyOne));
	}
	...
	void ReadProperties(uint32_t startIndex = 0, uint32_t count = 0xFFFFFFFF);
	void ReadChildAtoms();

	void WriteProperties(uint32_t startIndex = 0, uint32_t count = 0xFFFFFFFF);
