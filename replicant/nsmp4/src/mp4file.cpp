...
MP4File::~MP4File()
{
	NXURIRelease(m_filename);

	if (m_file)
		NXFileRelease(m_file);
	m_file=0;
...
void MP4File::Read(nx_uri_t filename)
{
	m_filename= NXURIRetain(filename);
	m_mode = nx_file_FILE_read_binary;

	Open(nx_file_FILE_read_binary);
...
ns_error_t MP4File::ReadFile(nx_uri_t filename, nx_file_t file)
{
	m_filename= NXURIRetain(filename);
	m_mode = nx_file_FILE_read_binary;

	ns_error_t ret = OpenFile(file, nx_file_FILE_read_binary);
	if (ret != NErr_Success)
		return ret;
...
void MP4File::Create(nx_uri_t filename, uint32_t flags, 
		     int add_ftyp, int add_iods, 
		     char* majorBrand, uint32_t minorVersion, 
		     char** supportedBrands, uint32_t supportedBrandsCount)
{
	m_filename = NXURIRetain(filename);
	m_mode = nx_file_FILE_write_binary;
	m_createFlags = flags;
	
	Open(nx_file_FILE_readwrite_binary);

	// generate a skeletal atom tree
	m_pRootAtom = MP4Atom::CreateAtomFromType(NULL, NULL);
...
void MP4File::Modify(nx_uri_t filename)
{
	m_filename = NXURIRetain(filename);
	m_mode = nx_file_FILE_read_binary;

	Open(nx_file_FILE_update_binary);
	...
	if (pMoovAtom == NULL) {
		...
		for (i = numAtoms - 1; i >= 0; i--) {
			...
			{ // last atom isn't moov
				...
				MP4Atom* pFreeAtom = m_pRootAtom->CreateAtom("free");
...
void MP4File::Optimize(nx_uri_t orgFileName, nx_uri_t newFileName)
{
	m_filename = NXURIRetain(orgFileName);
	m_mode = nx_file_FILE_read_binary;
	...
	{
		nx_uri_t temp;
		NXURICreateTempForFilepath(&temp, m_filename);
		NXURIRelease(m_filename);
		m_filename = temp;
	}
	else 
	{
		nx_uri_t temp;
		temp = NXURIRetain(newFileName);
		NXURIRelease(m_filename);
		m_filename = temp;
	}

	nx_file_t pReadFile = m_file;
	m_file = NULL;
	m_mode = nx_file_FILE_write_binary;
	Open(nx_file_FILE_write_binary);
	...
	NXFileRelease(m_file);
	m_file=0;
	NXFileRelease(pReadFile);

	// move temporary file into place
	if (newFileName == NULL) 
	{
		NXFile_move(orgFileName, m_filename);
	}
}

void MP4File::RewriteMdat(nx_file_t pReadFile, nx_file_t pWriteFile)
{
...
void MP4File::Open(nx_file_FILE_flags_t fmode)
{
	ASSERT(m_file == NULL);
	nx_file_t openFile = NULL;
...
//#ifdef O_LARGEFILE
	...
	switch(fmode)
	{
	case nx_file_FILE_read_binary:
		flags = O_LARGEFILE | O_RDONLY;
		break;
	case nx_file_FILE_write_binary:
		flags = O_LARGEFILE | O_CREAT | O_TRUNC | O_WRONLY;
		break;
	case nx_file_FILE_update_binary:
		flags = O_LARGEFILE | O_CREAT | O_RDWR;
		break;
	case nx_file_FILE_readwrite_binary:
		flags = O_LARGEFILE | O_CREAT | O_RDWR | O_TRUNC;
		break;
	}
	fd = open(m_filename->string, flags, 0666); 

	if (fd >= 0) {
		openFile = fdopen(fd, fmode);
	}
#endif
	ns_error_t ret = NXFileOpenFile(&openFile, m_filename, fmode);
	if (ret != NErr_Success) {
	...
	if (NXFileStat(openFile, &m_file_stats) != NErr_Success)
	{
		throw new MP4Error(errno, "stat returned error", "MP4Open");
	}
	
	if (m_mode == nx_file_FILE_read_binary) 
	{
		uint64_t length=0;
		NXFileLength(m_file, &length);
	  m_orgFileSize = m_fileSize = length;
	}
	else 
	{
...
ns_error_t MP4File::OpenFile(nx_file_t file, nx_file_FILE_flags_t fmode)
{
	if (!file)
		return NErr_BadParameter;

	m_file = NXFileRetain(file);
	ns_error_t ret = NXFileStat(file, &m_file_stats);
	if (ret != NErr_Success)
		return ret;
...
int MP4File::GetStat(nx_file_stat_t stats)
{
	*stats = m_file_stats;
	return NErr_Success;
}
...
void MP4File::FinishWrite()
{
	...
	if (GetSize() < m_orgFileSize) 
	{
		...
		MP4Atom* pFreeAtom = MP4Atom::CreateAtomFromType(NULL, "free");
		ASSERT(pFreeAtom);
...
void MP4File::Close()
{
	if (m_mode == nx_file_FILE_write_binary) 
	{
		SetIntegerProperty("moov.mvhd.modificationTime", MP4GetAbsTimestamp());
...
MP4Atom* MP4File::InsertChildAtom(MP4Atom* pParentAtom, const char* childName, 	uint32_t index)
{
	MP4Atom* pChildAtom = pParentAtom->CreateAtom(childName);
...
MP4Atom* MP4File::AddDescendantAtoms(MP4Atom* pAncestorAtom, const char* descendantNames)
{
...
{
  return GetTrackIntegerProperty(trackId, "mdia.minf.stbl.stsd.*.damr.modeSet");
}
...
void MP4File::AddChapter(MP4TrackId chapterTrackId, MP4Duration chapterDuration, uint32_t chapterNr, const char * chapterTitle) 
{ 
 if (0 == chapterTrackId) { 
...
{ 
 ...
 if (refTrack == MP4_INVALID_TRACK_ID)	 
  throw new MP4Error("Track index doesn't exist - track %d type %s",  "FindTrackId",  0, MP4_AUDIO_TRACK_TYPE); 
 ...
 for (uint32_t chapterIndex = 0 ; chapterIndex < chapterCount; ++chapterIndex) { 
 ...
 *chapterCount = 0; 
 ...
 uint32_t len = MIN(strlen(name), CHAPTERTITLELEN); 
...
void MP4File::DeleteTrack(MP4TrackId trackId)
{
	...
	uint16_t trakIndex;
	if (FindTrakAtomIndex(trackId, &trakIndex) != NErr_Success)
	{
		throw new MP4Error("Track id %d doesn't exist", "FindTrakAtomIndex", trackId); 
	}

	//uint32_t trakIndex = index; 
...
MP4TrackId MP4File::FindTrackId(uint16_t trackIndex, const char* type, uint8_t subType)
{
	if (type == NULL) 
	{
		if (!m_pTracks.ValidIndex(trackIndex))
...
int MP4File::FindTrakAtomIndex(MP4TrackId trackId, uint16_t *index)
{
	if (trackId) 
	{
