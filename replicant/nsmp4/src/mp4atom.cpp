...
MP4Atom *MP4AtomInfo::CreateAtom(MP4Atom *parent, const char* type)
{
	return MP4Atom::CreateAtomFromType(parent, type);
}
...
MP4Atom::MP4Atom(MP4Atom *parent, const char *type)
{
	SetType(type);
	m_unknownType = FALSE;
	m_pFile = NULL;
...
{
	MP4AtomInfo *pChildAtomInfo = FindAtomInfo(type);
	if (pChildAtomInfo)
	{
		return pChildAtomInfo->CreateAtom(this, type);
	}
	else
	{
		return CreateAtomFromType(this, type);
	}
}

MP4Atom* MP4Atom::CreateAtomFromType(MP4Atom *parent, const char* type)
{
	MP4Atom* pAtom = NULL;
	...
	if (type == NULL) {
		...
		case 'a':
			 if (ATOMID(type) == ATOMID("avcC")) {
				...
				pAtom = new MP4SoundAtom(parent, type);
			} 
			break;
		case 'c':
...
void MP4Atom::Generate()
{
	...
	for (i = 0; i < m_pChildAtomInfos.Size(); i++) 
	{
		if (m_pChildAtomInfos[i]->m_mandatory && m_pChildAtomInfos[i]->m_onlyOne) 
		{
			...
			MP4Atom* pChildAtom = m_pChildAtomInfos[i]->CreateAtom(this, m_pChildAtomInfos[i]->m_name);
...
MP4Atom* MP4Atom::ReadAtom(MP4File* pFile, MP4Atom* pParentAtom)
{
	...
	pAtom = pParentAtom->CreateAtom(type);
	pAtom->SetFile(pFile);
	pAtom->SetStart(pos);
...
void MP4Atom::ReadChildAtoms()
{
	...
	for (uint64_t position = m_pFile->GetPosition(); position < m_end; position = m_pFile->GetPosition()) 
	{
			...
			if (pChildAtom)
			{
				...
				{
					VERBOSE_READ(GetVerbosity(), printf("Warning: In atom %s unexpected child atom %s\n", GetType(), pChildAtom->GetType()));
				}
				...
				if (pChildAtomInfo) 
				{
					...
					if (pChildAtomInfo->m_onlyOne && pChildAtomInfo->m_count > 1) {
						VERBOSE_READ(GetVerbosity(), printf("Warning: In atom %s multiple child atoms %s\n", GetType(), pChildAtom->GetType()));
					}
				}
	...
	for (uint32_t i = 0; i < numAtomInfo; i++) {
		...
		{
				VERBOSE_READ(GetVerbosity(), printf("Warning: In atom %s missing child atom %s\n", GetType(), m_pChildAtomInfos[i]->m_name));
		}
	}
...
MP4AtomInfo* MP4Atom::FindAtomInfo(const char* name)
{
	...
	for (uint32_t i = 0; i < numAtomInfo; i++) 
	{
		if (m_pChildAtomInfos[i]->m_name == NULL || ATOMID(m_pChildAtomInfos[i]->m_name) == ATOMID(name)) 
		{
