...
MP4SoundAtom::MP4SoundAtom(MP4Atom *parent, const char *type) :  MP4Atom(parent, type) 
{
	...
	if (ATOMID(type) == ATOMID("mp4a")) {
		...
		ExpectChildAtom<MP4WaveAtom>("wave", Optional, OnlyOne);
	} else if (ATOMID(type) == ATOMID("alac")) {
		AddReserved("reserved3", 2); /* 8 */
		ExpectChildAtom<MP4AlacAtom>("alac", Optional, Optional);
		//AddProperty( new MP4BytesProperty("alacInfo", 36));
	} else if (ATOMID(type) == ATOMID("m4ae")) {
		AddReserved("reserved3", 2); /* 8 */
		ExpectChildAtom("esds", Required, OnlyOne);
	}
}
...
void MP4SoundAtom::Generate()
{
	...
	((MP4BytesProperty*)m_pProperties[3])->SetValue(reserved2, sizeof(reserved2));
