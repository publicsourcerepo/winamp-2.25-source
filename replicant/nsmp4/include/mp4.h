...
MP4FileHandle MP4Create(nx_uri_t fileName, uint32_t verbosity DEFAULT(0),	uint32_t flags DEFAULT(0));
MP4FileHandle MP4CreateEx(nx_uri_t fileName,
...
MP4FileHandle MP4Modify(nx_uri_t fileName, uint32_t verbosity DEFAULT(0),	uint32_t flags DEFAULT(0));

MP4FileHandle MP4Read(nx_uri_t fileName, uint32_t verbosity DEFAULT(0));
MP4FileHandle MP4ReadFile(nx_uri_t fileName, nx_file_t file, uint32_t verbosity DEFAULT(0));
int MP4GetStat(MP4FileHandle hFile, nx_file_stat_t stats);

 
void MP4Close(MP4FileHandle hFile);

/* closes the underlying file, but doesn't destroy the MP4 object */
void MP4CloseFile(MP4FileHandle hFile);

bool MP4Optimize(
	nx_uri_t existingFileName, 
	nx_uri_t newFileName DEFAULT(NULL), 
	uint32_t verbosity DEFAULT(0));

bool MP4Dump(MP4FileHandle hFile, FILE* pDumpFile DEFAULT(NULL), bool dumpImplicits DEFAULT(0));
...
uint8_t MP4GetVideoProfileLevel(MP4FileHandle hFile, MP4TrackId trackId DEFAULT(MP4_INVALID_TRACK_ID));
...
MP4TrackId MP4FindTrackId(MP4FileHandle hFile, uint16_t index, const char* type DEFAULT(NULL), uint8_t subType DEFAULT(0));
...
bool MP4MakeIsmaCompliant(nx_uri_t fileName, 
	uint32_t verbosity DEFAULT(0),
