...
static bool DoMatch(nx_string_t match_string, const wchar_t *string, int match_style)
{
	switch(match_style)
	{
	case obj_xml::MATCH_EXACT:
		return !wcscmp(match_string->string, string);
...
const wchar_t *XMLParser::AddPath(const wchar_t *node)
{
...
const wchar_t *XMLParser::BuildPath()
{
	return path_string;
}

const wchar_t *XMLParser::RemovePath(const wchar_t *node)
{
	...
	if (path_string[0])
	{
		const wchar_t *last_node = wcsrchr(path_string, '\f');
		if (last_node)
		{
