...
struct PS_DEC {
  ...
  int   delayBufIndex;
  float aaRealDelayBufferQmf[NO_QMF_ALLPASS_CHANNELS][2];
  float aaImagDelayBufferQmf[NO_QMF_ALLPASS_CHANNELS][2];
	float aaRealDelayBufferLong[NO_DELAY_CHANNELS][LONG_DELAY];
  float aaImagDelayBufferLong[NO_DELAY_CHANNELS][LONG_DELAY];
	float aaRealDelayBufferShort[NO_DELAY_CHANNELS][SHORT_DELAY];
  float aaImagDelayBufferShort[NO_DELAY_CHANNELS][SHORT_DELAY];
  int aDelayRBufIndexSer[NO_SERIAL_ALLPASS_LINKS];
  float aaaRealDelayRBufferSerQmf[NO_SERIAL_ALLPASS_LINKS][5][NO_QMF_ALLPASS_CHANNELS];
  float aaaImagDelayRBufferSerQmf[NO_SERIAL_ALLPASS_LINKS][5][NO_QMF_ALLPASS_CHANNELS];
  float aaaRealDelayRBufferSerSubQmf[NO_SERIAL_ALLPASS_LINKS][5][SUBQMF_GROUPS];
  float aaaImagDelayRBufferSerSubQmf[NO_SERIAL_ALLPASS_LINKS][5][SUBQMF_GROUPS];
  float aaRealDelayBufferSubQmf[SUBQMF_GROUPS][DELAY_ALLPASS];
  float aaImagDelayBufferSubQmf[SUBQMF_GROUPS][DELAY_ALLPASS];
  float aPeakDecayFast[NO_BINS];
  float aPrevNrg[NO_BINS];
  float aPrevPeakDiff[NO_BINS];
  float mHybridRealLeft[SUBQMF_GROUPS];
  float mHybridImagLeft[SUBQMF_GROUPS];
  float mHybridRealRight[SUBQMF_GROUPS];
  float mHybridImagRight[SUBQMF_GROUPS];
  HYBRID Hybrid;
