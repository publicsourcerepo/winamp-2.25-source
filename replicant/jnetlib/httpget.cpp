...
void JNL_HTTPGet::addheader(const char *header)
{
	...
	else
	{
		...
		if (t)
		{
			...
			StringCchCopyExA(t, len, header, &t, &len, 0);
			StringCchCopyExA(t, len, "\r\n", &t, &len, 0);
			free(m_sendheaders);
...
void JNL_HTTPGet::addheadervalue(const char *header, const char *value)
{
	...
	if (!m_sendheaders)
	{
		...
		if (m_sendheaders)
		{
			...
			StringCchCopyExA(p, additional, ": ", &p, &additional, 0);
			StringCchCopyExA(p, additional, value, &p, &additional, 0);
			StringCchCopyExA(p, additional, "\r\n", &p, &additional, 0);
		}
	}
	else
	{
		...
		if (t)
		{
			...
			StringCchCopyExA(p, alloc_len, header, &p, &alloc_len, 0);
			StringCchCopyExA(p, alloc_len, ": ", &p, &alloc_len, 0);
			StringCchCopyExA(p, alloc_len, value, &p, &alloc_len, 0);
			StringCchCopyExA(p, alloc_len, "\r\n", &p, &alloc_len, 0);
			free(m_sendheaders);
			m_sendheaders = t;
		}
	}
...
{
	int cnt = 0;
	if (m_http_state == -1 || !m_con) return HTTPGET_RUN_ERROR; // error
...
run_again:
	...
	if (m_con->get_state() == JNL_Connection::STATE_CLOSED) return HTTPGET_RUN_CONNECTION_CLOSED;
...
{
	if (m_http_state < 0) return HTTPGET_STATUS_ERROR;
	if (m_http_state < 2) return HTTPGET_STATUS_CONNECTING;
	if (m_http_state == 2) return HTTPGET_STATUS_READING_HEADERS;
	if (m_http_state == 3) return HTTPGET_STATUS_READING_CONTENT;
