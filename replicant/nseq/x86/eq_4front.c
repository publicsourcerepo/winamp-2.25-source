...
void eq10_setup(eq10_t *eq, unsigned int channels, double rate, const double *frequencies)
{
	unsigned int t,k;

	for(k=0;k<channels;k++,eq++)
	{
...
void eq10_processf(eq10_t *eq, const float *buf, float *outbuf, size_t sz, size_t index, size_t step, int limit)
{
	...
	for(k=0;k<EQ10_NOFBANDS;k++)
	{
		...
		{
			/* 2-pole IIR */
			y0 = (in[0]-x2)*a0 + y1*b1 + y2*b2 + 1e-30; /* last is for dernomal */
