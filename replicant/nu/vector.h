...
public:
	...
	void eraseAt(size_t index)
	{
		if (index < numPtrs)
		{
			Type *oldTable = NULL;
			Type *from = values;
			if (allocSize - numPtrs > 48)
			{
				oldTable = values;
				values = new Type[numPtrs + INCREMENT - 1];
				for (size_t j = 0;j < index;j++)
				{
					values[j] = oldTable[j];
				}
			}
			for (size_t k = index + 1; k < numPtrs; k++) values[k-1] = from[k];
			numPtrs--;
			if (oldTable)
			{
				allocSize = numPtrs + INCREMENT;
				delete[] oldTable;
			}
		}
	}
