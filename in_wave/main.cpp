...
{
public:
	SoundFile(const wchar_t *filename, int mode, SF_INFO *info) : sndFile(0)
	{
		info->format = 0;
...
void About(HWND hwndParent)
{
	...
	StringCchPrintfW(message, 1024, WASABI_API_LNGSTRINGW(IDS_ABOUT_TEXT),
					 plugin.description, __DATE__, ver);
...
void Init()
{
	...
	GetPrivateProfileString("in_wave", "extensions", defaultExtensions, config_extensions, 1024, INI_FILE);
	SetFileExtensions(config_extensions);
}

void Quit()
{
	if (lstrcmpi(config_extensions, defaultExtensions))
		WritePrivateProfileString("in_wave", "extensions", config_extensions, INI_FILE);
	else
		WritePrivateProfileString("in_wave", "extensions", 0, INI_FILE);
...
int InfoBox(const wchar_t *file, HWND hwndParent)
{
	...
	if (metaFile)
	{
		...
		char temp[1024];
		StringCchPrintfA(temp, 1024, WASABI_API_LNGSTRING(IDS_INFO_STR_FMT),
						 formatInfo.name, info.channels, info.samplerate);
		MessageBox(NULL, temp, WASABI_API_LNGSTRING(IDS_FILE_INFORMATION), MB_OK);
		sf_close(metaFile);
	}
...
{
	return !lstrcmpi(mainString, keyword);
}
