...
{
	svc_fileReader *reader = (svc_fileReader *)user_data;
	return reader->read((__int8 *)ptr, (size_t)count);
}

sf_count_t VirtualWrite(const void *ptr, sf_count_t count, void *user_data) 
{
	svc_fileReader *reader = (svc_fileReader *)user_data;
	return reader->write((__int8 *)ptr, (size_t)count);
}
...
{
	waServiceFactory *sf = WASABI_API_SVC->service_getServiceByGuid(HTTPReaderGUID );
	if (sf)
	{
		svc_fileReader *reader = (svc_fileReader *)sf->getInterface();
		if (reader)
		{
			reader->open(url);
		}
