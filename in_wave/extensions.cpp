...
int SizeAvailableTypes()
{
	...
	for (int i=0;i<numTypes;i++)
	{
		...
		size += lstrlen(info.extension) + 1 /* ; or \0 */;
...
void BuildDefaultExtensions()
{
	...
	for (int i=0; i<numTypes; i++)
	{
		...
		if (ExtensionExists(info.extension, defaultExtensions))
			continue;
		if (*CharPrev(defaultExtensions,extPtr))
			StringCchCatEx(extPtr, extPtrCch, ";", &extPtr, &extPtrCch, 0);
		StringCchCatEx(extPtr, extPtrCch, info.extension, &extPtr, &extPtrCch, 0);
	}
}
...
void SetFileExtensions(const char *extList)
{
	...
	char* end = 0;
	StringCchCopyEx(fileExtensionsString, 1200, extList, &end, 0, 0);
	StringCchCopyEx(end+1, 1200, WASABI_API_LNGSTRING(IDS_SOUND_FILES), 0, 0, 0);
...
BOOL GetExtensionName(LPCWSTR pszExt, LPWSTR pszDest, INT cchDest)
{
	...
	for (index = 0; index < maxCount; index++)
	{
		...
		{
			INT len1, len2;
			len1 = lstrlen(info.extension);
			len2 = lstrlen(info.name);
			if (len2 > len1)
			{
				if (CSTR_EQUAL == CompareStringA(lcid, NORM_IGNORECASE, info.extension, len1, info.name, len1)) { info.name += len1; len2 -= len1; }
				if (len2 > 0 && ' ' == *info.name) { info.name++; len2--; }
				if (len2 > 0 && '(' == *info.name) { info.name++; len2--; }
				if (len2 > 0 && ')' == info.name[len2 - 1]) len2--;
			}
