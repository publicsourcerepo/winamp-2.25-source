...
int WindacPlay::open(char drive, int track) //called by winampGetExtendedRead
{
	driveLetter = drive;
	if (!inited && !LoadASPI()) 
	{
	...
	if (getSCSIIDFromDrive(drive, &host, &id, &lun))
	{
		int found = 0;
...
#ifndef WINAMPX
	int do_limit = 0;
	do_limit = config_maxextractspeed + 1;
	if (do_limit > 5 && getRegVer() <= 0) do_limit = 5;
	if (do_limit > 6)
		speedLimiter.NoLimit();
	else
		speedLimiter.SetLimit(do_limit);
#endif

	return 0;
}

int WindacPlay::play(char drive, int track) //called by winamp2's normal(old) play() interface
{
	if (!config_use_dae2) return 1;
