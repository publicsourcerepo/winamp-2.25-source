...
extern "C" __declspec(dllexport) int winampGetExtendedFileInfoW(const wchar_t *fn, const char *data, wchar_t *dest, int destlen)
{
	...
	if (!_wcsnicmp(fn, L"cda://", 6)) // determine length of cd track via MCI
	{
		...
		if (!_stricmp(data, "cdengine"))
		{
			dest[0] = 0;
			if (g_cdplay && device && g_cdplay->IsPlaying(device))
			{
				if (g_cdplay == veritasPlayer)
					lstrcpynW(dest,  L"Sonic", destlen);
		...
		else if (!_stricmp(data, "album") || !_stricmp(data, "artist") || !_stricmp(data, "year") || !_stricmp(data, "genre") ||
			!_stricmp(data, "title") || !_stricmp(data, "comment") || !_stricmp(data, "tuid") || !_stricmp(data, "albumartist")
			...
			)
		{
			...
			//OutputDebugString(cached ? "requesting metadata (cached)" : "requesting metadata (uncached)");
			...
			if (myps.CDDBID)
			{
				...
				{
					PostMessage(line.hMainWindow, WM_WA_IPC, (WPARAM)L"cda://", IPC_REFRESHPLCACHE);
				}
		...
		else if (!_stricmp(data, "reloadsonic"))
		{
			// benski> 22 may 2008 - we now ignore this since we moved sonic to a different DLL
			// not really sure what the purpose was anyway?
		}
		else
			return 0;
...
extern "C" __declspec(dllexport) int winampSetExtendedFileInfoW(const wchar_t *fn, const char *data, wchar_t *val)
{
	...
	char drive;
	int tracknum;
	if (!ParseName(AutoChar(fn), drive, tracknum)) // TODO: remove AutoChar here, I'm just being lazy
		return 0;
...
static HRESULT CALLBACK Cddb_LookupCallback(HRESULT result, ICddbDisc *pDisc, DWORD *pdwAutoCloseDelay, ULONG_PTR user)
{
	...
	if (S_OK == result)
	{
		...
		if (SUCCEEDED(hr))
		{
			hr = pCache->StoreDiscByToc(data->szTOC, data->disc);
			pCache->Release();
		}
	}
	else
		...
		if (SUCCEEDED(hr))
		{
			...
			if (pDisc)
			{
				...
				if (uiFlags & UI_DATA_CHANGED)
				{
					...
					if (SUCCEEDED(hr))
					{
						hr = pCache->StoreDiscByToc(data->szTOC, pDisc);
						pCache->Release();
					}
...
bool SubmitEdit(ICddbDisc *pDisc)
{
	...
	if (SUCCEEDED(hr)) 
	{
		...
		long val;
		hr=pControl->SubmitDisc(pDisc, 0, &val);
		pControl->Release();
		if (val == 0)
			return true;
	}
...
static void FillDialog_CDText(HWND hwndDlg, const DINFO &info)
{
	...
	for (int i=0;i<info.ntracks;i++)
	{
		...
		if (track.composer)
			listview.SetItemText(index, 3, track.composer);
	}
}
