...
winampMediaLibraryPlugin plugin = {MLHDR_VER,"nullsoft(ml_transcode.dll)",init,quit,PluginMessageProc};
...
void transcode(nu::PtrList<const wchar_t> &filenames, HWND parent);
...
int init()
{
	...
	StringCchPrintfW(szDescription, ARRAYSIZE(szDescription),
					 WASABI_API_LNGSTRINGW(IDS_NULLSOFT_FORMAT_CONVERTER), PLUGIN_VERSION);
...
{
public:
	TranscodePlaylistLoader(nu::PtrList<const wchar_t> *_fileList);
	void OnFile(const wchar_t *filename, const wchar_t *title, int lengthInMS, ifc_plentryinfo *info);

protected:
	nu::PtrList<const wchar_t> *fileList;
	RECVS_DISPATCH;
};

TranscodePlaylistLoader::TranscodePlaylistLoader(nu::PtrList<const wchar_t> *_fileList)
{
	fileList = _fileList;
}
...
public:
	...
	void makefn(const wchar_t* folder, const wchar_t *outfile)
	{
		if(GetPrivateProfileInt(L"transcoder",L"usefilename",0,inifile))
		{
			...
			this->outfile = (wchar_t*)malloc(len * sizeof(this->outfile[0]));
	...
	~TranscodeItem() { free(outfile); freeRecord(&ice); }
};
...
static BOOL CALLBACK transcode_dlgproc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam)
{
	...
	switch(uMsg)
	{
		case WM_INITDIALOG:
		{
			...
			cfs = (convertFileStructW *)malloc(sizeof(convertFileStructW));
		...
		case WM_TRANSCODE_START:
			for(;;)
			{
				...
				{
					MessageBox(hwndDlg,cfs->error,WASABI_API_LNGSTRINGW(IDS_TRANSCODING_FAILED),0);
		...
		case WM_TRANSCODE_ABORT:
		{
			...
			while((t = (TranscodeItem*)transcodeQueue.Poll()) != NULL)
			{
				...
				{
					wchar_t title[64], prompt[512], file[MAX_PATH];
...
void transcode(nu::PtrList<const wchar_t> &filenames, HWND parent)
{
	wchar_t format[2048], folder[MAX_PATH];
	unsigned int fourcc = transcodeGatherSettings(format, folder, 2048, parent);
	if(!fourcc) return;

	for(size_t i=0; i != filenames.size(); i++)
	{
		addTrackToTranscodeQueue(filenames[i],fourcc,format,folder);
	}
}
...
extern "C" {
	...
	__declspec( dllexport ) int winampUninstallPlugin(HINSTANCE hDllInst, HWND hwndDlg, int param) {
		...
		wchar_t title[128];
		StringCchPrintf(title, 128, L"%s %hs", WASABI_API_LNGSTRINGW(IDS_NULLSOFT_FORMAT_CONVERTER),PLUGIN_VERSION);
