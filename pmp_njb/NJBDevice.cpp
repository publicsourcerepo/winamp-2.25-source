...
static void FillSongFromMeta(BYTE * buf,Song * song) {
  ...
  for(int i=0; i<count; i++)
    ...
    int l=min(sizeof(itemname)-1,NameLen);
...
static BYTE * makeMetaFromItemRecord(itemRecordW * item, wchar_t * file, long * size) {
  ...
  if(!_wcsicmp(ext,L"mp3")) strcpy(codec,"MP3");
  else if(!_wcsicmp(ext,L"wma")) strcpy(codec,"WMA");

  if(!item->album) item->album = _wcsdup(L"");
  if(!item->artist) item->artist = _wcsdup(L"");
  if(!item->title) item->title = _wcsdup(L"");

  *size = 2/*count*/+2/*type*/+6/*namelen+datalen*/+strlen(TITLE)+2*wcslen(item->title)
  ...
  int count = 6;
  if (item->year > 0 ){
		*size+=2+6+strlen(YEAR)+sizeof(short);
    count++;
  }
  if (item->genre) {
		*size+=2+6+strlen(GENRE)+2*wcslen(item->genre);
    count++;
  }
  if (item->track>0) {
    *size+=2+6+strlen(TRACKNUM)+sizeof(short);
    count++;
  }
  ...
  ptr = setAttrib(ptr,2,TITLE,(BYTE*)((wchar_t*)(item->title)),wcslen(item->title)*2);
  ptr = setAttrib(ptr,2,ARTIST,(BYTE*)((wchar_t*)(item->artist)),wcslen(item->artist)*2);
  ptr = setAttrib(ptr,2,ALBUM,(BYTE*)((wchar_t*)(item->album)),wcslen(item->album)*2);
  if(item->genre) ptr = setAttrib(ptr,2,GENRE,(BYTE*)((wchar_t*)(item->genre)),wcslen(item->genre)*2);
  short v = item->track;
  if(item->track>0) ptr = setAttrib(ptr,1,TRACKNUM,(BYTE*)&v,2);
  v = item->year;
  if(item->year>0) ptr = setAttrib(ptr,1,YEAR,(BYTE*)&v,2);
  ptr = setAttrib(ptr,0,CODEC,(BYTE*)codec,strlen(codec));
...
void NJBDevice::setTrackArtist(songid_t songid, const wchar_t * value) {
  ...
  DAPSDK_ID item = {s->trackid,kAudioTrackType,s->title};
  m_pCTJukebox2->SetItemAttribute(id,(IUnknown*)&item,L"ARTIST",2,wcslen(value)*2+2,(IUnknown*)value);
}
...
void NJBDevice::setTrackAlbum(songid_t songid, const wchar_t * value) {
  ...
  DAPSDK_ID item = {s->trackid,kAudioTrackType,s->title};
  m_pCTJukebox2->SetItemAttribute(id,(IUnknown*)&item,L"ALBUM",2,wcslen(value)*2+2,(IUnknown*)value);
}
...
void NJBDevice::setTrackTitle(songid_t songid, const wchar_t * value) {
  ...
  DAPSDK_ID item = {s->trackid,kAudioTrackType,s->title};
  m_pCTJukebox2->SetItemAttribute(id,(IUnknown*)&item,L"TITLE",2,wcslen(value)*2+2,(IUnknown*)value);
}
...
void NJBDevice::setTrackGenre(songid_t songid, const wchar_t * value) {
  ...
  DAPSDK_ID item = {s->trackid,kAudioTrackType,s->title};
  m_pCTJukebox2->SetItemAttribute(id,(IUnknown*)&item,L"GENRE",2,wcslen(value)*2+2,(IUnknown*)value);
}
