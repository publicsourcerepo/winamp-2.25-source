...
void P4SDevice::Load() {
	...
	if (SUCCEEDED(hr) && !error) {
		...
		if(store->GetStorage(L"Playlists",&playlists) == S_FALSE) {
			if(store->GetStorage(L"My Playlists",&playlists) == S_FALSE) playlists = NULL;
		}
		if(playlists) { traverseStorage(playlists); playlists->QueryInterface(&playlistsDir); playlists->Release(); }

		IWMDMStorage * songs;
		if(store->GetStorage(L"Music",&songs) == S_OK) { traverseStorage(songs); songs->Release(); } 
		else if(store->GetStorage(L"My Music",&songs) == S_OK) { traverseStorage(songs); songs->Release(); musicDir = L"My Music";}
		else if(store->GetStorage(L"Music Files",&songs) == S_OK) { traverseStorage(songs); songs->Release(); musicDir = L"Music Files";}
...
static void commitPlaylist(Playlist * pl) {
	int l = pl->songs.GetSize();
	IWMDMStorage ** newOrder = (IWMDMStorage **)malloc(sizeof(IWMDMStorage *)*l);
...
#define MKVALIDFN(x) { wchar_t * n = x; while(*n == L'.') *(n++)=L'_'; while(*n) { if(*n == L'|' || *n == L'\\' || *n == L'/' || *n == L'?' || *n == L'<' || *n == L'>' || *n == L':' || *n == L'*'  || *n == L'"') *n=L'_'; n++; } n = x+wcslen(x)-1; while(*n==L'.' && n>=x) *(n--)=0; }
...
static IWMDMStorage4* GetOrCreateFolder(IWMDMStorage4 * store, wchar_t * name, P4SDevice * dev, bool album, const itemRecordW * item) {
	...
	IWMDMMetaData * meta;
	store->CreateEmptyMetadataObject(&meta);
	hr = storeControl->Insert3(WMDM_MODE_BLOCK|WMDM_CONTENT_FOLDER,WMDM_FILE_ATTR_FOLDER,NULL,name,NULL,NULL,meta,NULL,&store0);
	meta->Release();
						...
						if (parent)
						{
							...
							if (parent4)
							{
								...
								if(album0) {
									...
									newrefs = (IWMDMStorage **)malloc(sizeof(void*)* (refc + 1));
...
static BYTE* GetMetadataItem(IWMDMMetaData *meta, const WCHAR * name) {
	...
	if((hr=meta->QueryByName(name,&type,&value,&len)) != S_OK) { return NULL; /*value;*/ } //wchar_t buf[100]; wsprintf(buf,L"meta fail: %x %s",hr,name); OutputDebugString(buf);
	return value;
}
...
static BYTE* GetMetadataItem(Song * song, const WCHAR * name) {
	...
	if((hr=song->meta->QueryByName(name,&type,&value,&len)) != S_OK) { return NULL; /*value;*/ } //wchar_t buf[100]; wsprintf(buf,L"meta fail: %x %s",hr,name); OutputDebugString(buf);
	return value;
}
...
void P4SDevice::setTrackYear(songid_t songid, int value) {
	wchar_t buf[10]; wsprintf(buf,L"%d",value);
...
void P4SDevice::setArt(songid_t songid, void *buf, int w, int h) { //buf is in format ARGB32*
	...
	if(meta || meta2) {
		if(buf) {
			...
			waServiceFactory *sf = WASABI_API_SVC->service_getServiceByGuid(JPEGwriteguid);
			...
			if(jpgWrite) {
				...
				if(jpeg) {
					...
					if(meta) {
						HRESULT hr;
						hr=meta->AddItem(WMDM_TYPE_BINARY,g_wszWMDMAlbumCoverData,(BYTE*)jpeg,length);
						hr=meta->AddItem(WMDM_TYPE_DWORD,g_wszWMDMAlbumCoverWidth,(BYTE*)&w,sizeof(DWORD));
						hr=meta->AddItem(WMDM_TYPE_DWORD,g_wszWMDMAlbumCoverHeight,(BYTE*)&h,sizeof(DWORD));
						hr=meta->AddItem(WMDM_TYPE_DWORD,g_wszWMDMAlbumCoverSize,(BYTE*)&length,sizeof(DWORD));
						hr=meta->AddItem(WMDM_TYPE_DWORD,g_wszWMDMAlbumCoverFormat,(BYTE*)&fmt,sizeof(DWORD));
					}
					if(meta2) {
						HRESULT hr;
						hr=meta2->AddItem(WMDM_TYPE_BINARY,g_wszWMDMAlbumCoverData,(BYTE*)jpeg,length);
						hr=meta2->AddItem(WMDM_TYPE_DWORD,g_wszWMDMAlbumCoverWidth,(BYTE*)&w,sizeof(DWORD));
						hr=meta2->AddItem(WMDM_TYPE_DWORD,g_wszWMDMAlbumCoverHeight,(BYTE*)&h,sizeof(DWORD));
						hr=meta2->AddItem(WMDM_TYPE_DWORD,g_wszWMDMAlbumCoverSize,(BYTE*)&length,sizeof(DWORD));
						hr=meta2->AddItem(WMDM_TYPE_DWORD,g_wszWMDMAlbumCoverFormat,(BYTE*)&fmt,sizeof(DWORD));
					}
					WASABI_API_MEMMGR->sysFree(jpeg);
				}
				sf->releaseInterface(jpgWrite);
			}
		} else { // remove art
			if(meta) {
...
public:
	...
	ARGB32 * GetImage() {
		...
		waServiceFactory *sf = WASABI_API_SVC->service_getServiceByGuid(JPEGguid);
		if(sf) jpgLoad = (svc_imageLoader*)sf->getInterface();
		if(jpgLoad) {
			data = jpgLoad->loadImage(jpegData,jpegDataLen,&w,&h);
			sf->releaseInterface(jpgLoad);
		}
		resized=0;
		return data;
	}
...
bool P4SDevice::hasArt(songid_t songid) {return true;} // fucko
