...
static bool Audio_IsSupported(int type)
{
	...
	while (factory = WASABI_API_SVC->service_enumService(WaSvc::FLVDECODER, n++))
	{
...
static bool CheckSWF()
{
	if (!checked_in_swf)
	{
		...
		char in_swf_path[MAX_PATH];
		PathCombineA(in_swf_path, pluginsDir, "in_swf.dll");
		in_swf = LoadLibraryA(in_swf_path);
		checked_in_swf = true;
	}
	return !!in_swf;
}
			...
			case FLV::FRAME_TYPE_VIDEO: // video
				...
				if (m_need_seek == -1 || !Video_DecoderReady())
				{
					...
					if (videoHeader.Read(data, bytesRead))
					{
						...
						uint8_t *videodata = (uint8_t *)malloc(dataSize);
						if (videodata)
						{
			...
			case FLV::FRAME_TYPE_METADATA: // metadata
				{
					...
					uint8_t *metadatadata= (uint8_t *)malloc(dataSize);
					if (metadatadata)
					{
						...
						{
							FLVMetadata::Tag *tag = metadata.tags[i];
							if (!wcscmp(tag->name.str, L"onMetaData"))
							{
