...
#include "wmaudiosdk.h"  // for IWMAudioWriter
...
{
public:
	CustomIndexStatus( HANDLE _done ) : done(_done), IWMStatusCallback()
	{}
...
AudioCoderWMA::AudioCoderWMA(int numchannels, int samplerate, int bitspersamp, configtype *cfg, char *configfile) : AudioCoder()
{
	...
	hr = CreateAndConfigureWriter(numchannels, samplerate, bitspersamp, configfile);
...
void AudioCoderWMA::PrepareToFinish()
{
	...
	if (writer && begin_writing)
	{
		...
		HRESULT res = writer->EndWriting();
		// TODO: do we have to wait for this shit to finish?
...
void AudioCoderWMA::OnFinished(const wchar_t *wfname)
{
	...
	hr = pMyIndexer->Configure(0,                          // Stream Number, use all.
	...
	// Start the indexer.
	hr = pMyIndexer->StartIndexing(tempFilename, &status, NULL);
...
HRESULT AudioCoderWMA::SelectAndLoadResampler(int numchannels, int samplerate, int bitspersamp)
{
	...
	hr = writer->GetInputCount(&inCount);
	if(!FAILED(hr)){
		...
		if(!FAILED(hr)){
			...
			for(size_t i = 0;i < fmtCount;i++){
				...
				hr = writer->GetInputFormat(0, i, &pProps);
				if(!FAILED(hr)){
					DWORD cbSize = 0;
...
HRESULT AudioCoderWMA::CreateAndConfigureWriter(int numchannels, int samplerate, int bitspersamp, char *configfile)
{
	HRESULT hr = S_OK;
	...
	if ( !FAILED(hr) )
	{
		...
		WAVEFORMATEX WaveLimits =
		{
			WAVE_FORMAT_PCM,
				numchannels,
				samplerate,
				samplerate * numchannels * bitspersamp / 8,
				numchannels * bitspersamp / 8,
				bitspersamp,
				0
		};
