...
void readconfig(char *configfile, configtype *cfg)
{
	...
	if (configfile)
	{
		GetPrivateProfileStruct("audio_wma", "conf", cfg, sizeof(configtype), configfile);
	}
}
...
void writeconfig(char *configfile, configtype *cfg)
{
	if (configfile)
	{
		WritePrivateProfileStruct("audio_wma", "conf", cfg, sizeof(configtype), configfile);
	}
}
		...
		for (DWORD numPasses = 1;numPasses <= MAX_PASSES;numPasses++)
			for (DWORD i = 0;i != numCodecs;i++)
			{
				...
				else
				{
					...
					WideCharToMultiByteSZ(CP_ACP, 0, codecName, -1, eName, 4999, NULL, NULL);
					encp->encoderName = _strdup(eName);
				...
				for (DWORD f = 0;f != formatCount;f++)
				{
					...
					WideCharToMultiByteSZ(CP_ACP, 0, fDesc, -1, fName, 4999, NULL, NULL);
