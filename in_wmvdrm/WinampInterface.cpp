...
void WinampInterface::FlushAudits()
{
  PostMessage(GetWinampWindow(), WM_WA_IPC, 0, IPC_FLUSHAUDITS);
}
...
{
	{
		...
		StringCchCopy(statusFilename, 1040, activePlaylist.GetFileName());
	}
...
bool WinampInterface::GetStatus(wchar_t *title, size_t titleLen, const wchar_t *filename)
{
	...
	if (status[0] && title && filename && !lstrcmp(statusFilename, filename))
	{
