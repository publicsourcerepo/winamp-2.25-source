...
WMInformation::WMInformation(IWMReader *_reader)
		...
		editor(0), editor2(0), header(0), header3(0), header2(0)
{
...
WMInformation::WMInformation(IWMMetadataEditor *_editor)
		: editor(_editor), editor2(0), header(0), header3(0), reader(0), header2(0)
{
	editor->AddRef();
...
void WMInformation::SetAttribute_BinString(const wchar_t *attrName, wchar_t *value)
{
	...
	AutoChar data(value);
	header->SetAttribute(0, attrName, WMT_TYPE_BINARY, (BYTE *)(char *)data, strlen(data));
}
...
void WMInformation::SetAttribute(const wchar_t *attrName, wchar_t *value, WMT_ATTR_DATATYPE defaultType)
{
	...
	{
		BOOL boolValue;
		if (!wcscmp(L"True", value) || !wcscmp(L"true", value))
			boolValue = TRUE;
		else
			boolValue = FALSE;
...
void WMInformation::SetUserText(const wchar_t *description, const wchar_t *valueStr)
{
	...
	WORD index;
	HRESULT hr=header3->AddAttribute(0, L"WM/Text", &index, WMT_TYPE_BINARY, 0, (BYTE *) &userText, sizeof(userText));
	hr=hr;
}
