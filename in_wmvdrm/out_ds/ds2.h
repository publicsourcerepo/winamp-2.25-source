#ifndef NULLSOFT_DS2H
#define NULLSOFT_DS2H
...
#include "../../out_ds/SoundBlockList.h"
#include "../../out_ds/Config.h"
#include "../../out_ds/VolCtrl.h"
#include "../../out_ds/DevEnum.h"
...
public:
	~DS2();
	int WriteData(void * data, UINT size, bool doSilence, bool *killswitch); //returns 1 on success and 0 on failure
	...
	int ForceWriteData(void * data, UINT size, bool doSilence); //sleep-less force-write all the data w/o sleep/canwrite (async buffer has no size limit), use with caution
	...
	*/
	...
	inline UINT BufferStatusPercent() { return MulDiv(data_buffered + BlockList.DataSize(), buf_size, 100);}
	void ForcePlay();
	void KillEndGap();
...
private:
	...
	static Nullsoft::Utility::LockGuard ds2Guard;
...
public:
	...
	void GetRealtimeStat(struct DS2_REALTIME_STAT * stat);
