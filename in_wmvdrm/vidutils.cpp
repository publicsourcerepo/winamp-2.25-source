	...
	if (p || sr || wnd)
	{
		...
		if (!initted)
		{
			...
			if (h)
			{
				...
				Gmi = (BOOL (WINAPI *)(HMONITOR, LPMONITORINFOEX)) GetProcAddress(h, "GetMonitorInfoA");
			}
			initted = 1;
		}
		...
		{
			HMONITOR hm = NULL;
			if (sr) hm = Mfr(sr, MONITOR_DEFAULTTONEAREST);
			else if (wnd) hm = Mfw(wnd, MONITOR_DEFAULTTONEAREST);
			else if (p) hm = Mfp(*p, MONITOR_DEFAULTTONEAREST);
			if (hm)
			{
				MONITORINFOEX mi;
