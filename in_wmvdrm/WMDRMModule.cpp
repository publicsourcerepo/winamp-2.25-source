...
extern Out_Module waveMod, dsMod;
HINSTANCE cfg_orig_dll = 0;
void InitOutputs(HWND hMainWindow, HMODULE hDllInstance)
{
	ds.winampWnd = hMainWindow;
	ds.dllInstance = hDllInstance;
	ds.Init();
	wave.winampWnd = hMainWindow;
	wave.dllInstance = hDllInstance;
	wave.Init();
}
...
void WMDRM::AssignOutput()
{
	...
	if (plugin.outMod == &dsMod)
	{
		wmConfig.cfg_int(L"default_ds", 1) = 1;
		out = &ds;
	}
	else if (plugin.outMod == &waveMod)
	{
		wmConfig.cfg_int(L"default_ds", 0) = 0;
		out = &wave;
	}
	else if (WMIsContentProtected(activePlaylist.GetFileName(), &prot) == S_FALSE
	         && prot == FALSE)
	{
		out = &pluginOut;
	}
	else // TODO have a config option for preferred output when a non-allowed plugin is used.
	{
		if (wmConfig.cfg_int(L"default_ds", 1) == 1)
			out = &ds;
		else
			out = &wave;
	}
...
WMDRM::WMDRM()
		: paused(false),
		clock(0), audio(0), video(0), wait(0), info(0), buffer(0), seek(0), gain(0),
#ifndef NO_DRM
		drm(0),
#endif
...
void WMDRM::InitWM()
{
	...
	if (!triedInit)
	{
		...
		callback >> seek >> buffer >> clock
#ifndef NO_DRM
		>> drm
#endif
		>> video >> audio >> wait >> gain >> this;
	...
	*length_in_ms = -1000;
	...
	if (file && file[0])
	{
		...
		if (config_http_metadata || !isURL)
		{
			...
			*length_in_ms = getFileInfo.GetLengthMilliseconds();
		}
		else
		{
			if (title)
	...
	else if (activePlaylist.GetFileName())
	{
		...
		if (!wait->IsOpen()) // if it's not open, fill in with some default data... WMDRM::Opened() will refresh the title ...
		{
...
int WMDRM::Play(const wchar_t * fn)
{
	...
	startAtMilliseconds = 0;
	int result = seek->Open(activePlaylist.GetFileName(), &callback);
	if (result == 0)
	{
		//Tag->  Flush any pending audit pings
		winamp.FlushAudits();
	}
	return result;
}
