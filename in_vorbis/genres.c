...
extern const char *INI_DIRECTORY;

static void file_init(char * file_path,char * fn)
{
	PathCombine(file_path, INI_DIRECTORY, fn);
}
...
void genres_read(HWND wnd,char* fn)
{
	...
	while(!eof)
	{
		...
		{
			temp[ptr]=0;
			SendMessage(wnd,CB_ADDSTRING,0,(long)temp);
			if (add[0])
			{
	...
	CloseHandle(f);
	if (add[0]) SendMessage(wnd,CB_ADDSTRING,0,(long)add);
}
...
void genres_write(HWND wnd,char * fn)
{
	...
	HANDLE f;
	{
		...
		for(n=0;n<max;n++)
		{
			SendMessage(wnd,CB_GETLBTEXT,n,(long)temp);
			if (!_stricmp(temp,add)) return;
		}
		SendMessage(wnd,CB_ADDSTRING,0,(long)add);
	}
	file_init(file_path,fn);
	...
	{
		SendMessage(wnd,CB_GETLBTEXT,n,(long)temp);
		WriteFile(f,temp,strlen(temp),&bw,0);
		WriteFile(f,eol,2,&bw,0);
	}
	CloseHandle(f);
}