...
static INT_PTR CALLBACK aboutProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam) 
{
	switch (uMsg)
	{
		...
		case WM_LBUTTONDBLCLK :
			//easter eggs :)
			if (++m_effect >= M_NUM_FX) m_effect=0;
		...
		case WM_TIMER:
		{
			...
			if (m_effect == 0) // oldsk00l sine scroll
			{
				...
				if((scrolloffs&7)==7)
				{
					...
					else TextOut(scrolldc,100,0,&scrolltext[o],1);
				}
			} 
			...
			else if(m_effect==2) //gayfire
			{
				...
				if (fire_textcnt-- <= 0)
				{
					...
					DrawText(m_hdc,fire_texts[fire_textpos%(sizeof(fire_texts)/sizeof(fire_texts[0]))],-1,&r,DT_CENTER|DT_VCENTER|DT_SINGLELINE);
					if (fire_textcnt < -30)
					{
