...
void fdct_short_C ( INT16 *DCTDataBuffer, INT16 *DCT_codes )
{
	...
	for ( i = 0; i < 64; i++ )
		DCTDataBuffer[i] = (short)(DCTDataBuffer[i] << FDCT_PRECISION_BITS);
	...
	{	
		...
		if ( DCT_codes[i]  >= 0 )
			DCT_codes[i] = (short)((DCT_codes[i]) >> FDCT_PRECISION_BITS);
		else
			DCT_codes[i] = (short)((DCT_codes[i] + FDCT_PRECISION_NEG_ADJ) >> FDCT_PRECISION_BITS);
	}
