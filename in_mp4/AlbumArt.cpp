...
bool IsMyExtension(const wchar_t *filename)
{
	...
	if (extension && *extension)
	{
		...
		GetPrivateProfileStringW(L"in_mp4", L"extensionlist", AutoWide(defaultExtensions), exts, 1024, AutoWide(m_ini));
...
static int MimeTypeToFlags(const wchar_t *mime_type)
{
	...
	if (!wcsicmp(mime_type, L"jpeg")
		|| !wcsicmp(mime_type, L"jpg")
		|| !wcsicmp(mime_type, L"image/jpeg")
		|| !wcsicmp(mime_type, L"image/jpg"))
		return 13; /* JPEG */

if (!wcsicmp(mime_type, L"png")
		|| !wcsicmp(mime_type, L"image/png"))
		return 14; /* PNG */

if (!wcsicmp(mime_type, L"gif")
		|| !wcsicmp(mime_type, L"image/gif"))
		return 12; /* GIF */

if (!wcsicmp(mime_type, L"bmp")
		|| !wcsicmp(mime_type, L"image/bmp"))
		return 27; /* BMP */
