...
static const char defaultExtensions_nonpro[] = {"M4A;MP4"};
static const char defaultExtensions_pro[] = {"M4A;MP4;M4V"};
const char *defaultExtensions=defaultExtensions_nonpro;
...
static int getRegVer()
{
	...
	SendMessage(mod.hMainWindow, WM_WA_IPC, (WPARAM)s, IPC_GETREGISTEREDVERSION);
...
void init()
{
	...
	if (getRegVer() > 0)
	{
		defaultExtensions = defaultExtensions_pro;
	}
	...
	if (sf)
		WASABI_API_DLMGR = (api_downloadManager *)sf->getInterface();
	...
	const char *inipath = (char *)SendMessage(mod.hMainWindow, WM_WA_IPC, 0, IPC_GETINIDIRECTORY);

	PathCombineA(m_ini, inipath, "Plugins");
	CreateDirectoryA(m_ini, NULL);

	PathAppendA(m_ini, "in_mp4.ini");
...
void quit()
{
	...
	sf = WASABI_API_SVC->service_getServiceByGuid(DownloadManagerGUID);
	if (sf)
		sf->releaseInterface(WASABI_API_DLMGR);
		...
		if (length_in_ms) // calculate length
		{
			*length_in_ms = -1000; // the default is unknown file length (-1000).
			...
			if (hMp4)
			{
				...
				if (audio_track != -1)
				{
					int m_timescale = MP4GetTrackTimeScale(hMp4, audio_track);
					...
					*length_in_ms = (int)(m_length * 1000);
				}
				MP4Close(hMp4);
			}
		}
