...
typedef Vector<INFOKEYWORD, 8> KeywordList;
...
extern "C"
{
	__declspec( dllexport ) int winampGetExtendedFileInfoW(const wchar_t *fn, const char *data, wchar_t *dest, size_t destlen)
	{
		...
		if (!getFileInfoMP4 || lstrcmp(getFileInfoFn, fn) || HasFileTimeChanged(fn)) // different file than last time?
		{
...
static int SetExtendedInfoMP4(const char *data, const wchar_t *val)
{
	...
	{
		if (!setFileInfoMP4 || lstrcmp(m_last_ext_fn, fn)) // different file than last time?
		{
			m_last_err = 0;
