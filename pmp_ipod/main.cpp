...
static bool createBlankDatabase(wchar_t drive) {
	...
	if(!fh) return false;
	if(fwrite(data,l,1,fh)) _wunlink(ipodtest2);
	fclose(fh);
...
int init()
{
	...
	char mlipod[MAX_PATH];
	wsprintf(mlipod,"%s\\%s",charDir,"ml_ipod.dll");
	FILE * f = fopen(mlipod,"rb");
	if(f) {
		fclose(f);
		return -1;
	}
