...
void MakeDateString(__time64_t convertTime, wchar_t *date_str, size_t len)
{
	...
	GetDateFormat(LOCALE_USER_DEFAULT, DATE_SHORTDATE, &sysTime, NULL, date_str, len);
...
static wchar_t *wday[] = {L"Sun", L"Mon", L"Tue", L"Wed", L"Thu", L"Fri", L"Sat"};
static wchar_t *mon[] = {L"Jan", L"Feb", L"Mar", L"Apr", L"May", L"Jun", L"Jul", L"Aug", L"Sep", L"Oct", L"Nov", L"Dec"};
...
int getMonthOfYear(const wchar_t *str)
{
	...
	*/
	...
	if (!wcsncmp(str, L"Jan", 3))
		mon = 0;
	else if (!wcsncmp(str, L"Feb", 3))
		mon = 1;
	else if (!wcsncmp(str, L"Mar", 3))
		mon = 2;
	else if (!wcsncmp(str, L"Apr", 3))
		mon = 3;
	else if (!wcsncmp(str, L"May", 3))
		mon = 4;
	else if (!wcsncmp(str, L"Jun", 3))
		mon = 5;
	else if (!wcsncmp(str, L"Jul", 3))
		mon = 6;
	else if (!wcsncmp(str, L"Aug", 3))
		mon = 7;
	else if (!wcsncmp(str, L"Sep", 3))
		mon = 8;
	else if (!wcsncmp(str, L"Oct", 3))
		mon = 9;
	else if (!wcsncmp(str, L"Nov", 3))
		mon = 10;
	else if (!wcsncmp(str, L"Dec", 3))
		mon = 11;
...
int getGMOffset(const wchar_t *str)
{
	...
	if (!wcsncmp(str, L"UT", 2) || !wcsncmp(str, L"GMT", 3))
		secs = 0;
	else if (!wcsncmp(str, L"EDT", 3))
		secs = -4 * 3600;
	else if (!wcsncmp(str, L"PST", 3))
		secs = -8 * 3600;
	else if (!wcsncmp(str, L"EST", 3) || !wcsncmp(str, L"CDT", 3))
		secs = -5 * 3600;
	else if (!wcsncmp(str, L"MST", 3) || !wcsncmp(str, L"PDT", 3))
		secs = -7 * 3600;
	else if (!wcsncmp(str, L"CST", 3) || !wcsncmp(str, L"MDT", 3))
		secs = -6 * 3600;
