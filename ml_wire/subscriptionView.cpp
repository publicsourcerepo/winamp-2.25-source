...
{
	static const INT szButtons[] = { IDC_PLAY, IDC_ENQUEUE, IDC_DOWNLOAD, };
	HWND hControl;
...
static HRESULT SubscriptionView_FormatFont(LPWSTR pszBuffer, INT cchBufferMax, HFONT hFont)
{
	...
	LPCWSTR style = (0 == lf.lfItalic) ? L"normal" : L"italic";

	return StringCchPrintfEx(pszBuffer, cchBufferMax, NULL, NULL, STRSAFE_IGNORE_NULLS, 
				szTemplate, style, lf.lfWeight, lf.lfHeight, lf.lfFaceName);
}
...
static void SubscriptionView_UptateInfo(HWND hwnd)
{
	...
	if (podcast->description != infoText &&
		CSTR_EQUAL != CompareString(CSTR_INVARIANT, 0, podcast->description, -1, infoText, -1))
	{
...
static INT SubscriptionView_Play(HWND hwnd, size_t iChannel, size_t *indexList, size_t indexCount, BOOL fEnqueue, BOOL fForce)
{
	...
	cchBuffer++;
	LPWSTR pszBuffer = (LPWSTR)malloc(sizeof(WCHAR) * cchBuffer);
...
static INT SubscriptionView_PlayChannel(HWND hwnd, size_t iChannel, BOOL fEnqueue, BOOL fForce)
{
	...
	if (0 != count) 
	{
		list = (size_t*)malloc(sizeof(size_t) * count);
...
static void PodcastItem_InitializeList(HWND hwnd)
{
	...
	if (NULL != imageList)
	{
		...
		if (NULL != prevList) ImageList_Destroy(prevList);
	}
...
static void PodcastItem_SelectionChanged(HWND hwnd, BOOL fImmediate)
{
	...
	SubscriptionView_UptateInfo(hwnd);
}
...
static void PodcastChannel_SelectionChanged(HWND hwnd, BOOL fImmediate)
{
	...
	if (-1 != iSelected && FALSE == podcast->channelAscending)
		iSelected = channels.size() - iSelected - 1;
...
static void SubscriptionView_ListContextMenu(HWND hwnd, INT controlId, POINTS pts)
{
	...
	if (NULL != menu)
		Menu_TrackPopup(menu, TPM_LEFTALIGN | TPM_TOPALIGN | 0x1000/*TPM_VERPOSANIMATION*/, pt.x, pt.y, hwnd, NULL);
...
static void SubscriptionView_UpdateLayout(HWND hwnd, BOOL fRedraw, UINT uReason, HRGN validRegion, POINTS layoutOffset)
{
	...
	const INT szItems[LI_LAST] = {  IDC_CHANNELLIST, IDC_ITEMLIST, IDC_VDIV, 
                                    ...
                                    IDC_PLAY, IDC_ENQUEUE, IDC_DOWNLOAD, IDC_VISIT,	IDC_STATUS};

	LAYOUTITEM *layout = (LAYOUTITEM*)malloc(sizeof(LAYOUTITEM) * ARRAYSIZE(szItems));
	...
	LONG t, t2;
	t = (clientWidth - layout[LI_VERT].cx) * podcast->vertDivider / 10000;
	t2 = clientRect.right - layout[LI_VERT].cx;

	if (LAYOUTREASON_DIV_LEFT == uReason && t < clientRect.left + 32)
		t = clientRect.left;

	if (LAYOUTREASON_DIV_RIGHT == uReason && t > (t2 - 32))
		t = t2;

	if (t < clientRect.left) t = clientRect.left;
	else if (t > t2) t = t2;
	...
	layout[LI_HORZ].cx = clientWidth - layout[LI_HORZ].x*2;
	layout[LI_EPISODE_INFO].cx = clientWidth - layout[LI_EPISODE_INFO].x*2;
	layout[LI_INFO].cx = clientWidth - layout[LI_INFO].x*2;
	LI_SET_R(layout[LI_STATUS], clientWidth);
	if (layout[LI_STATUS].cx < 20) layout[LI_STATUS].cx = 0;

	t = clientHeight - layout[LI_PLAY].cy;
	const INT szBottomRow[] = { LI_PLAY, LI_ENQUEUE, LI_DOWNLOAD, LI_VISIT, LI_STATUS, };
	...
	if (LAYOUTREASON_DIV_TOP == uReason && t < clientRect.top + 36)
		t = clientRect.top;

	if (LAYOUTREASON_DIV_BOTTOM == uReason && t > (t2 - 36))
		t = t2;

	if (t < clientRect.top) t = clientRect.top;
	else if (t > t2) t = t2;
	...
	for (INT i = 0; i < ARRAYSIZE(szMiddleRow); i++)
	{
		...
		p->cy = (t < 80) ? 0 : podcast->middleRowHeight;
		p->y = t - p->cy;
	}

	layout[LI_HORZ].y = LI_GET_B(layout[LI_FINDNEW]);
	layout[LI_EPISODE_INFO].y = LI_GET_B(layout[LI_HORZ]);
	if (clientRect.bottom - layout[LI_EPISODE_INFO].y  < episode_info_cy)
	{
	...
	LI_SET_B(layout[LI_INFO], layout[LI_PLAY].y - podcast->bottomRowSpace);
	...
	for (INT i = 0; i < ARRAYSIZE(szImages); i++)
	{
		...
		if (NULL != bitmap)
		{
			...
			if (sizeof(dib) == GetObject(bitmap, sizeof(dib), &dib))
			{
				...
				if (filter.cy < 0) filter.cy = -filter.cy;
...
static void SubscriptionView_UpdateSkin(HWND hwnd)
{
	...
	for (INT i = 0; i < ARRAYSIZE(szLists); i++)
	{
		...
		{		
			...
			imageList = (HIMAGELIST)SendMessage(hControl, LVM_GETIMAGELIST, LVSIL_SMALL, 0L);
			if (NULL != imageList)
			{
...
static void SubscriptionView_InitMetrics(HWND hwnd)
{
	...
	{
		podcast->middleRowHeight = rect.bottom - rect.top;
		t = rect.top;
...
static void PodcastItem_OnKeyDown(HWND hwnd, NMLVKEYDOWN *pkd)
{
	...
	switch(pkd->wVKey)
	{
		...
		case 'A':		
			if (!shift && ctrl)
			{
				INT iCount = (INT)SNDMSG(pkd->hdr.hwndFrom, LVM_GETITEMCOUNT, 0, 0L);
				if (0 != iCount)
				{
					INT iSelected = (INT)SNDMSG(pkd->hdr.hwndFrom, LVM_GETSELECTEDCOUNT, 0, 0L);
					if (iSelected != iCount)
					{
						LVITEM lvi;
						lvi.state = LVIS_SELECTED;
						lvi.stateMask = LVIS_SELECTED;
						SNDMSG(pkd->hdr.hwndFrom, LVM_SETITEMSTATE, (WPARAM)-1, (LPARAM)&lvi);
					}
				}
			}
			break;
...
static INT_PTR SubscriptionView_OnInitDialog(HWND hwnd, HWND hFocus, LPARAM lParam)
{
	...
	const INT szControls[] = { IDC_FINDNEW, IDC_ADD, IDC_EDIT, IDC_DELETE, IDC_REFRESH, IDC_PLAY, IDC_ENQUEUE, IDC_DOWNLOAD, IDC_VISIT, IDC_STATUS, };
	SubscriptionView_SkinControls(hwnd, szControls, ARRAYSIZE(szControls), SKINNEDWND_TYPE_AUTO, SWS_USESKINCOLORS | SWS_USESKINCURSORS);
	
	HWND hControl;

	hControl = GetDlgItem(hwnd, IDC_HDIV);
...
static void PodcastCommand_OnDeleteChannel(HWND hwnd)
{
	...
	WCHAR szText[1024], szBuffer[1024];
	...
	{
		channels.RemoveChannel(iChannel);
...
static void PodcastCommand_OnPlaySelection(HWND hwnd, BOOL fEnqueue, BOOL fForce)
{
	...
	size_t indexCount = 0;
	size_t *indexList = (size_t*)malloc(sizeof(size_t) * selectedCount);
...
static void SubscriptionView_OnCommand(HWND hwnd, INT controlId, INT eventId, HWND hControl)
{
	switch (controlId)
	{
		case IDC_PLAY:			PodcastCommand_OnPlaySelection(hwnd, FALSE, TRUE); break;
		case IDC_ENQUEUE:		PodcastCommand_OnPlaySelection(hwnd, TRUE, TRUE);break;
		case IDC_PLAYACTION:	PodcastCommand_OnPlaySelection(hwnd, FALSE, FALSE); break;
		case IDC_ENQUEUEACTION:	PodcastCommand_OnPlaySelection(hwnd, TRUE, FALSE); break;
