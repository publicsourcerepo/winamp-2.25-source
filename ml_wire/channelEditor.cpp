...
static INT_PTR ChannelEditor_SaveChannel(HWND hwnd)
{
	...
	if (channel->autoUpdate) cloud.Pulse();
	return IDOK;
}
...
static DWORD AddUrlThread(void *vBuffer, HWND hwndDlg)
{
	...
	{
		wchar_t error_msg[1024], titleStr[64];
		StringCchPrintf(error_msg, 1024, WASABI_API_LNGSTRINGW(IDS_CHANNEL_ALREADY_PRESENT),
						newFeed.title, newFeed.url);
		MessageBox(hwndDlg, error_msg,
				   WASABI_API_LNGSTRINGW_BUF(IDS_DUPLICATE_CHANNEL,titleStr,64), MB_OK);
...
static INT ChannelEditor_AddUrl(HWND hwnd)
{
	...
	GetDlgItemText(hwnd, IDC_EDITURL, buffer, length);
...
static INT_PTR ChannelEditor_CreateChannel(HWND hwnd)
{
	...
	{
		WCHAR szMessage[512], szTitle[128];
