...
BOOL Layout_Perform(HWND hwnd, LAYOUTITEM *layout, INT layoutCount, BOOL fRedraw)
{
	...
	for (INT i = 0; i < layoutCount; i++)
	{
		...
		if (0 != (SWP_HIDEWINDOW & item->flags))
		{
		...
		if ((SWP_NOSIZE | SWP_NOMOVE) != ((SWP_NOSIZE | SWP_NOMOVE | SWP_FRAMECHANGED) & flags))
		{
			...
			if (NULL == hdwpTemp) break;
			hdwp = hdwpTemp;
		}
	}

	BOOL result = (NULL != hdwp) ? 	EndDeferWindowPos(hdwp) : FALSE;
...
{
	if (NULL == rect || NULL == layout) return FALSE;
...
{
	if (NULL == rect || NULL == indexList || NULL == layout) return FALSE;
...
BOOL Layout_GetValidRgn(HRGN validRgn, POINTS parrentOffset, const RECT *validRect, LAYOUTITEM *layout, INT layoutCount)
{
	...
	for (INT i = 0; i < layoutCount; i++)
	{
		...
		if (NULL != item->hwnd && 0 == ((SWP_HIDEWINDOW | SWP_SHOWWINDOW) & item->flags))
		{
			...
			r = l + item->cx;
			b = t + item->cy;
			if (0 != (SWP_NOREDRAW & item->flags) ||
				(l == item->rect.left && t == item->rect.top && r == item->rect.right && b == item->rect.bottom))
			{
				if (NULL != validRect)
				{
					if (l < validRect->left) l = validRect->left;
					if (t < validRect->top) t = validRect->top;
					if (r > validRect->right) r = validRect->right;
					if (b > validRect->bottom) b = validRect->bottom;
				}
