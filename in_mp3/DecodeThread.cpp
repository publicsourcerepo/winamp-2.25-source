...
void DecodeLoop::SetupStream()
{
	...
	while (!killDecodeThread && file.Peek(buf, 1024, &len) == SSC_OK && !len)
		Sleep(50);
...
{
	int decoderDelaySamples = decoder->GetDecoderDelay();
	...
	if (!config_fastvis || (decoder->GetLayer() != 3 || g_ds))
	{
		...
		if (csa || is_vis_running)
		{
			...
			size_t pos = 0;
			{
				...
				{
					int a, b;
					if (mod.SAGetMode()) mod.SAAddPCMData((char *)g_samplebuf + pos, g_nch, g_bits, ti);
					if (mod.VSAGetMode(&a, &b)) mod.VSAAddPCMData((char *)g_samplebuf + pos, g_nch, g_bits, ti);
...
void DecodeLoop::Decode()
{
	while (g_samplebuf_used < (size_t)g_framesize && !killDecodeThread && seek_needed == -1)
	{
		...
		if (config_gapless && endCut)
			endCutter.SetEndSize(endCut*sampleFrameSize);
...
void DecodeLoop::PreBuffer()
{
	...
	else
	{
		...
		if (bpos != last_bpos)
		{
			...
			last_bpos = bpos;
			EnterCriticalSection(&g_lfnscs);
			if (strcmp(lastfn_status, "stream temporarily interrupted"))
...
void DecodeLoop::Seek(int seekPosition)
{
	...
	int br = decoder->GetCurrentBitrate();
...
DWORD DecodeLoop::OpenDecoder()
{
	...
	if (isAac)
	{
		if (isEAAC)
		{
				waServiceFactory *factory = WASABI_API_SVC->service_getServiceByGuid(adts_aac_guid);
				if (factory)
	...
	{
		decoder = new ADTS_MP2;
		mod.UsesOutputPlug|=8;
	}

	if (decoder 
		&& decoder->Initialize(AGAVE_API_CONFIG->GetBool(playbackConfigGroupGUID, L"mono", false), config_downmix == 2,  AGAVE_API_CONFIG->GetBool(playbackConfigGroupGUID, L"surround", true), AGAVE_API_CONFIG->GetUnsigned(playbackConfigGroupGUID, L"bits", 16), true, false) == adts::SUCCESS
		&& decoder->Open(&file))
	{
		// sync to stream
		while (1)
...
DWORD DecodeLoop::Loop()
{
	...
	if (!SSC_SUCCESS(file.Open(lastfn, config_max_bufsize_k)))
	{
		if (!killDecodeThread) Sleep(200);
		if (!killDecodeThread) PostMessage(mod.hMainWindow, WM_WA_MPEG_EOF, 0, 0);
		return 0;
...
// TODO?	if (decoder != &aacp) // hack because aac+ bitrate isn't accurate at this point
	...
	need_prebuffer = CalcPreBuffer(config_http_prebuffer, br);
	...
	sampleFrameSize = g_nch * (g_bits/8);

	if (config_gapless)
		CalculateCodecDelay();

	if (OpenOutput(g_nch, g_srate, g_bits) == 0)
		return 0;

	/* ----- send info to winamp and vis: bitrate, etc ----- */
	g_br = decoder->GetCurrentBitrate();
				...
				// factor of two (for tempo adjustment).
			{
				...
				if ((g_samplebuf_used >= (size_t)g_framesize || (done && g_samplebuf_used > 0)) && seek_needed == -1)
				{
					...
					if (file.isSeekReset())
						...
						decode_pos_ms += (g_samplebuf_used / sampleFrameSize  * 1000) / g_srate;

					// if we have a DSP plug-in, then call it on our samples
					if (mod.dsp_isactive())
					{
						g_samplebuf_used = mod.dsp_dosamples((short *)g_samplebuf, g_samplebuf_used / sampleFrameSize, g_bits, g_nch, g_srate) * sampleFrameSize;
					}
					Viz();
					endCutter.Write((char *)g_samplebuf, g_samplebuf_used);
					g_samplebuf_used = 0;
					needsleep = 0;
