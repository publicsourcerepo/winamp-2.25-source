...
private:
	...
	int preCut, preCutSize;
};

GapCutter::GapCutter() :
		preCutSize(0), preCut(0)
{}
...
int GapCutter::Write(void *dest, void *input, size_t inputBytes) // returns # of bytes written
{
	...
	int outWrite = max(0, (intptr_t)(inputBytes - remainingFill));
	if (outWrite)
...
struct ExtendedRead
{
	ExtendedRead() : decoder(0), initialData(0){}
	~ExtendedRead()
	{
		file.Close();
		if (decoder)
...
{
	if (!SSC_SUCCESS(file.Open(fn, config_max_bufsize_k)))
		return false;
	...
	*bps = bits = numBits;
	...
	if (config_gapless)
		cutter.SetSize((file.prepad + decoder->GetDecoderDelay())*frameSize, (file.postpad - decoder->GetDecoderDelay())*frameSize);
	...
	else // no helpful info to go on
	{
		...
		int len_ms = MulDiv(file.GetContentLength(), 8, bitrate);
...
extern "C"
{
		...
		while (len)
		{
			...
			if (!ext->initialData)
			{
				...
				if (config_gapless && endCut)
					ext->cutter.SetEndSize((endCut - ext->decoder->GetDecoderDelay())*ext->frameSize);
	...
	__declspec(dllexport) int winampGetExtendedRead_setTime(intptr_t handle, int millisecs)
	{
		...
		if (!br) br = ext->decoder->GetCurrentBitrate();
