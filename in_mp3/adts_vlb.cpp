...
int ADTS_VLB::Initialize(bool forceMono, bool reverseStereo, bool allowSurround, int maxBits, bool allowRG, bool _useFloat)
{
	return 0;
}
bool ADTS_VLB::Open(ifc_mpeg_stream_reader *file)
{
	waServiceFactory *factory = WASABI_API_SVC->service_getServiceByGuid(obj_vlbDecoderGUID);
	if (factory)
...
void ADTS_VLB::CalculateFrameSize(int *frameSize)
{
	*frameSize = 576 * 2 * params.num_channels;
	if (*frameSize > 576*2*2) *frameSize = 576 * 2 * 2;
}
