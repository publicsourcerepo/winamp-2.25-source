...
{
	unsigned char buffer[512]; /* don't want to read too much, since most MP3's start at 0 */
	int buflen;

	int checked=0;
	if (file->Peek(buffer, sizeof(buffer), &buflen) != SSC_OK)
		return false;
...
{
	unsigned char buffer[1448 + 4]; /* large enough for one max-size frame and the header of the second */
	int buflen;
...
void GetFileDescription(const wchar_t *file, CGioFile &_file, wchar_t *data, size_t datalen)
{
	...
	else
	{
		...
		unsigned char mp3syncbuf[INFO_READ_SIZE];
		int inputRead;
		...
		decoder.Connect(&_file);
#ifndef NO_MP3SURROUND
		SADEC_HANDLE saDecHandle = 0;
		bool surround = false;
		...
		unsigned char ancBytes[8192];
		int numAncBytes   = 0;

again:
		SSC ssc = decoder.DecodeFrame(floatTemp, sizeof(floatTemp), &outputFrameSize,
		                              ancBytes, &numAncBytes, 1, &delay, &totalLength);

		if (ssc == SSC_I_MPGA_NOMAINDATA)
		{
			goto again;
		}
		else if (ssc == SSC_OK)
		{
			if (delay && !_file.prepad)
			{
				// validate
				if (delay >= 529)
				{
				encoderDelay = delay;
				padding = 1152 - ((totalLength + delay) % 1152);
				encoderDelay -= 529; // subtract out decoder delay
				padding+=529; // add decoder delay to end
				padding+=encoderDelay;
				_file.m_vbr_samples = totalLength;
				}
			}
#ifndef NO_MP3SURROUND
			if (!numAncBytes && saDecHandle)
			{
				ssc = decoder.DecodeFrame(floatTemp, sizeof(floatTemp), &outputFrameSize, ancBytes, &numAncBytes);
				if (ssc != SSC_OK)
				{
			...
			if (numAncBytes && saDecHandle)
			{
				...
				{
					IIS_SADec_InitInfo(saDecHandle, decoder.GetStreamInfo()->GetSFreq(), decoder.GetStreamInfo()->GetChannels());
					SA_DEC_INFO saInfo = IIS_SADec_GetInfo(saDecHandle);
					if (saInfo.configuredMode != SA_DEC_BYPASS)
						surround = true;
				}
				else
				{
...
#endif
			int framelen = decoder.m_Mbs.GetHdr()->GetFrameLen();
			...
			int iLen = (is_vbr_lens ? is_vbr_lens/1000 : ((flen * 8) / info->GetBitrate()));
			if(iLen > 0)
				StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_LENGTH_X_SECONDS, langbuf, 256), iLen);
			else
			{
				float fLen = (is_vbr_lens ? is_vbr_lens/1000.0f : ((flen * 8.0f) / info->GetBitrate()));
				StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_LENGTH_X_PART_SECONDS, langbuf, 256), fLen);
			}

			StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_S_LAYER_X, langbuf, 256), GetMPEGVersionString(info->GetMpegVersion()), info->GetLayer());

			int frames = _file.m_vbr_frames;

			int is_vbr = _file.m_vbr_flag || _file.m_vbr_gayhdr;
			if (!is_vbr || _file.encodingMethod == ENCODING_METHOD_CBR)
			{
				if (frames)
					StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_X_KBIT,  langbuf,256), info->GetBitrate() / 1000, frames);
				else
					StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_X_KBIT_APPROX, langbuf, 256), info->GetBitrate() / 1000, MulDiv(flen, 8, framelen));
			}
				...
				StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_X_KBIT_VBR,  langbuf,256), _file.GetAvgVBRBitrate(), _file.m_vbr_gayhdr?"I":"", frames);

			StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_X_HZ_S, langbuf, 256), info->GetSFreq(), GetChannelModeString(header->GetMode()));
			StringCchPrintfEx(mt, size, &mt, &size, 0, L"\r\nCRC: %s", WASABI_API_LNGSTRINGW_BUF((header->GetCrcCheck() ? IDS_YES : IDS_NO), langbuf, 256));
			wchar_t tmp[16];
			StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_COPYRIGHTED, langbuf, 256), WASABI_API_LNGSTRINGW_BUF((header->GetCopyright() ? IDS_YES : IDS_NO),tmp,16));
			StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_ORIGINAL, langbuf, 256), WASABI_API_LNGSTRINGW_BUF((header->GetOriginal() ? IDS_YES : IDS_NO),tmp,16));
			StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_EMPHASIS, langbuf, 256), GetEmphasisString(header->GetEmphasis()));
#ifndef NO_MP3SURROUND
			if (surround)
				StringCchPrintfEx(mt, size, &mt, &size, 0, WASABI_API_LNGSTRINGW_BUF(IDS_EXT_MP3_SURROUND, langbuf, 256));
#endif
...
void GetAudioInfo(const wchar_t *filename, CGioFile *file, int *len, int *channels, int *bitrate, int *vbr, int *sr)
{
	...
	*channels=0;
	if (file)
	{
		...
		if (!_wcsicmp(ext, L".aac") || !_wcsicmp(ext, L".vlb"))
		{
			unsigned char t[INFO_READ_SIZE*2];
			unsigned char *a = t;
			int n;

			if (file->Read(t, sizeof(t), &n) != SSC_OK)
				return;
		...
		else
		{
			...
			{
				*len = file->m_vbr_ms;
				*vbr = file->m_vbr_flag || file->m_vbr_gayhdr;
			}
			...
			if (file->Peek(t, sizeof(t), &n) != SSC_OK)
				return;
