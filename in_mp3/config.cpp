	...
	case WM_NOTIFY:
		{
			...
			if (pnmh->code == PSN_APPLY)
			{
				...
				if (!strcmp(config_rating_email, "rating@winamp.com\0")) config_rating_email[0] = 0;
	...
	case WM_NOTIFY:
		{
			...
			if (pnmh->code == PSN_APPLY)
			{
				...
				config_http_prebuffer = SendMessage(GetDlgItem(hwndDlg, IDC_PREBUFSLIDER), TBM_GETPOS, 0, 0) * 2;
				config_http_prebuffer_underrun = SendMessage(GetDlgItem(hwndDlg, IDC_PREBUFSLIDER2), TBM_GETPOS, 0, 0) * 2;
