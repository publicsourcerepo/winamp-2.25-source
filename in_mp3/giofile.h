...
/* ------------------------ includes --------------------------------------*/
#include "../mp3/mp3dec/giobase.h"
#include "../mp3/mp3dec/mp3sscdef.h"
...
class CGioFile : public CGioBase, public DataIOControl, public ifc_mpeg_stream_reader
{
public:
	CGioFile();
	virtual ~CGioFile();

	SSC  Open(const wchar_t *pszName, int maxbufsizek);
	SSC  Close();
	SSC  Read(void *pBuffer, int cbToRead, int *pcbRead);
	SSC  Peek(void *pBuffer, int cbToRead, int *pcbRead);
	...
	int PercentAvailable()
	{
		...
		uint64_t bytes_100 = m_connection->GetReceiveBytesAvailable()*100;
	...
	int RunStream()
	{
		if (m_is_stream && m_connection)
		{
			...
			else m_connection->run();
			int p=m_connection->get_state();
			if (p==CONNECTION_STATE_ERROR||
			    p==CONNECTION_STATE_CLOSED)
				return 2;
			return 1;
		}
		return 0;
