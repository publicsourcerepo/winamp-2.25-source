...
wchar_t *ID3_GetUnicodeString(ID3_Frame *frame, ID3_FieldID fldName, size_t nIndex)
{
	...
	{
		size_t nText = frame->Field(fldName).Size();
		text = (wchar_t *)malloc(sizeof(wchar_t) * (nText + 1));
		text[0] = 0;
...
{
	ID3_Frame *frame;
	frame = tag->Find(ID3FID_COMMENT, ID3FN_DESCRIPTION, L"");
	if (frame)
	{
...
wchar_t *ID3_GetRating(ID3_Tag *tag, wchar_t *dest, size_t destlen)
{
	...
	if (frame)
	{ 
		int rating = frame->Field(ID3FN_RATING).Get();
...
wchar_t *ID3_GetGracenoteTagID(ID3_Tag *tag)
{
	...
	ID3_Frame* frame = NULL;
	frame = tag->Find(ID3FID_UNIQUEFILEID, ID3FN_OWNER, L"http://www.cddb.com/id3/taginfo1.html");
	if (frame)
	{
		uchar data[64];
		luint dataSize = frame->Field(ID3FN_DATA).Size();
			frame->Field(ID3FN_DATA).Get(data, 64);
		int converted = MultiByteToWideChar(CP_ACP, 0, (const char *)data, dataSize, 0, 0);
		wchar_t *dest = (wchar_t *)malloc((converted+1)*sizeof(wchar_t));
		converted = MultiByteToWideChar(CP_ACP, 0, (const char *)data, dataSize, dest, converted);
		dest[converted]=0;
		return dest;
	}
	return 0;
...
wchar_t *ID3_GetGracenoteTagID(ID3_Tag *tag, wchar_t *dest, size_t destlen)
{
	...
	if (frame)
	{
		...
		luint dataSize = frame->Field(ID3FN_DATA).Size();
			frame->Field(ID3FN_DATA).Get(data, 64);
		int converted = MultiByteToWideCharSZ(CP_ACP, 0, (const char *)data, dataSize, dest, destlen);
		dest[converted]=0;
		return dest;
	}
	return 0;
...
void ID3_AddSetGracenoteTagID(ID3_Tag *tag, const wchar_t *tagID)
{
	...
	if (frame)
	{
		...
		else
		{
			...
			uchar data[64];
			luint dataSize = WideCharToMultiByte(CP_ACP, 0, tagID, origLen, (char *)data, 64, 0, 0);
...
{
	ID3_Frame *frame;
	frame = tag->Find(ID3FID_USERTEXT, ID3FN_DESCRIPTION, desc);
	if (frame)
	{
		if (!value || !value[0])
			tag->RemoveFrame(frame);
