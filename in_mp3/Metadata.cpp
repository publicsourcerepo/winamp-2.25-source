...
Metadata::Metadata() : filename(0), sampleRate(0), bitrate(0), vbr(0), channels(0), length_ms(0), refs(1)
{
	filename=0;
}
...
int Metadata::Open(const wchar_t *_filename)
{
	...
	if (file.Open(filename, INFO_READ_SIZE/1024) != SSC_OK)
		return 1;
...
Metadata::Metadata(CGioFile *_file, const wchar_t *_filename) : filename(0), sampleRate(0), bitrate(0), vbr(0), channels(0), length_ms(0), refs(1)
{
	...
	if (bitrate = _file->GetAvgVBRBitrate()*1000)
	{
		...
		vbr = _file->m_vbr_flag || _file->m_vbr_gayhdr;
	}
}
...
static int ID3Write(const wchar_t *filename, HANDLE infile, DWORD offset, void *data, DWORD len)
{
	...
	if (outfile != INVALID_HANDLE_VALUE)
	{
		...
		do
		{
			...
			if (read)
				WriteFile(outfile, data, read, &written, NULL);
		}
		while (read != 0);
		CloseHandle(outfile);
		CloseHandle(infile);
...
int Metadata::Save()
{
	...
	/* Strip Lyrics3 tag */
	if (!strippedLyrics3
	    && config_parse_lyrics3 && lyrics3.IsDirty())
	{
		uint32_t len;
	...
	if (config_parse_apev2 && config_write_apev2 && apev2.IsDirty() && apev2.HasData())
	{
		...
		if (apev2_data && apev2.Encode(apev2_data, apev2_len) == APEv2::APEV2_SUCCESS)
		{
			...
			WriteFile(metadataFile, apev2_data, apev2_len, &bytesWritten, 0);
			free(apev2_data);
	...
	if (config_parse_id3v2 && config_write_id3v2 && id3v2.IsDirty())
	{
		...
		else
		{
			...
			if (newlen <= oldlen) // if we can fit in the old tag
			{
				...
				newlen = oldlen;
				uint8_t *new_id3v2_data = (uint8_t *)malloc(newlen);
...
int Metadata::GetString(const char *tag, wchar_t *data, int dataLen)
{
	...
	else if (!_stricmp(tag, "endoffset"))
	{
		if (file.m_vbr_frames)
		{
			...
			if (totalFrames > 8)
			{
				float percent = (float)(totalFrames - 8) / (totalFrames);
				int seekPoint = 0;
