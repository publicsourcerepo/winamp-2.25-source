...
#ifdef DS2_HAVE_DEVICES
		pDirectSoundEnumerate=(tDirectSoundEnumerate)GetProcAddress((HMODULE)hdsound,"DirectSoundEnumerateA");
...
//	SYNCFUNC;	//inside sync already
	...
	if (!pDS)
	{
		...
		{
#ifdef DS2_HAVE_DEVICES
			cfg->SetErrorCodeMsg(DsDevEnumGuid(cur_dev) ? WASABI_API_LNGSTRING(IDS_BAD_DS_DRIVER) : WASABI_API_LNGSTRING(IDS_DEVICE_NOT_FOUND_SELECT_ANOTHER),hr);
#else
...

	{
		...
		if (coop_mode!=new_coop)
		{
			...
			{
				pDS->Release();pDS=0;
				cfg->SetErrorCodeMsg(WASABI_API_LNGSTRING(IDS_ERROR_SETTING_DS_COOPERATIVE_LEVEL),hr);
				return 0;
			}
		}
	...
	if (FAILED(hr) || !pDSB)
	{
		cfg->SetErrorCodeMsg(WASABI_API_LNGSTRING(IDS_ERROR_CREATING_DS_BUFFER),hr);
		return 0;
	}
