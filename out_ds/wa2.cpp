...
void Init()
{
	...
	char temp[MAX_PATH];
	GetModuleFileName(mod.hDllInstance, temp, sizeof(temp));
	p = temp +strlen(temp);
	while (*p != '\\' && p >= temp)
	{
		if (*p == '.')
...
{ //messy
	...
	if (!pDS2)
	{
		...
		if (!pDS2)
		{
			...
			{
				char errStr[128];
				wsprintf(errStr,WASABI_API_LNGSTRING(IDS_ERROR),mod.description);
				MessageBox(mod.hMainWindow, moo, errStr, MB_ICONERROR);
			}
			KILL_SSRC;
			SYNC_OUT;
			return -1;
...
HMODULE thisMod=0;
Out_Module *(*dsGetter)(HINSTANCE) = 0;
HMODULE inWMDLL = 0;
...
extern "C"
{
	__declspec(dllexport) Out_Module * winampGetOutModule()
	{
		...
		GetModuleFileName(thisMod, thisName, MAX_PATH);
		char *filespec = PathFindFileName(thisName);
		if (filespec && !lstrcmpi(filespec, "out_ds.dll"))
		{
			inWMDLL = GetModuleHandle("in_wm.dll");
			if (inWMDLL)
			{
				dsGetter = (Out_Module * (*)(HINSTANCE))GetProcAddress(inWMDLL, "GetDS");
				if (dsGetter)
					return dsGetter(thisMod);
			}
		}
		return &mod;
	}
...
extern void format_fade(char * txt, FadeCfgCopy * c, int idx);
...
{
	WNDCLASS wc;
	if (!GetClassInfo(mod.hDllInstance, DS_IPC_CLASSNAME, &wc))
	{
		...
		if (!_r)
		{
			log_write("could not create ipc");
		}
	}

	ipcWnd = CreateWindow(DS_IPC_CLASSNAME, "", WS_CHILD, 0, 0, 1, 1, mod.hMainWindow, NULL, mod.hDllInstance, NULL);
