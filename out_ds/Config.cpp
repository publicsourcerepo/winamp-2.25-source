...
void DS2config::SetErrorCodeMsg(const TCHAR *msg, DWORD code)
{
	if (code)
	{
		TCHAR boo[512], buf[512];
#ifdef UNICODE
		wsprintf(boo,WASABI_API_LNGSTRINGW_BUF_DS(IDS_ERROR_CODE_08X,buf,512),msg,code);
#else
