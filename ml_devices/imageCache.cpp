...
typedef nu::PtrList<DeviceColoredImage> DeviceColoredImageList;
...
typedef nu::PtrList<DeviceImage> DeviceImageList;
...
DeviceImageCache_GetImage(DeviceImageCache *self, const wchar_t *path, int width, int height, DeviceImageLoader loader, void *user)
{
	...
	image_ptr = (DeviceImage**)bsearch(&searchParam, self->list.begin(), self->list.size(), 
										sizeof(DeviceImage**),
	...
	if (NULL != image)
	{
		...
		qsort(self->list.begin(), self->list.size(), sizeof(DeviceImage**), DeviceImageCache_SortCb);
	}
...
{
	size_t listSize;
	DeviceColoredImage **image_ptr, *image;
	...
	if (listSize > 0)
	{
		image_ptr = (DeviceColoredImage**)bsearch(&searchParam, self->list.begin(), listSize, 
	...
	if (listSize > 1)
	{
		qsort(self->list.begin(), self->list.size(), sizeof(DeviceColoredImage**), 
