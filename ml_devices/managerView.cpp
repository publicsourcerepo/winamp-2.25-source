...
#define VIEW_OFFSET_TOP_PX					1
#define VIEW_OFFSET_RIGHT_PX				2
#define VIEW_OFFSET_BOTTOM_PX				0
...
HWND ManagerView_CreateWindow(HWND parentWindow)
{
	...
	hwnd = WASABI_API_CREATEDIALOGPARAMW((INT_PTR)IDD_MANAGER_VIEW, parentWindow, 
...
{
	ManagerView *self;
	HWND buttonWindow, widgetWindow, sliderWindow, statusBar;
	RECT clientRect, elementRect;
	LONG bottomBarHeight, buttonWidth, sliderWidth;
	HDWP hdwp;
	UINT swpFlags;
	...
	statusBar = MANAGERVIEW_STATUS_BAR(hwnd);
	if (NULL != statusBar)
	{
		...
		elementRect.top = elementRect.bottom - statusBarHeight - (bottomBarHeight - statusBarHeight)/2;
	...
	widgetWindow = MANAGERVIEW_WIDGET(hwnd);
	if (NULL != widgetWindow)
	{
		CopyRect(&elementRect, &clientRect);
		if (0 != bottomBarHeight)
		{
			elementRect.bottom -= bottomBarHeight;
			elementRect.bottom -= MANAGERVIEW_DLU_TO_VERT_PX(self, BOTTOM_BAR_OFFSET_TOP_DLU);
		}
...
ManagerView_UpdateSkin(HWND hwnd)
{
	...
	color = Graphics_GetSkinColor(WADLG_WNDBG);
...
ManagerView_GetIdealButtonSize(HWND buttonWindow, SIZE *buttonSize)
{
	...
	skinSize = MLSkinnedButton_GetIdealSize(buttonWindow, NULL);
	if (0 != skinSize)
	{
...
ManagerView_UpdateDiscoverButtonFont(HWND hwnd, HFONT font, SIZE *size)
{
	...
	result = SetWindowPos(buttonWindow, NULL, 0, 0, buttonSize.cx, buttonSize.cy, 
	...
	while(S_OK == enumerator->Next(&object, 1, NULL))
	{
		if (SUCCEEDED(object->QueryInterface(IFC_Device, (void**)&device)))
		{
			if(FALSE == device->GetHidden() &&
			   ...
			   strcmp(device->GetConnection(), "cloud"))
				devicesPresent = TRUE;
...
{
	ManagerView *self;
	HWND widgetWindow, discoverButton, statusBar;
	...
	discoverButton = MANAGERVIEW_DISCOVER_BUTTON(hwnd);
	...
	statusBar = StatusBar_CreateWindow(0, NULL, WS_VISIBLE, 0, 0, 0, 0, hwnd, MANAGERVIEW_STATUSBAR_ID);
	self->discoveryStatus = STATUS_ERROR;
	...
	widgetWindow = ManagerView_UpdateWidget(hwnd);
...
ManagerView_OnWindowPosChanged(HWND hwnd, WINDOWPOS *windowPos)
{
	if ((SWP_NOSIZE | SWP_NOMOVE) != ((SWP_NOSIZE | SWP_NOMOVE | SWP_FRAMECHANGED) & windowPos->flags))
	{
		...
		ManagerView_Layout(hwnd, 0 == (SWP_NOREDRAW & windowPos->flags));
	}
}
