...
typedef nu::PtrList<StatusRecord> StatusRecordList;
						...
						HWND parentWindow, unsigned int controlId)
{
	...
	klassAtom = StatusBar_GetClassAtom(instance);
	if (0 == klassAtom)
		return NULL;
	
	hwnd = CreateWindowExW(WS_EX_NOPARENTNOTIFY | windowExStyle, (LPCWSTR)MAKEINTATOM(klassAtom), text,
							WS_CHILD | WS_CLIPSIBLINGS | WS_CLIPCHILDREN | windowStyle,
							x, y, width, height, 
							parentWindow, (HMENU)controlId, instance, NULL);
	...
	switch(moveCommand)
	{
		...
		case STATUS_MOVE_BOTTOM:
			if (0 == index)
				return TRUE;
			self->list.eraseindex(index);
			self->list.insertBefore(0, record);
			newIndex = 0;
			break;
		...
		case STATUS_MOVE_UP:
			...
			self->list.eraseindex(index);
			newIndex = index + 1;
			self->list.insertBefore(newIndex, record);
			break;
