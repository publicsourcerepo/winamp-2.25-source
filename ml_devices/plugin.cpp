...
typedef nu::HandleList<PluginUnloadCallback> UnloadCallbackList;
...
Plugin_GetResourceString(const wchar_t *resourceName, const wchar_t *resourceType, wchar_t *buffer, size_t bufferMax)
{
	...
	filenameLength = GetModuleFileName(Plugin_GetInstance(), buffer, bufferMax);
...
Plugin_ShowHelp()
{
	...
	if (L'\0' == buffer[0])
	{
		if (FAILED(StringCchCopy(buffer, ARRAYSIZE(buffer), 
					L"http://www.winamp.com/help/Main_Page#Portables_Interface")))
		{
			return FALSE;
		}
	}
