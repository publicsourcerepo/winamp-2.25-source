					   ...
					   HFONT sourceFont, WidgetStyleAssignFlags assignFlags)
{
	...
	*font = (sizeof(lf) == GetObjectW(sourceFont, sizeof(lf), &lf)) ?
					  CreateFontIndirectW(&lf) : NULL;
...
WidgetStyle_GetBorderColor(COLORREF backColor, COLORREF textColor)
{
	...
	for(int step = 0;; step++)
	{
		...
		distance = Graphics_GetColorDistance(borderColor, backColor);
		if (distance < 0) distance = -distance;
		if (distance >= 40)
			break;
	}
...
WidgetStyle_UpdateDefaultFonts(WidgetStyle *style, HFONT baseFont, long unitWidth, long unitHeight)
{
	...
	tempFont = Graphics_DuplicateFont(baseFont, 2, FALSE, TRUE);
