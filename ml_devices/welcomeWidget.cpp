...
WelcomeWidget_CreateTitleFont(WidgetStyle *style, HDC hdc)
{
	...
	if (NULL != textFont)
	{
		...
		{
			long test = MulDiv(tm.tmHeight - tm.tmInternalLeading, 72, pixelsY);
	...
	StringCchCopy(lf.lfFaceName, ARRAYSIZE(lf.lfFaceName), L"Arial Black");
...
WelcomeWidget_CreateContext(HWND hwnd, WidgetStyle *style, HBITMAP *prevBitmapOut, long *widthOut, long *heightOut)
{
	...
	HBITMAP prevTargetBitmap, prevSourceBitmap;
...
WelcomeWidget_InitCb(HWND hwnd, void **object, void *param)
{
	...
	self = (WelcomeWidget*)malloc(sizeof(WelcomeWidget));
	if (NULL == self)
		return FALSE;
	...
	helpLink = CommandLink_CreateWindow(WS_EX_NOPARENTNOTIFY, 
							buffer, 
