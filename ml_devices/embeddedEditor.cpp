...
EmbeddedEditorParent_RemoveChild(HWND hwnd, HWND child)
{
	...
	self->editorList.eraseAtFast(index);
...
EmbdeddedEditor_DrawBorder(EmbeddedEditor *self, HWND hwnd, HDC hdc)
{
	...
	if (NULL != borderRgn)
	{
		...
		frameRgn = CreatePolyPolygonRgn(polygons, 
										 polygonsPointCount, 
										 ARRAYSIZE(polygonsPointCount),
										 WINDING);
		if (NULL != frameRgn)
		{
			combineResult = CombineRgn(frameRgn, borderRgn, frameRgn, RGN_AND);
...
EmbeddedEditor_Resize(EmbeddedEditor *self, HWND hwnd, BOOL redraw)
{
	...
	else
	{
		...
		result = DrawText(hdc, text, textLength, &rect, 
						   ...
						   WPARAM wParam, LPARAM lParam, LRESULT *result)
{
	...
	switch(uMsg)
	{
		...
		case WM_KILLFOCUS:
			return EmbeddedEditor_OnKillFocus(self, hwnd, (HWND)wParam);
		case WM_MOUSEWHEEL:
