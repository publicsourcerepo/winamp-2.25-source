		...
		case ML_MSG_NAVIGATION_CONTEXTMENU:
		{
			...
			else
			{
				...
				if (FindTreeItem(nvItem.id))
				{
					...
					int r = Menu_TrackPopup(menu, TPM_RETURNCMD | TPM_RIGHTBUTTON | TPM_LEFTBUTTON, pt.x, pt.y, wnd, NULL);
					switch (r)
					{
						case IDC_PLAY:
						{
						...
						break;
						case IDC_NEWPLAYLIST:  playlists_Add(wnd);	break;
						case IDC_DELETE: DeletePlaylist(tree_to_guid_map[nvItem.id], wnd, true); break;
						case IDC_RENAME: RenamePlaylist(tree_to_guid_map[nvItem.id], wnd); break;
						case IDC_VIEWLIST: mediaLibrary.SelectTreeItem(nvItem.id); break;
						default:
						{
...
int playlists_ContextMenu(INT_PTR param1, HWND hHost, POINTS pts)
{
	...
	switch (r)
	{
		case IDC_NEWPLAYLIST: playlists_Add(hHost);	break;
		case IDC_IMPORT_PLAYLIST_FROM_FILE: Playlist_importFromFile(hHost);	break;
		case IDC_IMPORT_WINAMP_PLAYLIST: Playlist_importFromWinamp();	break;
		case ID_PLAYLISTSMENU_IMPORTPLAYLISTFROMFOLDERS: Playlist_importFromFolders(hHost);	break;
		case ID_SORTPLAYLIST_TITLE_A_Z: playlists_Sort(SORT_TITLE_ASCENDING); break;
		case ID_SORTPLAYLIST_TITLE_Z_A: playlists_Sort(SORT_TITLE_DESCENDING); break;
		case ID_SORTPLAYLIST_NUMBEROFITEMSASCENDING: playlists_Sort(SORT_NUMBER_ASCENDING); break;
		case ID_SORTPLAYLIST_NUMBEROFITEMSDESCENDING: playlists_Sort(SORT_NUMBER_DESCENDING); break;
		case ID_PLAYLISTS_HELP: myOpenURL(hHost, L"http://www.winamp.com/help/Main_Page#Winamp_Playlists"); break;
	}
	Sleep(100);
	MSG msg;
		...
		case ML_ACTION_ENTER:
		{
			if (FindTreeItem(treeId))
			{
				...
				if (info.Associate(treeId))
				{
					...
					if ((!!g_config->ReadInt("enqueuedef", 0)) ^ flip)
						mediaLibrary.EnqueueFile(info.GetFilename());
					else
						mediaLibrary.PlayFile(info.GetFilename());
				}
