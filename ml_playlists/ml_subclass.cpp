...
{
	PlaylistInfo pl;
	wchar_t filename[1024+256]; // use a longer buffer than MAX_PATH here because createPlayListDBFileName needs it
	...
	AddPlaylist(true, title, filename, !!(addPlaylist->flags & PL_FLAG_SHOW), g_config->ReadInt("cloud", 1), numItems, length);
}
