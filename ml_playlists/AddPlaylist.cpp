...
wchar_t *createPlayListDBFileName(wchar_t *filename) // filename is ignored but used for temp space, make sure it's 1024+256 chars =)
{
	...
	for (;;)
	{
		...
		{
			if (g_config->ReadInt("playlist_m3u8", 1))
				lstrcpyW(filename + lstrlenW(filename) - 4, L".m3u8");
			else
				lstrcpyW(filename + lstrlenW(filename) - 4, L".m3u");
		}
...
{
	if (g_config->ReadInt("cloud_always", 0) && !g_config->ReadInt("cloud_prompt", 0) && g_config->ReadInt("cloud", 1))
	{
		wchar_t titleStr[64];
		...
		{
			g_config->WriteInt("cloud", (IsDlgButtonChecked(hwndDlg, IDC_CLOUD) == BST_CHECKED));
		}
		else
		{
			CheckDlgButton(hwndDlg, IDC_CLOUD, playlists_CloudAvailable() && g_config->ReadInt("cloud", 1));
		}
		g_config->WriteInt("cloud_prompt", 1);
	}
	else
	{
		g_config->WriteInt("cloud", (IsDlgButtonChecked(hwndDlg, IDC_CLOUD) == BST_CHECKED));
...
int AddToCloud()
{
	...
	{
		if (g_config->ReadInt("cloud_always", 1))
		{
			g_config->WriteInt("cloud", 1);
			return 1;
		}
		else
		{
			g_config->WriteInt("cloud", 0);
		}
	}
	return 0;
}
		...
		case WM_COMMAND:
			switch (LOWORD(wParam))
			{
				case IDOK:
				{
					...
					AddPlaylist(callback, name, filename, true, (playlists_CloudAvailable() ? g_config->ReadInt("cloud", 1) : 0)); 
