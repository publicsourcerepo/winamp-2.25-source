...
int init() 
{
	...
	importerFactory.Register(WASABI_API_SVC, &importAPI);
...
void quit() {
	...
	importerFactory.Deregister(WASABI_API_SVC);
}
...
static int pickFile(HWND hwndDlg, StringW *file) 
{
	...
	temp = (wchar_t *)GlobalAlloc(GMEM_FIXED,len*sizeof(*temp));
	temp[0] = 0;
	l.hwndOwner = hwndDlg;
...
void exportDatabase() {
	...
	if (w.pickFile(plugin.hwndLibraryParent)) {
		...
		for (int x = 0; x < progress[1]; x ++) {
			...
			// changed in 5.7 to use the 'realsize' if it's available, otherwise map to filesize scaled to bytes (stored as kb otherwise)
