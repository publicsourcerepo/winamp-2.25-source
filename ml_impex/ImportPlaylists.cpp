...
typedef PtrMap<int64_t, iTunesFileInfo> FilesList;
...
{
	plistDict *item_dict = (plistDict *)items->enumItem(item);
	if (item_dict)
	{
...
static void AddPlaylist(plistDict *playlist,  FilesList &files)
{
		...
		&& (!GetInteger(playlist, L"Visible", &visible) || visible))
	{
		...
		{		
			...
			size_t cloud = (cloudAvailable() ? GetPrivateProfileIntW(L"gen_ml_config", L"cloud_always", 1, ini_file) : 0);

			AGAVE_API_PLAYLISTMANAGER->Save(destination, &plist_playlist);
			AGAVE_API_PLAYLISTS->Lock();
			int new_index = (!cloud ? AGAVE_API_PLAYLISTS->AddPlaylist(destination, name) : AGAVE_API_PLAYLISTS->AddCloudPlaylist(destination, name));
			if (new_index >= 0)
			{
				uint32_t numItems = plist_playlist.GetNumItems();
