...
static Vector<wchar_t*> blacklist;
...
static void blacklistLoad() {
	...
	int l = GetPrivateProfileIntA("pmp_android","blacklistnum",0,winampini);
	for(int i=l>100?l-100:0; i<l; i++) {
		char buf[100];
		buf[0] = '\0';

		StringCchPrintfA(keyname, 64, "blacklist-%d", i);
		GetPrivateProfileStringA("pmp_android", keyname, "", buf, 100, winampini);
		if(buf[0]) 
		{
			blacklist.push_back(AutoWideDup(buf, CP_UTF8));
		}
	}
}
...
static void blacklistSave() {
	...
	StringCchPrintfA(buf, 64, "%d",blacklist.size());
	WritePrivateProfileStringA("pmp_android","blacklistnum",buf,winampini);
	for(size_t i=0; i<blacklist.size(); i++) 
	{
		StringCchPrintfA(buf, 64, "blacklist-%d",i);
		WritePrivateProfileStringA("pmp_android",buf,AutoChar((const wchar_t*)blacklist.at(i), CP_UTF8),winampini);
	}
}
...
{
	const wchar_t test[] = {drive, L":\\Winamp\\"TAG_CACHE};
	wchar_t title[128];
...
void connectDrive(wchar_t drive, bool checkSize=true, bool checkBlacklist=true) 
{
	...
	for (Vector<AndroidDevice*>::const_iterator e = devices.begin(); e != devices.end(); e++) 
	{
		if ((*e)->drive == drive) 
			return;
	}
