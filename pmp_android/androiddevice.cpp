...
extern Vector<AndroidDevice*> devices;
...
{
	return androidPlaylists.at(playlistnumber)->size();
}
...
void AndroidDevice::deletePlaylist(int playlistnumber)
{
	AndroidPlaylist * pl = (AndroidPlaylist*)androidPlaylists.at(playlistnumber);
	_wunlink(pl->filename);
	androidPlaylists.eraseAt(playlistnumber);
	delete pl;
}
...
int AndroidDevice::newPlaylist(const wchar_t *name)
{
	...
	for (Vector<AndroidPlaylist*>::iterator itr=androidPlaylists.begin();itr!=androidPlaylists.end();itr++)
	{
		AndroidPlaylist *p = *itr;
		if (!wcsicmp(p->playlistName, plname))
			return -1;
	}
	...
	return androidPlaylists.size()-1;
}
...
pmpart_t AndroidDevice::getArt(songid_t songid)
{
	...
	ARGB32 *bits;
	int w, h;
	if (AGAVE_API_ALBUMART->GetAlbumArt(song->filename, L"cover", &w, &h, &bits) == ALBUMART_SUCCESS && bits)
	{
...
{
	for (Vector<AndroidPlaylist*>::const_iterator e = androidPlaylists.begin(); e != androidPlaylists.end(); e++)
	{
