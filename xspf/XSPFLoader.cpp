...
wchar_t *ItemRecordTagProvider::GetTag(const wchar_t *tag, ifc_tagparams *parameters) 
{
	...
	if (!wcsicmp(tag, L"artist"))	value = t->artist;
	else if (!wcsicmp(tag, L"album"))	value = t->album;
	else if (!wcsicmp(tag, L"filename"))	value = t->filename;
	else if (!wcsicmp(tag, L"title"))	value = t->title;
	else if (!wcsicmp(tag, L"year"))
	{
		if (t->year > 0)
		{
	...
	else if (!wcsicmp(tag, L"genre"))	value = t->genre;
	else if (!wcsicmp(tag, L"comment")) value = t->comment;
	else if (!wcsicmp(tag, L"tracknumber") || !wcsicmp(tag, L"track"))
	{
		if (t->track > 0)
		{
			if (t->tracks > 0)
	...
	else if (!wcsicmp(tag, L"playcount"))
	{
		if (t->playcount > 0)
		{
	...
	else if (!wcsicmp(tag, L"albumartist"))	value = t->albumartist;
	else if (!wcsicmp(tag, L"publisher"))	value = t->publisher;
	else if (!wcsicmp(tag, L"composer"))	value = t->composer;
	else if (!wcsicmp(tag, L"replaygain_album_gain"))	value = t->replaygain_album_gain;
	else if (!wcsicmp(tag, L"replaygain_track_gain"))	value = t->replaygain_track_gain;
	else if (!wcsicmp(tag, L"GracenoteFileID"))	
	{
		copy=true;
		value = getRecordExtendedItem(t, L"GracenoteFileID");
	}
	else if (!wcsicmp(tag, L"GracenoteExtData"))
	{
		copy=true;
			...
			if (query[0])
			{
				...
				{/*
					if (HasTagzAPI())
					{
						...
						AGAVE_API_TAGZ->format(L"%artist% - %title%", title, 2048, &provider, 0);
						int length = -1;
...
void XSPFLoaderCallback::xmlReaderOnCharacterDataCallback(const wchar_t *xmlpath, const wchar_t *xmltag, const wchar_t *str)
{
	if (element!=ELEMENT_NONE)
	{
		...
		else
		{
			...
			wchar_t *&newstr = elements[element];
