...
static const ValueInfo valueInfoMap[] =
{
	...
	*/ 
	{ "Artist",			StringGroup::ValueId_String },
	{ "Artist_SortKey",	StringGroup::ValueId_SortKey },
};

static int
ArtistGroupProvider_FindValueInfoCb(const void *target, const void *element)
{
	ValueInfo *valueInfo;
...
ArtistGroupProvider::ArtistGroupProvider()
...
HRESULT ArtistGroupProvider::CreateInstance(ifc_groupprovider **instance)
{
	if (NULL == instance) 
		return E_POINTER;

	*instance = new (std::nothrow) ArtistGroupProvider();
	if (NULL == *instance)
...
size_t ArtistGroupProvider::AddRef()
{
	return InterlockedIncrement((LONG*)&ref);
}

size_t ArtistGroupProvider::Release()
{
	if (0 == ref)
		return ref;
...
int ArtistGroupProvider::QueryInterface(GUID interface_guid, void **object)
{
	if (NULL == object) 
		return E_POINTER;
...
const char *ArtistGroupProvider::GetName()
{
	return "Artist";
}

HRESULT ArtistGroupProvider::GetDisplayName(wchar_t *buffer, size_t bufferSize)
{
	if(NULL == ResourceString_CopyTo(buffer, bufferSize, MAKEINTRESOURCE(IDS_GROUP_ARTIST)))
	{
		return E_OUTOFMEMORY;
	}
...
HRESULT ArtistGroupProvider::GetCounterText(wchar_t *buffer, size_t bufferSize)
{
	if(NULL == ResourceString_CopyTo(buffer, bufferSize, MAKEINTRESOURCE(IDS_GROUP_ARTIST_COUNTER)))
	{
		return E_OUTOFMEMORY;
	}
...
HRESULT ArtistGroupProvider::GetEmptyText(wchar_t *buffer, size_t bufferSize)
{
	if(NULL == ResourceString_CopyTo(buffer, bufferSize, MAKEINTRESOURCE(IDS_GROUP_ARTIST_EMPTY)))
	{
		return E_OUTOFMEMORY;
	}
...
HRESULT ArtistGroupProvider::Bind(ifc_dataprovider *provider)
{
	const char *valueName[] = { "Artist", "Artist_SortKey" };
...
HRESULT ArtistGroupProvider::ResolveNames(const char **names, size_t count, size_t *valueIds)
{
...
HRESULT ArtistGroupProvider::CreateSummaryGroup(ifc_dataobject **group)
{
	return SummaryGroup::CreateInstance((SummaryGroup**)group);
}

HRESULT ArtistGroupProvider::CreateGroup(LCID localeId, ifc_dataobject *object, ifc_dataobject **group)
{
	HRESULT hr;
	wchar_t buffer[512];
...
int ArtistGroupProvider::CompareObjects(LCID localeId, ifc_dataobject *object1, ifc_dataobject *object2)
{
	int result;
...
size_t ArtistGroupProvider::ResolveName(const char *name)
{
	ValueInfo *valueInfo;
	...
	valueInfo = (ValueInfo*)bsearch(name, valueInfoMap, ARRAYSIZE(valueInfoMap), 
							sizeof(ValueInfo), ArtistGroupProvider_FindValueInfoCb);
...
#define CBCLASS ArtistGroupProvider
START_DISPATCH;
CB(ADDREF, AddRef)
CB(RELEASE, Release)
