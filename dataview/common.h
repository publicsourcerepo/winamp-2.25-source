#ifndef _NULLSOFT_WINAMP_DATAVIEW_COMMON_HEADER
#define _NULLSOFT_WINAMP_DATAVIEW_COMMON_HEADER
...
#define SetPtrValSafe(_ptr, _val)\
	{if (NULL != (_ptr)) { (*(_ptr)) = (_val);}}

#define SafeRelease(_ptr)\
	{ if (NULL != (_ptr)) {(_ptr)->Release(); (_ptr) = NULL;}}

#define SafeDelete(_ptr)\
	{ if (NULL != (_ptr)) {delete (_ptr); (_ptr) = NULL;}}

#define RETURN_HRESULT_FROM_LAST_ERROR()\
		{unsigned long _errorCode = GetLastError();\
		return HRESULT_FROM_WIN32(_errorCode);}

#endif //_NULLSOFT_WINAMP_DATAVIEW_COMMON_HEADER
