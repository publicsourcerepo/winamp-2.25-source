...
HRESULT AddressEncoder_AppendAnsiString(ENCODEBUFFER *decoder, LPCSTR pszString, size_t cchString)
{
	...
	INT cchConverted; 
	while(0 ==(cchConverted = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, pszString, cchString, decoder->cursor, decoder->remaining)))
	{
		DWORD errorCode = GetLastError();
		if (ERROR_INSUFFICIENT_BUFFER == errorCode)
		{
			INT cchNeed = MultiByteToWideChar(CP_UTF8, 0, pszString, cchString, NULL, 0) - decoder->remaining;
...
HRESULT AddressEncoder_GetEscapeBlock(LPCWSTR pszEscape, LPCWSTR *ppszEnd, size_t *pcchEscapeLen, LPSTR pszBuffer, UINT *pcbBufferMax)
{
	...
	{		
		...
		binaryData = ((szDigit[0] - ((szDigit[0] <= L'9' ? L'0' : L'A' - 10))) << 4) & 0xf0;
...
HRESULT AddressEncoder_GetWideBlock(LPCWSTR pszWide, LPCWSTR *pszEnd, LPWSTR pszBuffer, size_t *pcchBufferMax)
{
	...
	{
		size_t bytesCount = WideCharToMultiByte(CP_UTF8, 0, pszWide, (cursor - pszWide), NULL, 0, NULL, NULL);
		if (0 == bytesCount)
		{
			DWORD errorCode = GetLastError();
		...
		else
		{
			...
			if (NULL != pszBuffer)
			{
				if (*pcchBufferMax >= cchBuffer)
				{
					...
					WideCharToMultiByte(CP_UTF8, 0, pszWide,  (cursor - pszWide), (LPSTR)bytes, bytesCount, NULL, NULL);
...
HRESULT AddressEncoder_EncodeWideChars(LPCWSTR pszAddress, size_t cchAddress, LPWSTR *ppResult)
{
	...
	size_t resultLen = lstrlen(result);
