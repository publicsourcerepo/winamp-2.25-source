...
extern int config_prompt_save_preset, config_reuseonresize;
...
void my_getViewport(RECT *r, RECT *sr) {
	if (sr) 
	{
		...
		BOOL (WINAPI *Gmi)(HMONITOR mon, LPMONITORINFO lpmi) = (BOOL (WINAPI *)(HMONITOR,LPMONITORINFO)) GetProcAddress(h,"GetMonitorInfoA");    
			if (Mfr && Gmi) {
				HMONITOR hm=Mfr(sr,MONITOR_DEFAULTTONEAREST/*MONITOR_DEFAULTTONULL*/);
				if (hm) {
					MONITORINFOEX mi;
...
void SetTransparency(HWND hWnd, int enable, int amount)
{
	...
	if (!_stricmp(classname, "BaseWindow_RootWnd")) return;
	// --
#endif
...
int w = (prc->right - prc->left), h = (prc->bottom - prc->top);
	...
	HMONITOR hMonitor = g_pfnMonitorFromRect(prc,MONITOR_DEFAULTTONEAREST);
	MONITORINFO mi = {sizeof(mi),0};
		g_pfnGetMonitorInfo(hMonitor,(LPMONITORINFO)&mi);
		rc = mi.rcWork;
	}
	else{
		SystemParametersInfo(SPI_GETWORKAREA,0,&rc,0);
	}*/
	my_getViewport(&rc,prc);
...
#define AVS_SECTION "AVS_L"
	...
	// makes sure that the avs window is on screen when we're creating it
	// should handle cases when screen resolution is altered and it's lost off screen
	ClipOrCenterRectToMonitor(&myWindowState.r);
#else
...
void toggleWharfAmpDock(HWND hwnd)
{
	...
	if (!inWharf)
	{
		...
		SetWindowPos(GetDlgItem(g_hwndDlg,IDC_TREE1),NULL,0,0,r2.right-r2.left,r.top - 6 - r2.top,SWP_NOMOVE|SWP_NOZORDER|SWP_NOACTIVATE);
			...
			if (!(d.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
			{
				...
				if (lastpreset)
				{
					...
					if (dir > 0)
					{
						...
						if (!_stricmp(dirmask,lastpreset)) *state=1;
					}

					if (dir < 0)
					{
						if (!_stricmp(dirmask,lastpreset))
						{
...
void next_preset(HWND hwnd) {
	...
	if (readyToLoadPreset(hwnd,0))
	{
		...
		if (dirmask[0] && _stricmp(last_preset,dirmask))
		{
...
static LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	if (DDraw_IsFullScreen() && !cfg_fs_use_overlay &&
        ((message == WM_KEYDOWN && wParam == VK_ESCAPE) || message == WM_LBUTTONUP ||
		  (message == WM_NCACTIVATE && !wParam) || message == WM_KILLFOCUS))
	{
		Wnd_GoWindowed(hwnd);
		return 0;
	}
	...
	switch (message)
	{
		...
		case WM_DROPFILES:
		{
			...
			if (readyToLoadPreset(hwnd,0))
			{
				if (!_stricmp(extension(temp),"avs"))
				{
					if (g_render_transition->LoadPreset(temp,1) != 2)
  						lstrcpyn(last_preset,temp,sizeof(last_preset));
				}
