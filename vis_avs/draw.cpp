	...
	if (fs && !fsovl)
	{
		...
		if((cfg_bkgnd_render&1) || (g_fs && fsovl)) {
			...
			if(!FAILED(ddrval))
			{
				...
				// Make the call to UpdateOverlay() which actually displays the overlay on the screen.
...
#ifndef NO_MMX
bool CopyRGBSurfaceToYUVSurfaceMMX(LPDDSURFACEDESC pddsd1, LPDDSURFACEDESC pddsd2, fourcc_enum eOverlayFormat)
{
...
#else
bool CopyRGBSurfaceToYUVSurface(LPDDSURFACEDESC pddsd1, LPDDSURFACEDESC pddsd2, fourcc_enum eOverlayFormat)
{
	if (pddsd1->dwWidth != pddsd2->dwWidth) 
		return false;
	if (pddsd1->dwHeight != pddsd2->dwHeight) 
...
void DDraw_Exit(int which)
{
	...
	if (g_fs && !cfg_fs_use_overlay)
	{
		...
		if (g_bpp != 32) // non-32bpp mode
		{
			...
			{
				DDSURFACEDESC d={sizeof(d),};
				HRESULT han = g_lpPrimSurfBack->Lock(NULL,&d,DDLOCK_WAIT,NULL);
...
void DDraw_SetFullScreen(int fs, int w, int h, int dbl, int bpp)
{
	...
	else DD_CreateSurfaces(w, h, (min(max(1,cfg_fs_height),100)*h)/100, 1, bpp, cfg_fs_flip, !!dbl, cfg_fs_use_overlay);
}
...
HRESULT WINAPI _cb(LPDDSURFACEDESC lpDDSurfaceDesc,  LPVOID lpContext)
{
...
HRESULT WINAPI _cb2(LPDDSURFACEDESC lpDDSurfaceDesc,  LPVOID lpContext)
{
...
HRESULT WINAPI _cb3(LPDDSURFACEDESC lpDDSurfaceDesc,  LPVOID lpContext)
{
	if ((lpDDSurfaceDesc->ddpfPixelFormat.dwFlags & DDPF_RGB) &&
		(lpDDSurfaceDesc->ddpfPixelFormat.dwRGBBitCount==32 ||
		lpDDSurfaceDesc->ddpfPixelFormat.dwRGBBitCount==24 ||
