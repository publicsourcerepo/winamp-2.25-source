...
class C_THISCLASS : public C_RBASE2 {
	...
	public:
		...
		virtual char *get_desc() { static char desc[128]; return (!desc[0]?WASABI_API_LNGSTRING_BUF(IDS_TRANS_MOVEMENT,desc,128):desc); }
...
int C_THISCLASS::smp_begin(int max_threads, char visdata[2][2][576], int isBeat, int *framebuffer, int *fbout, int w, int h)
{
    ...
    else if (trans_effect == 32767 || effect_uses_eval(trans_effect))
    {
		...
		*ph=h;
		...
		codehandle=compileCode(trans_effect == 32767 ? effect_exp.get() : descriptions[trans_effect].eval_desc);
...
static BOOL CALLBACK g_DlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam)
{
	...
	switch (uMsg)
	{
		case WM_INITDIALOG:
			{
				for (int x = 0; x < sizeof(descriptions)/sizeof(descriptions[0]); x ++) 
				{
					SendDlgItemMessage(hwndDlg,IDC_LIST1,LB_ADDSTRING,0,(LPARAM)WASABI_API_LNGSTRING(descriptions[x].list_desc));
				}
				...
				SendDlgItemMessage(hwndDlg,IDC_LIST1,LB_ADDSTRING,0,(LPARAM)WASABI_API_LNGSTRING(IDS_USER_DEFINED));
		...
		case WM_COMMAND:
			...
			{
				int t=SendDlgItemMessage(hwndDlg,IDC_LIST1,LB_GETCURSEL,0,0);
