...
void C_THISCLASS::getWord(int n, char *buf, int maxlen)
{
	...
	*d=0;
	...
	{
		char *endp;
		if ((!_strnicmp(p,"$(playpos",9) || !_strnicmp(p,"$(playlen",9)) && (endp = strstr(p+9,")")))
		{
			char buf[128];
			int islen=_strnicmp(p,"$(playpos",9);
			int add_dig=0;
			if (p[9]=='.')
				add_dig=atoi(p+10);
		...
		else if (!_strnicmp(p,"$(title",7) && (endp = strstr(p+7,")")))
		{
...
//			use: $(reg00), $(reg00:4.5), $(title), $(title:32), $(title:n32)
		...
		else if (!_strnicmp(p,"$(reg",5) && p[5] >= '0' && p[5] <= '9' && p[6] >= '0' && p[6] <= '9' && (endp = strstr(p+7,")")))
		{
			char buf[128];
			char fmt[32];
			int wr=atoi(p+5);
		...
		else if (!_strnicmp(p,"$(playlist",10) && p[10] >= '0' && p[10] <= '9' && p[11] >= '0' && p[11] <= '9' && (endp = strstr(p+10,")")))
		{
			int num = (p[10] - '0')*10 + (p[11] - '0'); // playlist number to get
			int skipnum=1;
			int max_fmtlen=0;
			if (p[12] == ':')
			{
			...
			if(!max_fmtlen) max_fmtlen=300;
			fileinfo2 f={num,"",""};
			//SendMessage(playlist_wnd,WM_WA_IPC,IPC_PE_GETINDEXTITLE,(LPARAM)&f);
			HWND playlist_wnd = (HWND) SendMessage(hwnd_WinampParent, WM_WA_IPC, IPC_GETWND_PE, IPC_GETWND);
			if (IsWindow(playlist_wnd)) 
			{
				DWORD id;
				if (!SendMessageTimeout(playlist_wnd,WM_WA_IPC,IPC_PE_GETINDEXTITLE,(LPARAM)&f,SMTO_BLOCK,50,&id) || !id) 
				{
					// failed...
				}
			}
			char foo[300]="";
			if(!skipnum)
				wsprintf(foo,"%d. %s",num+1,f.filetitle);
			else
				wsprintf(foo,"%s",f.filetitle);

			int l=strlen(foo);
			if (l > maxlen) l=maxlen;
			if (max_fmtlen >0 && max_fmtlen < l) l=max_fmtlen;
			memcpy(d,foo,l);
			maxlen-=l;
			d += l;
			p=endp+1;
...
static BOOL CALLBACK g_DlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
			...
			if (!g_ConfigThis->blend && !g_ConfigThis->blendavg) CheckDlgButton(hwndDlg,IDC_REPLACE,BST_CHECKED);
			if (g_ConfigThis->enabled) CheckDlgButton(hwndDlg,IDC_CHECK1,BST_CHECKED);
