...
static presetType presets[]=
{
	...
	{IDS_4X_RED_BRIGHTNESS_2X_GREEN_1X_BLUE,"","red=4*red; green=2*green;","","",0},
	{IDS_SOLARIZATION,"","red=(min(1,red*2)-red)*2;\r\ngreen=red; blue=red;","","",0},
	{IDS_DOUBLE_SOLARIZATION,"","red=(min(1,red*2)-red)*2;\r\nred=(min(1,red*2)-red)*2;\r\ngreen=red; blue=red;","","",0},
	{IDS_INVERSE_SOLARIZATION_SOFT,"","red=abs(red - .5) * 1.5;\r\ngreen=red; blue=red;","","",0},
	{IDS_BIG_BRIGHTNESS_ON_BEAT,"scale=1.0","red=red*scale;\r\ngreen=red; blue=red;","scale=0.07 + (scale*0.93)","scale=16",1},
	{IDS_BIG_BRIGHTNESS_ON_BEAT_INTERPOLATIVE,"c = 200; f = 0;","red = red * t;\r\ngreen=red;blue=red;","f = f + 1;\r\nt = (1.025 - (f / c)) * 5;","c = f;f = 0;",1},
	{IDS_PULSING_BRIGHTNESS_BEAT_INTERPOLATIVE,"c = 200; f = 0;","red = red * st;\r\ngreen=red;blue=red;","f = f + 1;\r\nt = (f * 2 * $PI) / c;\r\nst = sin(t) + 1;","c = f;f = 0;",1},
	{IDS_ROLLING_SOLARIZATION_BEAT_INTERPOLATIVE,"c = 200; f = 0;","red=(min(1,red*st)-red)*st;\r\nred=(min(1,red*2)-red)*2;\r\ngreen=red; blue=red;","f = f + 1;\r\nt = (f * 2 * $PI) / c;\r\nst = ( sin(t) * .75 ) + 2;","c = f;f = 0;",1},
	{IDS_ROLLING_TONE_BEAT_INTERPOLATIVE,"c = 200; f = 0;","red = red * st;\r\ngreen = green * ct;\r\nblue = (blue * 4 * ti) - red - green;","f = f + 1;\r\nt = (f * 2 * $PI) / c;\r\nti = (f / c);\r\nst = sin(t) + 1.5;\r\nct = cos(t) + 1.5;","c = f;f = 0;",1},
	{IDS_RANDOM_INVERSE_TONE_SWITCH_ON_BEAT,"","dd = red * 1.5;\r\nred = pow(dd, dr);\r\ngreen = pow(dd, dg);\r\nblue = pow(dd, db);","","token = rand(99) % 3;\r\ndr = if (equal(token, 0), -1, 1);\r\ndg = if (equal(token, 1), -1, 1);\r\ndb = if (equal(token, 2), -1, 1);",1},
};
