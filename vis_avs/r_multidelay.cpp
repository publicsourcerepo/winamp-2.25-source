		...
		case WM_COMMAND:
			...
			if (objectcode >= 1100)
			{
				if(IsDlgButtonChecked(hwndDlg,objectcode)==1)
				{
					...
					for (size_t i=1100;i<1103;i++)	if (objectcode != i) CheckDlgButton(hwndDlg,i,BST_UNCHECKED);
				}
			    return 0;
			}
			// frames stuff
			...
			if (objectcode >= 1000)
			{
				if(IsDlgButtonChecked(hwndDlg,objectcode)==1)
				{
					...
					for (size_t i=1000;i<1006;i++)	if (objectcode != i) CheckDlgButton(hwndDlg,i,BST_UNCHECKED);
				}
			    return 0;
			}
	}
