...
/*		ShowWindow(GetDlgItem(hwndDlg, IDC_CURBPM), cfg_smartbeat ? SW_NORMAL : SW_HIDE);
	...
	case WM_TIMER:
		...
		{
			char lBuf[16];
			wsprintf(txt, predictionBpm ? "%d%s"/*/%d"*/ : WASABI_API_LNGSTRING_BUF(IDS_LEARNING,lBuf,16),
					 predictionBpm, cfg_smartbeatsticky && sticked ? WASABI_API_LNGSTRING(IDS_GOT_IT) : ""/*, Bpm*/);
...
#endif
		if (strncmp(songName, lastSongName, 255))
		{
			lstrcpyn(lastSongName, songName, 255);
			return TRUE;
		}
	}
	return FALSE;
...
BOOL TCHistStep(BeatType *t, DWORD _Avg, int *_halfDiscriminated, int *_hdPos, DWORD *_lastTC, DWORD TC, int Type)
{
	...
	if (thisLen < Avg/2 - Avg*0.2)
	{
		if (learning)
		{
			if ((long)abs((long)(Avg - (long)(TC - t[1].TC))) < (long)abs((long)(Avg - (long)(t[0].TC - t[1].TC))))
			{
/*				if (predictionLastTC && t[0].Type == BEAT_GUESSED && Type == BEAT_REAL)
	...
	if (learning) 
		for (offI=2;offI<offIMax;offI++) // Try to see if this beat is in the middle of our current Bpm, or maybe 1/3, 1/4 etc... to offIMax
			if ((float)abs(int(float((Avg/offI)-thisLen))) < (float)(Avg/offI)*0.2)
			{
